...
#include <XMZZip/write.h>         /* #includes <zzip/lib.h> */
                      ...
                      zzip_strings_t * ext, zzip_plugin_io_t io)
{
    ...
    if (! _ZZIP_TRY)
    {
         ...
         */
        //if (! kdMkdir(name, o_mode) || errno == EEXIST)
		if (! kdMkdir(name) || kdGetError ( ) == KD_EEXIST )
            kdSetError ( KD_ENOENT );//KD_EROFS );
        return 0;
    } else
    {
#       define MAX_EXT_LEN 10
        ...
        ___ int fd =
            __zzip_try_open(dir->realname, KD_O_EXCL | KD_O_TRUNC | KD_O_WRONLY, ext,
                            io);
        if (fd != -1)
        ...
        ____;
        fd = io->fd.open(dir->realname, KD_O_CREAT | KD_O_TRUNC | KD_O_WRONLY, o_mode);
...
zzip_createdir(zzip_char_t * name, int o_mode)
{
    if (o_mode & S_IWGRP)
    {
		...
		if (-1 == kdMkdir(name) && kdGetError ( ) != KD_EEXIST )      /* fail */
            return 0;
...
zzip_write(ZZIP_FILE * file, const void *ptr, zzip_size_t len)
{
    if (zzip_file_real(file))
        return kdWrite(zzip_realfd(file), (void*)ptr, len);
    else
