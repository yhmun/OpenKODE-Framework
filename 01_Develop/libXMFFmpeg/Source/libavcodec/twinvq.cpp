                     ...
                     float *out)
{
    ...
    float step     = (float)(AMP_MAX     / ((1 <<     GAIN_BITS) - 1));
    float sub_step = (float)(SUB_AMP_MAX / ((1 << SUB_GAIN_BITS) - 1));
                         ...
                         int ch, float *out, float gain, enum FrameType ftype)
{
	...
	const float _t[] = {0.4f, 0.35f, 0.28f};
    float val = _t[ftype];
                             ...
                             int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    TwinContext *tctx = (TwinContext *)avctx->priv_data;
    GetBitContext gb;
    const ModeTab *mtab = tctx->mtab;
    float *out = NULL;
    enum FrameType ftype;
...
static av_cold int init_mdct_win(TwinContext *tctx)
{
    ...
    FF_ALLOC_OR_GOTO(float*, tctx->avctx, tctx->tmp_buf,
                     mtab->size * sizeof(*tctx->tmp_buf), alloc_fail);

    FF_ALLOC_OR_GOTO(float*, tctx->avctx, tctx->spectrum,
                     2 * mtab->size * channels * sizeof(*tctx->spectrum),
                     alloc_fail);
    FF_ALLOC_OR_GOTO(float*, tctx->avctx, tctx->curr_frame,
                     2 * mtab->size * channels * sizeof(*tctx->curr_frame),
                     alloc_fail);
    FF_ALLOC_OR_GOTO(float*, tctx->avctx, tctx->prev_frame,
    ...
    for (i = 0; i < 3; i++) {
        ...
        FF_ALLOC_OR_GOTO(float*, tctx->avctx, tctx->cos_tabs[i],
...
static av_cold void init_bitstream_params(TwinContext *tctx)
{
    ...
    for (frametype = FT_SHORT; frametype <= FT_PPC; frametype = (FrameType)(frametype + 1) )
...
static av_cold int twin_decode_init(AVCodecContext *avctx)
{
    ...
    memset_float(tctx->bark_hist[0][0], 0.1f, FF_ARRAY_ELEMS(tctx->bark_hist));
