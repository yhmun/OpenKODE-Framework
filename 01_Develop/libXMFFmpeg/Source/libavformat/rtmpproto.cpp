...
#include "XMFFmpeg/libavutil/sha.h"
#include "internal.h"
#include "network.h"
#include "flv.h"
#include "rtmp.h"
                        ...
                        const char *host, int port)
{
    ...
    if (rt->is_input) {
        snprintf((char*)ver, sizeof(ver), "%s %d,%d,%d,%d", RTMP_CLIENT_PLATFORM, RTMP_CLIENT_VER1,
                 RTMP_CLIENT_VER2, RTMP_CLIENT_VER3, RTMP_CLIENT_VER4);
    } else {
        snprintf((char*)ver, sizeof(ver), "FMLE/3.0 (compatible; %s)", LIBAVFORMAT_IDENT);
        ff_amf_write_field_name(&p, "type");
        ff_amf_write_string(&p, "nonprivate");
    }
    ff_amf_write_field_name(&p, "flashVer");
    ff_amf_write_string(&p, (const char *)ver);
                             ...
                             const uint8_t *key, int keylen, uint8_t *dst)
{
    ...
    sha = (struct AVSHA *)av_mallocz(av_sha_size);
    ...
    case RTMP_PT_INVOKE:
        ...
        if (!memcmp(pkt->data, "\002\000\006_error", 9)) {
            ...
            if (!ff_amf_get_field_value(pkt->data + 9, data_end,
                                        (const uint8_t *)"description", tmpstr, sizeof(tmpstr)))
            ...
            t = ff_amf_get_field_value(ptr, data_end,
                                       (const uint8_t *)"level", tmpstr, sizeof(tmpstr));
            ...
            t = ff_amf_get_field_value(ptr, data_end,
                                       (const uint8_t *)"code", tmpstr, sizeof(tmpstr));
...
{
    RTMPContext *rt = (RTMPContext *)s->priv_data;
    int ret;
    uint8_t *p;
    const uint8_t *next;
    uint32_t data_size;
    ...
    for (;;) {
        ...
        if (rpkt.type == RTMP_PT_VIDEO || rpkt.type == RTMP_PT_AUDIO ||
           (rpkt.type == RTMP_PT_NOTIFY && !memcmp("\002\000\012onMetaData", rpkt.data, 13))) {
            ...
            rt->flv_data = p = (uint8_t *)av_realloc(rt->flv_data, rt->flv_size);
            ...
            rt->flv_data = (uint8_t *)av_realloc(rt->flv_data, rt->flv_size);
            /* rewrite timestamps */
            next = rpkt.data;
            ts = rpkt.timestamp;
...
{
    RTMPContext *rt = (RTMPContext *)h->priv_data;
...
static int rtmp_open(URLContext *s, const char *uri, int flags)
{
    ...
    ff_url_join((char *)buf, sizeof(buf), "tcp", NULL, hostname, port, NULL);

    if (ffurl_open(&rt->stream, (const char *)buf, AVIO_FLAG_READ_WRITE,
...
static int rtmp_write(URLContext *s, const uint8_t *buf, int size)
{
    ...
    do {
        if (rt->skip_bytes) {
            int skip = FFMIN(rt->skip_bytes, KDsizeemp);
            buf_temp       += skip;
            KDsizeemp      -= skip;
            rt->skip_bytes -= skip;
            continue;
        ...
        if (rt->flv_header_bytes < 11) {
            ...
            int copy = FFMIN(11 - rt->flv_header_bytes, KDsizeemp);
            ...
            ff_rtmp_packet_create(&rt->out_pkt, RTMP_SOURCE_CHANNEL, (RTMPPacketType)pkttype, ts, pktsize);
        ...
        if (rt->flv_size - rt->flv_off > KDsizeemp) {
            bytestream_get_buffer(&buf_temp, rt->flv_data + rt->flv_off, KDsizeemp);
            rt->flv_off += KDsizeemp;
            KDsizeemp = 0;
        } else {
            bytestream_get_buffer(&buf_temp, rt->flv_data + rt->flv_off, rt->flv_size - rt->flv_off);
            KDsizeemp   -= rt->flv_size - rt->flv_off;
