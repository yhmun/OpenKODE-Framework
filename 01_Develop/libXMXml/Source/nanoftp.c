...
static char *proxyUser = KD_NULL;	/* user for proxy authentication */
static char *proxyPasswd = KD_NULL;/* passwd for proxy authentication */
...
{
    __xmlSimpleError(XML_FROM_FTP, XML_ERR_NO_MEMORY, KD_NULL, KD_NULL, extra);
}
...
xmlNanoFTPCleanup(void) {
    ...
    if (proxyUser != KD_NULL) {
	xmlFree(proxyUser);
	proxyUser = KD_NULL;
    }
    if (proxyPasswd != KD_NULL) {
	xmlFree(proxyPasswd);
	proxyPasswd = KD_NULL;
    }
#ifdef _WINSOCKAPI_
...
xmlNanoFTPScanURL(void *ctx, const char *URL) {
    ...
    if (uri->user != KD_NULL) {
	char *cptr;
	if ((cptr=strchr(uri->user, ':')) == KD_NULL)
...
xmlNanoFTPScanProxy(const char *URL) {
	...
	(strcmp(uri->scheme, "ftp")) || (uri->server == KD_NULL)) {
 ...
 *
 * Returns an FTP context or KD_NULL in case of error.
 */
...
xmlNanoFTPNewCtxt(const char *URL) {
    ...
    unescaped = xmlURIUnescapeString(URL, 0, KD_NULL);
...
xmlNanoFTPGetMore(void *ctx) {
    ...
    if ((ctxt == KD_NULL) || (ctxt->controlFd == INVALID_SOCKET)) return(-1);
...
xmlNanoFTPCheckResponse(void *ctx) {
    ...
    switch(select(ctxt->controlFd + 1, &rfd, KD_NULL, KD_NULL, &tv)) {
	case 0:
	    return(0);
	case -1:
...
xmlNanoFTPConnect(void *ctx) {
	...
	if (proxy) {
	    if (getaddrinfo (proxy, KD_NULL, &hints, &result) != 0) {
		__xmlIOErr(XML_FROM_FTP, 0, "getaddrinfo failed");
		return (-1);
	    }
	}
	else
	    if (getaddrinfo (ctxt->hostname, KD_NULL, &hints, &result) != 0) {
		__xmlIOErr(XML_FROM_FTP, 0, "getaddrinfo failed");
		return (-1);
	    }
 ...
 *
 * Returns an fTP context or KD_NULL if it failed
 */
...
xmlNanoFTPConnectTo(const char *server, int port) {
    ...
    if (port <= 0)
	return(KD_NULL);
    ctxt = (xmlNanoFTPCtxtPtr) xmlNanoFTPNewCtxt(KD_NULL);
...
xmlNanoFTPDele(void *ctx, const char *file) {
    ...
    if ((ctxt == KD_NULL) || (ctxt->controlFd == INVALID_SOCKET) || (file == KD_NULL)) return(-1);
    if (file == KD_NULL) return (0);
...
xmlNanoFTPGetConnection(void *ctx) {
    ...
    if (ctxt == KD_NULL) return INVALID_SOCKET;
...
xmlNanoFTPCloseConnection(void *ctx) {
    ...
    res = select(ctxt->controlFd + 1, &rfd, KD_NULL, &efd, &tv);
    if (res < 0) {
#ifdef DEBUG_FTP
	perror("select");
#endif
...
xmlNanoFTPList(void *ctx, ftpListCallback callback, void *userData,
	...
	res = select(ctxt->dataFd + 1, &rfd, KD_NULL, &efd, &tv);
	if (res < 0) {
#ifdef DEBUG_FTP
	    perror("select");
#endif
 ...
 * xmlNanoFTPGetSocket:
 * @ctx:  an FTP context
 * @filename:  the file to retrieve (or KD_NULL if path is in context).
 *
...
xmlNanoFTPGetSocket(void *ctx, const char *filename) {
    ...
    if (ctx == KD_NULL)
	return INVALID_SOCKET;
    if ((filename == KD_NULL) && (ctxt->path == KD_NULL))
	return INVALID_SOCKET;
    ctxt->dataFd = xmlNanoFTPGetConnection(ctxt);
    if (ctxt->dataFd == INVALID_SOCKET)
	return INVALID_SOCKET;
...
xmlNanoFTPGet(void *ctx, ftpDataCallback callback, void *userData,
	...
	FD_ZERO(&rfd);
	FD_SET(ctxt->dataFd, &rfd);
	res = select(ctxt->dataFd + 1, &rfd, KD_NULL, KD_NULL, &tv);
	if (res < 0) {
#ifdef DEBUG_FTP
	    perror("select");
#endif
 ...
 *
 * Returns an FTP context, or KD_NULL 
 */
...
xmlNanoFTPOpen(const char *URL) {
    ...
    if (strncmp("ftp://", URL, 6)) return(KD_NULL);
...
int main(int argc, char **argv) {
    ...
    xmlNanoFTPList(ctxt, ftpList, KD_NULL, tstfile);
