...
{
    WtvContext *wctx = (WtvContext *)s->priv_data;
    AVIOContext *pb = s->pb;
...
static int finish_file(AVFormatContext *s, enum WtvFileIndex index, int64_t start_pos)
{
    ...
    pad = ((int64_t)1 << sector_bits) - (w->length % ((int64_t)1 << sector_bits));
    if (pad) {
        nb_sectors++;
        write_pad(pb, pad);
    }
...
static const AVCodecTag* const _ff_wtv_muxer[] = { ff_codec_bmp_tags, ff_codec_wav_tags, 0 };
