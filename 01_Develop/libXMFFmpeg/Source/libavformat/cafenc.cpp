...
static int caf_write_header(AVFormatContext *s)
{
    ...
    ffio_wfourcc(pb, (const uint8_t *)"caff"); //< mFileType
    avio_wb16(pb, 1);         //< mFileVersion
    avio_wb16(pb, 0);         //< mFileFlags

    ffio_wfourcc(pb, (const uint8_t *)"desc");                         //< Audio Description chunk
    ...
    if (enc->channel_layout) {
        ffio_wfourcc(pb, (const uint8_t *)"chan");
        avio_wb64(pb, 12);
        ff_mov_write_chan(pb, enc->channel_layout);
    }

    if (enc->codec_id == CODEC_ID_ALAC) {
        ffio_wfourcc(pb, (const uint8_t *)"kuki");
        avio_wb64(pb, 12 + enc->extradata_size);
        avio_write(pb, (const unsigned char *)"\0\0\0\14frmaalac", 12);
        avio_write(pb, enc->extradata, enc->extradata_size);
    } else if (enc->codec_id == CODEC_ID_AMR_NB) {
        ffio_wfourcc(pb, (const uint8_t *)"kuki");
        avio_wb64(pb, 29);
        avio_write(pb, (const unsigned char *)"\0\0\0\14frmasamr", 12);
        avio_wb32(pb, 0x11); /* size */
        avio_write(pb, (const unsigned char *)"samrFFMP", 8);
    ...
    ffio_wfourcc(pb, (const uint8_t *)"data"); //< Audio Data chunk
...
static int caf_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (!s->streams[0]->codec->block_align) {
        ...
        if (alloc_size < 0) {
            ...
            caf->pkt_sizes = (uint8_t *)av_fast_realloc(caf->pkt_sizes,
...
static int caf_write_trailer(AVFormatContext *s)
{
    ...
    if (pb->seekable) {
        ...
        if (!enc->block_align) {
            ffio_wfourcc(pb, (const uint8_t *)"pakt");
...
static const AVCodecTag* const _ff_caf_muxer[] = {ff_codec_caf_tags, 0};
