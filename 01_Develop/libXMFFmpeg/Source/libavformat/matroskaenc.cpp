                            ...
                            const void *buf, int size)
{
    ...
    avio_write(pb, (const unsigned char *)buf, size);
}
...
static ebml_master start_ebml_master(AVIOContext *pb, unsigned int elementid, uint64_t expectedsize)
{
	...
	{
		ebml_master _r = { avio_tell(pb), bytes };
    return _r;
	}
}
...
{
    mkv_seekhead *new_seekhead = (mkv_seekhead *)av_mallocz(sizeof(mkv_seekhead));
...
static int mkv_add_seekhead_entry(mkv_seekhead *seekhead, unsigned int elementid, uint64_t filepos)
{
    ...
    entries = (mkv_seekhead_entry *)av_realloc(entries, (seekhead->num_entries + 1) * sizeof(mkv_seekhead_entry));
    if (entries == NULL)
...
{
    mkv_cues *cues = (mkv_cues *)av_mallocz(sizeof(mkv_cues));
    if (cues == NULL)
        return NULL;
...
static int mkv_add_cuepoint(mkv_cues *cues, int stream, int64_t ts, int64_t cluster_pos)
{
    ...
    entries = (mkv_cuepoint *)av_realloc(entries, (cues->num_entries + 1) * sizeof(mkv_cuepoint));
    if (entries == NULL)
        return AVERROR(ENOMEM);
...
{
    MatroskaMuxContext *mkv = (MatroskaMuxContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    ebml_master tracks;
    int i, j, ret;
...
{
    uint8_t *key = (uint8_t *)av_strdup(t->key);
    uint8_t *p   = key;
    const uint8_t *lang = NULL;
    ebml_master tag;

    if ((p = (uint8_t *)strrchr(p, '-')) &&
        (lang = (const uint8_t *)av_convert_lang_to((const char *)(p + 1), AV_LANG_ISO639_2_BIBL)))
        *p = 0;
    ...
    put_ebml_string(pb, MATROSKA_ID_TAGNAME, (const char *)key);
    if (lang)
        put_ebml_string(pb, MATROSKA_ID_TAGLANG, (const char *)lang);
...
static int mkv_write_header(AVFormatContext *s)
{
    ...
    mkv->tracks = (mkv_track *)av_mallocz(s->nb_streams * sizeof(*mkv->tracks));
    if (!mkv->tracks)
        return AVERROR(ENOMEM);
...
static int mkv_write_ass_blocks(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    while (data_size) {
        ...
        end = (uint8_t *)memchr(data, '\n', data_size);
        size = line_size = end ? end-data+1 : data_size;
        size -= end ? (end[-1]=='\r')+1 : 0;
        start = data;
        for (i=0; i<3; i++, start++)
            if (!(start = (uint8_t *)memchr(start, ',', size-(start-data))))
                return max_duration;
        size -= start - data;
        ...
        avio_w8(pb, 0);
        avio_write(pb, (const unsigned char *)buffer, size);
...
static int mkv_copy_packet(MatroskaMuxContext *mkv, const AVPacket *pkt)
{
    ...
    mkv->cur_audio_pkt.data = (uint8_t *)av_fast_realloc(data, &mkv->audio_buffer_size, pkt->size);
