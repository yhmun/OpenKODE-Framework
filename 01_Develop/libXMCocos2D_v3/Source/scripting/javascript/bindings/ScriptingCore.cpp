...
void js_log(const char *format, ...) {
    ...
    va_list vl;
    va_start(vl, format);
    int len = kdVsnprintf(_js_log_buf, kMaxLogLen, format, vl);
    va_end(vl);
    if (len > 0)
    {
        CCLOG("JS: %s\n", _js_log_buf);
...
JSBool JSBCore_version(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    char version[256];
    kdSnprintf(version, sizeof(version)-1, "%s", cocos2dVersion());
...
int ScriptingCore::handleNodeEvent(void* data)
{
    ...
    Node* node = static_cast<Node*>(basicScriptData->pNativeObject);
...
int ScriptingCore::handleTouchesEvent(void* data)
{
    ...
    if (NULL == touchesScriptData->pNativeObject || touchesScriptData->aTouches.empty())
        return 0;
    
    Layer* pLayer = static_cast<Layer*>(touchesScriptData->pNativeObject);
    EventTouch::EventCode eventType = touchesScriptData->eActionType;
    const std::vector<Touch*>& touches = touchesScriptData->aTouches;
...
int ScriptingCore::handleTouchEvent(void* data)
{
    ...
    Layer* pLayer = static_cast<Layer*>(touchScriptData->pNativeObject);
    EventTouch::EventCode eventType = touchScriptData->eActionType;
    Touch *pTouch = touchScriptData->pTouch;
...
int ScriptingCore::handleAccelerometerEvent(void* data)
{
    ...
    Layer* layer = static_cast<Layer*>(basicScriptData->pNativeObject);
...
int ScriptingCore::handleKeypadEvent(void* data)
{
	...
	js_proxy_t * p = jsb_get_native_proxy(keypadScriptData->pNativeObject);
