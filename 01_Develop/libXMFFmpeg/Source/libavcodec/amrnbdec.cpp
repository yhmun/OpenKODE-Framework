...
#define AMR_TILT_GAMMA_T   0.8f
/** Adaptive gain control factor used in post-filter */
#define AMR_AGC_ALPHA      0.9f
...
{
    AMRContext *p = (AMRContext *)avctx->priv_data;
    int i;
                                  ...
                                  int buf_size)
{
    ...
    mode = (Mode) get_bits(&gb, 4);                  // frame type
...
static void lsf2lsp_3(AMRContext *p)
{
    ...
    lsf_quantizer = lsf_3_2[lsf_param[1] << (int16_t)(p->cur_frame_mode <= MODE_5k15)];
                                ...
                                const int subframe)
{
    ...
    pitch_lag_frac <<= (int)(p->cur_frame_mode != MODE_12k2);
                                ...
                                const enum Mode mode, const int subframe)
{
    ...
    if (mode == MODE_12k2) {
        ff_decode_10_pulses_35bits((const int16_t *)pulses, fixed_sparse, gray_decode, 5, 3);
    } else if (mode == MODE_10k2) {
        decode_8_pulses_31bits((const int16_t *)pulses, fixed_sparse);
    } else {
                              ...
                              int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    AMRContext *p = (AMRContext *)avctx->priv_data;        // pointer to private data
...
static const enum AVSampleFormat _ff_amrnb_decoder[] = {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_NONE};
