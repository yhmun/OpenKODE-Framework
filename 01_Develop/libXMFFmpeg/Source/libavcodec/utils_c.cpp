...
#include "XMFFmpeg/libavutil/samplefmt.h"
...
void *av_fast_realloc(void *ptr, unsigned int *size, KDsize min_size)
{
    if(min_size < *size)
        return ptr;
...
static inline int ff_fast_malloc(void *ptr, unsigned int *size, KDsize min_size, int zero_realloc)
{
    void **p = (void **)ptr;
    if (min_size < *size)
        return 0;
...
void av_fast_malloc(void *ptr, unsigned int *size, KDsize min_size)
{
    ff_fast_malloc(ptr, size, min_size, 0);
}

void av_fast_padded_malloc(void *ptr, unsigned int *size, KDsize min_size)
{
                             ...
                             int buf_size, int align)
{
    ...
    if (planar && nb_channels > AV_NUM_DATA_POINTERS) {
        if (!(frame->extended_data = (uint8_t **)av_mallocz(nb_channels *
    ...
    if ((ret = av_samples_fill_arrays(frame->extended_data, &frame->linesize[0],
                                      (uint8_t *)buf, nb_channels, frame->nb_samples,
                                      sample_fmt, align)) < 0) {
...
static int audio_get_buffer(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    if (!avci->buffer) {
        avci->buffer = (InternalBuffer *)av_mallocz(sizeof(InternalBuffer));
        if (!avci->buffer)
            return AVERROR(ENOMEM);
    }
    buf = avci->buffer;
    ...
    if (!buf->extended_data) {
        if (!buf->data[0]) {
            if (!(buf->data[0] = (uint8_t *)av_mallocz(buf_size)))
                return AVERROR(ENOMEM);
...
static int video_get_buffer(AVCodecContext *s, AVFrame *pic)
{
    ...
    if (!avci->buffer) {
        avci->buffer = (InternalBuffer *)av_mallocz((INTERNAL_BUFFER_SIZE+1) *
...
void avcodec_get_frame_defaults(AVFrame *pic){
    ...
    pic->pkt_pos = -1;
    pic->key_frame= 1;
    pic->sample_aspect_ratio = av_rational(0, 1);
...
int attribute_align_arg avcodec_open2(AVCodecContext *avctx, AVCodec *codec, AVDictionary **options)
{
    ...
    avctx->internal = (AVCodecInternal *)av_mallocz(sizeof(AVCodecInternal));
    if (!avctx->internal) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    ...
    if (codec->priv_data_size > 0) {
      if(!avctx->priv_data){
        ...
        if (codec->priv_class) {
            *(AVClass**)avctx->priv_data= (AVClass *)codec->priv_class;
        ...
        *got_packet_ptr = 0;
        ...
        if (!ret && *got_packet_ptr &&
            ...
            avpkt->duration = av_rescale_q(frame->nb_samples,
                                           av_rational( 1, avctx->sample_rate ),
                                           avctx->time_base);
        }
    } else {
                         ...
                         const AVFrame *pict)
{
    ...
    if((avctx->codec->capabilities & CODEC_CAP_DELAY) || pict){
        int ret = avctx->codec->encode(avctx, buf, (int)buf_size, (void*)pict);
        avctx->frame_number++;
                            ...
                            const AVSubtitle *sub)
{
    ...
    ret = avctx->codec->encode(avctx, buf, (int)buf_size, (void *)sub);
    avctx->frame_number++;
    return ret;
}
...
KDsize av_get_codec_tag_string(char *buf, KDsize buf_size, unsigned int codec_tag)
{
    int i, len, ret = 0;
