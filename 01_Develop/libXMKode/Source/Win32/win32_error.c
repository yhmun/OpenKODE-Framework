...
static KDint        l_errno = 0;

#endif

// This is the error table that defines the mapping between OS error codes and errno values
typedef struct XMErrEntry 
{
	unsigned long	oscode;							// OS return value 
	int				errnocode;						// System V error code 
} XMErrEntry;

static XMErrEntry   l_err_table[] = {
...
// size of the table 
#define ERRTABLESIZE                 ( sizeof ( l_err_table ) / sizeof ( l_err_table[ 0 ] ) )
...
KDvoid xmWinDosMapErr ( unsigned long oserrno )
{
	...
	{
		if ( oserrno == l_err_table[ i ].oscode )
		{
			err = l_err_table[i].errnocode;
			goto set;
		}
	}
