...
#include "XMFFmpeg/libavutil/pca.h"
...
PCA *ff_pca_init(int n){
    ...
    pca= (PCA*)av_mallocz(sizeof(PCA));
    pca->n= n;
    pca->count=0;
    pca->covariance= (double*)av_mallocz(sizeof(double)*n*n);
    pca->mean= (double*)av_mallocz(sizeof(double)*n);
...
int ff_pca(PCA *pca, double *eigenvector, double *eigenvalue){
    ...
    int k=0;
    const int n= pca->n;
    double* z = (double*)av_malloc(sizeof(double)*n);
