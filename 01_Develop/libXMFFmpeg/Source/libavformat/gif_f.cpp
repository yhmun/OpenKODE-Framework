                                  ...
                                  uint32_t *palette)
{
    ...
    avio_write(pb, (const unsigned char *)"GIF", 3);
    avio_write(pb, (const unsigned char *)"89a", 3);
    avio_wl16(pb, width);
    avio_wl16(pb, height);
    ...
    avio_w8(pb, 0xff);
    avio_w8(pb, 0x0b);
        avio_write(pb, (const unsigned char *)"NETSCAPE2.0", sizeof("NETSCAPE2.0") - 1);  // bytes 4 to 14
...
static av_always_inline unsigned char gif_clut_index(uint8_t r, uint8_t g, uint8_t b)
{
...
typedef struct {
    AVClass *_class;         /** Class for private options. */
...
{
    GIFContext *gif = (GIFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
...
static const AVOption options[] = {
    { "loop", "Number of times to loop the output.", OFFSET(loop), AV_OPT_TYPE_INT, 0, NULL, 0, 65535, ENC },
    { NULL },
};
