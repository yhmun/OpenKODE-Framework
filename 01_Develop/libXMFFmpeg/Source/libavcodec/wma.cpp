                          ...
                          const CoefVLCTable *vlc_table)
{
    ...
    run_table   = (uint16_t *)av_malloc(n * sizeof(uint16_t));
    level_table = (uint16_t *)av_malloc(n * sizeof(uint16_t));
    flevel_table= (float *)av_malloc(n * sizeof(*flevel_table));
    int_table   = (uint16_t *)av_malloc(n * sizeof(uint16_t));
    i = 2;
    level = 1;
    k = 0;
    while (i < n) {
...
int ff_wma_init(AVCodecContext *avctx, int flags2)
{
    ...
    if (s->use_noise_coding) {
...
#else
        {
            ...
            float norm;
            seed = 1;
            norm = (1.0 / (float)(1LL << 31)) * sqrt((double)3) * s->noise_mult;
            for (i = 0; i < NOISE_TAB_SIZE; i++) {
                seed = seed * 314159 + 1;
