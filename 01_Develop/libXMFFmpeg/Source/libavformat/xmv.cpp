...
{
    XMVDemuxContext *xmv = (XMVDemuxContext *)s->priv_data;
    AVIOContext     *pb  = s->pb;
    AVStream        *vst = NULL;
    ...
    xmv->audio = (XMVAudioPacket *)av_malloc(xmv->audio_track_count * sizeof(XMVAudioPacket));
    if (!xmv->audio)
        return AVERROR(ENOMEM);
