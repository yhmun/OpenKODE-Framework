                          ...
                          const SiprParameters *parm)
{
    ...
    lsfnew[LP_FILTER_ORDER - 1] *= (float)(6.153848 / M_PI);
}
...
static void postfilter_5k0(SiprContext *ctx, const float *lpc, float *samples)
{
    ...
    ff_tilt_compensation(&ctx->tilt_mem, 0.4f, pole_out, SUBFR_SIZE);
                         ...
                         float *out_data)
{
    ...
    for (i = 0; i < subframe_count; i++) {
        ...
        gain_code = ff_amr_set_fixed_gain(gain_cb[params->gc_index[i]][1],
                                          ...
                                          (float)(34 - 15.0/(0.05*M_LN10/M_LN2)),
                                          pred);
    ...
    if (ctx->mode == MODE_5k0) {
        for (i = 0; i < subframe_count; i++) {
            ...
            ff_adaptive_gain_control(&synth[i * SUBFR_SIZE],
                                     ...
                                     SUBFR_SIZE, 0.9f, &ctx->postfilter_agc);
        }
	...
	{
		const float _t1[2] = {-1.99997f   , 1.000000000f};
		const float _t2[2] = {-1.93307352f, 0.935891986f};
...
{
    SiprContext *ctx = (SiprContext *)avctx->priv_data;
    int i;
