...
static const float quant_lut_mul[7] = { 0.0f,  0.0f,  2.0f,  2.0f,  5.0f, 12.0f,  36.6f };
static const float quant_lut_add[7] = { 0.0f,  0.0f,  2.0f,  7.0f, 21.0f, 56.0f, 157.0f };
...
{
    NellyMoserEncodeContext *s = (NellyMoserEncodeContext *)avctx->priv_data;
    int i;
    ...
    if (s->avctx->trellis) {
        s->opt  = (float (*)[23])av_malloc(NELLY_BANDS * OPT_SIZE * sizeof(float  ));
        s->path = (uint8_t (*)[23])av_malloc(NELLY_BANDS * OPT_SIZE * sizeof(uint8_t));
    }
...
static const enum AVSampleFormat _ff_nellymoser_encoder[] = {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_NONE};
AVCodec ff_nellymoser_encoder = {
	...
	/*.sample_fmts			= */	_ff_nellymoser_encoder,
};
