...
{
    BinkAudioContext *s = (BinkAudioContext *)avctx->priv_data;
    ...
    s->root          = 2.0 / sqrt((double)s->frame_len);
    for (i = 0; i < 96; i++) {
    ...
    s->bands = (unsigned int *)av_malloc((s->num_bands + 1) * sizeof(*s->bands));
    if (!s->bands)
                        ...
                        int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    if (!get_bits_left(gb)) {
        ...
        buf = (uint8_t *)av_realloc(s->packet_buffer, avpkt->size + FF_INPUT_BUFFER_PADDING_SIZE);
        if (!buf)
            return AVERROR(ENOMEM);
        s->packet_buffer = buf;
