 ...
 * esoteric implementations of KDVaListKHR (i.e. it may need
...
xmlTextReaderFreeProp(xmlTextReaderPtr reader, xmlAttrPtr cur) {
    ...
    if ((cur->parent != KD_NULL) && (cur->parent->doc != KD_NULL) &&
	((cur->parent->doc->intSubset != KD_NULL) ||
	 (cur->parent->doc->extSubset != KD_NULL))) {
    ...
    DICT_FREE(cur->name);
    if ((reader != KD_NULL) && (reader->ctxt != KD_NULL) &&
	...
	if (cur->type != XML_DTD_NODE) {
	    ...
	    if (((cur->type == XML_ELEMENT_NODE) ||
		 (cur->type == XML_TEXT_NODE)) &&
	        (reader != KD_NULL) && (reader->ctxt != KD_NULL) &&
...
xmlTextReaderFreeDoc(xmlTextReaderPtr reader, xmlDocPtr cur) {
     ...
     */
    if (cur->ids != KD_NULL) xmlTextReaderFreeIDTable((xmlIDTablePtr) cur->ids);
    cur->ids = KD_NULL;
    ...
    if (cur->children != KD_NULL) xmlTextReaderFreeNodeList(reader, cur->children);

    if (cur->version != KD_NULL) xmlFree((char *) cur->version);
    if (cur->name != KD_NULL) xmlFree((char *) cur->name);
    if (cur->encoding != KD_NULL) xmlFree((char *) cur->encoding);
    if (cur->oldNs != KD_NULL) xmlFreeNsList(cur->oldNs);
    if (cur->URL != KD_NULL) xmlFree((char *) cur->URL);
    if (cur->dict != KD_NULL) xmlDictFree(cur->dict);
...
xmlTextReaderEntPop(xmlTextReaderPtr reader)
{
    ...
    reader->entTab[reader->entNr] = KD_NULL;
    return (ret);
}
 ...
 * @atts:  An array of name/value attributes pairs, KD_NULL terminated
 *
...
#endif
    if ((reader != KD_NULL) && (reader->startElement != KD_NULL)) {
	reader->startElement(ctx, fullname, atts);
	if ((ctxt->node != KD_NULL) && (ctxt->input != KD_NULL) &&
	    (ctxt->input->cur != KD_NULL) && (ctxt->input->cur[0] == '/') &&
	    (ctxt->input->cur[1] == '>'))
...
#endif
    if ((reader != KD_NULL) && (reader->endElement != KD_NULL)) {
...
#endif
    if ((reader != KD_NULL) && (reader->startElementNs != KD_NULL)) {
...
#endif
    if ((reader != KD_NULL) && (reader->endElementNs != KD_NULL)) {
...
#endif
    if ((reader != KD_NULL) && (reader->characters != KD_NULL)) {
...
#endif
    if ((reader != KD_NULL) && (reader->cdataBlock != KD_NULL)) {
...
xmlTextReaderPushData(xmlTextReaderPtr reader) {
    ...
    if ((reader->input == KD_NULL) || (reader->input->buffer == KD_NULL))
	return(-1);
...
xmlTextReaderValidatePush(xmlTextReaderPtr reader ATTRIBUTE_UNUSED) {
    ...
    if ((reader->validate == XML_TEXTREADER_VALIDATE_DTD) &&
        (reader->ctxt != KD_NULL) && (reader->ctxt->validate == 1)) {
    ...
    if ((reader->validate == XML_TEXTREADER_VALIDATE_RNG) &&
               (reader->rngValidCtxt != KD_NULL)) {
	int ret;

	if (reader->rngFullNode != KD_NULL) return;
...
xmlTextReaderValidatePop(xmlTextReaderPtr reader) {
	...
	if (reader->rngFullNode != KD_NULL) {
	    if (node == reader->rngFullNode)
	        reader->rngFullNode = KD_NULL;
	    return;
	}
...
xmlTextReaderValidateEntity(xmlTextReaderPtr reader) {
	...
	if (node->type == XML_ENTITY_REF_NODE) {
	     ...
	     * and walk it.
	     */
	    if ((node->children == KD_NULL) && (ctxt->sax != KD_NULL) &&
		...
		(node->children->children != KD_NULL)) {
	...
	do {
		...
		(reader->ent != KD_NULL) && (reader->ent->children == node)) {
	...
	} while ((node != KD_NULL) && (node != oldnode));
    } while ((node != KD_NULL) && (node != oldnode));
    reader->node = oldnode;
}
 ...
 *
 * Returns the successor node or KD_NULL
 */
static xmlNodePtr
...
xmlTextReaderDoExpand(xmlTextReaderPtr reader) {
    ...
    if ((reader == KD_NULL) || (reader->node == KD_NULL) || (reader->ctxt == KD_NULL))
        return(-1);
    do {
	if (reader->ctxt->instate == XML_PARSER_EOF) return(1);

        if (xmlTextReaderGetSuccessor(reader->node) != KD_NULL)
	    return(1);
 ...
 *
 *  Returns a string containing the content, or KD_NULL in case of error.
 */
static xmlChar *
xmlTextReaderCollectSiblings(xmlNodePtr node)
{
    ...
    for ( ; node != KD_NULL; node = node->next) {
       switch (node->type) {
       case XML_TEXT_NODE:
...
xmlTextReaderRead(xmlTextReaderPtr reader) {
    ...
    xmlNodePtr oldnode = KD_NULL;


    if (reader == KD_NULL)
	return(-1);
    reader->curnode = KD_NULL;
    if (reader->doc != KD_NULL)
        return(xmlTextReaderReadTree(reader));
    if (reader->ctxt == KD_NULL)
	return(-1);
	...
	} while ((reader->ctxt->node == KD_NULL) &&
		 ((reader->mode != XML_TEXTREADER_MODE_EOF) &&
		  (reader->state != XML_TEXTREADER_DONE)));
	if (reader->ctxt->node == KD_NULL) {
	    if (reader->ctxt->myDoc != KD_NULL) {
...
get_next_node:
     ...
     */
    while ((reader->node != KD_NULL) && (reader->node->next == KD_NULL) &&
	   (reader->ctxt->nodeNr == olddepth) &&
           ((oldstate == XML_TEXTREADER_BACKTRACK) ||
            (reader->node->children == KD_NULL) ||
	    (reader->node->type == XML_ENTITY_REF_NODE) ||
	    ((reader->node->children != KD_NULL) &&
	     (reader->node->children->type == XML_TEXT_NODE) &&
	     (reader->node->children->next == KD_NULL)) ||
	    (reader->node->type == XML_DTD_NODE) ||
	    (reader->node->type == XML_DOCUMENT_NODE) ||
	    (reader->node->type == XML_HTML_DOCUMENT_NODE)) &&
	   ((reader->ctxt->node == KD_NULL) ||
	...
	if ((reader->node->children != KD_NULL) &&
	    (reader->node->type != XML_ENTITY_REF_NODE) &&
	 ...
	 * Cleanup of the old node
	 */
	if ((oldnode != KD_NULL) && (reader->preserves == 0) &&
#ifdef LIBXML_XINCLUDE_ENABLED
	    (reader->in_xinclude == 0) &&
#endif
	    (reader->entNr == 0) &&
...
node_found:
     ...
     */
    if ((reader->node != KD_NULL) &&
        (reader->node->next == KD_NULL) &&
        ((reader->node->type == XML_TEXT_NODE) ||
	 (reader->node->type == XML_CDATA_SECTION_NODE))) {
            if (xmlTextReaderExpand(reader) == KD_NULL)
	        return -1;
    }
     ...
     */
    if ((reader->xinclude) && (reader->node != KD_NULL) &&
	(reader->node->type == XML_ELEMENT_NODE) &&
	(reader->node->ns != KD_NULL) &&
	((xmlStrEqual(reader->node->ns->href, XINCLUDE_NS)) ||
	 (xmlStrEqual(reader->node->ns->href, XINCLUDE_OLD_NS)))) {
	if (reader->xincctxt == KD_NULL) {
    ...
    if ((reader->node != KD_NULL) && (reader->node->type == XML_XINCLUDE_START)) {
        reader->in_xinclude++;
	goto get_next_node;
    }
    if ((reader->node != KD_NULL) && (reader->node->type == XML_XINCLUDE_END)) {
        reader->in_xinclude--;
	goto get_next_node;
    }
#endif
    ...
    if ((reader->node != KD_NULL) &&
	(reader->node->type == XML_ENTITY_REF_NODE) &&
	(reader->ctxt != KD_NULL) && (reader->ctxt->replaceEntities == 1)) {
	/*
	 * Case where the underlying tree is not availble, lookup the entity
	 * and walk it.
	 */
	if ((reader->node->children == KD_NULL) && (reader->ctxt->sax != KD_NULL) &&
	    (reader->ctxt->sax->getEntity != KD_NULL)) {
	...
	if ((reader->node->children != KD_NULL) &&
	    ...
	    (reader->node->children->children != KD_NULL)) {
	    xmlTextReaderEntPush(reader, reader->node);
	    reader->node = reader->node->children->children;
	}
#ifdef LIBXML_REGEXP_ENABLED
    } else if ((reader->node != KD_NULL) &&
	       (reader->node->type == XML_ENTITY_REF_NODE) &&
	       (reader->ctxt != KD_NULL) && (reader->validate)) {
	xmlTextReaderValidateEntity(reader);
#endif /* LIBXML_REGEXP_ENABLED */
    }
    if ((reader->node != KD_NULL) &&
	(reader->node->type == XML_ENTITY_DECL) &&
	(reader->ent != KD_NULL) && (reader->ent->children == reader->node)) {
	reader->node = xmlTextReaderEntPop(reader);
	reader->depth++;
        goto get_next_node;
    }
#ifdef LIBXML_REGEXP_ENABLED
    if ((reader->validate) && (reader->node != KD_NULL)) {
	xmlNodePtr node = reader->node;
 ...
 *         or KD_NULL in case of error.
 */
xmlNodePtr
xmlTextReaderExpand(xmlTextReaderPtr reader) {
    if ((reader == KD_NULL) || (reader->node == KD_NULL))
        return(KD_NULL);
    if (reader->doc != KD_NULL)
        return(reader->node);
    if (reader->ctxt == KD_NULL)
...
xmlTextReaderNext(xmlTextReaderPtr reader) {
    ...
    cur = reader->node;
    if ((cur == KD_NULL) || (cur->type != XML_ELEMENT_NODE))
 ...
 *
 * Returns a string containing the XML content, or KD_NULL if the current node
...
/*
    ...
    for (cur_node = reader->node->children; cur_node != KD_NULL;
 ...
 * Reads the contents of the current node, including child nodes and markup.
 *
 * Returns a string containing the node and any XML content, or KD_NULL if the 
 ...
 *         or KD_NULL if the reader is positioned on any other type of node.
...
xmlTextReaderReadString(xmlTextReaderPtr reader)
{
    ...
    node = (reader->curnode != KD_NULL) ? reader->curnode : reader->node;
    switch (node->type) {
    case XML_TEXT_NODE:
...
xmlTextReaderReadBase64(xmlTextReaderPtr reader,
    ...
    if ((reader->node == KD_NULL) || (reader->node->type == XML_ELEMENT_NODE))
	return(0);
    TODO
    return(0);
}
...
xmlTextReaderNextTree(xmlTextReaderPtr reader)
{
    ...
    if (reader->node == KD_NULL) {
        if (reader->doc->children == KD_NULL) {
            reader->state = XML_TEXTREADER_END;
            return(0);
        }
	...
	/* if reader->node->next is KD_NULL mean no subtree for current node,
 ...
 *
 * Returns the new xmlTextReaderPtr or KD_NULL in case of error
 */
xmlTextReaderPtr
xmlNewTextReader(xmlParserInputBufferPtr input, const char *URI) {
    ...
    kdMemset(ret, 0, sizeof(xmlTextReader));
    ret->doc = KD_NULL;
    ret->entTab = KD_NULL;
    ret->entMax = 0;
    ret->entNr = 0;
	...
	ret->startElementNs = KD_NULL;
	ret->endElementNs = KD_NULL;
    }
#endif /* LIBXML_SAX1_ENABLED */
    ...
    if (ret->input->buffer->use >= 4) {
	ret->ctxt = xmlCreatePushParserCtxt(ret->sax, KD_NULL,
			(const char *) ret->input->buffer->content, 4, URI);
	ret->base = 0;
	ret->cur = 4;
    } else {
	ret->ctxt = xmlCreatePushParserCtxt(ret->sax, KD_NULL, KD_NULL, 0, URI);
	ret->base = 0;
	ret->cur = 0;
    }
...
xmlNewTextReaderFilename(const char *URI) {
    ...
    if (ret->ctxt->directory == KD_NULL)
        directory = xmlParserGetDirectory(URI);
    if ((ret->ctxt->directory == KD_NULL) && (directory != KD_NULL))
        ret->ctxt->directory = (char *) xmlStrdup((xmlChar *) directory);
    if (directory != KD_NULL)
	xmlFree(directory);
    return(ret);
}
...
xmlFreeTextReader(xmlTextReaderPtr reader) {
	...
	return;
#ifdef LIBXML_SCHEMAS_ENABLED
    if (reader->rngSchemas != KD_NULL) {
	xmlRelaxNGFree(reader->rngSchemas);
	reader->rngSchemas = KD_NULL;
    }
    if (reader->rngValidCtxt != KD_NULL) {
	xmlRelaxNGFreeValidCtxt(reader->rngValidCtxt);
	reader->rngValidCtxt = KD_NULL;
    }
    if (reader->xsdPlug != KD_NULL) {
	xmlSchemaSAXUnplug(reader->xsdPlug);
	reader->xsdPlug = KD_NULL;
    }
    if (reader->xsdValidCtxt != KD_NULL) {
	if (! reader->xsdPreserveCtxt)
	    xmlSchemaFreeValidCtxt(reader->xsdValidCtxt);
	reader->xsdValidCtxt = KD_NULL;
    }
    if (reader->xsdSchemas != KD_NULL) {
	xmlSchemaFree(reader->xsdSchemas);
	reader->xsdSchemas = KD_NULL;
    }
#endif
#ifdef LIBXML_XINCLUDE_ENABLED
    if (reader->xincctxt != KD_NULL)
...
#endif
#ifdef LIBXML_PATTERN_ENABLED
    if (reader->patternTab != KD_NULL) {
        int i;
	for (i = 0;i < reader->patternNr;i++) {
	    if (reader->patternTab[i] != KD_NULL)
	        xmlFreePattern(reader->patternTab[i]);
	}
	xmlFree(reader->patternTab);
    }
#endif
    if (reader->faketext != KD_NULL) {
	...
	if ((reader->ctxt->vctxt.vstateTab != KD_NULL) &&
	    (reader->ctxt->vctxt.vstateMax > 0)){
	    xmlFree(reader->ctxt->vctxt.vstateTab);
	    reader->ctxt->vctxt.vstateTab = KD_NULL;
	    reader->ctxt->vctxt.vstateMax = 0;
	}
	if (reader->allocs & XML_TEXTREADER_CTXT)
	    xmlFreeParserCtxt(reader->ctxt);
    }
    if (reader->sax != KD_NULL)
	xmlFree(reader->sax);
    if ((reader->input != KD_NULL)  && (reader->allocs & XML_TEXTREADER_INPUT))
	xmlFreeParserInputBuffer(reader->input);
    if (reader->buffer != KD_NULL)
        xmlBufferFree(reader->buffer);
    if (reader->entTab != KD_NULL)
	xmlFree(reader->entTab);
    if (reader->dict != KD_NULL)
 ...
 *
 * Returns a string containing the value of the specified attribute, or KD_NULL
...
xmlTextReaderGetAttributeNo(xmlTextReaderPtr reader, int no) {
    ...
    if (ret == KD_NULL) return(xmlStrdup((xmlChar *)""));
    return(ret);
}
...
xmlTextReaderGetAttribute(xmlTextReaderPtr reader, const xmlChar *name) {
    ...
    if ((reader == KD_NULL) || (name == KD_NULL))
	return(KD_NULL);
    if (reader->node == KD_NULL)
	return(KD_NULL);
    if (reader->curnode != KD_NULL)
    ...
    if (xmlStrEqual(prefix, BAD_CAST "xmlns")) {
		ns = reader->node->nsDef;
		while (ns != KD_NULL) {
			if ((ns->prefix != KD_NULL) && (xmlStrEqual(ns->prefix, localname))) {
...
xmlTextReaderGetAttributeNs(xmlTextReaderPtr reader, const xmlChar *localName,
    ...
    if ((reader == KD_NULL) || (localName == KD_NULL))
	return(KD_NULL);
    if (reader->node == KD_NULL)
	return(KD_NULL);
    if (reader->curnode != KD_NULL)
    ...
    if (xmlStrEqual(namespaceURI, BAD_CAST "http://www.w3.org/2000/xmlns/")) {
		...
		ns = reader->node->nsDef;
		while (ns != KD_NULL) {
			if ((prefix == KD_NULL && ns->prefix == KD_NULL) ||
				((ns->prefix != KD_NULL) && (xmlStrEqual(ns->prefix, localName)))) {
				return xmlStrdup(ns->href);
			}
			ns = ns->next;
		}
 ...
 *
 * Returns the xmlParserInputBufferPtr attached to the XML or KD_NULL
 *    in case of error.
 */
xmlParserInputBufferPtr
xmlTextReaderGetRemainder(xmlTextReaderPtr reader) {
    xmlParserInputBufferPtr ret = KD_NULL;
 ...
 *          the default namespace, specify KD_NULL
 *
 * Resolves a namespace prefix in the scope of the current element.
 *
 * Returns a string containing the namespace URI to which the prefix maps
 *    or KD_NULL in case of error. The string must be deallocated by the caller.
 */
xmlChar *
...
xmlTextReaderMoveToAttribute(xmlTextReaderPtr reader, const xmlChar *name) {
	...
	while (prop != KD_NULL) {
		...
		((prop->ns == KD_NULL) || (prop->ns->prefix == KD_NULL))) {
	...
	if ((xmlStrEqual(prop->name, localname)) &&
	    (prop->ns != KD_NULL) && (xmlStrEqual(prop->ns->prefix, prefix))) {
	    reader->curnode = (xmlNodePtr) prop;
	    goto found;
	}
	prop = prop->next;
...
xmlTextReaderMoveToAttributeNs(xmlTextReaderPtr reader,
    ...
    if ((reader == KD_NULL) || (localName == KD_NULL) || (namespaceURI == KD_NULL))
	return(-1);
    if (reader->node == KD_NULL)
	return(-1);
    if (reader->node->type != XML_ELEMENT_NODE)
...
xmlTextReaderMoveToFirstAttribute(xmlTextReaderPtr reader) {
    ...
    if (reader->node->nsDef != KD_NULL) {
	reader->curnode = (xmlNodePtr) reader->node->nsDef;
	return(1);
    }
    if (reader->node->properties != KD_NULL) {
...
xmlTextReaderReadAttributeValue(xmlTextReaderPtr reader) {
	...
	if (reader->curnode->children == KD_NULL)
	    return(0);
	...
	if (reader->faketext == KD_NULL) {
	    reader->faketext = xmlNewDocText(reader->node->doc,
            ...
            if ((reader->faketext->content != KD_NULL) &&
	...
	if (reader->curnode->next == KD_NULL)
	    return(0);
	reader->curnode = reader->curnode->next;
    }
    return(1);
 ...
 *
 * Returns a string containing the encoding of the document or KD_NULL in
 ...
 *
 * Returns the local name or KD_NULL if not available,
 *   if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
 ...
 * The local name of the node.
 *
 * Returns the local name or KD_NULL if not available, the
 ...
 * The qualified name of the node, equal to Prefix :LocalName.
 *
 * Returns the local name or KD_NULL if not available, the string is
 ...
 *
 * Returns the prefix or KD_NULL if not available,
 *    if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
 ...
 * A shorthand reference to the namespace associated with the node.
 *
 * Returns the prefix or KD_NULL if not available, the string is deallocated
 *         with the reader.
 */
const xmlChar *
 ...
 *
 * Returns the namespace URI or KD_NULL if not available,
 *    if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
 ...
 * The URI defining the namespace associated with the node.
 *
 * Returns the namespace URI or KD_NULL if not available, the string
 ...
 *
 * Returns the base URI or KD_NULL if not available,
 *    if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
xmlTextReaderBaseUri(xmlTextReaderPtr reader) {
    if ((reader == KD_NULL) || (reader->node == KD_NULL))
	return(KD_NULL);
    return(xmlNodeGetBase(KD_NULL, reader->node));
}
 ...
 * The base URI of the node.
 *
 * Returns the base URI or KD_NULL if not available, the string
 *         will be deallocated with the reader
 */
const xmlChar *
xmlTextReaderConstBaseUri(xmlTextReaderPtr reader) {
    ...
    if ((reader == KD_NULL) || (reader->node == KD_NULL))
	return(KD_NULL);
    tmp = xmlNodeGetBase(KD_NULL, reader->node);
    if (tmp == KD_NULL)
        return(KD_NULL);
    ret = CONSTSTR(tmp);
    xmlFree(tmp);
...
xmlTextReaderHasAttributes(xmlTextReaderPtr reader) {
    ...
    if ((node->type == XML_ELEMENT_NODE) &&
	((node->properties != KD_NULL) || (node->nsDef != KD_NULL)))
	return(1);
    /* TODO: handle the xmlDecl */
    return(0);
}
 ...
 *
 * Returns the string or KD_NULL if not available. The result must be deallocated
 *     with xmlFree()
 */
xmlChar *
xmlTextReaderValue(xmlTextReaderPtr reader) {
    ...
    switch (node->type) {
	    ...
	    if (attr->parent != KD_NULL)
		return (xmlNodeListGetString
			(attr->parent->doc, attr->children, 1));
	    else
		return (xmlNodeListGetString(KD_NULL, attr->children, 1));
	    break;
	}
        case XML_TEXT_NODE:
        case XML_CDATA_SECTION_NODE:
 ...
 * Provides the text value of the node if present
 *
 * Returns the string or KD_NULL if not available. The result will be
 ...
 *
 * Returns the xml:lang value or KD_NULL if none exists.,
 *    if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
 ...
 *
 * Returns an interned copy of the string or KD_NULL in case of error. The
 *         string will be deallocated with the reader.
 */
const xmlChar *
 ...
 *
 * Returns the xmlNodePtr or KD_NULL in case of error.
 */
xmlNodePtr
 ...
 * @namespaces: the prefix definitions, array of [URI, prefix] or KD_NULL
 *
                             ...
                             const xmlChar **namespaces)
{
    ...
    if ((reader == KD_NULL) || (pattern == KD_NULL))
	return(-1);
 ...
 *
 * Returns the xmlDocPtr or KD_NULL in case of error.
 */
xmlDocPtr
xmlTextReaderCurrentDoc(xmlTextReaderPtr reader) {
    ...
    if (reader->doc != KD_NULL)
        return(reader->doc);
    if ((reader->ctxt == KD_NULL) || (reader->ctxt->myDoc == KD_NULL))
	return(KD_NULL);
...
#ifdef LIBXML_SCHEMAS_ENABLED
static char *xmlTextReaderBuildMessage(const char *msg, KDVaListKHR ap);
 ...
 * if @schema is KD_NULL, then RelaxNG validation is desactivated.
 ...
 * Activation is only possible before the first Read().
 * if @schema is KD_NULL, then Schema validation is desactivated.
 @ The @schema should not be freed until the reader is deallocated
 * or its use has been deactivated.
 *
 ...
 * @reader:  the xmlTextReaderPtr used
 * @rng:  the path to a RelaxNG schema or KD_NULL
 *
 * Use RelaxNG to validate the document as it is processed.
 * Activation is only possible before the first Read().
 * if @rng is KD_NULL, then RelaxNG validation is deactivated.
 *
 ...
 * @reader:  the xmlTextReaderPtr used
 * @xsd:  the path to a W3C XSD schema or KD_NULL
 * @ctxt: the XML Schema validation context or KD_NULL
 * @options: options (not used yet)
 *
 * Validate the document as it is processed using XML Schema.
 * Activation is only possible before the first Read().
 * If both @xsd and @ctxt are KD_NULL then XML Schema validation is deactivated.
 *
 ...
 * Activation is only possible before the first Read().
 * If @ctxt is KD_NULL, then XML Schema validation is deactivated.
 *
 * Returns 0 in case the schemas validation could be (de)activated and
 *         -1 in case of error.
 */
...
xmlTextReaderSchemaValidateCtxt(xmlTextReaderPtr reader,
				    ...
				    int options)
{
    return(xmlTextReaderSchemaValidateInternal(reader, KD_NULL, ctxt, options));
}
 ...
 * Activation is only possible before the first Read().
 * If @xsd is KD_NULL, then XML Schema validation is deactivated.
 *
 * Returns 0 in case the schemas validation could be (de)activated and
 *         -1 in case of error.
 */
...
{
    return(xmlTextReaderSchemaValidateInternal(reader, xsd, KD_NULL, 0));
}
#endif
 ...
 *
 * Returns a string containing the XML version of the document or KD_NULL
...
/* helper to build a xmlMalloc'ed string from a format and KDVaListKHR */
static char *
xmlTextReaderBuildMessage(const char *msg, KDVaListKHR ap) {
    int size = 0;
    int chars;
    char *larger;
    char *str = KD_NULL;
    ...
    while (1) {		
        KD_VA_COPY(aq, ap);
        chars = kdVsnprintfKHR ( str, size, msg, aq );
        KD_VA_END_KHR ( aq );
        if (chars < 0) {
	...
	size = chars + 1;
        ...
        if ((larger = (char *) xmlRealloc(str, size)) == KD_NULL) {
	    xmlGenericError(xmlGenericErrorContext, "xmlRealloc failed !\n");
	    if (str)
                xmlFree(str);
            return KD_NULL;
...
xmlTextReaderLocatorLineNumber(xmlTextReaderLocatorPtr locator) {
	...
	input = ctx->input;
	if ((input->filename == KD_NULL) && (ctx->inputNr > 1))
 ...
 *
 * Returns the base URI or KD_NULL in case of error,
 *    if non KD_NULL it need to be freed by the caller.
 */
xmlChar *
xmlTextReaderLocatorBaseURI(xmlTextReaderLocatorPtr locator) {
    ...
    if (locator == KD_NULL)
        return(KD_NULL);
    if (ctx->node != KD_NULL) {
	ret = xmlNodeGetBase(KD_NULL,ctx->node);
    }
    else {
	/* inspired from error.c */
	xmlParserInputPtr input;
 ...
 *
 * If @f is KD_NULL, the default error and warning handlers are restored.
 */
void
                             ...
                             xmlTextReaderErrorFunc f, void *arg)
{
    if (f != KD_NULL) {
        ...
        reader->ctxt->sax->serror = KD_NULL;
        ...
        if (reader->rngValidCtxt) {
            xmlRelaxNGSetValidErrors(reader->rngValidCtxt,
                                     ...
                                     xmlTextReaderValidityWarningRelay,
                                     reader);
            xmlRelaxNGSetValidStructuredErrors(reader->rngValidCtxt, KD_NULL,
                                               reader);
        }
        if (reader->xsdValidCtxt) {
            xmlSchemaSetValidErrors(reader->xsdValidCtxt,
                                    ...
                                    xmlTextReaderValidityWarningRelay,
                                    reader);
            xmlSchemaSetValidStructuredErrors(reader->xsdValidCtxt, KD_NULL,
                                              reader);
        }
#endif
    } else {
        ...
        reader->errorFunc = KD_NULL;
        reader->sErrorFunc = KD_NULL;
        reader->errorFuncArg = KD_NULL;
#ifdef LIBXML_SCHEMAS_ENABLED
        if (reader->rngValidCtxt) {
            xmlRelaxNGSetValidErrors(reader->rngValidCtxt, KD_NULL, KD_NULL,
                                     reader);
            xmlRelaxNGSetValidStructuredErrors(reader->rngValidCtxt, KD_NULL,
                                               reader);
        }
        if (reader->xsdValidCtxt) {
            xmlSchemaSetValidErrors(reader->xsdValidCtxt, KD_NULL, KD_NULL,
                                    reader);
            xmlSchemaSetValidStructuredErrors(reader->xsdValidCtxt, KD_NULL,
                                              reader);
        }
...
#endif
    if ((reader->ctxt != KD_NULL) && (reader->ctxt->validate == 1))
 ...
 * @reader:  the xmlTextReaderPtr used
 * @f:	the callback function or KD_NULL is no callback has been registered
 * @arg:    a user argument
 *
                   ...
                   const char *encoding, int options)
{
    ...
    if ((input != KD_NULL) && (reader->input != KD_NULL) &&
        (reader->allocs & XML_TEXTREADER_INPUT)) {
	xmlFreeParserInputBuffer(reader->input);
	reader->input = KD_NULL;
	reader->allocs -= XML_TEXTREADER_INPUT;
    ...
    if (reader->sax->initialized == XML_SAX2_MAGIC) {
        ...
        reader->startElementNs = KD_NULL;
        reader->endElementNs = KD_NULL;
    }
#endif /* LIBXML_SAX1_ENABLED */
    ...
    if (input != KD_NULL) {
        ...
        if (reader->ctxt == KD_NULL) {
            if (reader->input->buffer->use >= 4) {
                reader->ctxt = xmlCreatePushParserCtxt(reader->sax, KD_NULL,
		       (const char *) reader->input->buffer->content, 4, URL);
                reader->base = 0;
                reader->cur = 4;
            } else {
                reader->ctxt =
                    xmlCreatePushParserCtxt(reader->sax, KD_NULL, KD_NULL, 0, URL);
                reader->base = 0;
                reader->cur = 0;
            }
        } else {
    ...
    while (reader->patternNr > 0) {
        reader->patternNr--;
	if (reader->patternTab[reader->patternNr] != KD_NULL) {
	    xmlFreePattern(reader->patternTab[reader->patternNr]);
            reader->patternTab[reader->patternNr] = KD_NULL;
	}
    }
#endif
    ...
    if ((URL != KD_NULL) && (reader->ctxt->input != KD_NULL) &&
        (reader->ctxt->input->filename == KD_NULL))
 ...
 *
 * Returns the new reader or KD_NULL in case of error.
 */
xmlTextReaderPtr
...
xmlReaderForFile(const char *filename, const char *encoding, int options)
{
    ...
    if (reader == KD_NULL)
        return (KD_NULL);
    xmlTextReaderSetup(reader, KD_NULL, KD_NULL, encoding, options);
    return (reader);
}
                   ...
                   const char *encoding, int options)
{
    ...
    reader->allocs |= XML_TEXTREADER_INPUT;
    xmlTextReaderSetup(reader, KD_NULL, URL, encoding, options);
    return (reader);
}
...
xmlReaderNewWalker(xmlTextReaderPtr reader, xmlDocPtr doc)
{
    ...
    reader->state = XML_TEXTREADER_START;
    if (reader->dict == KD_NULL) {
        if ((reader->ctxt != KD_NULL) && (reader->ctxt->dict != KD_NULL))
	    reader->dict = reader->ctxt->dict;
	else
                ...
                unsigned char *to, unsigned long *tolen)
{
    ...
    if ((in == KD_NULL) || (inlen == KD_NULL) || (to == KD_NULL) || (tolen == KD_NULL))
        return (-1);
