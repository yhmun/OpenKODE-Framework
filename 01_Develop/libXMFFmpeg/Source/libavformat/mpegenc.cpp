...
#include "XMFFmpeg/libavutil/fifo.h"
...
{
    MpegMuxContext *s = (MpegMuxContext *)ctx->priv_data;
    PutBitContext pb;
...
static int put_system_header(AVFormatContext *ctx, uint8_t *buf,int only_for_stream_id)
{
    ...
    if (s->is_dvd) {
        ...
        for(i=0;i<ctx->nb_streams;i++) {
            StreamInfo *stream = (StreamInfo *)ctx->streams[i]->priv_data;
...
static int get_system_header_size(AVFormatContext *ctx)
{
    ...
    private_stream_coded = 0;
    for(i=0;i<ctx->nb_streams;i++) {
        stream = (StreamInfo *)ctx->streams[i]->priv_data;
        if (stream->id < 0xc0) {
            if (private_stream_coded)
                continue;
            private_stream_coded = 1;
...
static int mpeg_mux_init(AVFormatContext *ctx)
{
    ...
    for(i=0;i<ctx->nb_streams;i++) {
        st = ctx->streams[i];
        stream = (StreamInfo *)av_mallocz(sizeof(StreamInfo));
        if (!stream)
            goto fail;
        st->priv_data = stream;
...
static av_always_inline void put_timestamp(AVIOContext *pb, int id, int64_t timestamp)
{
    avio_w8(pb,
             (id << 4) |
             (((timestamp >> 30) & 0x07) << 1) |
...
{
    MpegMuxContext *s = (MpegMuxContext *)ctx->priv_data;
    StreamInfo *stream = (StreamInfo *)ctx->streams[stream_index]->priv_data;
    uint8_t *buf_ptr;
    ...
    if (packet_size > 0) {
        ...
        av_fifo_generic_read(stream->fifo, ctx->pb, payload_size - stuffing_size, (void (*)(void *,void *,int))avio_write);
...
//av_log(ctx, AV_LOG_DEBUG, "dts:%f pts:%f flags:%d stream:%d nopts:%d\n", dts/90000.0, pts/90000.0, pkt->flags, pkt->stream_index, pts != AV_NOPTS_VALUE);
    ...
    *stream->next_packet=
    pkt_desc= (PacketDesc *)av_mallocz(sizeof(PacketDesc));
    pkt_desc->pts= pts;
    pkt_desc->dts= dts;
...
static const AVOption options[] = {
    { "muxrate", NULL, OFFSET(mux_rate), AV_OPT_TYPE_INT, 0, NULL, 0, INT_MAX, E },
    { "preload", "Initial demux-decode delay in microseconds.", OFFSET(preload),  AV_OPT_TYPE_INT, 500000, NULL, 0, INT_MAX, E},
    { NULL },
};
