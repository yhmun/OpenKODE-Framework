...
{
    GXFStreamContext *sc = (GXFStreamContext *)st->priv_data;
    int i;
...
{
    GXFContext *gxf = (GXFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    int64_t pos;
    ...
    avio_write(pb, (const unsigned char *)ES_NAME_PATTERN, sizeof(ES_NAME_PATTERN) - 1);
...
static int gxf_write_material_data_section(AVFormatContext *s)
{
    ...
    avio_write(pb, (const unsigned char *)SERVER_PATH, sizeof(SERVER_PATH) - 1);
    avio_write(pb, (const unsigned char *)filename, len);
    avio_w8(pb, 0);
...
static int gxf_write_track_description_section(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; ++i)
        gxf_write_track_description(s, (GXFStreamContext *)s->streams[i]->priv_data, i);
...
static int gxf_write_map_packet(AVFormatContext *s, int rewrite)
{
    ...
    if (!rewrite) {
        if (!(gxf->map_offsets_nb % 30)) {
            gxf->map_offsets = (uint64_t *)av_realloc_f(gxf->map_offsets,
...
static int gxf_write_umf_track_description(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; ++i) {
        GXFStreamContext *sc = (GXFStreamContext *)s->streams[i]->priv_data;
...
static int gxf_write_umf_media_description(AVFormatContext *s)
{
    ...
    for (i = 0; i <= s->nb_streams; ++i) {
        ...
        if (i == s->nb_streams)
            ...
            sc = (GXFStreamContext *)s->streams[i]->priv_data;
        ...
        avio_wl32(pb, 0); /* mark in */
        avio_wl32(pb, gxf->nb_fields); /* mark out */
        avio_write(pb, (const unsigned char *)ES_NAME_PATTERN, strlen(ES_NAME_PATTERN));
        avio_wb16(pb, sc->media_info);
...
static int gxf_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; ++i) {
        AVStream *st = s->streams[i];
        GXFStreamContext *sc = (GXFStreamContext *)av_mallocz(sizeof(*sc));
        if (!sc)
    ...
    if (ff_audio_interleave_init(s, GXF_samples_per_frame, av_rational(1, 48000)) < 0)
        return -1;
...
static int gxf_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
        if (!(gxf->flt_entries_nb % 500)) {
            gxf->flt_entries = (unsigned int *)av_realloc_f(gxf->flt_entries,
...
static int gxf_compare_field_nb(AVFormatContext *s, AVPacket *next, AVPacket *cur)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        sc[i] = (GXFStreamContext *)st->priv_data;
        if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO) {
