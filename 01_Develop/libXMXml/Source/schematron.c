...
#define IS_SCHEMATRON(node, elem)					\
   ((node != KD_NULL) && (node->type == XML_ELEMENT_NODE ) &&		\
    (node->ns != KD_NULL) &&						\
...
#define NEXT_SCHEMATRON(node)						\
   while (node != KD_NULL) {						\
       if ((node->type == XML_ELEMENT_NODE ) && (node->ns != KD_NULL) && 	\
           ((xmlStrEqual(node->ns->href, xmlSchematronNs)) ||		\
	    (xmlStrEqual(node->ns->href, xmlOldSchematronNs))))		\
	   break;							\
       node = node->next;						\
...
struct _xmlSchematronValidCtxt {
    ...
    KDFile *outputFile;		/* if using XML_SCHEMATRON_OUT_KDFile */
 ...
 *
 * Returns the new pointer or KD_NULL in case of error
 */
                     ...
                     xmlNodePtr node, xmlChar *test, xmlChar *report)
{
    ...
    if ((ctxt == KD_NULL) || (rule == KD_NULL) || (node == KD_NULL) ||
        (test == KD_NULL))
        return(KD_NULL);
    ...
    kdMemset(ret, 0, sizeof(xmlSchematronTest));
    ret->type = type;
    ret->node = node;
    ret->test = test;
    ret->comp = comp;
...
xmlSchematronFreeTests(xmlSchematronTestPtr tests) {
    ...
    while (tests != KD_NULL) {
        next = tests->next;
	if (tests->test != KD_NULL)
	    xmlFree(tests->test);
	if (tests->comp != KD_NULL)
	    xmlXPathFreeCompExpr(tests->comp);
	if (tests->report != KD_NULL)
		     ...
		     xmlChar *context, xmlChar *report)
{
    ...
    kdMemset(ret, 0, sizeof(xmlSchematronRule));
    ret->node = node;
    ret->context = context;
    ret->pattern = pattern;
    ret->report = report;
    ret->next = KD_NULL;
    if (schema->rules == KD_NULL) {
	schema->rules = ret;
    } else {
    ...
    ret->patnext = KD_NULL;
    if (pat->rules == KD_NULL) {
	pat->rules = ret;
    } else {
        xmlSchematronRulePtr prev = pat->rules;

	while (prev->patnext != KD_NULL)
	     prev = prev->patnext;
        prev->patnext = ret;
    }
    return (ret);
                     ...
                     xmlSchematronPtr schema, xmlNodePtr node, xmlChar *name)
{
    ...
    if ((ctxt == KD_NULL) || (schema == KD_NULL) || (node == KD_NULL) || (name == KD_NULL))
        return(KD_NULL);
    ...
    kdMemset(ret, 0, sizeof(xmlSchematronPattern));
    ret->name = name;
    ret->next = KD_NULL;
    if (schema->patterns == KD_NULL) {
	schema->patterns = ret;
    } else {
...
xmlSchematronFreePatterns(xmlSchematronPatternPtr patterns) {
    ...
    while (patterns != KD_NULL) {
        next = patterns->next;
	if (patterns->name != KD_NULL)
...
xmlSchematronNewSchematron(xmlSchematronParserCtxtPtr ctxt)
{
    ...
    if (ret == KD_NULL) {
        xmlSchematronPErrMemory(ctxt, "allocating schema", KD_NULL);
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlSchematron));
    ret->dict = ctxt->dict;
...
xmlSchematronFree(xmlSchematronPtr schema)
{
    ...
    if ((schema->doc != KD_NULL) && (!(schema->preserve)))
        xmlFreeDoc(schema->doc);

    if (schema->namespaces != KD_NULL)
...
xmlSchematronNewParserCtxt(const char *URL)
{
    ...
    if (ret == KD_NULL) {
        xmlSchematronPErrMemory(KD_NULL, "allocating schema parser context",
                                KD_NULL);
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlSchematronParserCtxt));
    ret->type = XML_STRON_CTXT_PARSER;
    ret->dict = xmlDictCreate();
    ret->URL = xmlDictLookup(ret->dict, (const xmlChar *) URL, -1);
    ret->includes = KD_NULL;
    ret->xctxt = xmlXPathNewContext(KD_NULL);
    if (ret->xctxt == KD_NULL) {
        xmlSchematronPErrMemory(KD_NULL, "allocating schema parser XPath context",
                                KD_NULL);
                        ...
                        xmlDocPtr doc, xmlNodePtr cur)
{
	...
	if (ctxt->includes == KD_NULL) {
	    xmlSchematronPErrMemory(KD_NULL, "allocating parser includes",
				    KD_NULL);
	    return;
	}
        ctxt->nbIncludes = 0;
 ...
 *
 * Returns the node immediately following the include or KD_NULL if the
                          ...
                          const xmlChar *prefix, const xmlChar *ns)
{
	...
	if (ctxt->namespaces == KD_NULL) {
	    xmlSchematronPErrMemory(KD_NULL, "allocating parser namespaces",
				    KD_NULL);
	    return;
	}
        ctxt->nbNamespaces = 0;
    ...
    ctxt->nbNamespaces++;
    ctxt->namespaces[2 * ctxt->nbNamespaces] = KD_NULL;
    ctxt->namespaces[2 * ctxt->nbNamespaces + 1] = KD_NULL;
		       ...
		       xmlNodePtr rule)
{
    ...
    if ((ctxt == KD_NULL) || (rule == KD_NULL)) return;
	...
	xmlSchematronPErr(ctxt, rule,
	    XML_SCHEMAP_NOROOT,
	    "rule has no assert nor report element", KD_NULL, KD_NULL);
    }
}
...
xmlSchematronParsePattern(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr pat)
{
    ...
    if ((ctxt == KD_NULL) || (pat == KD_NULL)) return;
	...
	if (IS_SCHEMATRON(cur, "rule")) {
	    ...
	    xmlSchematronPErr(ctxt, cur,
		XML_SCHEMAP_NOROOT,
		"Expecting a rule element instead of %s", cur->name, KD_NULL);
	}
	cur = cur->next;
	NEXT_SCHEMATRON(cur);
    }
	...
	xmlSchematronPErr(ctxt, pat,
	    XML_SCHEMAP_NOROOT,
	    "Pattern has no rule element", KD_NULL, KD_NULL);
    }
}
...
xmlSchematronLoadInclude(xmlSchematronParserCtxtPtr ctxt, xmlNodePtr cur)
{
	...
	xmlSchematronPErr(ctxt, cur,
	    XML_SCHEMAP_NOROOT,
	    "Include has no href attribute", KD_NULL, KD_NULL);
	return(cur->next);
    }
    ...
    doc = xmlReadFile((const char *) URI, KD_NULL, SCHEMATRON_PARSE_OPTIONS);
    if (doc == KD_NULL) {
	xmlSchematronPErr(ctxt, cur,
...
xmlSchematronParse(xmlSchematronParserCtxtPtr ctxt)
{
     ...
     */
    if (ctxt->URL != KD_NULL) {
        doc = xmlReadFile((const char *) ctxt->URL, KD_NULL,
	                  SCHEMATRON_PARSE_OPTIONS);
        if (doc == KD_NULL) {
	    xmlSchematronPErr(ctxt, KD_NULL,
			  XML_SCHEMAP_FAILED_LOAD,
	...
	ctxt->preserve = 0;
        ...
        doc = xmlReadMemory(ctxt->buffer, ctxt->size, KD_NULL, KD_NULL,
	                    SCHEMATRON_PARSE_OPTIONS);
        if (doc == KD_NULL) {
	    xmlSchematronPErr(ctxt, KD_NULL,
	...
	if ((uri == KD_NULL) || (uri[0] == 0)) {
	    xmlSchematronPErr(ctxt, cur,
		XML_SCHEMAP_NOROOT,
		"ns element has no uri", KD_NULL, KD_NULL);
	}
	if ((prefix == KD_NULL) || (prefix[0] == 0)) {
	    xmlSchematronPErr(ctxt, cur,
		XML_SCHEMAP_NOROOT,
		"ns element has no prefix", KD_NULL, KD_NULL);
	}
	if ((prefix) && (uri)) {
	...
	if (IS_SCHEMATRON(cur, "pattern")) {
	    ...
	    xmlSchematronPErr(ctxt, cur,
		XML_SCHEMAP_NOROOT,
		"Expecting a pattern element instead of %s", cur->name, KD_NULL);
	}
	cur = cur->next;
	NEXT_SCHEMATRON(cur);
    }
...
xmlSchematronGetNode(xmlSchematronValidCtxtPtr ctxt,
    ...
    if ((ctxt == KD_NULL) || (cur == KD_NULL) || (xpath == KD_NULL))
        return(KD_NULL);
    ...
    if ((ret->type == XPATH_NODESET) &&
        (ret->nodesetval != KD_NULL) && (ret->nodesetval->nodeNr > 0))
 ...
 *
 * Returns a report string or KD_NULL in case of error. The string needs
...
xmlSchematronReportSuccess(xmlSchematronValidCtxtPtr ctxt, 
    ...
    if ((ctxt == KD_NULL) || (cur == KD_NULL) || (test == KD_NULL))
        return;
...
#if 0
	if ((test->report != KD_NULL) && (test->report[0] != 0))
	    report = test->report;
#endif
	if (test->node != KD_NULL)
	...
	if (report == KD_NULL) {
	    ...
	    kdSnprintfKHR  (msg, 999, "%s line %ld: %s\n", (const char *) path,
    ...
    if (ctxt->flags & XML_SCHEMATRON_OUT_ERROR) {
        ...
        __xmlRaiseError(schannel, channel, data,
                        KD_NULL, cur, XML_FROM_SCHEMATRONV,
                        (test->type == XML_SCHEMATRON_ASSERT)?XML_SCHEMATRONV_ASSERT:XML_SCHEMATRONV_REPORT,
                        XML_ERR_ERROR, KD_NULL, line,
                        (pattern == KD_NULL)?KD_NULL:((const char *) pattern->name),
                        (const char *) path,
	...
	if ((path != KD_NULL) && (path != (xmlChar *) cur->name))
	    xmlFree(path);
    }
}
...
xmlSchematronReportPattern(xmlSchematronValidCtxtPtr ctxt, 
			   xmlSchematronPatternPtr pattern) {
    if ((ctxt == KD_NULL) || (pattern == KD_NULL))
        return;
	...
	if (pattern->name == KD_NULL)
	    return;
	kdSnprintfKHR  (msg, 999, "Pattern: %s\n", (const char *) pattern->name);
	xmlSchematronReportOutput(ctxt, KD_NULL, &msg[0]);
    }
}
...
xmlSchematronNewValidCtxt(xmlSchematronPtr schema, int options)
{
    ...
    if (ret == KD_NULL) {
        xmlSchematronVErrMemory(KD_NULL, "allocating validation context",
                                KD_NULL);
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlSchematronValidCtxt));
    ...
    for (i = 0;i < schema->nbNamespaces;i++) {
        if ((schema->namespaces[2 * i] == KD_NULL) ||
            (schema->namespaces[2 * i + 1] == KD_NULL))
	    break;
...
xmlSchematronNextNode(xmlNodePtr cur) {
	...
	if (cur->type == XML_DOCUMENT_NODE) return(KD_NULL);
	if (cur->next != KD_NULL) {
	    cur = cur->next;
	    return(cur);
	}
...
xmlSchematronValidateDoc(xmlSchematronValidCtxtPtr ctxt, xmlDocPtr instance)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->schema == KD_NULL) ||
        (ctxt->schema->rules == KD_NULL) || (instance == KD_NULL))
        return(-1);
    ctxt->nberrors = 0;
...
main(void)
{
    ...
    xmlSchematronPtr schema = KD_NULL;

    pctxt = xmlSchematronNewParserCtxt("tst.sct");
    if (pctxt == KD_NULL) {
        kdFprintfKHR (stderr, "failed to build schematron parser\n");
    } else {
        schema = xmlSchematronParse(pctxt);
	if (schema == KD_NULL) {
	    kdFprintfKHR (stderr, "failed to compile schematron\n");
	}
	xmlSchematronFreeParserCtxt(pctxt);
    }
    instance = xmlReadFile("tst.sct", KD_NULL,
                           XML_PARSE_NOENT | XML_PARSE_NOCDATA);
    if (instance == KD_NULL) {
	kdFprintfKHR (stderr, "failed to parse instance\n");
    }
    if ((schema != KD_NULL) && (instance != KD_NULL)) {
        vctxt = xmlSchematronNewValidCtxt(schema);
	if (vctxt == KD_NULL) {
	    kdFprintfKHR (stderr, "failed to build schematron validator\n");
	} else {
