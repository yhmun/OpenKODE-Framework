...
{
    kdMemset(_techniqueChannel, 0, sizeof(_techniqueChannel));
}
...
C3DMaterial* C3DMaterial::create(const char* vshPath, const char* fshPath, const char* defines)
{
	...
	tpNodePass->setElement("vertexShader", (vshPath == NULL) ? "" : vshPath);
	tpNodePass->setElement("fragmentShader", (fshPath == NULL) ? "" : fshPath);
	if(defines != NULL)
