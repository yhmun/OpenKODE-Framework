...
{
    SRTContext *s = (SRTContext *)avctx->priv_data;
    s->avctx = avctx;
    s->ass_ctx = ff_ass_split((const char *)avctx->subtitle_header);
    return s->ass_ctx ? 0 : AVERROR_INVALIDDATA;
}

static void srt_text_cb(void *priv, const char *text, int len)
{
    SRTContext *s = (SRTContext *)priv;
...
{
    srt_print((SRTContext *)priv, "\r\n");
}

static void srt_style_cb(void *priv, char style, int close)
{
    srt_stack_push_pop((SRTContext *)priv, style, close);
    if (!close)
        srt_print((SRTContext *)priv, "<%c>", style);
}
...
static void srt_color_cb(void *priv, unsigned int color, unsigned int color_id)
{
    if (color_id > 1)
        return;
    srt_stack_push_pop((SRTContext *)priv, 'f', color == 0xFFFFFFFF);
    if (color != 0xFFFFFFFF)
        srt_print((SRTContext *)priv, "<font color=\"#%06x\">",
              (color & 0xFF0000) >> 16 | color & 0xFF00 | (color & 0xFF) << 16);
}

static void srt_font_name_cb(void *priv, const char *name)
{
    srt_stack_push_pop((SRTContext *)priv, 'f', !name);
    if (name)
        srt_print((SRTContext *)priv, "<font face=\"%s\">", name);
}

static void srt_font_size_cb(void *priv, int size)
{
    srt_stack_push_pop((SRTContext *)priv, 'f', size < 0);
    if (size >= 0)
        srt_print((SRTContext *)priv, "<font size=\"%d\">", size);
}
...
static void srt_alignment_cb(void *priv, int alignment)
{
    ...
    if (!s->alignment_applied && alignment >= 0) {
        srt_print((SRTContext *)s, "{\\an%d}", alignment);
        s->alignment_applied = 1;
    }
}
...
{
    srt_stack_push_pop((SRTContext *)priv, 0, 1);
    srt_style_apply((SRTContext *)priv, style);
}
                            ...
                            unsigned char *buf, int bufsize, void *data)
{
    ...
    len = av_strlcpy((char *)buf, s->buffer, bufsize);
