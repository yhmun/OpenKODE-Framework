...
struct concat_data {
    ...
    KDsize               length;   ///< number of cat'ed nodes
    KDsize               current;  ///< index of currently read node
};
...
static av_cold int concat_close(URLContext *h)
{
    int err = 0;
    KDsize i;
    struct concat_data  *data  = (struct concat_data *)h->priv_data;
...
static av_cold int concat_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (!(nodes = (struct concat_nodes *)av_malloc(sizeof(*nodes) * len))) {
    ...
    for (i = 0; *uri; i++) {
        ...
        if (!(tmp_uri = (char *)av_realloc(node_uri, len+1))) {
            err = AVERROR(ENOMEM);
            break;
        } else
            node_uri = tmp_uri;
    ...
    if (err < 0)
        concat_close(h);
    else if (!(nodes = (struct concat_nodes *)av_realloc(nodes, data->length * sizeof(*nodes)))) {
        concat_close(h);
        err = AVERROR(ENOMEM);
    } else
        data->nodes = nodes;
