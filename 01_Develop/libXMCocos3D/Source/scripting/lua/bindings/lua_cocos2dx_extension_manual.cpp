...
#include "scripting/lua/bindings/CCBProxy.h"
...
static int tolua_cocos2d_CCBProxy_readCCBFromFile(lua_State* tolua_S)
{
    ...
    if (2 == argc || 3 == argc)
    {
        ...
        ID = (tolua_ret) ? (int)tolua_ret->m_uID : -1;
        luaID = (tolua_ret) ? &tolua_ret->m_nLuaID : NULL;
...
static int tolua_cocos2d_CCBReader_load(lua_State* tolua_S)
{
    ...
    if (argc >= 1 && argc <= 3)
    {
        ...
        if (1 == argc)
        {
            ...
            int ID = (tolua_ret) ? (int)tolua_ret->m_uID : -1;
            int* luaID = (tolua_ret) ? &tolua_ret->m_nLuaID : NULL;
            toluafix_pushusertype_ccobject(tolua_S, ID, luaID, (void*)tolua_ret,"Node");
            return 1;
        }
...
public:
    ...
    virtual Size tableCellSizeForIndex(TableView *table, KDint32 idx)
    {
        if (nullptr != table )
        {
    ...
    virtual TableViewCell* tableCellAtIndex(TableView *table, KDint32 idx)
    {
        if (nullptr != table )
        {
    ...
    virtual KDint32 numberOfCellsInTableView(TableView *table)
    {
        if (nullptr != table )
        {
...
static int lua_cocos2dx_TableView_create(lua_State* L)
{
    ...
    if (2 == argc || 1 == argc)
    {
        ...
        int  nID = (int)ret->m_uID;
        int* pLuaID =  &ret->m_nLuaID;
