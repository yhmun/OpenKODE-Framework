...
void RenderTextureSave::onTouchesMoved(const std::vector<Touch*>& touches, Event* event)
{
    ...
    if (distance > 1)
    {
        ...
        for (int i = 0; i < d; i++)
        {
            ...
            _brush->setRotation(kdRand() % 360);
            float r = (float)(kdRand() % 50 / 50.f) + 0.25f;
            _brush->setScale(r);
            /*_brush->setColor(Color3B(CCRANDOM_0_1() * 127 + 128, 255, 255));*/
            // Use CCRANDOM_0_1() will cause error when loading libtests.so on android, I don't know why.
            _brush->setColor(Color3B(kdRand() % 127 + 128, 255, 255));
...
void RenderTextureTargetNode::update(float dt)
{
    static float time = 0;
    float r = 80;
    sprite1->setPosition(Point(kdCosf(time * 2) * r, kdSinf(time * 2) * r));
    sprite2->setPosition(Point(kdSinf(time * 2) * r, kdCosf(time * 2) * r));
