                                                ...
                                                float stopband, float ripple)
{
    ...
    FF_ALLOCZ_OR_GOTO(FFIIRFilterCoeffs*, avc, c,     sizeof(FFIIRFilterCoeffs),
                      init_fail);
    FF_ALLOC_OR_GOTO (int*, avc, c->cx, sizeof(c->cx[0]) * ((order >> 1) + 1),
                      init_fail);
    FF_ALLOC_OR_GOTO (float*, avc, c->cy, sizeof(c->cy[0]) * order,
                      init_fail);
    c->order = order;
...
{
    FFIIRFilterState* s = (FFIIRFilterState *)av_mallocz(sizeof(FFIIRFilterState) + sizeof(s->x[0]) * (order - 1));
    return s;
}
