...
#include "XMFFmpeg/libavutil/colorspace.h"
...
static void png_save2(const char *filename, uint32_t *bitmap, int w, int h)
{
    ...
    kdSnprintf(fname, sizeof(fname), "%s.ppm", filename);
    ...
    kdSnprintf(fname2, sizeof(fname2), "%s-a.pgm", filename);
    ...
    //kdSnprintf(command, sizeof(command), "pnmtopng -alpha %s %s > %s.png 2> /dev/null", fname2, fname, filename);
    //system(command);

    //kdSnprintf(command, sizeof(command), "rm %s %s", fname, fname2);
    //system(command);
}
#endif
                                        ...
                                        const uint8_t *buf, int buf_size)
{
    ...
    if (!clut) {
        clut = (DVBSubCLUT *)av_malloc(sizeof(DVBSubCLUT));
                                        ...
                                        const uint8_t *buf, int buf_size)
{
    ...
    if (!region) {
        region = (DVBSubRegion *)av_mallocz(sizeof(DVBSubRegion));
    ...
    if (region->width * region->height != region->buf_size) {
        ...
        region->pbuf = (uint8_t *)av_malloc(region->buf_size);
    ...
    while (buf + 5 < buf_end) {
        ...
        if (!object) {
            object = (DVBSubObject *)av_mallocz(sizeof(DVBSubObject));
        ...
        display = (DVBSubObjectDisplay *)av_mallocz(sizeof(DVBSubObjectDisplay));
                                        ...
                                        const uint8_t *buf, int buf_size)
{
    ...
    while (buf + 5 < buf_end) {
        ...
        if (!display)
            display = (DVBSubRegionDisplay *)av_mallocz(sizeof(DVBSubRegionDisplay));
...
static void save_display_set(DVBSubContext *ctx)
{
    ...
    if (x_pos >= 0) {
        ...
        pbuf = (uint32_t*) av_malloc(width * height * 4);
                                                    ...
                                                    int buf_size)
{
    ...
    if (!display_def) {
        display_def             = (DVBSubDisplayDefinition *)av_mallocz(sizeof(*display_def));
                                        ...
                                        int buf_size, AVSubtitle *sub)
{
    ...
    if (sub->num_rects > 0){
        sub->rects = (AVSubtitleRect **)av_mallocz(sizeof(*sub->rects) * sub->num_rects);
        for(i=0; i<sub->num_rects; i++)
            sub->rects[i] = (AVSubtitleRect *)av_mallocz(sizeof(*sub->rects[i]));
    ...
    for (display = ctx->display_list; display; display = display->next) {
        ...
        rect->pict.data[1] = (uint8_t *)av_mallocz(AVPALETTE_SIZE);
        memcpy(rect->pict.data[1], clut_table, (1 << region->depth) * sizeof(uint32_t));

        rect->pict.data[0] = (uint8_t *)av_malloc(region->buf_size);
