...
{
    AVRational pts_info = { 2997, 125 };  /* default: 23.976 fps */
    MicroDVDContext *microdvd = (MicroDVDContext *)s->priv_data;
    ...
    for (i=0; i<FF_ARRAY_ELEMS(microdvd->lines); i++) {
        ...
        ff_get_line(s->pb, (char *)microdvd->lines[i], sizeof(microdvd->lines[i]));
        ...
        if (sscanf(microdvd->lines[i], "{DEFAULT}{}%c", &c) == 1) {
            st->codec->extradata = (uint8_t *)av_strdup((const char *)(microdvd->lines[i] + 11));
