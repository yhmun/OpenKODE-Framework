...
#include "XMFFmpeg/libavutil/tree.h"
#include "internal.h"
...
static int get_str(AVIOContext *bc, char *string, unsigned int maxlen){
    ...
    if(len && maxlen)
        avio_read(bc, (unsigned char *)string, FFMIN(len, maxlen));
    while(len > maxlen){
        avio_r8(bc);
        len--;
    }
...
static int decode_main_header(NUTContext *nut){
    ...
    nut->time_base= (AVRational *)av_malloc(nut->time_base_count * sizeof(AVRational));
    ...
    if(end > avio_tell(bc) + 4){
        ...
        for(i=1; i<nut->header_count; i++){
            ...
            nut->header[i]= (const uint8_t *)av_malloc(nut->header_len[i]);
            avio_read(bc, (unsigned char*)nut->header[i], nut->header_len[i]);
        }
    ...
    nut->stream = (StreamContext *)av_mallocz(sizeof(StreamContext)*stream_count);
...
static int decode_stream_header(NUTContext *nut){
    ...
    switch(av_class)
    {
        case 0:
			...
			{
				const AVCodecTag * const _code_tag[] = { ff_codec_bmp_tags, ff_nut_video_tags, 0 };
    ...
    if(av_class<3 && st->codec->codec_id == CODEC_ID_NONE)
...
static int find_and_decode_index(NUTContext *nut){
    ...
    syncpoints= (int64_t *)av_malloc(sizeof(int64_t)*syncpoint_count);
    has_keyframe= (int8_t *)av_malloc(sizeof(int8_t)*(syncpoint_count+1));
...
static int read_seek(AVFormatContext *s, int stream_index, int64_t pts, int flags){
    ...
    if(st->index_entries){
        ...
        if(!(flags & AVSEEK_FLAG_BACKWARD)){
            ...
            av_tree_find(nut->syncpoints, &dummy, (int (*)(void *,const void *)) ff_nut_sp_pos_cmp,
                         (void **) next_node);
        ...
        dummy.pos= pos;
        sp= (Syncpoint *)av_tree_find(nut->syncpoints, &dummy, (int (*)(void *,const void *)) ff_nut_sp_pos_cmp,
                         NULL);
...
static int nut_read_close(AVFormatContext *s)
{
    ...
    ff_nut_free_sp(nut);
    for(i = 1; i < nut->header_count; i++)
        av_freep((void*)&nut->header[i]);

    return 0;
}

#if CONFIG_NUT_DEMUXER
static const AVCodecTag * const _ff_nut_demuxer[] = { ff_codec_bmp_tags, ff_nut_video_tags, ff_codec_wav_tags, ff_nut_subtitle_tags, 0 };
