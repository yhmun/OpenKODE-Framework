...
static ChunkType get_chunk_header(MMSHContext *mmsh, int *len)
{
    ...
    if (res != CHUNK_HEADER_LENGTH) {
        ...
        return (ChunkType)AVERROR(EIO);
    }
    chunk_type = (ChunkType)AV_RL16(chunk_header);
    ...
    default:
        ...
        return (ChunkType)AVERROR_INVALIDDATA;
    }
    ...
    for (;;) {
        ...
        if (res < 0) {
            ...
            if (!mms->header_parsed) {
                ...
                mms->asf_header = (uint8_t *)av_mallocz(len);
                if (!mms->asf_header) {
...
static int mmsh_open_internal(URLContext *h, const char *uri, int flags, int timestamp, int64_t pos)
{
    ...
    mms = &mmsh->mms;
    av_strlcpy((char *)mmsh->location, uri, sizeof(mmsh->location));

    av_url_split(NULL, 0, NULL, 0,
        host, sizeof(host), &port, path, sizeof(path), (const char *)mmsh->location);
    if (port<0)
    ...
    stream_selection = (char *)av_mallocz(mms->stream_num * 19 + 1);
    if (!stream_selection)
        return AVERROR(ENOMEM);
                        ...
                        int64_t timestamp, int flags)
{
    ...
    ret= mmsh_open_internal(h, (const char *)mmsh->location, 0, FFMAX(timestamp, 0), 0);
