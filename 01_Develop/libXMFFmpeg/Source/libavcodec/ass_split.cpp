...
static uint8_t *realloc_section_array(ASSSplitContext *ctx)
{
    ...
    uint8_t *tmp = (uint8_t *)av_realloc(*section_ptr, (*count+1)*section->size);
    if (!tmp)
        return NULL;
    *section_ptr = tmp;
    ...
    while (buf && *buf) {
        ...
        if (buf[0] == ';' || (buf[0] == '!' && buf[1] == ':')) {
            ...
            while (!is_eol(*buf)) {
                buf = skip_space(buf);
                len = strcspn(buf, ", \r\n");
                if (!(tmp = (int *)av_realloc(order, (*number + 1) * sizeof(*order))))
                    return NULL;
                order = tmp;
                order[*number] = -1;
            ...
            if (!strncmp(buf, section->fields_header, len) && buf[len] == ':') {
                ...
                for (i=0; !is_eol(*buf) && i < *number; i++) {
                    ...
                    if (order[i] >= 0) {
                        ASSFieldType type = (ASSFieldType)section->fields[order[i]].type;
            ...
            if (buf[len] == ':') {
                for (i=0; section->fields[i].name; i++)
                    if (!strncmp(buf, section->fields[i].name, len)) {
                        ASSFieldType type = (ASSFieldType)section->fields[i].type;
...
{
    ASSSplitContext *ctx = (ASSSplitContext *)av_mallocz(sizeof(*ctx));
    ctx->current_section = -1;
