...
static void render_charset(AVCodecContext *avctx, uint8_t *charset,
                           uint8_t *colrammap)
{
    A64Context *c = (A64Context *)avctx->priv_data;
    uint8_t row1, row2;
    int charpos, x, y;
    int a, b;
    uint8_t pix;
...
static av_cold int a64multi_init_encoder(AVCodecContext *avctx)
{
    ...
    if (!(c->mc_meta_charset = (int *)av_malloc(32000 * c->mc_lifetime * sizeof(int))) ||
       !(c->mc_best_cb       = (int *)av_malloc(CHARSET_CHARS * 32 * sizeof(int)))     ||
       !(c->mc_charmap       = (int *)av_mallocz(1000 * c->mc_lifetime * sizeof(int))) ||
       !(c->mc_colram        = (uint8_t *)av_mallocz(CHARSET_CHARS * sizeof(uint8_t)))     ||
       !(c->mc_charset       = (uint8_t *)av_malloc(0x800 * (INTERLACED+1) * sizeof(uint8_t)))) {
        av_log(avctx, AV_LOG_ERROR, "Failed to allocate buffer memory.\n");
        return AVERROR(ENOMEM);
    }

    /* set up extradata */
    if (!(avctx->extradata = (uint8_t *)av_mallocz(8 * 4 + FF_INPUT_BUFFER_PADDING_SIZE))) {
...
static const enum PixelFormat _ff_a64multi_encoder[] = {PIX_FMT_GRAY8, PIX_FMT_NONE};
AVCodec ff_a64multi_encoder = {
	...
	/*.pix_fmts				= */	_ff_a64multi_encoder,
    /*.long_name			= */	NULL_IF_CONFIG_SMALL("Multicolor charset for Commodore 64"),
};

static const enum PixelFormat _ff_a64multi5_encoder[] = {PIX_FMT_GRAY8, PIX_FMT_NONE};
