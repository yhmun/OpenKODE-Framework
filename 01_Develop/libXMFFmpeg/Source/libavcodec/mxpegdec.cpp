...
{
    MXpegDecodeContext *s = (MXpegDecodeContext *)avctx->priv_data;
                            ...
                            const uint8_t *buf_ptr, int buf_size)
{
    ...
    if (s->bitmask_size != bitmask_size) {
        ...
        s->mxm_bitmask = (uint8_t *)av_malloc(bitmask_size);
        if (!s->mxm_bitmask) {
            av_log(s->jpg.avctx, AV_LOG_ERROR,
        ...
        s->completion_bitmask = (uint8_t *)av_mallocz(bitmask_size);
