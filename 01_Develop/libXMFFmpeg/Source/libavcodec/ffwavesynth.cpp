...
{
    struct wavesynth_context *ws = (struct wavesynth_context *)avc->priv_data;
    struct ws_interval *in;
    ...
    if (ws->nb_inter < 0)
        return AVERROR(EINVAL);
    ws->inter = (ws_interval *)av_calloc(ws->nb_inter, sizeof(*ws->inter));
    if (!ws->inter)
    ...
    for (i = 0; i < ws->nb_inter; i++) {
        ...
        in->type     = (ws_interval_type)AV_RL32(edata + 16);
...
static av_cold int wavesynth_init(AVCodecContext *avc)
{
    ...
    ws->sin = (int32_t *)av_malloc(sizeof(*ws->sin) << SIN_BITS);
    if (!ws->sin) {
        r = AVERROR(ENOMEM);
        goto fail;
    }
