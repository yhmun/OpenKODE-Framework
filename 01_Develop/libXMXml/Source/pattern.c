 ...
 *   currently push(KD_NULL, KD_NULL) means a reset of the streaming context
...
#define XML_PAT_FREE_STRING(c, r) if ((c)->comp->dict == KD_NULL) xmlFree(r);

typedef struct _xmlStreamStep xmlStreamStep;
typedef xmlStreamStep *xmlStreamStepPtr;
struct _xmlStreamStep {
    int flags;			/* properties of that step */
    const xmlChar *name;	/* first string value if KD_NULL accept all */
 ...
 *
 * Returns the newly allocated xmlPatternPtr or KD_NULL in case of error
 */
...
xmlNewPattern(void) {
    ...
    if (cur == KD_NULL) {
	ERROR(KD_NULL, KD_NULL, KD_NULL,
		"xmlNewPattern : malloc failed\n");
	return(KD_NULL);
    }
    kdMemset(cur, 0, sizeof(xmlPattern));
    cur->maxStep = 10;
...
xmlFreePattern(xmlPatternPtr comp) {
    ...
    kdMemset(comp, -1, sizeof(xmlPattern));
    xmlFree(comp);
}
 ...
 * @dict:  the inherited dictionary or KD_NULL
 * @namespaces: the prefix definitions, array of [URI, prefix] terminated
 *              with [KD_NULL, KD_NULL] or KD_NULL if no namespace is used
 *
 * Create a new XML pattern parser context
 *
 * Returns the newly allocated xmlPatParserContextPtr or KD_NULL in case of error
 */
...
xmlNewPatParserContext(const xmlChar *pattern, xmlDictPtr dict,
    ...
    kdMemset(cur, 0, sizeof(xmlPatParserContext));
    cur->dict = dict;
    cur->cur = pattern;
    cur->base = pattern;
    if (namespaces != KD_NULL) {
        int i;
	for (i = 0;namespaces[2 * i] != KD_NULL;i++);
        cur->nb_namespaces = i;
    } else {
        cur->nb_namespaces = 0;
    }
...
xmlFreePatParserContext(xmlPatParserContextPtr ctxt) {
    if (ctxt == KD_NULL)
	return;    
    kdMemset(ctxt, -1, sizeof(xmlPatParserContext));
    xmlFree(ctxt);
}
...
xmlReversePattern(xmlPatternPtr comp) {
    ...
    comp->steps[comp->nbStep].value = KD_NULL;
    comp->steps[comp->nbStep].value2 = KD_NULL;
...
xmlPatPushState(xmlStepStates *states, int step, xmlNodePtr node) {
    if ((states->states == KD_NULL) || (states->maxstates <= 0)) {
        states->maxstates = 4;
	states->nbstates = 0;
...
xmlPatMatch(xmlPatternPtr comp, xmlNodePtr node) {
    int i;
    xmlStepOpPtr step;
    xmlStepStates states = {0, 0, KD_NULL}; /* // may require backtrack */

    if ((comp == KD_NULL) || (node == KD_NULL)) return(-1);
    i = 0;
restart:
    for (;i < comp->nbStep;i++) {
	step = &comp->steps[i];
	switch (step->op) {
		...
		/* Namespace test */
		if (node->ns == KD_NULL) {
		    if (step->value2 != KD_NULL)
			goto rollback;
		} else if (node->ns->href != KD_NULL) {
		    if (step->value2 == KD_NULL)
			goto rollback;
 ...
 *                | "'" [^']* "'"
 *
 * Returns the Literal parsed or KD_NULL
 */
...
xmlCompileAttributeTest(xmlPatParserContextPtr ctxt) {
    ...
    if (name == KD_NULL) {
	if (CUR == '*') {
	    PUSH(XML_OP_ATTR, KD_NULL, KD_NULL);
	    NEXT;
	} else {
	    ERROR(KD_NULL, KD_NULL, KD_NULL,
	...
	if (IS_BLANK_CH(CUR)) {	    
	    ERROR5(KD_NULL, KD_NULL, KD_NULL, "Invalid QName.\n", KD_NULL);
	...
	*/
	    ...
	    (prefix[3] == 0))
	{
		...
		ERROR5(KD_NULL, KD_NULL, KD_NULL,
...
xmlCompileStepPattern(xmlPatParserContextPtr ctxt) {
	...
	*/
	NEXT;
	PUSH(XML_OP_ELEM, KD_NULL, KD_NULL);
	return;
    }
    if (CUR == '@') {
	/*
	...
	*/
	...
	if (CUR == '*') {
	    NEXT;
	    PUSH(XML_OP_ALL, KD_NULL, KD_NULL);
	    return;
	} else {
	    ERROR(KD_NULL, KD_NULL, KD_NULL,
	...
	if (CUR != ':') {
		...
		if (CUR == ':') {
		    ...
		    */
		    ...
		    PUSH(XML_OP_CHILD, name, KD_NULL);
		return;
	...
	NEXT;
	PUSH(XML_OP_ALL, token, KD_NULL);
    } else {
	PUSH(XML_OP_ELEM, name, KD_NULL);
    }
    return;
error:
    if (URL != KD_NULL)
...
xmlCompilePathPattern(xmlPatParserContextPtr ctxt) {
    ...
    if ((CUR == '/') && (NXT(1) == '/')) {
	PUSH(XML_OP_ANCESTOR, KD_NULL, KD_NULL);
	NEXT;
	NEXT;
	...
	if (CUR != 0) {
        ...
        if (CUR == '/') {
	    PUSH(XML_OP_ROOT, KD_NULL, KD_NULL);
	    NEXT;
	    /* Check for incompleteness. */
	    SKIP_BLANKS;
	    if (CUR == 0) {
	...
	while (CUR == '/') {
		...
		if (ctxt->error != 0)
	        ...
	        PUSH(XML_OP_PARENT, KD_NULL, KD_NULL);
		NEXT;
		SKIP_BLANKS;
		if (CUR == 0) {
		    ERROR5(KD_NULL, KD_NULL, KD_NULL,
...
xmlDebugStreamComp(xmlStreamCompPtr stream) {
    ...
    if (stream == KD_NULL) {
        printf("Stream: KD_NULL\n");
	return;
    }
    printf("Stream: %d steps\n", stream->nbStep);
    for (i = 0;i < stream->nbStep;i++) {
	if (stream->steps[i].ns != KD_NULL) {
	    printf("{%s}", stream->steps[i].ns);
	}
        if (stream->steps[i].name == KD_NULL) {
	    printf("* ");
	} else {
...
xmlNewStreamComp(int size) {
    ...
    kdMemset(cur, 0, sizeof(xmlStreamComp));
 ...
 * @comp: the compiled pattern for streaming
 * @name: the first string, the name, or KD_NULL for *
...
xmlStreamCompile(xmlPatternPtr comp) {
    ...
    if ((comp == KD_NULL) || (comp->steps == KD_NULL))
        return(-1);
    /*
     * special case for .
     */
	...
	(comp->steps[0].value == KD_NULL) &&
	(comp->steps[0].value2 == KD_NULL)) {
	...
	step = comp->steps[i];
		...
		break;
	    case XML_OP_NS:
		s = xmlStreamCompAddStep(stream, KD_NULL, step.value,
		...
		break;
	    case XML_OP_ELEM:		
	        if ((step.value == KD_NULL) && (step.value2 == KD_NULL)) {
		    /*
			...
			*/
			...
			s = xmlStreamCompAddStep(stream, KD_NULL, KD_NULL,
...
xmlNewStreamCtxt(xmlStreamCompPtr stream) {
    ...
    kdMemset(cur, 0, sizeof(xmlStreamCtxt));
    cur->states = (int *) xmlMalloc(4 * 2 * sizeof(int));
    if (cur->states == KD_NULL) {
	xmlFree(cur);
	ERROR(KD_NULL, KD_NULL, KD_NULL,
	      "xmlNewStreamCtxt: malloc failed\n");
	return(KD_NULL);
...
xmlFreeStreamCtxt(xmlStreamCtxtPtr stream) {
    ...
    while (stream != KD_NULL) {
        next = stream->next;
        if (stream->states != KD_NULL)
 ...
 * Both @name and @ns being KD_NULL means the / i.e. the root of the document.
...
xmlStreamPushInternal(xmlStreamCtxtPtr stream,
    ...
    if ((stream == KD_NULL) || (stream->nbState < 0))
        return(-1);
	...
	*/
	...
	while (i < m) {
	    ...
	    */
	    ...
	    } else if (((step.ns != KD_NULL) == (ns != KD_NULL)) &&
		(name != KD_NULL) &&
		(step.name[0] == name[0]) &&
...
{    
    if (streamCtxt == KD_NULL)
	return(-1);
    while (streamCtxt != KD_NULL) {
 ...
 * Compile a pattern.
 *
 * Returns the compiled form of the pattern or KD_NULL in case of error
 */
xmlPatternPtr
xmlPatterncompile(const xmlChar *pattern, xmlDict *dict, int flags,
                  const xmlChar **namespaces) {
    xmlPatternPtr ret = KD_NULL, cur;
    xmlPatParserContextPtr ctxt = KD_NULL;
    const xmlChar *_or, *start;
    xmlChar *tmp = KD_NULL;
    int type = 0;
    int streamable = 1;
	...
	while ((*_or != 0) && (*_or != '|')) _or++;
	    ...
	    tmp = xmlStrndup(start, _or - start);
	    if (tmp != KD_NULL) {
		ctxt = xmlNewPatParserContext(tmp, dict, namespaces);
	    }
	    _or++;
	}
	if (ctxt == KD_NULL) goto error;	
	cur = xmlNewPattern();
	if (cur == KD_NULL) goto error;
	/*
	* Assign string dict.
	*/
	if (dict) {	    
    ...
    return(ret);
error:
    if (ctxt != KD_NULL) xmlFreePatParserContext(ctxt);
    if (ret != KD_NULL) xmlFreePattern(ret);
 ...
 *
 * Returns a pointer to the context or KD_NULL in case of failure
 */
xmlStreamCtxtPtr
xmlPatternGetStreamCtxt(xmlPatternPtr comp)
{
    xmlStreamCtxtPtr ret = KD_NULL, cur;

    if ((comp == KD_NULL) || (comp->stream == KD_NULL))
        return(KD_NULL);
