...
static int read_atom(AVFormatContext *s, _Atom *atom)
{
...
static int r3d_read_red1(AVFormatContext *s)
{
    ...
    avio_read(s->pb, (unsigned char *)filename, 257);
...
static int r3d_read_rdvo(AVFormatContext *s, _Atom *atom)
{
    R3DContext *r3d = (R3DContext *)s->priv_data;
    AVStream *st = s->streams[0];
    int i;

    r3d->video_offsets_count = (atom->size - 8) / 4;
    r3d->video_offsets = (unsigned int *)av_malloc(atom->size);
...
static int r3d_read_redv(AVFormatContext *s, AVPacket *pkt, _Atom *atom)
{
    AVStream *st = s->streams[0];
    int tmp;
    int av_unused tmp2;
...
static int r3d_read_reda(AVFormatContext *s, AVPacket *pkt, _Atom *atom)
{
