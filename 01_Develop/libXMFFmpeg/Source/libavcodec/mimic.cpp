/*
 * Copyright (C) 2005  Ole Andr?Vadla Ravn? <oleavr@gmail.com>
...
{
    MimicContext *ctx = (MimicContext *)avctx->priv_data;
...
{
    MimicContext *dst = (MimicContext *)avctx->priv_data, *src = (MimicContext *)avctx_from->priv_data;
                              ...
                              int *data_size, AVPacket *avpkt)
{
    ...
    av_fast_padded_malloc(&ctx->swap_buf, (unsigned int *)&ctx->swap_buf_size, swap_buf_size);
    if(!ctx->swap_buf)
    ...
    ctx->dsp.bswap_buf((uint32_t *)ctx->swap_buf,
                        (const uint32_t*) buf,
                        swap_buf_size>>2);
    init_get_bits(&ctx->gb, (const uint8_t *)ctx->swap_buf, swap_buf_size << 3);
