...
static av_cold int adpcm_encode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->trellis) {
        ...
        FF_ALLOC_OR_GOTO(TrellisPath*, avctx, s->paths,
                         max_paths * sizeof(*s->paths), error);
        FF_ALLOC_OR_GOTO(TrellisNode*, avctx, s->node_buf,
                         2 * frontier * sizeof(*s->node_buf),  error);
        FF_ALLOC_OR_GOTO(TrellisNode**, avctx, s->nodep_buf,
                         2 * frontier * sizeof(*s->nodep_buf), error);
        FF_ALLOC_OR_GOTO(uint8_t*, avctx, s->trellis_hash,
    ...
    case CODEC_ID_ADPCM_MS:
        ...
        extradata = avctx->extradata = (uint8_t *)av_malloc(avctx->extradata_size);
        if (!extradata)
                              ...
                              unsigned char *frame, int buf_size, void *data)
{
    ...
    short *samples;
    unsigned char *dst;
    ADPCMEncodeContext *c = (ADPCMEncodeContext *)avctx->priv_data;
    uint8_t *buf;
    ...
    case CODEC_ID_ADPCM_IMA_WAV:
        ...
        *dst++ = 0; /* unknown */
        ...
        if (avctx->trellis > 0) {
            FF_ALLOC_OR_GOTO(uint8_t*, avctx, buf, 2 * n * 8, error);
    ...
    case CODEC_ID_ADPCM_SWF:
    {
        ...
        if (avctx->trellis > 0) {
            FF_ALLOC_OR_GOTO(uint8_t*, avctx, buf, 2 * n, error);
    ...
    case CODEC_ID_ADPCM_YAMAHA:
        ...
        if (avctx->trellis > 0) {
            FF_ALLOC_OR_GOTO(uint8_t*, avctx, buf, 2 * n * 2, error);
            n *= 2;
            if (avctx->channels == 1) {
                adpcm_compress_trellis(avctx, samples, buf, &c->status[0], n);
                for (i = 0; i < n; i += 2)
...
static const enum AVSampleFormat _ADPCM_ENCODER[] = { AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_NONE };
