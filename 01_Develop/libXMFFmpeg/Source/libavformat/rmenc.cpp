...
#define ffio_wfourcc(p,s)	ffio_wfourcc(p, (const uint8_t *)(s))
...
{
    RMMuxContext *rm = (RMMuxContext *)ctx->priv_data;
    ...
    for(i=0;i<ctx->nb_streams;i++) {
        ...
        if (stream->enc->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            /* audio codec info */
            avio_write(s, (const unsigned char *)".ra", 3);
            avio_w8(s, 0xfd);
...
{
    RMMuxContext *rm = (RMMuxContext *)s->priv_data;
    StreamInfo *stream;
    int n;
...
static const AVCodecTag* const _ff_rm_muxer[] = {ff_rm_codec_tags, 0};
