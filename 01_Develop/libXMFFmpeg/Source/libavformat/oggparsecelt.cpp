...
static int celt_header(AVFormatContext *s, int idx)
{
    ...
    struct oggcelt_private *priv = (struct oggcelt_private *)os->_private;
    ...
    if (os->psize == 60 &&
        ...
        extradata = (uint8_t *)av_malloc(2 * sizeof(uint32_t) +
                              FF_INPUT_BUFFER_PADDING_SIZE);
        priv = (struct oggcelt_private *)av_malloc(sizeof(struct oggcelt_private));
        if (!extradata || !priv) {
            av_free(extradata);
            av_free(priv);
