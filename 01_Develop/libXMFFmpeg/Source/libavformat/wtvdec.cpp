...
static const ff_asf_guid  _ff_asf_guids[] = {
...
static int wtvfile_read_packet(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    while(nread < buf_size) {
        int n;
        int remaining_in_sector = ((int64_t)1 << wf->sector_bits) - (wf->position & (((int64_t)1 << wf->sector_bits) - 1));
...
static AVIOContext * wtvfile_open_sector(int first_sector, uint64_t length, int depth, AVFormatContext *s)
{
    ...
    wf = (WtvFile *)av_mallocz(sizeof(WtvFile));
    if (!wf)
        return NULL;

    if (depth == 0) {
        wf->sectors = (uint32_t *)av_malloc(sizeof(uint32_t));
        if (!wf->sectors) {
            av_free(wf);
            return NULL;
        }
        ...
        wf->sectors = (uint32_t *)av_malloc(WTV_SECTOR_SIZE);
        if (!wf->sectors) {
            av_free(wf);
            return NULL;
        }
        ...
        wf->sectors = (uint32_t *)av_malloc(nb_sectors1 << WTV_SECTOR_BITS);
        if (!wf->sectors) {
            av_free(wf);
            return NULL;
        }
    ...
    buffer = (uint8_t *)av_malloc(1 << wf->sector_bits);
    if (!buffer) {
        av_free(wf->sectors);
        av_free(wf);
        return NULL;
...
static void get_attachment(AVFormatContext *s, AVIOContext *pb, int length)
{
    ...
    st->codec->extradata  = (uint8_t *)av_mallocz(filesize);
...
static AVStream * new_stream(AVFormatContext *s, AVStream *st, int sid, int codec_type)
{
    if (st) {
        ...
        WtvStream *wst = (WtvStream *)av_mallocz(sizeof(WtvStream));
        if (!wst)
            return NULL;
        st = avformat_new_stream(s, NULL);
        if (!st)
...
{
	const uint8_t _guid[] = {FF_MEDIASUBTYPE_BASE_GUID};
    ...
    while (!url_feof(pb)) {
        ...
        if (!ff_guidcmp(g, ff_stream_guid)) {
            ...
            if (stream_index >= 0) {
                ...
                if (language[0]) {
                    language[3] = 0;
                    av_dict_set(&st->metadata, "language", (const char *)language, 0);
            ...
            if (mode == SEEK_TO_DATA && stream_index >= 0 && len > 32) {
                WtvStream *wst = (WtvStream *)s->streams[stream_index]->priv_data;
                wst->seen_data = 1;
                if (len_ptr) {
                    *len_ptr = len;
                }
            ...
            !ff_guidcmp(g, _ff_asf_guids [0] /* DSATTRIB_CAPTURE_STREAMTIME */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [1] /* DSATTRIB_PicSampleSeq */ ) ||
            !ff_guidcmp(g, ff_DSATTRIB_TRANSPORT_PROPERTIES /* DSATTRIB_TRANSPORT_PROPERTIES */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [2] /* dvr_ms_vid_frame_rep_data */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [3] /* EVENTID_ChannelChangeSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [4] /* EVENTID_ChannelInfoSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [5] /* EVENTID_ChannelTypeSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [6] /* EVENTID_PIDListSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [7] /* EVENTID_SignalAndServiceStatusSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [8] /* EVENTID_StreamTypeSpanningEvent */ ) ||
            !ff_guidcmp(g, _ff_asf_guids [9]  ) ||
            !ff_guidcmp(g, _ff_asf_guids [10] ) ||
            !ff_guidcmp(g, _ff_asf_guids [11] ) ||
            !ff_guidcmp(g, _ff_asf_guids [12] ) ||
            !ff_guidcmp(g, _ff_asf_guids [13] ) ||
            !ff_guidcmp(g, _ff_asf_guids [14] ) ||
            !ff_guidcmp(g, _ff_asf_guids [15] ) ||
            !ff_guidcmp(g, _ff_asf_guids [16] ) ||
