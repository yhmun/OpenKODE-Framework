...
#include "XMFFmpeg/libavutil/aes.h"
...
static int mxf_decrypt_triplet(AVFormatContext *s, AVPacket *pkt, KLVPacket *klv)
{
    ...
    if (!mxf->aesc && s->key && s->keylen == 16) {
        mxf->aesc = (AVAES *)av_malloc(av_aes_size);
        if (!mxf->aesc)
...
static int mxf_read_primer_pack(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    mxf->local_tags = (uint8_t *)av_malloc(item_num*item_len);
    if (!mxf->local_tags)
        return AVERROR(ENOMEM);
...
static int mxf_read_partition_pack(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    tmp_part = (MXFPartition *)av_realloc(mxf->partitions, (mxf->partitions_count + 1) * sizeof(*mxf->partitions));
    if (!tmp_part)
        return AVERROR(ENOMEM);
...
static int mxf_add_metadata_set(MXFContext *mxf, void *metadata_set)
{
    ...
    if (mxf->metadata_sets_count+1 >= UINT_MAX / sizeof(*mxf->metadata_sets))
        return AVERROR(ENOMEM);
    tmp = (MXFMetadataSet **)av_realloc(mxf->metadata_sets, (mxf->metadata_sets_count + 1) * sizeof(*mxf->metadata_sets));
    if (!tmp)
        return AVERROR(ENOMEM);
    mxf->metadata_sets = tmp;
    mxf->metadata_sets[mxf->metadata_sets_count] = (MXFMetadataSet *)metadata_set;
...
static int mxf_read_content_storage(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x1901:
        ...
        if (mxf->packages_count >= UINT_MAX / sizeof(UID))
            return AVERROR_INVALIDDATA;
        mxf->packages_refs = (UID (*))av_malloc(mxf->packages_count * sizeof(UID));
...
static int mxf_read_material_package(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x4403:
        ...
        if (package->tracks_count >= UINT_MAX / sizeof(UID))
            return AVERROR_INVALIDDATA;
        package->tracks_refs = (UID (*))av_malloc(package->tracks_count * sizeof(UID));
...
static int mxf_read_sequence(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x1001:
        ...
        if (sequence->structural_components_count >= UINT_MAX / sizeof(UID))
            return AVERROR_INVALIDDATA;
        sequence->structural_components_refs = (UID (*))av_malloc(sequence->structural_components_count * sizeof(UID));
...
static int mxf_read_index_entry_array(AVIOContext *pb, MXFIndexTableSegment *segment)
{
    ...
    if (!(segment->temporal_offset_entries=(int8_t *)av_calloc(segment->nb_index_entries, sizeof(*segment->temporal_offset_entries))) ||
        !(segment->flag_entries          = (int *)av_calloc(segment->nb_index_entries, sizeof(*segment->flag_entries))) ||
        !(segment->stream_offset_entries = (uint64_t *)av_calloc(segment->nb_index_entries, sizeof(*segment->stream_offset_entries))))
        return AVERROR(ENOMEM);
...
static int mxf_read_generic_descriptor(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x3F01:
        ...
        if (descriptor->sub_descriptors_count >= UINT_MAX / sizeof(UID))
            return AVERROR_INVALIDDATA;
        descriptor->sub_descriptors_refs = (UID (*))av_malloc(descriptor->sub_descriptors_count * sizeof(UID));
    ...
    default:
        ...
        if (IS_KLV_KEY(uid, mxf_sony_mpeg4_extradata)) {
            descriptor->extradata = (uint8_t *)av_malloc(size + FF_INPUT_BUFFER_PADDING_SIZE);
...
static int mxf_get_sorted_table_segments(MXFContext *mxf, int *nb_sorted_segments, MXFIndexTableSegment ***sorted_segments)
{
    ...
    if (!(unsorted_segments = (MXFIndexTableSegment **)av_calloc(nb_segments, sizeof(*unsorted_segments))) ||
        !(*sorted_segments  = (MXFIndexTableSegment **)av_calloc(nb_segments, sizeof(**sorted_segments)))) {
...
static int mxf_compute_ptses_fake_index(MXFContext *mxf, MXFIndexTable *index_table)
{
    ...
    if (!(index_table->ptses      = (int64_t *)av_calloc(index_table->nb_ptses, sizeof(int64_t))) ||
        !(index_table->fake_index = (AVIndexEntry *)av_calloc(index_table->nb_ptses, sizeof(AVIndexEntry)))) {
...
static int mxf_compute_index_tables(MXFContext *mxf)
{
    ...
    if (!(mxf->index_tables = (MXFIndexTable *)av_calloc(mxf->nb_index_tables, sizeof(MXFIndexTable)))) {
    ...
    for (i = j = 0; j < mxf->nb_index_tables; i += mxf->index_tables[j++].nb_segments) {
        ...
        if (!(t->segments = (MXFIndexTableSegment **)av_calloc(t->nb_segments, sizeof(MXFIndexTableSegment*)))) {
...
static int mxf_parse_structural_metadata(MXFContext *mxf)
{
    ...
    for (i = 0; i < mxf->packages_count; i++) {
        material_package = (MXFPackage *)mxf_resolve_strong_ref(mxf, &mxf->packages_refs[i], MaterialPackage);
    ...
    for (i = 0; i < material_package->tracks_count; i++) {
        ...
        if (!(material_track = (MXFTrack *)mxf_resolve_strong_ref(mxf, &material_package->tracks_refs[i], Track))) {
            av_log(mxf->fc, AV_LOG_ERROR, "could not resolve material track strong ref\n");
            continue;
        }

        if (!(material_track->sequence = (MXFSequence *)mxf_resolve_strong_ref(mxf, &material_track->sequence_ref, Sequence))) {
        ...
        for (j = 0; j < material_track->sequence->structural_components_count; j++) {
            ...
            component = (MXFStructuralComponent *)mxf_resolve_strong_ref(mxf, &material_track->sequence->structural_components_refs[j], SourceClip);
            if (!component)
                continue;

            for (k = 0; k < mxf->packages_count; k++) {
                temp_package = (MXFPackage *)mxf_resolve_strong_ref(mxf, &mxf->packages_refs[k], SourcePackage);
                if (!temp_package)
                    continue;
            ...
            for (k = 0; k < source_package->tracks_count; k++) {
                if (!(temp_track = (MXFTrack *)mxf_resolve_strong_ref(mxf, &source_package->tracks_refs[k], Track))) {
        ...
        if (!(source_track->sequence = (MXFSequence *)mxf_resolve_strong_ref(mxf, &source_track->sequence_ref, Sequence))) {
        ...
        st->codec->codec_type = (AVMediaType)codec_ul->id;

        source_package->descriptor = (MXFDescriptor *)mxf_resolve_strong_ref(mxf, &source_package->descriptor_ref, AnyType);
        if (source_package->descriptor) {
            if (source_package->descriptor->type == MultipleDescriptor) {
                for (j = 0; j < source_package->descriptor->sub_descriptors_count; j++) {
                    MXFDescriptor *sub_descriptor = (MXFDescriptor *)mxf_resolve_strong_ref(mxf, &source_package->descriptor->sub_descriptors_refs[j], Descriptor);
        ...
        st->codec->codec_id = (CodecID)codec_ul->id;
        if (descriptor->extradata) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (st->codec->codec_id == CODEC_ID_NONE)
                st->codec->codec_id = (CodecID)container_ul->id;
...
static int mxf_read_local_tags(MXFContext *mxf, KLVPacket *klv, MXFMetadataReadFunc *read_child, int ctx_size, enum MXFMetadataSetType type)
{
    ...
    MXFMetadataSet *ctx = (MXFMetadataSet *)(ctx_size ? av_mallocz(ctx_size) : mxf);
    ...
    while (!url_feof(s->pb)) {
        ...
        if (IS_KLV_KEY(klv.key, mxf_essence_element_key) ||
            ...
            if (s->streams[index]->codec->codec_type == AVMEDIA_TYPE_VIDEO)
                mxf_packet_timestamps((MXFContext *)s->priv_data, pkt);   /* offset -> EditUnit -> DTS/PTS */
