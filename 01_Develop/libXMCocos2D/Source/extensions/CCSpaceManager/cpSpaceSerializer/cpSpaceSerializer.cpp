...
cpShape *cpSpaceSerializer::createShape(TiXmlElement *elm)
{
	...
	long id = createValue<long>("id", elm);
    _shapeMap[id] = shape;
...
{	
	...
	if (itr == _bodyMap.end())
	{
		...
		if (bodyElm)
		{
			...
			body->a = createValue<cpFloat>("a", bodyElm);
...
{
	long id_a = createValue<long>("body_a_id", elm);
	long id_b = createValue<long>("body_b_id", elm);
...
cpConstraint *cpSpaceSerializer::createConstraint(TiXmlElement *elm)
{
	...
	long id = createValue<cpFloat>("id", elm);
	constraint->maxForce = createValue<cpFloat>("maxForce", elm);
	//constraint->biasCoef = createValue<cpFloat>("biasCoef", elm);
...
TiXmlElement *cpSpaceSerializer::createConstraintElm(cpConstraint* constraint)
{
	...
	//-elm->LinkEndChild(createValueElm("biasCoef", constraint->biasCoef));
