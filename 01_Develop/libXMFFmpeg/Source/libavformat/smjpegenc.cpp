...
static int smjpeg_write_header(AVFormatContext *s)
{
    ...
    avio_write(pb, (const unsigned char *)SMJPEG_MAGIC, 8);
    avio_wb32(pb, 0);
    avio_wb32(pb, 0);
    ...
    while ((t = av_dict_get(s->metadata, "", t, AV_DICT_IGNORE_SUFFIX))) {
        ...
        avio_write(pb, (const unsigned char *)t->key, strlen(t->key));
        avio_write(pb, (const unsigned char *)" = ", 3);
        avio_write(pb, (const unsigned char *)t->value, strlen(t->value));
    }
...
{
    SMJPEGMuxContext *smc = (SMJPEGMuxContext *)s->priv_data;
    AVIOContext *pb = s->pb;
...
static const AVCodecTag *const _ff_smjpeg_muxer[] = { ff_codec_smjpeg_video_tags, ff_codec_smjpeg_audio_tags, 0 };
