...
#define GET_MVDATA(_dmv_x, _dmv_y)                                      \
    ...
    if (!index) {                                                       \
        ...
        if (KDsizeable[index1] - val > 0)                               \
            val = get_bits(gb, KDsizeable[index1] - val);               \
        else                                   val = 0;                 \
        sign = 0 - (val&1);                                             \
    ...
    if (coded) {
        ...
        if (v->s.ac_pred) {
            if (!use_pred && v->fcm == ILACE_FRAME) {
                zz_table = (const uint8_t *)v->zzi_8x8;
            } else {
                if (!dc_pred_dir) // top
                    zz_table = v->zz_8x8[2];
                else // left
...
static const int KDsizeable  [6] = { 0, 2, 3, 4,  5,  8 };
...
{
    VC1Context *v     = (VC1Context *)avctx->priv_data;
    MpegEncContext *s = &v->s;
...
static av_cold int vc1_decode_init_alloc_tables(VC1Context *v)
{
    ...
    v->mv_type_mb_plane = (uint8_t *)av_malloc (s->mb_stride * s->mb_height);
    v->direct_mb_plane  = (uint8_t *)av_malloc (s->mb_stride * s->mb_height);
    v->forward_mb_plane = (uint8_t *)av_malloc (s->mb_stride * s->mb_height);
    v->fieldtx_plane    = (uint8_t *)av_mallocz(s->mb_stride * s->mb_height);
    v->acpred_plane     = (uint8_t *)av_malloc (s->mb_stride * s->mb_height);
    v->over_flags_plane = (uint8_t *)av_malloc (s->mb_stride * s->mb_height);

    v->n_allocated_blks = s->mb_width + 2;
    v->block            = (DCTELEM (*)[6][64])av_malloc(sizeof(*v->block) * v->n_allocated_blks);
    v->cbp_base         = (uint32_t *)av_malloc(sizeof(v->cbp_base[0]) * 2 * s->mb_stride);
    v->cbp              = v->cbp_base + s->mb_stride;
    v->ttblk_base       = (int *)av_malloc(sizeof(v->ttblk_base[0]) * 2 * s->mb_stride);
    v->ttblk            = v->ttblk_base + s->mb_stride;
    v->is_intra_base    = (uint8_t *)av_mallocz(sizeof(v->is_intra_base[0]) * 2 * s->mb_stride);
    v->is_intra         = v->is_intra_base + s->mb_stride;
    v->luma_mv_base     = (int16_t (*)[2])av_malloc(sizeof(v->luma_mv_base[0]) * 2 * s->mb_stride);
    v->luma_mv          = v->luma_mv_base + s->mb_stride;

    /* allocate block type info in that way so it could be used with s->block_index[] */
    v->mb_type_base = (uint8_t *)av_malloc(s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2);
    v->mb_type[0]   = v->mb_type_base + s->b8_stride + 1;
    v->mb_type[1]   = v->mb_type_base + s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride + 1;
    v->mb_type[2]   = v->mb_type[1] + s->mb_stride * (s->mb_height + 1);

    /* allocate memory to store block level MV info */
    v->blk_mv_type_base = (uint8_t *)av_mallocz(     s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2);
    v->blk_mv_type      = v->blk_mv_type_base + s->b8_stride + 1;
    v->mv_f_base        = (uint8_t *)av_mallocz(2 * (s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2));
    v->mv_f[0]          = v->mv_f_base + s->b8_stride + 1;
    v->mv_f[1]          = v->mv_f[0] + (s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2);
    v->mv_f_last_base   = (uint8_t *)av_mallocz(2 * (s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2));
    ...
    v->mv_f_next_base   = (uint8_t *)av_mallocz(2 * (s->b8_stride * (s->mb_height * 2 + 1) + s->mb_stride * (s->mb_height + 1) * 2));
...
//            return -1;
    ...
    if (s->avctx->codec_id == CODEC_ID_WMV3IMAGE || s->avctx->codec_id == CODEC_ID_VC1IMAGE) {
        for (i = 0; i < 4; i++)
            if (!(v->sr_rows[i >> 1][i & 1] = (uint8_t *)av_malloc(v->output_width))) return -1;
    }
...
static av_cold int vc1_decode_init(AVCodecContext *avctx)
{
    ...
    if (avctx->codec_id == CODEC_ID_WMV3 || avctx->codec_id == CODEC_ID_WMV3IMAGE) {
        ...
        buf2  = (uint8_t *)av_mallocz(avctx->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
                            ...
                            int *data_size, AVPacket *avpkt)
{
    ...
    if (avctx->codec_id == CODEC_ID_VC1 || avctx->codec_id == CODEC_ID_VC1IMAGE) {
        int buf_size2 = 0;
        buf2 = (uint8_t *)av_mallocz(buf_size + FF_INPUT_BUFFER_PADDING_SIZE);
        ...
        if (IS_MARKER(AV_RB32(buf))) { /* frame starts with marker and needs to be parsed */
            ...
            for (start = buf, end = buf + buf_size; next < end; start = next) {
                ...
                case VC1_CODE_FIELD: {
                    int buf_size3;
                    slices = (struct _SLICE*)av_realloc(slices, sizeof(*slices) * (n_slices+1));
                    if (!slices)
                        goto err;
                    slices[n_slices].buf = (uint8_t *)av_mallocz(buf_size + FF_INPUT_BUFFER_PADDING_SIZE);
            ...
            if ((divider == (buf + buf_size)) || AV_RB32(divider) != VC1_CODE_FIELD) {
                ...
                tmp = (struct _SLICE*)av_realloc(slices, sizeof(*slices) * (n_slices+1));
                if (!tmp)
                    goto err;
                slices = tmp;
                slices[n_slices].buf = (uint8_t *)av_mallocz(buf_size + FF_INPUT_BUFFER_PADDING_SIZE);
    ...
    s->current_picture.f.pict_type = (AVPictureType)s->pict_type;
...
static const enum PixelFormat _ff_wmv3_vdpau_decoder[] = {PIX_FMT_VDPAU_WMV3, PIX_FMT_NONE};
AVCodec ff_wmv3_vdpau_decoder = {
	...
	/*.flush				= */	0,
	/*.supported_framerates	= */	0,
	/*.pix_fmts				= */	_ff_wmv3_vdpau_decoder,
...
#if CONFIG_VC1_VDPAU_DECODER
static const enum PixelFormat _ff_vc1_vdpau_decoder[] = {PIX_FMT_VDPAU_VC1, PIX_FMT_NONE};
AVCodec ff_vc1_vdpau_decoder = {
	...
	/*.flush				= */	0,
	/*.supported_framerates	= */	0,
	/*.pix_fmts				= */	_ff_vc1_vdpau_decoder,
