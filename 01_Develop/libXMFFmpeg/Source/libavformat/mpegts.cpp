...

{
    ...
    if (pid >= NB_PID_MAX || ts->pids[pid])
        return NULL;
    filter = (MpegTSFilter *)av_mallocz(sizeof(MpegTSFilter));
    if (!filter)
        return NULL;
    ts->pids[pid] = filter;
    ...
    sec->opaque = opaque;
    sec->section_buf = (uint8_t *)av_malloc(MAX_SECTION_SIZE);
...
static av_always_inline int get8(const uint8_t **pp, const uint8_t *p_end)
{
    const uint8_t *p;
    int c;
...
static av_always_inline int get16(const uint8_t **pp, const uint8_t *p_end)
{
    const uint8_t *p;
    int c;
                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    if ((prog_reg_desc == AV_RL32("HDMV") ||
        ...
        if (pes->stream_type == 0x83) {
            ...
            PESContext *sub_pes = (PESContext *)av_malloc(sizeof(*sub_pes));
            if (!sub_pes)
    ...
    if (st->codec->codec_id == CODEC_ID_NONE){
        st->codec->codec_id  = (CodecID)old_codec_id;
        st->codec->codec_type= (AVMediaType)old_codec_type;
    }
        ...
        case MPEGTS_HEADER:
            ...
            if (pes->data_index == PES_START_SIZE) {
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    /* allocate pes buffer */
                    pes->buffer = (uint8_t *)av_malloc(pes->total_size+FF_INPUT_BUFFER_PADDING_SIZE);
                    if (!pes->buffer)
                        return AVERROR(ENOMEM);
...
static PESContext *add_pes_stream(MpegTSContext *ts, int pid, int pcr_pid)
{
    ...
    pes = (PESContext *)av_mallocz(sizeof(PESContext));
    if (!pes)
        return 0;
    pes->ts = ts;
...
static int parse_MP4DecConfigDescrTag(MP4DescrParseContext *d, int64_t off, int len)
{
    ...
    if (!descr)
        return -1;
    d->active_descr->dec_config_descr = (uint8_t *)av_malloc(len);
...
{
    MpegTSContext *ts = (MpegTSContext *)filter->u.section_filter.opaque;
    SectionHeader h;
                              ...
                              MpegTSContext *ts)
{
    ...
    case 0x1F: /* FMC descriptor */
        ...
        if (mp4_descr_count > 0 && (st->codec->codec_id == CODEC_ID_AAC_LATM || st->request_probe>0) &&
            ...
            if (st->codec->codec_id == CODEC_ID_AAC &&
                st->codec->extradata_size > 0){
                st->request_probe= st->need_parsing = AVSTREAM_PARSE_NONE;
                st->codec->codec_type= AVMEDIA_TYPE_AUDIO;
            }
        }
        break;
    ...
    case 0x59: /* subtitling descriptor */
        ...
        if (st->codec->extradata) {
            ...
            st->codec->extradata = (uint8_t *)av_malloc(4 + FF_INPUT_BUFFER_PADDING_SIZE);
            if (st->codec->extradata) {
...
{
    MpegTSContext *ts = (MpegTSContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    uint8_t buf[8*1024];
    int len;
    int64_t pos;
...
MpegTSContext *ff_mpegts_parse_open(AVFormatContext *s)
{
    ...
    ts = (MpegTSContext *)av_mallocz(sizeof(MpegTSContext));
    if (!ts)
        return NULL;
