...
static av_cold void alloc_temp(HYuvContext *s){
    ...
    if(s->bitstream_bpp<24){
        for(i=0; i<3; i++){
            s->temp[i]= (uint8_t *)av_malloc(s->width + 16);
        }
    }else{
        s->temp[0]= (uint8_t *)av_mallocz(4*s->width + 16);
    }
}

static av_cold int common_init(AVCodecContext *avctx){
    HYuvContext *s = (HYuvContext *)avctx->priv_data;
...
//  printf("extradata:%X, extradata_size:%d\n", *(uint32_t*)avctx->extradata, avctx->extradata_size);
    ...
    if(s->version==2){
        ...
        s->predictor= (Predictor)(method&63);
...
static av_cold int encode_init(AVCodecContext *avctx)
{
    ...
    avctx->extradata= (uint8_t *)av_mallocz(1024*30); // 256*3+4 == 772
    avctx->stats_out= (char *)av_mallocz(1024*30); // 21*256*3(%llu ) + 3(\n) + 1(0) = 16132
    s->version=2;
    ...
    s->predictor= (Predictor)avctx->prediction_method;
...
static const enum PixelFormat _ff_huffyuv_encoder[] = {PIX_FMT_YUV422P, PIX_FMT_RGB24, PIX_FMT_RGB32, PIX_FMT_NONE};
AVCodec ff_huffyuv_encoder = {
	...
	/*.flush				= */	0,
	/*.supported_framerates	= */	0,
	/*.pix_fmts				= */	_ff_huffyuv_encoder,
    /*.long_name			= */	NULL_IF_CONFIG_SMALL("Huffyuv / HuffYUV"),
};
#endif

#if CONFIG_FFVHUFF_ENCODER
static const enum PixelFormat _ff_ffvhuff_encoder[] = {PIX_FMT_YUV420P, PIX_FMT_YUV422P, PIX_FMT_RGB24, PIX_FMT_RGB32, PIX_FMT_NONE};
