...
int ff_snow_alloc_blocks(SnowContext *s){
    ...
    av_free(s->block);
    s->block= (BlockNode *)av_mallocz(w * h * sizeof(BlockNode) << (s->block_max_depth*2));
    return 0;
}
...
static void mc_block(Plane *p, uint8_t *dst, const uint8_t *src, int stride, int b_w, int b_h, int dx, int dy){
	...
	uint8_t* tmp2t[3] = { (uint8_t*)av_malloc(stride*(32+HTAPS_MAX)), //[stride*(32+HTAPS_MAX)];
						  (uint8_t*)av_malloc(stride*(32+HTAPS_MAX)),
						  (uint8_t*)av_malloc(stride*(32+HTAPS_MAX)) };
    int16_t *tmpI= tmpIt;
...
//    dec += FFMAX(s->chroma_h_shift, s->chroma_v_shift);
    ...
    s->spatial_idwt_buffer= (IDWTELEM *)av_mallocz(width*height*sizeof(IDWTELEM));
    s->spatial_dwt_buffer= (DWTELEM *)av_mallocz(width*height*sizeof(DWTELEM)); //FIXME this does not belong here
    ...
    s->scratchbuf = (uint8_t *)av_malloc(s->mconly_picture.linesize[0]*7*MB_SIZE);
    ...
    for(plane_index=0; plane_index<3; plane_index++){
        ...
        for(level=s->spatial_decomposition_count-1; level>=0; level--){
            for(orientation=level ? 1 : 0; orientation<4; orientation++){
                ...
                av_freep(&b->x_coeff);
                b->x_coeff=(x_and_coeff *)av_mallocz(((b->width+1) * b->height+1)*sizeof(x_and_coeff));
            }
            w= (w+1)>>1;
            h= (h+1)>>1;
        }
