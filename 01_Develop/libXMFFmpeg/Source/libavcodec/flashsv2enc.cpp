...
{
    FlashSV2Context *s = (FlashSV2Context *)avctx->priv_data;
    ...
    s->encbuffer     = (uint8_t *)av_mallocz(s->frame_size);
    s->keybuffer     = (uint8_t *)av_mallocz(s->frame_size);
    s->databuffer    = (uint8_t *)av_mallocz(s->frame_size * 6);
    s->current_frame = (uint8_t *)av_mallocz(s->frame_size);
    s->key_frame     = (uint8_t *)av_mallocz(s->frame_size);
    s->frame_blocks  = (Block *)av_mallocz(s->blocks_size);
    s->key_blocks    = (Block *)av_mallocz(s->blocks_size);
...
{
    int res = compress2(buf, (z_uLongf *)buf_size, b->sl_begin, b->sl_end - b->sl_begin, comp);
    return res == Z_OK ? 0 : -1;
}
                        ...
                        int keyframe)
{
    ...
    if (b->len > 0) {
        ...
        if (!keyframe) {
            res = encode_zlibprime(b, prev, buf, (int *)&buf_size, comp);
            if (res)
                goto end;
                                   ...
                                   int stride)
{
    ...
    if (s->rows * s->cols != s->blocks_size / sizeof(Block)) {
        if (s->rows * s->cols > s->blocks_size / sizeof(Block)) {
            s->frame_blocks = (Block *)av_realloc(s->frame_blocks, s->rows * s->cols * sizeof(Block));
            s->key_blocks = (Block *)av_realloc(s->key_blocks, s->cols * s->rows * sizeof(Block));
...
{
    FlashSV2Context *const s = (FlashSV2Context *const)avctx->priv_data;
...
static enum PixelFormat _ff_flashsv2_encoder[] = {PIX_FMT_BGR24, PIX_FMT_NONE};
