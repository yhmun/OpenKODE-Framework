        ...
        int buf_size, void *data)
{
    ...
    bytestream_put_buffer(&buf, (const uint8_t *)"icpf", 4);

    bytestream_put_be16(&buf, header_size);
    bytestream_put_be16(&buf, 0);
    bytestream_put_buffer(&buf, (const uint8_t *)"fmpg", 4);
...
static av_cold int prores_encode_init(AVCodecContext *avctx)
{
    ...
    if ((avctx->height & 0xf) || (avctx->width & 0xf)) {
        ctx->fill_y = (uint8_t *)av_malloc(4 * (DEFAULT_SLICE_MB_WIDTH << 8));
        if (!ctx->fill_y)
...
static const enum PixelFormat _ff_prores_encoder[] = {PIX_FMT_YUV422P10, PIX_FMT_NONE};
