...
static av_cold int wmavoice_decode_init(AVCodecContext *ctx)
{
    ...
    WMAVoiceContext *s = (WMAVoiceContext *)ctx->priv_data;
                       ...
                       int fcb_type,      int pitch)
{
    ...
    adaptive_gain_control(samples, synth_pf, synth, size, 0.99f,
                          &s->postfilter_agc);
    ...
    if (s->dc_level > 8) {
		...
		const float _t1[2] = { -1.99997f,      1.0f };
		const float _t2[2] = { -1.9330735188f, 0.93589198496f };
        ff_acelp_apply_order_2_transfer_function(samples, samples,
            _t1,
            _t2,
            0.93980580475f, s->dcf_mem, size);
    }
}
/**
 * @}
                                ...
                                float *excitation)
{
    static const float gain_coeff[6] = {
        0.8169f, -0.06545f, 0.1726f, 0.0185f, -0.0359f, 0.0458f
    };
    ...
    pred_err = av_clipf(wmavoice_gain_codebook_fcb[idx],
                        -2.9957322736f /* log(0.05) */,
                         1.6094379124f /* log(5.0)  */);
