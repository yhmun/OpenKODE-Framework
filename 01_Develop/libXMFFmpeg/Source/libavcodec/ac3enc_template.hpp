...
int AC3_NAME(allocate_sample_buffers)(AC3EncodeContext *s)
{
    ...
    FF_ALLOC_OR_GOTO(SampleType*, s->avctx, s->windowed_samples, AC3_WINDOW_SIZE *
                     sizeof(*s->windowed_samples), alloc_fail);
    FF_ALLOC_OR_GOTO(SampleType**, s->avctx, s->planar_samples, s->channels * sizeof(*s->planar_samples),
                     alloc_fail);
    for (ch = 0; ch < s->channels; ch++) {
        FF_ALLOCZ_OR_GOTO(SampleType*, s->avctx, s->planar_samples[ch],
...
static void apply_mdct(AC3EncodeContext *s)
{
    ...
    for (ch = 0; ch < s->channels; ch++) {
        for (blk = 0; blk < s->num_blocks; blk++) {
            ...
            s->mdct.mdct_calcw(&s->mdct, (FFTDouble*)block->mdct_coef[ch+1],
                               s->windowed_samples);
        }
    }
}
...
{
	DECLARE_ALIGNED(16, CoefType, cpl_coords) [AC3_MAX_BLOCKS][AC3_MAX_CHANNELS][16];
                           ...
                           int buf_size, void *data)
{
    ...
    const SampleType *samples = (const SampleType *)data;
    int ret;
