...
static av_cold int tgv_decode_init(AVCodecContext *avctx){
    TgvContext *s = (TgvContext *)avctx->priv_data;
    s->avctx = avctx;
    avctx->time_base = av_rational(1, 15);
...
static int tgv_decode_inter(TgvContext * s, const uint8_t *buf, const uint8_t *buf_end){
    ...
    if (num_mvs > s->num_mvs) {
        s->mv_codebook = (int (*)[2])av_realloc(s->mv_codebook, num_mvs*2*sizeof(int));
        s->num_mvs = num_mvs;
    }

    if (num_blocks_packed > s->num_blocks_packed) {
        s->block_codebook = (unsigned char (*)[16])av_realloc(s->block_codebook, num_blocks_packed*16*sizeof(unsigned char));
                            ...
                            AVPacket *avpkt)
{
    ...
    if (!s->frame.data[0]) {
        ...
        s->frame.data[0] = (uint8_t *)av_malloc(s->width*s->height + 12);
        if (!s->frame.data[0])
            return AVERROR(ENOMEM);
        s->frame.data[1] = (uint8_t *)av_malloc(AVPALETTE_SIZE);
        if (!s->frame.data[1]) {
