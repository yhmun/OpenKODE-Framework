...
static int encode_frame(AVCodecContext *avctx, unsigned char *buf, int buf_size, void *data){
    PNGEncContext *s = (PNGEncContext *)avctx->priv_data;
    ...
    *p = *pict;
    ...
    s->zstream.zalloc = (void* (*)(void*, uInt, uInt))ff_png_zalloc;
    ...
    if (ret != Z_OK)
        return -1;
    crow_base = (uint8_t *)av_malloc((row_size + 32) << (s->filter_type == PNG_FILTER_VALUE_MIXED));
    if (!crow_base)
        goto fail;
    crow_buf = crow_base + 15; // pixel data should be aligned, but there's a control byte before it
    if (is_progressive) {
        progressive_buf = (uint8_t *)av_malloc(row_size + 1);
        if (!progressive_buf)
            goto fail;
    }
    if (is_progressive) {
        top_buf = (uint8_t *)av_malloc(row_size + 1);
        if (!top_buf)
            goto fail;
    }
...
static const enum PixelFormat _ff_png_encoder[] = {PIX_FMT_RGB24, PIX_FMT_RGBA,
