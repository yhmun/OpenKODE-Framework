...
static av_cold int flashsv_encode_init(AVCodecContext *avctx)
{
    ...
    s->tmpblock  = (uint8_t *)av_mallocz(3 * 256 * 256);
    s->encbuffer = (uint8_t *)av_mallocz(s->image_width * s->image_height * 3);
    ...
    for (j = 0; j < v_blocks + (v_part ? 1 : 0); j++) {
        ...
        for (i = 0; i < h_blocks + (h_part ? 1 : 0); i++) {
            ...
            if (res || *I_frame) {
                ...
                ret = compress2(ptr + 2, (z_uLongf *)&zsize, s->tmpblock,
...
{
    FlashSVContext * const s = (FlashSVContext *const)avctx->priv_data;
    ...
    *p = *pict;
    ...
    if (avctx->frame_number == 0) {
        s->previous_frame = (uint8_t *)av_mallocz(FFABS(p->linesize[0]) * s->image_height);
        if (!s->previous_frame) {
            av_log(avctx, AV_LOG_ERROR, "Memory allocation failed.\n");
            return AVERROR(ENOMEM);
        }
...
static const enum PixelFormat _ff_flashsv_encoder[] = {PIX_FMT_BGR24, PIX_FMT_NONE};
