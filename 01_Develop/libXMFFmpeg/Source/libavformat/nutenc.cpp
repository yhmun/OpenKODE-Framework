...
static void put_str(AVIOContext *bc, const char *string){
    ...
    ff_put_v(bc, len);
    avio_write(bc, (const unsigned char *)string, len);
}
...
static int write_streamheader(AVFormatContext *avctx, AVIOContext *bc, AVStream *st, int i){
    NUTContext *nut = (NUTContext *)avctx->priv_data;
...
static int nut_write_header(AVFormatContext *s){
    ...
    nut->stream   = (StreamContext *)av_mallocz(sizeof(StreamContext)*s->nb_streams);
    nut->chapter  = (ChapterContext *)av_mallocz(sizeof(ChapterContext)*s->nb_chapters);
    nut->time_base= (AVRational *)av_mallocz(sizeof(AVRational   )*(s->nb_streams +
                                                      s->nb_chapters));
    ...
    avio_write(bc, (const unsigned char *)ID_STRING, strlen(ID_STRING));
    avio_w8(bc, 0);
...
//FIXME: Ensure store_sp is 1 in the first place.
    ...
    if(ff_lsb2full(nus, coded_pts) != pkt->pts)
        coded_pts= pkt->pts + ((int64_t)1<<nus->msb_pts_shift);
...
static const AVCodecTag * const _ff_nut_muxer[] = { ff_codec_bmp_tags, ff_nut_video_tags, ff_codec_wav_tags, ff_nut_subtitle_tags, 0 };
