...
static int encode_picture_ls(AVCodecContext *avctx, unsigned char *buf, int buf_size, void *data){
    JpeglsContext * const s = (JpeglsContext *const)avctx->priv_data;
    ...
    buf2 = (uint8_t *)av_malloc(buf_size);
    ...
    *p = *pict;
    ...
    state = (JLSState *)av_mallocz(sizeof(JLSState));
    ...
    zero = (uint8_t *)av_mallocz(p->linesize[0]);
    last = zero;
    cur = p->data[0];
...
static const enum PixelFormat _ff_jpegls_encoder[] = {PIX_FMT_BGR24, PIX_FMT_RGB24, PIX_FMT_GRAY8, PIX_FMT_GRAY16, PIX_FMT_NONE};
