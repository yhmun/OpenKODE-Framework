...
{
    ZmbvEncContext * const c = (ZmbvEncContext *const)avctx->priv_data;
...
static av_cold int encode_init(AVCodecContext *avctx)
{
    ...
    if ((c->work_buf = (uint8_t *)av_malloc(c->comp_size)) == NULL) {
    ...
    if ((c->comp_buf = (uint8_t *)av_malloc(c->comp_size)) == NULL) {
        av_log(avctx, AV_LOG_ERROR, "Can't allocate compression buffer.\n");
        return -1;
    }
    c->pstride = FFALIGN(avctx->width, 16);
    if ((c->prev = (uint8_t *)av_malloc(c->pstride * avctx->height)) == NULL) {
...
static const enum PixelFormat _ff_zmbv_encoder[] = {PIX_FMT_PAL8, PIX_FMT_NONE};
