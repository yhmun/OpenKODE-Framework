...
static const AVOption options[] = {
    ...
    { "payload_type", "Specify RTP payload type", offsetof(RTPMuxContext, payload_type), AV_OPT_TYPE_INT, -1, NULL, -1, 127, AV_OPT_FLAG_ENCODING_PARAM },
    { NULL },
};
...
{
    RTPMuxContext *s = (RTPMuxContext *)s1->priv_data;
    ...
    if (max_packet_size <= 12)
        return AVERROR(EIO);
    s->buf = (uint8_t *)av_malloc(max_packet_size);
    if (s->buf == NULL) {
    ...
    if (s1->max_delay) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            s->max_frames_per_packet = av_rescale_q(s1->max_delay, av_rational(1, 1000000), st->codec->time_base);
        }
    }
...
static void rtcp_send_sr(AVFormatContext *s1, int64_t ntp_time)
{
    ...
    rtp_ts = av_rescale_q(ntp_time - s->first_rtcp_ntp_time, av_rational(1, 1000000),
