...
xmlIsXHTML(const xmlChar *systemID, const xmlChar *publicID) {
    if ((systemID == KD_NULL) && (publicID == KD_NULL))
	return(-1);
    if (publicID != KD_NULL) {
...
{
    __xmlSimpleError(XML_FROM_OUTPUT, XML_ERR_NO_MEMORY, KD_NULL, KD_NULL, extra);
}
...
xmlEscapeEntities(unsigned char* out, int *outlen,
    ...
    while ((in < inend) && (out < outend)) {
	    ...
	    *out++ = *in++;
	    ...
	    if (*in < 0xC0) {
		xmlSaveErr(XML_SAVE_NOT_UTF8, KD_NULL, KD_NULL);
		in++;
		goto error;
	    } else if (*in < 0xE0) {
		if (inend - in < 2) break;
		...
		xmlSaveErr(XML_SAVE_CHAR_INVALID, KD_NULL, KD_NULL);
		in++;
		goto error;
	    }
	    if (!IS_CHAR(val)) {
...
xmlSaveCtxtInit(xmlSaveCtxtPtr ctxt)
{
    ...
    if ((ctxt->encoding == KD_NULL) && (ctxt->escape == KD_NULL))
        ctxt->escape = xmlEscapeEntities;
    len = xmlStrlen((xmlChar *)xmlTreeIndentString);
    if ((xmlTreeIndentString == KD_NULL) || (len == 0)) {
        kdMemset(&ctxt->indent[0], 0, MAX_INDENT + 1);
    } else {
	ctxt->indent_size = len;
	ctxt->indent_nr = MAX_INDENT / ctxt->indent_size;
	for (i = 0;i < ctxt->indent_nr;i++)
	    kdMemcpy(&ctxt->indent[i * ctxt->indent_size], xmlTreeIndentString,
		   ctxt->indent_size);
...
xmlNewSaveCtxt(const char *encoding, int options)
{
    ...
    kdMemset(ret, 0, sizeof(xmlSaveCtxt));

    if (encoding != KD_NULL) {
        ret->handler = xmlFindCharEncodingHandler(encoding);
	if (ret->handler == KD_NULL) {
	    xmlSaveErr(XML_SAVE_UNKNOWN_ENCODING, KD_NULL, encoding);
            xmlFreeSaveCtxt(ret);
	    return(KD_NULL);
	}
...
static int xmlSaveSwitchEncoding(xmlSaveCtxtPtr ctxt, const char *encoding) {
    ...
    if ((encoding != KD_NULL) && (buf->encoder == KD_NULL) && (buf->conv == KD_NULL)) {
	buf->encoder = xmlFindCharEncodingHandler((const char *)encoding);
	if (buf->encoder == KD_NULL) {
	    xmlSaveErr(XML_SAVE_UNKNOWN_ENCODING, KD_NULL,
		       (const char *)encoding);
	    return(-1);
	}
	 ...
	 */
	xmlCharEncOutFunc(buf->encoder, buf->conv, KD_NULL);
    }
    return(0);
}
...
{
    int i;
    if ((ctxt == KD_NULL) || (ctxt->buf == KD_NULL))
        return;
...
xmlNsDumpOutput(xmlOutputBufferPtr buf, xmlNsPtr cur, xmlSaveCtxtPtr ctxt) {
    if ((cur == KD_NULL) || (buf == KD_NULL)) return;
    if ((cur->type == XML_LOCAL_NAMESPACE) && (cur->href != KD_NULL)) {
	if (xmlStrEqual(cur->prefix, BAD_CAST "xml"))
	    return;

	if (ctxt != KD_NULL && ctxt->format == 2)
...
xmlNsListDumpOutput(xmlOutputBufferPtr buf, xmlNsPtr cur) {
    while (cur != KD_NULL) {
        xmlNsDumpOutput(buf, cur, KD_NULL);
	cur = cur->next;
    }
}
...
xmlDtdDumpOutput(xmlSaveCtxtPtr ctxt, xmlDtdPtr dtd) {
    ...
    }  else if (dtd->SystemID != KD_NULL) {
	xmlOutputBufferWrite(buf, 8, " SYSTEM ");
	xmlBufferWriteQuotedString(buf->buffer, dtd->SystemID);
    }
    if ((dtd->entities == KD_NULL) && (dtd->elements == KD_NULL) &&
        (dtd->attributes == KD_NULL) && (dtd->notations == KD_NULL) &&
	(dtd->pentities == KD_NULL)) {
	xmlOutputBufferWrite(buf, 1, ">");
	return;
    }
     ...
     */
    if ((dtd->notations != KD_NULL) && ((dtd->doc == KD_NULL) ||
...
htmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
	...
	} else if (doc->encoding != KD_NULL) {
	    encoding = doc->encoding;
	}
    }

    if ((encoding != KD_NULL) && (doc != KD_NULL))
	htmlSetMetaEncoding(doc, (const xmlChar *) encoding);
    if ((encoding == KD_NULL) && (doc != KD_NULL))
	encoding = htmlGetMetaEncoding(doc);
    if (encoding == KD_NULL)
	encoding = BAD_CAST "HTML";
    if ((encoding != KD_NULL) && (oldctxtenc == KD_NULL) &&
	(buf->encoder == KD_NULL) && (buf->conv == KD_NULL)) {
     ...
     */
    if ((switched_encoding) && (oldctxtenc == KD_NULL)) {
...
xmlNodeDumpOutputInternal(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
    ...
    if (((cur->type != XML_NAMESPACE_DECL) && (cur->doc != KD_NULL) &&
	...
	(cur->children == KD_NULL) && ((ctxt->options & XML_SAVE_NO_EMPTY) == 0)) {
        if (ctxt->format == 2)
            xmlOutputBufferWriteWSNonSig(ctxt, 0);
        xmlOutputBufferWrite(buf, 2, "/>");
	ctxt->format = format;
...
xhtmlIsEmpty(xmlNodePtr node) {
	...
	return(0);
    if ((node->ns != KD_NULL) && (!xmlStrEqual(node->ns->href, XHTML_NS_NAME)))
	return(0);
...
xhtmlAttrListDumpOutput(xmlSaveCtxtPtr ctxt, xmlAttrPtr cur) {
    xmlAttrPtr xml_lang = KD_NULL;
    xmlAttrPtr lang = KD_NULL;
    xmlAttrPtr name = KD_NULL;
    xmlAttrPtr id = KD_NULL;
    xmlNodePtr parent;
    xmlOutputBufferPtr buf;
    ...
    parent = cur->parent;
    while (cur != KD_NULL) {
	if ((cur->ns == KD_NULL) && (xmlStrEqual(cur->name, BAD_CAST "id")))
	    id = cur;
	else
	if ((cur->ns == KD_NULL) && (xmlStrEqual(cur->name, BAD_CAST "name")))
	    name = cur;
	else
	if ((cur->ns == KD_NULL) && (xmlStrEqual(cur->name, BAD_CAST "lang")))
	    lang = cur;
	else
	if ((cur->ns != KD_NULL) && (xmlStrEqual(cur->name, BAD_CAST "lang")) &&
     ...
     * C.8
     */
    if ((name != KD_NULL) && (id == KD_NULL)) {
	if ((parent != KD_NULL) && (parent->name != KD_NULL) &&
     ...
     * C.7.
     */
    if ((lang != KD_NULL) && (xml_lang == KD_NULL)) {
	xmlOutputBufferWrite(buf, 11, " xml:lang=\"");
	xmlAttrSerializeContent(buf, lang);
	xmlOutputBufferWrite(buf, 1, "\"");
    } else 
    if ((xml_lang != KD_NULL) && (lang == KD_NULL)) {
...
xhtmlNodeDumpOutput(xmlSaveCtxtPtr ctxt, xmlNodePtr cur) {
	...
	(cur->ns == KD_NULL) && (cur->nsDef == KD_NULL))) {
	/*
    ...
    if ((cur->type == XML_ELEMENT_NODE) && (cur->children == KD_NULL)) {
	if (((cur->ns == KD_NULL) || (cur->ns->prefix == KD_NULL)) &&
    ...
    */
	...
	while (child != KD_NULL) {
		...
		if ((xmlStrchr(child->content, '<') == KD_NULL) &&
		    (xmlStrchr(child->content, '&') == KD_NULL) &&
		    (xmlStrstr(child->content, BAD_CAST "]]>") == KD_NULL)) {
 ...
 * @encoding:  the encoding name to use or KD_NULL
 * @options:  a set of xmlSaveOptions
 *
 * Create a document saving context serializing to a file descriptor
 * with the encoding and the options given.
 *
 * Returns a new serialization context or KD_NULL in case of error.
 */
xmlSaveCtxtPtr
...
xmlSaveDoc(xmlSaveCtxtPtr ctxt, xmlDocPtr doc)
{
    ...
    if ((ctxt == KD_NULL) || (doc == KD_NULL)) return(-1);
...
xmlSaveTree(xmlSaveCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    if ((ctxt == KD_NULL) || (node == KD_NULL)) return(-1);
    ...
    while (*cur != 0) {
        if (*cur == '\n') {
            ...
            if (base != cur)
                xmlBufferAdd(buf, base, cur - base);
            if (*cur < 0xC0) {
                xmlSaveErr(XML_SAVE_NOT_UTF8, (xmlNodePtr) attr, KD_NULL);
                if (doc != KD_NULL)
		...
		xmlSerializeHexCharRef(tmp, *cur);
            ...
            if ((l == 1) || (!IS_CHAR(val))) {
                xmlSaveErr(XML_SAVE_CHAR_INVALID, (xmlNodePtr) attr, KD_NULL);
                if (doc != KD_NULL)
                    doc->encoding = xmlStrdup(BAD_CAST "ISO-8859-1");
            ...
            int format)
{
    ...
    use = buf->use;
    xmlNodeDumpOutput(outbuf, doc, cur, level, format, KD_NULL);
    xmlFree(outbuf);
    ret = buf->use - use;
    return (ret);
}
 ...
 */
void
xmlElemDump(KDFile * f, xmlDocPtr doc, xmlNodePtr cur)
{
    xmlOutputBufferPtr outbuf;
    ...
    if (doc == KD_NULL) {
        xmlGenericError(xmlGenericErrorContext,
                        "xmlElemDump : doc == KD_NULL\n");
    }
#endif

    outbuf = xmlOutputBufferCreateFile(f, KD_NULL);
    if (outbuf == KD_NULL)
        return;
    if ((doc != KD_NULL) && (doc->type == XML_HTML_DOCUMENT_NODE)) {
#ifdef LIBXML_HTML_ENABLED
        htmlNodeDumpOutput(outbuf, doc, cur, KD_NULL);
#else
	xmlSaveErr(XML_ERR_INTERNAL_ERROR, cur, "HTML support not compiled in\n");
#endif /* LIBXML_HTML_ENABLED */
    } else
        xmlNodeDumpOutput(outbuf, doc, cur, 0, 1, KD_NULL);
                  ...
                  int level, int format, const char *encoding)
{
    ...
    if ((buf == KD_NULL) || (cur == KD_NULL)) return;
...
xmlDocDumpFormatMemoryEnc(xmlDocPtr out_doc, xmlChar **doc_txt_ptr,
    ...
    xmlSaveCtxt ctxt;
    int                         dummy = 0;
    xmlOutputBufferPtr          out_buff = KD_NULL;
    xmlCharEncodingHandlerPtr   conv_hdlr = KD_NULL;

    if (doc_txt_len == KD_NULL) {
    ...
    *doc_txt_len = 0;
    ...
    if (txt_encoding == KD_NULL)
	txt_encoding = (const char *) out_doc->encoding;
    if (txt_encoding != KD_NULL) {
	conv_hdlr = xmlFindCharEncodingHandler(txt_encoding);
	if ( conv_hdlr == KD_NULL ) {
    ...
    if ((out_buff = xmlAllocOutputBuffer(conv_hdlr)) == KD_NULL ) {
        xmlSaveErrMemory("creating buffer");
        return;
    }
	...
	*doc_txt_ptr = xmlStrndup(out_buff->buffer->content, *doc_txt_len);
    ...
    if ((*doc_txt_ptr == KD_NULL) && (*doc_txt_len > 0)) {
        *doc_txt_len = 0;
...
xmlDocDumpMemory(xmlDocPtr cur, xmlChar**mem, int *size) {
    xmlDocDumpFormatMemoryEnc(cur, mem, size, KD_NULL, 0);
}
...
xmlDocDumpFormatMemory(xmlDocPtr cur, xmlChar**mem, int *size, int format) {
    xmlDocDumpFormatMemoryEnc(cur, mem, size, KD_NULL, format);
}
 ...
 */
int
xmlDocFormatDump(KDFile *f, xmlDocPtr cur, int format) {
    xmlSaveCtxt ctxt;
    xmlOutputBufferPtr buf;
    const char * encoding;
 ...
 */
int
xmlDocDump(KDFile *f, xmlDocPtr cur) {
 ...
 * @encoding:  the name of the encoding to use or KD_NULL.
 ...
 * @cur:  the document
 * @encoding:  the name of an encoding (or KD_NULL)
 *
...
xmlSaveFormatFile(const char *filename, xmlDocPtr cur, int format) {
    return ( xmlSaveFormatFileEnc( filename, cur, KD_NULL, format ) );
}
...
xmlSaveFile(const char *filename, xmlDocPtr cur) {
    return(xmlSaveFormatFileEnc(filename, cur, KD_NULL, 0));
}
