...
{
    SoXContext *sox = (SoXContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    AVCodecContext *enc = s->streams[0]->codec;
    AVDictionaryEntry *comment;
    KDsize comment_len = 0, comment_size;
    ...
    if (enc->codec_id == CODEC_ID_PCM_S32LE) {
        ffio_wfourcc(pb, (const uint8_t *)".SoX");
        ...
        ffio_wfourcc(pb, (const uint8_t *)"XoS.");
    ...
    if (comment_len)
        avio_write(pb, (const unsigned char *)comment->value, comment_len);
