...
static av_cold int read_specific_config(ALSDecContext *ctx)
{
    ...
    sconf->ra_flag              = (RA_Flag)get_bits(&gb, 2);
    ...
    if (sconf->chan_sort && avctx->channels > 1) {
        ...
        if (!(sconf->chan_pos = (int *)av_malloc(avctx->channels * sizeof(*sconf->chan_pos))))
...
static int decode_frame(AVCodecContext *avctx, void *data, int *got_frame_ptr,
                        AVPacket *avpkt)
{
    ALSDecContext *ctx       = (ALSDecContext *)avctx->priv_data;
    ALSSpecificConfig *sconf = &ctx->sconf;
...
static av_cold int decode_init(AVCodecContext *avctx)
{
    ...
    ctx->quant_cof        = (int32_t **)av_malloc(sizeof(*ctx->quant_cof) * num_buffers);
    ctx->lpc_cof          = (int32_t **)av_malloc(sizeof(*ctx->lpc_cof)   * num_buffers);
    ctx->quant_cof_buffer = (int32_t *)av_malloc(sizeof(*ctx->quant_cof_buffer) *
                                      num_buffers * sconf->max_order);
    ctx->lpc_cof_buffer   = (int32_t *)av_malloc(sizeof(*ctx->lpc_cof_buffer) *
                                      num_buffers * sconf->max_order);
    ctx->lpc_cof_reversed_buffer = (int32_t *)av_malloc(sizeof(*ctx->lpc_cof_buffer) *
                                             sconf->max_order);
    ...
    ctx->const_block     = (int *)av_malloc (sizeof(*ctx->const_block) * num_buffers);
    ctx->shift_lsbs      = (unsigned int *)av_malloc (sizeof(*ctx->shift_lsbs)  * num_buffers);
    ctx->opt_order       = (unsigned int *)av_malloc (sizeof(*ctx->opt_order)   * num_buffers);
    ctx->store_prev_samples = (int *)av_malloc(sizeof(*ctx->store_prev_samples) * num_buffers);
    ctx->use_ltp         = (int *)av_mallocz(sizeof(*ctx->use_ltp)  * num_buffers);
    ctx->ltp_lag         = (int *)av_malloc (sizeof(*ctx->ltp_lag)  * num_buffers);
    ctx->ltp_gain        = (int **)av_malloc (sizeof(*ctx->ltp_gain) * num_buffers);
    ctx->ltp_gain_buffer = (int *)av_malloc (sizeof(*ctx->ltp_gain_buffer) *
                                      num_buffers * 5);
    ...
    if (sconf->mc_coding) {
        ctx->chan_data_buffer  = (ALSChannelData *)av_malloc(sizeof(*ctx->chan_data_buffer) *
                                           num_buffers * num_buffers);
        ctx->chan_data         = (ALSChannelData **)av_malloc(sizeof(*ctx->chan_data) *
                                           num_buffers);
        ctx->reverted_channels = (int *)av_malloc(sizeof(*ctx->reverted_channels) *
                                           num_buffers);
    ...
    ctx->prev_raw_samples = (int32_t *)av_malloc (sizeof(*ctx->prev_raw_samples) * sconf->max_order);
    ctx->raw_buffer       = (int32_t *)av_mallocz(sizeof(*ctx->     raw_buffer)  * avctx->channels * channel_size);
    ctx->raw_samples      = (int32_t **)av_malloc (sizeof(*ctx->     raw_samples) * avctx->channels);
    ...
    if (HAVE_BIGENDIAN != sconf->msb_first && sconf->crc_enabled &&
        ...
        ctx->crc_buffer = (uint8_t *)av_malloc(sizeof(*ctx->crc_buffer) *
