...
static av_always_inline void free_fragment_if_needed(PayloadContext * data)
{
    if (data->fragment) {
        uint8_t* p;
    ...
    if (fragmented == 0) {
        ...
        if (num_pkts > 0) {
            if (len > data->split_buf_size || !data->split_buf) {
                ...
                data->split_buf = (uint8_t *)av_malloc(data->split_buf_size);
                if (!data->split_buf) {
                    av_log(ctx, AV_LOG_ERROR, "Out of memory.\n");
                    av_free_packet(pkt);
                    return AVERROR(ENOMEM);
                     ...
                     AVCodecContext * codec, PayloadContext * xiph_data)
{
    ...
    ptr = codec->extradata = (uint8_t *)av_malloc(extradata_alloc);
    if (!ptr) {
                                ...
                                char *attr, char *value)
{
    ...
    if (!strcmp(attr, "sampling")) {
        ...
        int packet_size;
        KDsize decoded_alloc = strlen(value) / 4 * 3 + 4;

        if (decoded_alloc <= INT_MAX) {
            decoded_packet = (uint8_t *)av_malloc(decoded_alloc);
            if (decoded_packet) {
                packet_size =
