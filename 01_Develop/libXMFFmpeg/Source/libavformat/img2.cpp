...
typedef struct {
    const AVClass *av_class;  /**< Class for private options. */
    int img_first;
    int img_last;
    int img_number;
    int img_count;
...
{
    VideoData *s = (VideoData *)s1->priv_data;
...
{
    VideoData *img = (VideoData *)s->priv_data;
    const char *str;
            ...
            if(st->codec->extradata_size > 8 &&
               AV_RL32(st->codec->extradata+4) == MKTAG('j','p','2','h')){
                if(pkt->size < 8 || AV_RL32(pkt->data+4) != MKTAG('j','p','2','c'))
                    goto error;
                avio_wb32(pb[0], 12);
                ffio_wfourcc(pb[0], (const uint8_t *)"jP  ");
                avio_wb32(pb[0], 0x0D0A870A); // signature
                avio_wb32(pb[0], 20);
                ffio_wfourcc(pb[0], (const uint8_t *)"ftyp");
                ffio_wfourcc(pb[0], (const uint8_t *)"jp2 ");
                avio_wb32(pb[0], 0);
                ffio_wfourcc(pb[0], (const uint8_t *)"jp2 ");
...
static const AVOption options[] = {
    { "pixel_format", "", OFFSET(pixel_format), AV_OPT_TYPE_STRING, /*{.str = NULL}*/0, 0, 0, DEC },
    { "video_size",   "", OFFSET(video_size),   AV_OPT_TYPE_STRING, /*{.str = NULL}*/0, 0, 0, DEC },
    { "framerate",    "", OFFSET(framerate),    AV_OPT_TYPE_STRING, /*{.str = "25"}*/0, 0, 0, DEC },
    { "loop",         "", OFFSET(loop),         AV_OPT_TYPE_INT,    /*{.dbl = 0}*/0,    0, 1, DEC },
    { NULL },
};

static const AVOption muxoptions[] = {
    { "updatefirst",  "", OFFSET(updatefirst),  AV_OPT_TYPE_INT, /*{.dbl = 0}*/ 0,    0, 1, ENC },
    { NULL },
};
