...
{
    if ((ctxt == KD_NULL) || (name == KD_NULL) || (result == KD_NULL))
        return (-1);

    if (!kdStrcmp (name, "validate")) {
        *((int *) result) = ctxt->validate;
    } else if (!kdStrcmp (name, "keep blanks")) {
        *((int *) result) = ctxt->keepBlanks;
    } else if (!kdStrcmp (name, "disable SAX")) {
        *((int *) result) = ctxt->disableSAX;
    } else if (!kdStrcmp (name, "fetch external entities")) {
        *((int *) result) = ctxt->loadsubset;
    } else if (!kdStrcmp (name, "substitute entities")) {
        *((int *) result) = ctxt->replaceEntities;
    } else if (!kdStrcmp (name, "gather line info")) {
        *((int *) result) = ctxt->record_info;
    } else if (!kdStrcmp (name, "user data")) {
        *((void **) result) = ctxt->userData;
    } else if (!kdStrcmp (name, "is html")) {
        *((int *) result) = ctxt->html;
    } else if (!kdStrcmp (name, "is standalone")) {
        *((int *) result) = ctxt->standalone;
    } else if (!kdStrcmp (name, "document")) {
        *((xmlDocPtr *) result) = ctxt->myDoc;
    } else if (!kdStrcmp (name, "is well formed")) {
        *((int *) result) = ctxt->wellFormed;
    } else if (!kdStrcmp (name, "is valid")) {
        *((int *) result) = ctxt->valid;
    } else if (!kdStrcmp (name, "SAX block")) {
        *((xmlSAXHandlerPtr *) result) = ctxt->sax;
    } else if (!kdStrcmp (name, "SAX function internalSubset")) {
        *((internalSubsetSAXFunc *) result) = ctxt->sax->internalSubset;
    } else if (!kdStrcmp (name, "SAX function isStandalone")) {
        *((isStandaloneSAXFunc *) result) = ctxt->sax->isStandalone;
    } else if (!kdStrcmp (name, "SAX function hasInternalSubset")) {
        *((hasInternalSubsetSAXFunc *) result) =
            ctxt->sax->hasInternalSubset;
    } else if (!kdStrcmp (name, "SAX function hasExternalSubset")) {
        *((hasExternalSubsetSAXFunc *) result) =
            ctxt->sax->hasExternalSubset;
    } else if (!kdStrcmp (name, "SAX function resolveEntity")) {
        *((resolveEntitySAXFunc *) result) = ctxt->sax->resolveEntity;
    } else if (!kdStrcmp (name, "SAX function getEntity")) {
        *((getEntitySAXFunc *) result) = ctxt->sax->getEntity;
    } else if (!kdStrcmp (name, "SAX function entityDecl")) {
        *((entityDeclSAXFunc *) result) = ctxt->sax->entityDecl;
    } else if (!kdStrcmp (name, "SAX function notationDecl")) {
        *((notationDeclSAXFunc *) result) = ctxt->sax->notationDecl;
    } else if (!kdStrcmp (name, "SAX function attributeDecl")) {
        *((attributeDeclSAXFunc *) result) = ctxt->sax->attributeDecl;
    } else if (!kdStrcmp (name, "SAX function elementDecl")) {
        *((elementDeclSAXFunc *) result) = ctxt->sax->elementDecl;
    } else if (!kdStrcmp (name, "SAX function unparsedEntityDecl")) {
        *((unparsedEntityDeclSAXFunc *) result) =
            ctxt->sax->unparsedEntityDecl;
    } else if (!kdStrcmp (name, "SAX function setDocumentLocator")) {
        *((setDocumentLocatorSAXFunc *) result) =
            ctxt->sax->setDocumentLocator;
    } else if (!kdStrcmp (name, "SAX function startDocument")) {
        *((startDocumentSAXFunc *) result) = ctxt->sax->startDocument;
    } else if (!kdStrcmp (name, "SAX function endDocument")) {
        *((endDocumentSAXFunc *) result) = ctxt->sax->endDocument;
    } else if (!kdStrcmp (name, "SAX function startElement")) {
        *((startElementSAXFunc *) result) = ctxt->sax->startElement;
    } else if (!kdStrcmp (name, "SAX function endElement")) {
        *((endElementSAXFunc *) result) = ctxt->sax->endElement;
    } else if (!kdStrcmp (name, "SAX function reference")) {
        *((referenceSAXFunc *) result) = ctxt->sax->reference;
    } else if (!kdStrcmp (name, "SAX function characters")) {
        *((charactersSAXFunc *) result) = ctxt->sax->characters;
    } else if (!kdStrcmp (name, "SAX function ignorableWhitespace")) {
        *((ignorableWhitespaceSAXFunc *) result) =
            ctxt->sax->ignorableWhitespace;
    } else if (!kdStrcmp (name, "SAX function processingInstruction")) {
        *((processingInstructionSAXFunc *) result) =
            ctxt->sax->processingInstruction;
    } else if (!kdStrcmp (name, "SAX function comment")) {
        *((commentSAXFunc *) result) = ctxt->sax->comment;
    } else if (!kdStrcmp (name, "SAX function warning")) {
        *((warningSAXFunc *) result) = ctxt->sax->warning;
    } else if (!kdStrcmp (name, "SAX function error")) {
        *((errorSAXFunc *) result) = ctxt->sax->error;
    } else if (!kdStrcmp (name, "SAX function fatalError")) {
        *((fatalErrorSAXFunc *) result) = ctxt->sax->fatalError;
    } else if (!kdStrcmp (name, "SAX function getParameterEntity")) {
        *((getParameterEntitySAXFunc *) result) =
            ctxt->sax->getParameterEntity;
    } else if (!kdStrcmp (name, "SAX function cdataBlock")) {
        *((cdataBlockSAXFunc *) result) = ctxt->sax->cdataBlock;
    } else if (!kdStrcmp (name, "SAX function externalSubset")) {
...
{
    if ((ctxt == KD_NULL) || (name == KD_NULL) || (value == KD_NULL))
        return (-1);
    ...
    if (!kdStrcmp (name, "validate")) {
        ...
        if ((!ctxt->validate) && (newvalidate != 0)) {
            if (ctxt->vctxt.warning == KD_NULL)
                ctxt->vctxt.warning = xmlParserValidityWarning;
            if (ctxt->vctxt.error == KD_NULL)
 ...
 *
 * Returns the string parser or KD_NULL.
 */
xmlChar *
 ...
 *   DEPRECATED !!!
 * Returns KD_NULL this functionality had been removed
 */
xmlNsPtr
 ...
 * DEPRECATED
 *
 * Returns the xmlNsPtr or KD_NULL if none
 */
...
checkNamespace(void *ctx ATTRIBUTE_UNUSED,
               xmlChar * _namespace ATTRIBUTE_UNUSED)
{
