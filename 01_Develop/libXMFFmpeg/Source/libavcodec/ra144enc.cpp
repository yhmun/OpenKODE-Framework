...
static av_cold int ra144_encode_init(AVCodecContext * avctx)
{
    ...
    avctx->bit_rate = 8000;
    ractx = (RA144Context *)avctx->priv_data;
...
{
    RA144Context *ractx = (RA144Context *)avctx->priv_data;
                                  ...
                                  PutBitContext *pb)
{
    ...
    cba_idx = adaptive_cb_search((const int16_t *)ractx->adapt_cb, work + LPC_ORDER, coefs,
                                 data);
    if (cba_idx) {
        /**
         * The filtered vector from the adaptive codebook can be retrieved from
        ...
        ff_copy_and_dup(cba_vect, (const int16_t *)ractx->adapt_cb, cba_idx + BLOCKSIZE / 2 - 1);
    ...
    ff_subblock_synthesis(ractx, (const uint16_t *)lpc_coefs, cba_idx, cb1_idx, cb2_idx, rms,
                          gain);
}
                              ...
                              int buf_size, void *data)
{
    ...
    if (ff_eval_refl(lpc_refl, block_coefs[NBLOCKS - 1], avctx)) {
         ...
         */
        ff_int_to_int16(block_coefs[NBLOCKS - 1], (const int *)ractx->lpc_coef[1]);
        if (ff_eval_refl(lpc_refl, block_coefs[NBLOCKS - 1], avctx)) {
    ...
    ff_eval_coefs((int *)ractx->lpc_coef[0], lpc_refl);
    ...
    ff_int_to_int16(block_coefs[NBLOCKS - 1], (const int *)ractx->lpc_coef[0]);
...
static const enum AVSampleFormat _ff_ra_144_encoder[] = { AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_NONE };
AVCodec ff_ra_144_encoder = {
	...
	/*.sample_fmts			= */	_ff_ra_144_encoder,
};
