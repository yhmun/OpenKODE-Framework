...
static int decode_frame(AVCodecContext *avctx, void *data, int *data_size, AVPacket *avpkt)
{
    ...
    ZmbvContext * const c = (ZmbvContext *const)avctx->priv_data;
    ...
    if (c->flags & ZMBV_KEYFRAME) {
        ...
        case ZMBV_FMT_8BPP:
            c->bpp = 8;
            decode_intra = (void*)zmbv_decode_intra;
        ...
        c->cur  = (uint8_t *)av_realloc_f(c->cur, avctx->width * avctx->height,  (c->bpp / 8));
        c->prev = (uint8_t *)av_realloc_f(c->prev, avctx->width * avctx->height,  (c->bpp / 8));
        c->bx = (c->width + c->bw - 1) / c->bw;
        c->by = (c->height+ c->bh - 1) / c->bh;
        if (!c->cur || !c->prev)
            return -1;
        c->decode_intra= (int (*)(ZmbvContext *))decode_intra;
    }
...
static av_cold int decode_init(AVCodecContext *avctx)
{
    ...
    if (c->decomp_size) {
        if ((c->decomp_buf = (uint8_t *)av_malloc(c->decomp_size)) == NULL) {
            av_log(avctx, AV_LOG_ERROR,
