...
{
    AMRWBContext *ctx = (AMRWBContext *)avctx->priv_data;
    int i;
...
static int decode_mime_header(AMRWBContext *ctx, const uint8_t *buf)
{
    ...
    ctx->fr_cur_mode  = (Mode)get_bits(&gb, 4);
                          ...
                          uint16_t hb_idx, uint8_t vad)
{
    ...
    return av_clipf((1.0f - FFMAX(0.0f, tilt)) * (1.25f - 0.25f * wsp), 0.1f, 1.0f);
}
                         ...
                         const float *exc, const float *isf, const float *isf_past)
{
    ...
    if (mode == MODE_6k60) {
        ...
        lpc_weighting(hb_lpc, hb_lpc, 0.9f, LP_ORDER_16k);
    } else {
        lpc_weighting(hb_lpc, ctx->lp_coef[subframe], 0.6f, LP_ORDER);
    }
...
static const enum AVSampleFormat _ff_amrwb_decoder[] = {AV_SAMPLE_FMT_FLT,AV_SAMPLE_FMT_NONE};
