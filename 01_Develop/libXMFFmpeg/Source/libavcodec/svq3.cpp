...
static int svq3_decode_mb(SVQ3Context *svq3, unsigned int mb_type)
{
    ...
    if (IS_INTRA16x16(mb_type)) {
        ...
        if (svq3_decode_block(&s->gb, (DCTELEM *)h->mb_luma_dc, 0, 1)){
...
{
    SVQ3Context *svq3 = (SVQ3Context *)avctx->priv_data;
    H264Context *h = &svq3->h;
    ...
    if (!s->context_initialized) {
        ...
        if (marker_found) {
            ...
            if (svq3->unknown_flag) {
                ...
                if (uncompress(buf, (z_uLongf *)&buf_len, extradata + 8 + offset, size - offset) != Z_OK) {
                             ...
                             AVPacket *avpkt)
{
    ...
    if (svq3->watermark_key) {
        av_fast_malloc(&svq3->buf, (unsigned int *)&svq3->buf_size,
    ...
    if (avctx->debug&FF_DEBUG_PICT_INFO){
        av_log(h->s.avctx, AV_LOG_DEBUG, "%c hpel:%d, tpel:%d aqp:%d qp:%d, slice_num:%02X\n",
               av_get_picture_type_char((AVPictureType)s->pict_type), svq3->halfpel_flag, svq3->thirdpel_flag,
...
static const enum PixelFormat _ff_svq3_decoder[] = {PIX_FMT_YUVJ420P, PIX_FMT_NONE};
