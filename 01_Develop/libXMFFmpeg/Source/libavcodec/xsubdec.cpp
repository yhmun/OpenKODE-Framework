...
static int decode_frame(AVCodecContext *avctx, void *data, int *data_size,
    ...
    if (avpkt->pts != AV_NOPTS_VALUE)
        packet_time = av_rescale_q(avpkt->pts, AV_TIME_BASE_Q, av_rational(1, 1000));
    ...
    sub->rects[0]->pict.data[0] = (uint8_t *)av_malloc(w * h);
    sub->rects[0]->nb_colors = 4;
    sub->rects[0]->pict.data[1] = (uint8_t *)av_mallocz(AVPALETTE_SIZE);
