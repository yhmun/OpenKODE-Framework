...
void ff_cavs_init_top_lines(AVSContext *h) {
    ...
    h->top_qp       = (uint8_t *)av_malloc( h->mb_width);
    h->top_mv[0]    = (cavs_vector *)av_malloc((h->mb_width*2+1)*sizeof(cavs_vector));
    h->top_mv[1]    = (cavs_vector *)av_malloc((h->mb_width*2+1)*sizeof(cavs_vector));
    h->top_pred_Y   = (int *)av_malloc( h->mb_width*2*sizeof(*h->top_pred_Y));
    h->top_border_y = (uint8_t *)av_malloc((h->mb_width+1)*16);
    h->top_border_u = (uint8_t *)av_malloc( h->mb_width * 10);
    h->top_border_v = (uint8_t *)av_malloc( h->mb_width * 10);

    /* alloc space for co-located MVs and types */
    h->col_mv       = (cavs_vector *)av_malloc( h->mb_width*h->mb_height*4*sizeof(cavs_vector));
    h->col_type_base = (uint8_t *)av_malloc(h->mb_width*h->mb_height);
    h->block        = (DCTELEM *)av_mallocz(64*sizeof(DCTELEM));
}
