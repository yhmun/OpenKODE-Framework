...
static xmlHashTablePtr xmlSchemaTypesBank = KD_NULL;

/*
 * Basic types
 */
static xmlSchemaTypePtr xmlSchemaTypeStringDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeAnyTypeDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeAnySimpleTypeDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeDecimalDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeDatetimeDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeDateDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeTimeDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeGYearDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeGYearMonthDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeGDayDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeGMonthDayDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeGMonthDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeDurationDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeFloatDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeBooleanDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeDoubleDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeHexBinaryDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeBase64BinaryDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeAnyURIDef = KD_NULL;

/*
 * Derived types
 */
static xmlSchemaTypePtr xmlSchemaTypePositiveIntegerDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNonPositiveIntegerDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNegativeIntegerDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNonNegativeIntegerDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeIntegerDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeLongDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeIntDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeShortDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeByteDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeUnsignedLongDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeUnsignedIntDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeUnsignedShortDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeUnsignedByteDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNormStringDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeTokenDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeLanguageDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNameDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeQNameDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNCNameDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeIdDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeIdrefDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeIdrefsDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeEntityDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeEntitiesDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNotationDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNmtokenDef = KD_NULL;
static xmlSchemaTypePtr xmlSchemaTypeNmtokensDef = KD_NULL;
...
{
    __xmlSimpleError(XML_FROM_DATATYPE, XML_ERR_NO_MEMORY, node, KD_NULL, extra);
}
 ...
 *
 * Returns a pointer to the new value or KD_NULL in case of error
 */
...
xmlSchemaNewValue(xmlSchemaValType type) {
    ...
    kdMemset (value, 0, sizeof(xmlSchemaVal));
    value->type = type;
    return(value);
}
...
xmlSchemaInitBasicType(const char *name, xmlSchemaValType type, 
    ...
    if (ret == KD_NULL) {
        xmlSchemaTypeErrMemory(KD_NULL, "could not initialize basic types");
	return(KD_NULL);
    }
...
xmlSchemaAddParticle(void)
{
    ...
    if (ret == KD_NULL) {
	xmlSchemaTypeErrMemory(KD_NULL, "allocating particle component");
	return (KD_NULL);
    }
    kdMemset (ret, 0, sizeof(xmlSchemaParticle));
...
xmlSchemaInitTypes(void)
{
    ...
    */
	...
	if (sequence == KD_NULL) {
	    xmlSchemaTypeErrMemory(KD_NULL, "allocating model group component");
	    return;
	}
	kdMemset (sequence, 0, sizeof(xmlSchemaModelGroup));
	...
	if (wild == KD_NULL) {
	    xmlSchemaTypeErrMemory(KD_NULL, "allocating wildcard component");
	    return;
	}
	kdMemset (wild, 0, sizeof(xmlSchemaWildcard));
	...
	*/
	wild = (xmlSchemaWildcardPtr) xmlMalloc(sizeof(xmlSchemaWildcard));
	if (wild == KD_NULL) {
	    xmlSchemaTypeErrMemory(KD_NULL, "could not create an attribute "
		"wildcard on anyType");
	    return;
	}
	kdMemset (wild, 0, sizeof(xmlSchemaWildcard));
...
xmlSchemaCleanupTypes(void) {
    ...
    */
	...
	xmlSchemaTypeAnyTypeDef->subtypes = KD_NULL;	
    }
 ...
 * type by its type id.
 *
 * Returns the type if found, KD_NULL otherwise.
 */
xmlSchemaTypePtr
 ...
 *
 * Returns the next value or KD_NULL if there was none, or on
 *         API errors.
 */
xmlSchemaValPtr
 ...
 *
 * Returns the string value or KD_NULL if there was none, or on
 *         API errors.
 */
const xmlChar *
...
{    
    if ((val == KD_NULL) || (val->type != XML_SCHEMAS_BOOLEAN))
	return (0);
    return (val->value.b);
}
			...
			const xmlChar *value)
{
    ...
    kdMemset (val, 0, sizeof(xmlSchemaVal));
    val->type = type;
 ...
 * @ns: the notation namespace name or KD_NULL
 *
 ...
 * The given values are consumed and freed with the struct.
 *
 * Returns a pointer to the new value or KD_NULL in case of an error.
 */
xmlSchemaValPtr
...
xmlSchemaFreeValue(xmlSchemaValPtr value) {
	...
	switch (value->type) {
	    ...
	    case XML_SCHEMAS_ANYURI:
	    case XML_SCHEMAS_ANYSIMPLETYPE:
		if (value->value.str != KD_NULL)
		    xmlFree(value->value.str);
		break;
	    case XML_SCHEMAS_NOTATION:
	    case XML_SCHEMAS_QNAME:
		if (value->value.qname.uri != KD_NULL)
		    xmlFree(value->value.qname.uri);
		if (value->value.qname.name != KD_NULL)
		    xmlFree(value->value.qname.name);
		break;
	    case XML_SCHEMAS_HEXBINARY:
		if (value->value.hex.str != KD_NULL)
		    xmlFree(value->value.hex.str);
		break;
	    case XML_SCHEMAS_BASE64BINARY:
		if (value->value.base64.str != KD_NULL)
 ...
 * hierarchy of XML Schema Part 2: Datatypes, or KD_NULL in case of an error.
 */
xmlSchemaTypePtr
xmlSchemaGetBuiltInListSimpleTypeItemType(xmlSchemaTypePtr type)
{
    if ((type == KD_NULL) || (type->type != XML_SCHEMA_TYPE_BASIC))
	return (KD_NULL);
    switch (type->builtInType) {
	case XML_SCHEMAS_NMTOKENS: 
	    return (xmlSchemaTypeNmtokenDef );
 ...
 *
 * Returns the new string or KD_NULL if no change was required.
 */
static xmlChar *
xmlSchemaStrip(const xmlChar *value) {
    ...
    if ((start == value) && (f == end)) return(KD_NULL);
...
xmlSchemaCollapseString(const xmlChar *value) {
    ...
    if (start == KD_NULL) return(KD_NULL);
    g = (xmlChar *) (start + col);
    end = g;
    while (*end != 0) {
	if (IS_BLANK_CH(*end)) {
...
xmlSchemaValAtomicListNode(xmlSchemaTypePtr type, const xmlChar *value,
    ...
    while (cur != endval) {
	tmp = xmlSchemaValPredefTypeNode(type, cur, KD_NULL, node);
	if (tmp != 0)
	    break;
	while (*cur != 0) cur++;
	while ((*cur == 0) && (cur != endval)) cur++;
		       ...
		       int normOnTheFly, int applyNorm, int createStringValue)
{
	...
	case XML_SCHEMAS_ANYTYPE:
	case XML_SCHEMAS_ANYSIMPLETYPE:
	    if ((createStringValue) && (val != KD_NULL)) {
		...
		*/
		    ...
		    while IS_WSP_BLANK_CH(*cur) cur++;
                ...
                if (val != KD_NULL) {
                    if (type == xmlSchemaTypeFloatDef) {
                        ...
                        if (v != KD_NULL) {
			    /*
			    * TODO: kdSscanfKHR  seems not to give the correct
			    * value for extremely high/low values.
			    * E.g. "1E-149" results in zero.
			    */
                            if (kdSscanfKHR ((const char *) value, "%f",
                                 &(v->value.f)) == 1) {
                                *val = v;
                            } else {
                                xmlSchemaFreeValue(v);
			    ...
			    * value for extremely high/low values.
			    */
                            if (kdSscanfKHR ((const char *) value, "%lf",
                                 &(v->value.d)) == 1) {
                                *val = v;
                            } else {
                                xmlSchemaFreeValue(v);
        ...
        case XML_SCHEMAS_NAME:
            ...
            if ((ret == 0) && (val != KD_NULL) && (value != KD_NULL)) {
        ...
        case XML_SCHEMAS_ID:
            ...
            if ((ret == 0) && (node != KD_NULL) &&
                ...
                if (attr->atype != XML_ATTRIBUTE_ID) {
                    ...
                    if (strip != KD_NULL) {
                        res = xmlAddID(KD_NULL, node->doc, strip, attr);
                        xmlFree(strip);
                    } else
                        res = xmlAddID(KD_NULL, node->doc, value, attr);
                    if (res == KD_NULL) {
                        ret = 2;
                    } else {
		...
		*val = v;
            ...
            if ((ret == 0) && (node != KD_NULL) &&
                ...
                strip = xmlSchemaStrip(value);
                if (strip != KD_NULL) {
                    xmlAddRef(KD_NULL, node->doc, strip, attr);
                    xmlFree(strip);
                } else
                    xmlAddRef(KD_NULL, node->doc, value, attr);
        ...
        case XML_SCHEMAS_ENTITY:{
                ...
                ret = xmlValidateNCName(value, 1);
                if ((node == KD_NULL) || (node->doc == KD_NULL))
                    ret = 3;
                if (ret == 0) {
                    xmlEntityPtr ent;
        ...
        case XML_SCHEMAS_NOTATION:{
                ...
                if ((ret == 0) && (node != KD_NULL)) {
                    ...
                    if ((local != KD_NULL) && ((val == KD_NULL) || (ret != 0)))
                        xmlFree(local);
                    if (prefix != KD_NULL)
                        xmlFree(prefix);
                }
                if ((node == KD_NULL) || (node->doc == KD_NULL))
                    ret = 3;
                if (ret == 0) {
                    ret = xmlValidateNotationUse(KD_NULL, node->doc, value);
                    if (ret == 1)
                        ret = 0;
                    else
                        ret = 1;
...
xmlSchemaValidatePredefinedType(xmlSchemaTypePtr type, const xmlChar *value,
	                        xmlSchemaValPtr *val) {
    return(xmlSchemaValPredefTypeNode(type, value, val, KD_NULL));
}
 ...
 */
#define FQUOTIENT(a,b)                  (kdFloorKHR (((KDfloat64KHR)a/(KDfloat64KHR)b)))
 ...
 * the returned value.
 *
 * returns a pointer to a duplicated #xmlSchemaValPtr or KD_NULL if error.
 */
static xmlSchemaValPtr
xmlSchemaDupVal (xmlSchemaValPtr v)
{
    ...
    kdMemcpy (ret, v, sizeof(xmlSchemaVal));
    ret->next = KD_NULL;
    return ret;
}
 ...
 *
 * Returns the copy or KD_NULL if a copy for a data-type is not implemented.
 */
xmlSchemaValPtr
xmlSchemaCopyValue(xmlSchemaValPtr val)
{
    xmlSchemaValPtr ret = KD_NULL, prev = KD_NULL, cur;
 ...
 *
 * Returns a pointer to a new #xmlSchemaVal or KD_NULL if error.
 */
static xmlSchemaValPtr
 ...
 *
 * Returns a normalized copy of @dt or KD_NULL if error.
 */
static xmlSchemaValPtr
...
xmlSchemaCompareValues(xmlSchemaValPtr x, xmlSchemaValPtr y) {
    ...
    return(xmlSchemaCompareValuesInternal(x->type, x, KD_NULL, xws, y->type,
	y, KD_NULL, yws));
}
				     ...
				     xmlSchemaWhitespaceValueType ws)  
{
    ...
    if ((length == KD_NULL) || (facet == KD_NULL))
        return (-1);
    *length = 0;
    if ((facet->type != XML_SCHEMA_FACET_LENGTH) &&
    ...
    */
    ...
    if ((val != KD_NULL) && (val->type == XML_SCHEMAS_HEXBINARY))
	len = val->value.hex.total;
    else if ((val != KD_NULL) && (val->type == XML_SCHEMAS_BASE64BINARY))
	...
	switch (valType) {
		...
		if (ws == XML_SCHEMA_WHITESPACE_UNKNOWN) {
		    ...
		    */
		    if (valType == (xmlSchemaTypeType) XML_SCHEMAS_STRING)
...
xmlSchemaFormatFloat(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("INF"))
	    kdSnprintfKHR (buffer, buffersize, "INF");
	break;
    case -1:
	if (buffersize > (int)sizeof("-INF"))
	    kdSnprintfKHR (buffer, buffersize, "-INF");
	...
	if (xmlXPathIsNaN(number)) {
	    ...
	    kdSnprintfKHR (buffer, buffersize, "0.0E0");
	} else {
	    ...
	    */
	    ...
	    kdSnprintfKHR (work, sizeof(work),"%*.*e",
    ...
    *retValue = KD_NULL;
	...
	case XML_SCHEMAS_DECIMAL:
		...
		*retValue = xmlStrdup(BAD_CAST "0.0");
		...
		if (dec.hi != 0)
		    kdSnprintfKHR (offs, bufsize - (offs - buf),
		...
		if (dec.frac != 0) {
			...
			*/
			kdMemmove (offs + diff + 1, offs + diff, dec.frac +1);
			offs[diff] = '.';
		    } else {
			unsigned int i = 0;
			/*
			...
			*/			
			while (*(offs + i) != 0)
			    i++;
			if (i < dec.total) {
			    kdMemmove (offs + (dec.total - i), offs, i +1);
			    kdMemset (offs, '0', dec.total - i);
			}
		    }
		} else {
		    /*
	...
	case XML_SCHEMAS_UINT:
		...
		*retValue = (xmlChar*) xmlMalloc(bufsize);
		...
		if (dec.hi != 0) {
		    if (dec.sign)
			kdSnprintfKHR ((char *) *retValue, bufsize,
			    "-%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    else
			kdSnprintfKHR ((char *) *retValue, bufsize,
			    "%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    ...
		    if (dec.sign)
			kdSnprintfKHR ((char *) *retValue, bufsize, "-%lu", dec.lo);
		    else
			kdSnprintfKHR ((char *) *retValue, bufsize, "%lu", dec.lo);
		}
	    }
	...
	case XML_SCHEMAS_DURATION: {
		...
		*/
		year = (unsigned long) FQUOTIENT(kdAbs(val->value.dur.mon), 12);
		mon = kdAbs(val->value.dur.mon) - 12 * year;

		day = (unsigned long) FQUOTIENT(kdFabsKHR (val->value.dur.sec), 86400);
		left = kdFabsKHR (val->value.dur.sec) - day * 86400;
		if (left > 0) {
		...
		if ((val->value.dur.mon < 0) || (val->value.dur.sec < 0))
		    kdSnprintfKHR (buf, 100, "P%luY%luM%luDT%luH%luM%.14gS",
	...
	case XML_SCHEMAS_GYEAR: {
		...
		kdSnprintfKHR (buf, 30, "%04ld", val->value.date.year);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_GMONTH: {
		...
		*retValue = (xmlChar*) xmlMalloc(6);
		if (*retValue == KD_NULL)
		    return(-1);
		kdSnprintfKHR ((char *) *retValue, 6, "--%02u",
		    val->value.date.mon);
	    }
	    break;
        case XML_SCHEMAS_GDAY: {
		...
		*retValue = (xmlChar*) xmlMalloc(8);
		if (*retValue == KD_NULL)
		    return(-1);
		kdSnprintfKHR ((char *) *retValue, 8, "--%02u-%02u",
		...
		if (val->value.date.year < 0)
		    kdSnprintfKHR (buf, 35, "-%04ld-%02u",
			kdAbs(val->value.date.year), 
			val->value.date.mon);
		else
		    kdSnprintfKHR (buf, 35, "%04ld-%02u",
	...
	case XML_SCHEMAS_FLOAT: {
		...
		* the whole range.
		*/
		kdSnprintfKHR (buf, 30, "%01.14e", val->value.f);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_DOUBLE: {
		...
		* the whole range.
		*/
		kdSnprintfKHR (buf, 40, "%01.14e", val->value.d);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;	
	default:
