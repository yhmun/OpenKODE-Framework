...
#include "XMFFmpeg/libavutil/libm.h" // brought forward to work around cygwin header breakage
                                     ...
                                     int win, int group_len, const float lambda)
{
    ...
    for (i = stack_len - 1; i >= 0; i--) {
        ...
        for (j = 0; j < count; j++) {
            sce->band_type[win*16 + start] = (BandType)stackcb[i];
            start++;
        }
        while (count >= run_esc) {
        ...
        for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
            for (g = 0; g < sce->ics.num_swb; g++) {
                ...
                if (sce->sf_idx[w*16+g] != prevsc)
                    fflag = 1;
                sce->band_type[w*16+g] = (BandType)find_min_book(maxvals[w*16+g], sce->sf_idx[w*16+g]);
            }
        }
        its++;
    } while (fflag && its < 10);
                                       ...
                                       const float lambda)
{
    ...
    for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w]) {
        ...
        for (g = 0; g < sce->ics.num_swb; g++) {
            ...
            start += size;
            thr = pow(thr / (avg_energy * (end2 - start2)), 0.3f + 0.1f*(lastband - g) / lastband);
