        ...
        int srcSliceH, uint8_t* dst[], int dstStride[])
{
    ...
    switch (c->dstFormat) {
        case PIX_FMT_BGR24:
			{
				const uint8_t* _t1[] = {src[1], src[0], src[2]};
				int            _t2[] = {srcStride[1], srcStride[0], srcStride[2]};
				gbr24ptopacked24(_t1, _t2, dst[0] + srcSliceY * dstStride[0], dstStride[0], srcSliceH, c->srcW);
			}
            break;

        case PIX_FMT_RGB24:
			{
				const uint8_t* _t1[] = {src[2], src[0], src[1]};
				int            _t2[] = {srcStride[2], srcStride[0], srcStride[1]};
				gbr24ptopacked24(_t1, _t2, dst[0] + srcSliceY * dstStride[0], dstStride[0], srcSliceH, c->srcW);
			}
            break;
        ...
        case PIX_FMT_RGBA:
			{
				const uint8_t* _t1[] = {src[2], src[0], src[1]};
				int            _t2[] = {srcStride[2], srcStride[0], srcStride[1]};
				gbr24ptopacked32(_t1, _t2, dst[0] + srcSliceY * dstStride[0], dstStride[0], srcSliceH, alpha_first, c->srcW);
			}
            break;
                                  ...
                                  const int dstStride[])
{
    ...
    if (c->src0Alpha && !c->dst0Alpha && isALPHA(c->dstFormat)) {
        uint8_t *base;
        int x,y;
        rgb0_tmp = (uint8_t *)av_malloc(FFABS(srcStride[0]) * srcSliceH + 32);
