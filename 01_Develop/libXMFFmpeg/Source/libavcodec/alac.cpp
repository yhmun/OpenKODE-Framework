...
static int allocate_buffers(ALACContext *alac)
{
    ...
    for (ch = 0; ch < alac->numchannels; ch++) {
        ...
        FF_ALLOC_OR_GOTO(int32_t*, alac->avctx, alac->predicterror_buffer[ch],
                         buf_size, buf_alloc_fail);

        FF_ALLOC_OR_GOTO(int32_t*, alac->avctx, alac->outputsamples_buffer[ch],
                         buf_size, buf_alloc_fail);

        FF_ALLOC_OR_GOTO(int32_t*, alac->avctx, alac->extra_bits_buffer[ch],
                         buf_size, buf_alloc_fail);
    }
    return 0;
buf_alloc_fail:
