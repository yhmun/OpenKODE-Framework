...
{
    TtyDemuxContext *s = (TtyDemuxContext *)avctx->priv_data;
...
#define GET_EFI_META(name,size) \
    ...
    if (len < 1 || len > size) \
        return -1; \
    if (avio_read(pb, (unsigned char *)buf, size) == size) { \
        buf[len] = 0; \
...
static const AVOption options[] = {
    { "chars_per_frame", "", offsetof(TtyDemuxContext, chars_per_frame), AV_OPT_TYPE_INT, 6000, NULL, 1, INT_MAX, AV_OPT_FLAG_DECODING_PARAM},
