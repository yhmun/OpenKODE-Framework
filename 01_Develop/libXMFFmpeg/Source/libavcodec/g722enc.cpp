...
static av_cold int g722_encode_init(AVCodecContext * avctx)
{
    ...
    if (avctx->trellis) {
        ...
        int i;
        for (i = 0; i < 2; i++) {
            c->paths[i] = (TrellisPath *)av_mallocz(max_paths * sizeof(**c->paths));
            c->node_buf[i] = (TrellisNode *)av_mallocz(2 * frontier * sizeof(**c->node_buf));
            c->nodep_buf[i] = (TrellisNode **)av_mallocz(2 * frontier * sizeof(**c->nodep_buf));
        }
    }
...
static const enum AVSampleFormat _ff_adpcm_g722_encoder[] = {AV_SAMPLE_FMT_S16,AV_SAMPLE_FMT_NONE};
AVCodec ff_adpcm_g722_encoder = {
	...
	/*.sample_fmts			= */	_ff_adpcm_g722_encoder,
};
