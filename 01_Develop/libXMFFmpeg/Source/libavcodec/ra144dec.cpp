                              ...
                              int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    ff_eval_coefs((int *)ractx->lpc_coef[0], (const int *)lpc_refl);
    ractx->lpc_refl_rms[0] = ff_rms((const int *)lpc_refl);

    energy = ff_energy_tab[get_bits(&gb, 5)];

    refl_rms[0] = ff_interp(ractx, (int16_t *)block_coefs[0], 1, 1, ractx->old_energy);
    refl_rms[1] = ff_interp(ractx, (int16_t *)block_coefs[1], 2,
                            energy <= ractx->old_energy,
                            ff_t_sqrt(energy*ractx->old_energy) >> 12);
    refl_rms[2] = ff_interp(ractx, (int16_t *)block_coefs[2], 3, 0, energy);
    refl_rms[3] = ff_rescale_rms(ractx->lpc_refl_rms[0], energy);

    ff_int_to_int16((int16_t *)block_coefs[3], (const int *)ractx->lpc_coef[0]);
