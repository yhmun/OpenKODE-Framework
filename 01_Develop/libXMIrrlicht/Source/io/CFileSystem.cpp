...
CFileSystem::CFileSystem()
{
	...
	// getWorkingDirectory();
	WorkingDirectory[FILESYSTEM_NATIVE] = "/res/";
...
io::path CFileSystem::getAbsolutePath(const io::path& filename) const
{
	...
	tPath = szPath = xmGetNativePath ( filename.c_str ( ), KD_NULL );
...
IFileList* CFileSystem::createFileList()
{
	...
	if (FileSystemType == FILESYSTEM_NATIVE)
	{
		...
		KDDir*  pDir = kdOpenDir ( Path.c_str ( ) );
		if ( pDir )
		{
			struct KDDirent*  pEntry;
			while ( ( pEntry = kdReadDir ( pDir ) ) )
			{
				u32 size = 0;
				bool isDirectory = false;

				if ( ( kdStrcmp ( pEntry->d_name, "."  ) == 0 ) ||
				     ( kdStrcmp ( pEntry->d_name, ".." ) == 0 ) )
				{
					continue;
				}

				struct KDStat  tBuf;
				io::path  sTemp = Path + pEntry->d_name;
				if ( kdStat ( sTemp.c_str ( ), &tBuf ) == 0 )
				{
					size = tBuf.st_size;
					isDirectory = KD_ISDIR ( tBuf.st_mode ) != 0;
				}

				r->addItem ( Path + pEntry->d_name, 0, size, isDirectory, 0 );
			}
...
bool CFileSystem::existFile(const io::path& filename) const
{
    ...
    struct KDStat  info;
	return !kdStat ( filename.c_str ( ), &info );
}
