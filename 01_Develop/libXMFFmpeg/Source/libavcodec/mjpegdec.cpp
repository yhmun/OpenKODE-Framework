...
{
    int len, index, i, av_class, n, v, code_max;
    ...
    while (len > 0) {
        ...
        free_vlc(&s->vlcs[av_class][index]);
        av_log(s->avctx, AV_LOG_DEBUG, "class=%d index=%d nb_codes=%d\n",
               av_class, index, code_max + 1);
        if (build_vlc(&s->vlcs[av_class][index], bits_table, val_table,
                      code_max + 1, 0, av_class > 0) < 0)
            return -1;
...
int ff_mjpeg_decode_sof(MJpegDecodeContext *s)
{
    ...
    if (width != s->width || height != s->height) {
        ...
        s->qscale_table  = (int8_t *)av_mallocz((s->width + 15) / 16);
        s->first_picture = 0;
    }
...
//           s->interlaced, s->avctx->height);
    ...
    if (s->progressive) {
        ...
        for (i = 0; i < s->nb_components; i++) {
            ...
            s->blocks[i]       = (DCTELEM (*)[64])av_malloc(size * sizeof(**s->blocks));
            s->last_nnz[i]     = (uint8_t *)av_mallocz(size * sizeof(**s->last_nnz));
...
static int mjpeg_decode_com(MJpegDecodeContext *s)
{
    ...
    if (len >= 2 && 8 * len - 16 <= get_bits_left(&s->gb)) {
        char *cbuf = (char *)av_malloc(len - 1);
        if (cbuf) {
            int i;
            for (i = 0; i < len - 2; i++)
                         ...
                         int *unescaped_buf_size)
{
    ...
    av_fast_padded_malloc(&s->buffer, (unsigned int *)&s->buffer_size, buf_end - *buf_ptr);
    if (!s->buffer)
...
static const AVOption options[] = {
    { "extern_huff", "Use external huffman table.",
      OFFSET(extern_huff), AV_OPT_TYPE_INT, 0, NULL, 0, 1, VD },
    { NULL },
};
