...
static xmlCharEncodingHandlerPtr xmlUTF16LEHandler = KD_NULL;
static xmlCharEncodingHandlerPtr xmlUTF16BEHandler = KD_NULL;
...
static xmlCharEncodingAliasPtr xmlCharEncodingAliases = KD_NULL;
...
{
    __xmlSimpleError(XML_FROM_I18N, XML_ERR_NO_MEMORY, KD_NULL, KD_NULL, extra);
}
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL,
                    XML_FROM_I18N, error, XML_ERR_FATAL,
                    KD_NULL, 0, val, KD_NULL, KD_NULL, 0, 0, msg, val);
}
...
openIcuConverter(const char* name, int toUnicode)
{
  ...
  if (toUnicode) {
    ucnv_setToUCallBack(conv->uconv, UCNV_TO_U_CALLBACK_STOP,
                        KD_NULL, KD_NULL, KD_NULL, &status);
  }
  else {
...
isolat1ToUTF8(unsigned char* out, int *outlen,
    ...
    if ((out == KD_NULL) || (in == KD_NULL) || (outlen == KD_NULL) || (inlen == KD_NULL))
	return(-1);
           ...
           const unsigned char* inb, int *inlenb)
{
    ...
    if ((out == KD_NULL) || (inb == KD_NULL) || (outlen == KD_NULL) || (inlenb == KD_NULL))
	return(-1);
    if (*outlen > *inlenb) {
	len = *inlenb;
    } else {
            ...
            const unsigned char* in, int *inlen)
{
    ...
    /* UTF-16BE has no BOM */
    if ((outb == KD_NULL) || (outlen == KD_NULL) || (inlen == KD_NULL)) return(-1);
    if (in == KD_NULL) {
	*outlen = 0;
	*inlen = 0;
	return(0);
...
xmlCleanupEncodingAliases(void) {
    ...
    if (xmlCharEncodingAliases == KD_NULL)
	return;

    for (i = 0;i < xmlCharEncodingAliasesNb;i++) {
	if (xmlCharEncodingAliases[i].name != KD_NULL)
	    xmlFree((char *) xmlCharEncodingAliases[i].name);
	if (xmlCharEncodingAliases[i].alias != KD_NULL)
	    xmlFree((char *) xmlCharEncodingAliases[i].alias);
    }
    xmlCharEncodingAliasesNb = 0;
    xmlCharEncodingAliasesMax = 0;
    xmlFree(xmlCharEncodingAliases);
    xmlCharEncodingAliases = KD_NULL;
}
 ...
 *
 * Returns KD_NULL if not found, otherwise the original name
 */
const char *
xmlGetEncodingAlias(const char *alias) {
     ...
     */
    for (i = 0;i < xmlCharEncodingAliasesNb;i++) {
	if (!kdStrcmp(xmlCharEncodingAliases[i].alias, upper)) {
...
xmlDelEncodingAlias(const char *alias) {
     ...
     */
    for (i = 0;i < xmlCharEncodingAliasesNb;i++) {
	if (!kdStrcmp(xmlCharEncodingAliases[i].alias, alias)) {
	    xmlFree((char *) xmlCharEncodingAliases[i].name);
	    xmlFree((char *) xmlCharEncodingAliases[i].alias);
	    xmlCharEncodingAliasesNb--;
	    kdMemmove(&xmlCharEncodingAliases[i], &xmlCharEncodingAliases[i + 1],
...
xmlParseCharEncoding(const char* name)
{
    ...
    for (i = 0;i < 499;i++) {
        upper[i] = kdToupper(name[i]);
	if (upper[i] == 0) break;
    }
    upper[i] = 0;

    if (!kdStrcmp(upper, "")) return(XML_CHAR_ENCODING_NONE);
    if (!kdStrcmp(upper, "UTF-8")) return(XML_CHAR_ENCODING_UTF8);
    if (!kdStrcmp(upper, "UTF8")) return(XML_CHAR_ENCODING_UTF8);

    /*
     * NOTE: if we were able to parse this, the endianness of UTF16 is
     *       already found and in use
     */
    if (!kdStrcmp(upper, "UTF-16")) return(XML_CHAR_ENCODING_UTF16LE);
    if (!kdStrcmp(upper, "UTF16")) return(XML_CHAR_ENCODING_UTF16LE);

    if (!kdStrcmp(upper, "ISO-10646-UCS-2")) return(XML_CHAR_ENCODING_UCS2);
    if (!kdStrcmp(upper, "UCS-2")) return(XML_CHAR_ENCODING_UCS2);
    if (!kdStrcmp(upper, "UCS2")) return(XML_CHAR_ENCODING_UCS2);

    /*
     * NOTE: if we were able to parse this, the endianness of UCS4 is
     *       already found and in use
     */
    if (!kdStrcmp(upper, "ISO-10646-UCS-4")) return(XML_CHAR_ENCODING_UCS4LE);
    if (!kdStrcmp(upper, "UCS-4")) return(XML_CHAR_ENCODING_UCS4LE);
    if (!kdStrcmp(upper, "UCS4")) return(XML_CHAR_ENCODING_UCS4LE);


    if (!kdStrcmp(upper,  "ISO-8859-1")) return(XML_CHAR_ENCODING_8859_1);
    if (!kdStrcmp(upper,  "ISO-LATIN-1")) return(XML_CHAR_ENCODING_8859_1);
    if (!kdStrcmp(upper,  "ISO LATIN 1")) return(XML_CHAR_ENCODING_8859_1);

    if (!kdStrcmp(upper,  "ISO-8859-2")) return(XML_CHAR_ENCODING_8859_2);
    if (!kdStrcmp(upper,  "ISO-LATIN-2")) return(XML_CHAR_ENCODING_8859_2);
    if (!kdStrcmp(upper,  "ISO LATIN 2")) return(XML_CHAR_ENCODING_8859_2);

    if (!kdStrcmp(upper,  "ISO-8859-3")) return(XML_CHAR_ENCODING_8859_3);
    if (!kdStrcmp(upper,  "ISO-8859-4")) return(XML_CHAR_ENCODING_8859_4);
    if (!kdStrcmp(upper,  "ISO-8859-5")) return(XML_CHAR_ENCODING_8859_5);
    if (!kdStrcmp(upper,  "ISO-8859-6")) return(XML_CHAR_ENCODING_8859_6);
    if (!kdStrcmp(upper,  "ISO-8859-7")) return(XML_CHAR_ENCODING_8859_7);
    if (!kdStrcmp(upper,  "ISO-8859-8")) return(XML_CHAR_ENCODING_8859_8);
    if (!kdStrcmp(upper,  "ISO-8859-9")) return(XML_CHAR_ENCODING_8859_9);

    if (!kdStrcmp(upper, "ISO-2022-JP")) return(XML_CHAR_ENCODING_2022_JP);
    if (!kdStrcmp(upper, "SHIFT_JIS")) return(XML_CHAR_ENCODING_SHIFT_JIS);
    if (!kdStrcmp(upper, "EUC-JP")) return(XML_CHAR_ENCODING_EUC_JP);
...
static xmlCharEncodingHandlerPtr *handlers = KD_NULL;
static int nbCharEncodingHandler = 0;

/*
 * The default is UTF-8 for XML, that's also the default used for the
 * parser internals, so the default encoding handler is KD_NULL
 */

static xmlCharEncodingHandlerPtr xmlDefaultCharEncodingHandler = KD_NULL;
 ...
 *
 * Returns the xmlCharEncodingHandlerPtr created (or KD_NULL in case of error).
 */
...
xmlNewCharEncodingHandler(const char *name,
    ...
    kdMemset (handler, 0, sizeof(xmlCharEncodingHandler));
    handler->input = input;
    handler->output = output;
    handler->name = up;

#ifdef LIBXML_ICONV_ENABLED
    handler->iconv_in = KD_NULL;
    handler->iconv_out = KD_NULL;
#endif
#ifdef LIBXML_ICU_ENABLED
    handler->uconv_in = KD_NULL;
    handler->uconv_out = KD_NULL;
#endif
...
#ifdef LIBXML_HTML_ENABLED
    xmlNewCharEncodingHandler("HTML", KD_NULL, UTF8ToHtml);
#endif
#else
    xmlUTF16LEHandler =
          xmlNewCharEncodingHandler("UTF-16LE", UTF16LEToUTF8, KD_NULL);
    xmlUTF16BEHandler =
          xmlNewCharEncodingHandler("UTF-16BE", UTF16BEToUTF8, KD_NULL);
    xmlNewCharEncodingHandler("UTF-16", UTF16LEToUTF8, KD_NULL);
    xmlNewCharEncodingHandler("ISO-8859-1", isolat1ToUTF8, KD_NULL);
    xmlNewCharEncodingHandler("ASCII", asciiToUTF8, KD_NULL);
    xmlNewCharEncodingHandler("US-ASCII", asciiToUTF8, KD_NULL);
#endif /* LIBXML_OUTPUT_ENABLED */
#if !defined(LIBXML_ICONV_ENABLED) && !defined(LIBXML_ICU_ENABLED)
#ifdef LIBXML_ISO8859X_ENABLED
...
xmlCleanupCharEncodingHandlers(void) {
	...
	if (handlers[nbCharEncodingHandler] != KD_NULL) {
	    if (handlers[nbCharEncodingHandler]->name != KD_NULL)
    ...
    xmlDefaultCharEncodingHandler = KD_NULL;
}
...
xmlRegisterCharEncodingHandler(xmlCharEncodingHandlerPtr handler) {
    if (handlers == KD_NULL) xmlInitCharEncodingHandlers();
    if ((handler == KD_NULL) || (handlers == KD_NULL)) {
        xmlEncodingErr(XML_I18N_NO_HANDLER,
		"xmlRegisterCharEncodingHandler: KD_NULL handler !\n", KD_NULL);
	return;
    }
 ...
 *
 * Returns the handler or KD_NULL if not found
 */
xmlCharEncodingHandlerPtr
xmlGetCharEncodingHandler(xmlCharEncoding enc) {
    ...
    switch (enc) {
        ...
        case XML_CHAR_ENCODING_EBCDIC:
            ...
            if (handler != KD_NULL) return(handler);
            handler = xmlFindCharEncodingHandler("ebcdic");
            if (handler != KD_NULL) return(handler);
...
xmlFindCharEncodingHandler(const char *name) {
    ...
    if (handlers == KD_NULL) xmlInitCharEncodingHandlers();
    if (name == KD_NULL) return(xmlDefaultCharEncodingHandler);
    ...
    if (handlers != KD_NULL) {
        for (i = 0;i < nbCharEncodingHandler; i++) {
            if (!kdStrcmp(upper, handlers[i]->name)) {
#ifdef DEBUG_ENCODING
                xmlGenericError(xmlGenericErrorContext,
    ...
    if ((icv_in != (iconv_t) -1) && (icv_out != (iconv_t) -1)) {
		...
		return(KD_NULL);
            ...
            kdMemset (enc, 0, sizeof(xmlCharEncodingHandler));
    ...
    if (ucv_in != KD_NULL && ucv_out != KD_NULL) {
		...
		return(KD_NULL);
            ...
            kdMemset (encu, 0, sizeof(xmlCharEncodingHandler));
...
#endif
	    return encu;
    } else if (ucv_in != KD_NULL || ucv_out != KD_NULL) {
            closeIcuConverter(ucv_in);
            closeIcuConverter(ucv_out);
	    xmlEncodingErr(XML_ERR_INTERNAL_ERROR,
    ...
    if (alias != XML_CHAR_ENCODING_ERROR) {
        ...
        if ((canon != KD_NULL) && (kdStrcmp(name, canon))) {
...
xmlUconvWrapper(uconv_t *cd, int toUnicode, unsigned char *out, int *outlen,
    ...
    if (toUnicode) {
        ...
        ucnv_convertEx(cd->utf8, cd->uconv, &ucv_out, ucv_out + *outlen,
                       &ucv_in, ucv_in + *inlen, KD_NULL, KD_NULL, KD_NULL, KD_NULL,
                       0, TRUE, &err);
    } else {
...
xmlCharEncFirstLineInt(xmlCharEncodingHandler *handler, xmlBufferPtr out,
    ...
    if (handler == KD_NULL) return(-1);
    if (out == KD_NULL) return(-1);
...
#ifdef LIBXML_ICONV_ENABLED
    else if (handler->iconv_in != KD_NULL) {
...
#endif /* LIBXML_ICONV_ENABLED */
#ifdef LIBXML_ICU_ENABLED
    else if (handler->uconv_in != KD_NULL) {
                 ...
                 xmlBufferPtr in)
{
    ...
    switch (ret) {
	    ...
	    kdSnprintfKHR (&buf[0], 49, "0x%02X 0x%02X 0x%02X 0x%02X",
 ...
 * a first call with @in == KD_NULL has to be made firs to initiate the
...
retry:
    ...
    /*
     * First specific handling of in = KD_NULL, i.e. the initialization call
     */
    if (in == KD_NULL) {
        toconv = 0;
	if (handler->output != KD_NULL) {
...
#ifdef LIBXML_ICONV_ENABLED
	else if (handler->iconv_out != KD_NULL) {
	    ret = xmlIconvWrapper(handler->iconv_out, &out->content[out->use],
				  &written, KD_NULL, &toconv);
	    out->use += written;
	    out->content[out->use] = 0;
	}
#endif /* LIBXML_ICONV_ENABLED */
#ifdef LIBXML_ICU_ENABLED
	else if (handler->uconv_out != KD_NULL) {
    ...
    switch (ret) {
		 ...
		 */
		kdSnprintfKHR ( (char *) &charref[0], sizeof(charref), "&#%d;", cur);
...
xmlCharEncCloseFunc(xmlCharEncodingHandler *handler) {
    ...
    int tofree = 0;
    if (handler == KD_NULL) return(-1);
    if (handler->name == KD_NULL) return(-1);
#ifdef LIBXML_ICONV_ENABLED
    /*
     * Iconv handlers can be used only once, free the whole block.
     * and the associated icon resources.
     */
    if ((handler->iconv_out != KD_NULL) || (handler->iconv_in != KD_NULL)) {
        tofree = 1;
	if (handler->iconv_out != KD_NULL) {
	    if (iconv_close(handler->iconv_out))
		ret = -1;
	    handler->iconv_out = KD_NULL;
	}
	if (handler->iconv_in != KD_NULL) {
...
#endif /* LIBXML_ICONV_ENABLED */
#ifdef LIBXML_ICU_ENABLED
    if ((handler->uconv_out != KD_NULL) || (handler->uconv_in != KD_NULL)) {
        tofree = 1;
	if (handler->uconv_out != KD_NULL) {
	    closeIcuConverter(handler->uconv_out);
	    handler->uconv_out = KD_NULL;
	}
	if (handler->uconv_in != KD_NULL) {
...
xmlByteConsumed(xmlParserCtxtPtr ctxt) {
    ...
    in = ctxt->input;
    if (in == KD_NULL)  return(-1);
    if ((in->buf != KD_NULL) && (in->buf->encoder != KD_NULL)) {
...
UTF8ToISO8859x(unsigned char* out, int *outlen,
    ...
    if ((out == KD_NULL) || (outlen == KD_NULL) || (inlen == KD_NULL) ||
        (xlattable == KD_NULL))
	return(-1);
    if (in == KD_NULL) {
        /*
...
ISO8859xToUTF8(unsigned char* out, int *outlen,
    ...
    if ((out == KD_NULL) || (outlen == KD_NULL) || (inlen == KD_NULL) ||
        (in == KD_NULL) || (unicodetable == KD_NULL))
	return(-1);
    outend = out + *outlen;
    inend = in + *inlen;
    instop = inend;
