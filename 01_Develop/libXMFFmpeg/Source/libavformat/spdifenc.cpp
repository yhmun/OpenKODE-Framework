...
{ "spdif_flags", "IEC 61937 encapsulation flags", offsetof(IEC61937Context, spdif_flags), AV_OPT_TYPE_FLAGS, 0, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "spdif_flags" },
{ "be", "output in big-endian format (for use as s16be)", 0, AV_OPT_TYPE_CONST, SPDIF_FLAG_BIGENDIAN, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "spdif_flags" },
{ "dtshd_rate", "mux complete DTS frames in HD mode at the specified IEC958 rate (in Hz, default 0=disabled)", offsetof(IEC61937Context, dtshd_rate), AV_OPT_TYPE_INT, 0, NULL, 0, 768000, AV_OPT_FLAG_ENCODING_PARAM },
{ "dtshd_fallback_time", "min secs to strip HD for after an overflow (-1: till the end, default 60)", offsetof(IEC61937Context, dtshd_fallback), AV_OPT_TYPE_INT, 60, NULL, -1, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM },
{ NULL },
};
...
{
    IEC61937Context *ctx = (IEC61937Context *)s->priv_data;
    int bitstream_mode = pkt->data[5] & 0x7;

    ctx->data_type  = (IEC61937DataType)(IEC61937_AC3 | (bitstream_mode << 8));
...
static int spdif_header_eac3(AVFormatContext *s, AVPacket *pkt)
{
    ...
    ctx->hd_buf = (uint8_t *)av_fast_realloc(ctx->hd_buf, (unsigned int *)&ctx->hd_buf_size, ctx->hd_buf_filled + pkt->size);
    if (!ctx->hd_buf)
...
{
    IEC61937Context *ctx = (IEC61937Context *)s->priv_data;
    static const unsigned char dtshd_start_code[10] = { 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xfe, 0xfe };
    ...
    ctx->data_type = (IEC61937DataType)(IEC61937_DTSHD | subtype << 8);
    ...
    av_fast_malloc(&ctx->hd_buf, (unsigned int *)&ctx->hd_buf_size, ctx->out_bytes);
    if (!ctx->hd_buf)
        return AVERROR(ENOMEM);
...
static int spdif_header_truehd(AVFormatContext *s, AVPacket *pkt)
{
    ...
    const unsigned char mat_end_code[16] = { 0xC3, 0xC2, 0xC0, 0xC4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x97, 0x11 };

    if (!ctx->hd_buf_count) {
        const unsigned char mat_start_code[20] = { 0x07, 0x9E, 0x00, 0x03, 0x84, 0x01, 0x01, 0x01, 0x80, 0x00, 0x56, 0xA5, 0x3B, 0xF4, 0x81, 0x83, 0x49, 0x80, 0x77, 0xE0 };
        ...
        const unsigned char mat_middle_code[12] = { 0xC3, 0xC1, 0x42, 0x49, 0x3B, 0xFA, 0x82, 0x83, 0x49, 0x80, 0x77, 0xE0 };
...
static int spdif_write_header(AVFormatContext *s)
{
    ...
    case CODEC_ID_TRUEHD:
        ...
        ctx->hd_buf = (uint8_t *)av_malloc(MAT_FRAME_SIZE);
        if (!ctx->hd_buf)
            return AVERROR(ENOMEM);
        break;
    default:
...
static int spdif_write_packet(struct AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (ctx->extra_bswap ^ (ctx->spdif_flags & SPDIF_FLAG_BIGENDIAN)) {
        ...
        av_fast_malloc(&ctx->buffer, (unsigned int *)&ctx->buffer_size, ctx->out_bytes + FF_INPUT_BUFFER_PADDING_SIZE);
        if (!ctx->buffer)
            return AVERROR(ENOMEM);
