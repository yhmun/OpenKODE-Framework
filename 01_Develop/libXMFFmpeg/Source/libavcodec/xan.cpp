...
{
    XanContext *s = (XanContext *)avctx->priv_data;
    ...
    s->buffer1 = (unsigned char *)av_malloc(s->buffer1_size);
    if (!s->buffer1)
        return AVERROR(ENOMEM);
    s->buffer2_size = avctx->width * avctx->height;
    s->buffer2 = (unsigned char *)av_malloc(s->buffer2_size + 130);
    if (!s->buffer2) {
        av_freep(&s->buffer1);
        return AVERROR(ENOMEM);
    }
        ...
        while (buf_end - buf > 8 && tag != VGA__TAG) {
            ...
            case PALT_TAG:
                ...
                if (s->palettes_count >= PALETTES_MAX)
                    return AVERROR_INVALIDDATA;
                tmpptr = (unsigned int *)av_realloc(s->palettes,
