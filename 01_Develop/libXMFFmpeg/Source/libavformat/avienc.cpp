...
static av_always_inline AVIIentry* avi_get_ientry(AVIIndex* idx, int ent_id)
{
                                  ...
                                  const char* riff_tag, const char* list_tag)
{
    ...
    for (i=0; i<s->nb_streams; i++){
        AVIStream *avist= (AVIStream *)s->streams[i]->priv_data;
        avist->indexes.entry = 0;
    }

    avi->riff_start = ff_start_tag(pb, "RIFF");
    ffio_wfourcc(pb, (const uint8_t *)riff_tag);
    loff = ff_start_tag(pb, "LIST");
    ffio_wfourcc(pb, (const uint8_t *)list_tag);
    return loff;
}
...
static int avi_write_counters(AVFormatContext* s, int riff_id)
{
    ...
    AVIContext *avi = (AVIContext *)s->priv_data;
    unsigned int n;
	int au_byterate, au_ssize, au_scale, nb_frames = 0;
    int64_t file_size;
...
static int avi_write_header(AVFormatContext *s)
{
    AVIContext *avi = (AVIContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    int bitrate, nb_frames, au_byterate, au_ssize, au_scale;
	unsigned int n, i;
    ...
    /* avi header */
    ffio_wfourcc(pb, (const uint8_t *)"avih");
    avio_wl32(pb, 14 * 4);
    bitrate = 0;
    ...
    for(i=0;i<n;i++) {
        ...
        ffio_wfourcc(pb, (const uint8_t *)"strl");
        ...
        case AVMEDIA_TYPE_VIDEO: ffio_wfourcc(pb, (const uint8_t *)"vids"); break;
        case AVMEDIA_TYPE_AUDIO: ffio_wfourcc(pb, (const uint8_t *)"auds"); break;
//      case AVMEDIA_TYPE_TEXT : ffio_wfourcc(pb, (const uint8_t *)"txts"); break;
        case AVMEDIA_TYPE_DATA : ffio_wfourcc(pb, (const uint8_t *)"dats"); break;
        default:;
        }
        if(stream->codec_type == AVMEDIA_TYPE_VIDEO ||
      ...
      if(stream->codec_type != AVMEDIA_TYPE_DATA){
        ...
        if (pb->seekable) {
            ...
            ffio_wfourcc(pb, (const uint8_t *)avi_stream2fourcc((char *)tag, i, stream->codec_type));
                                    /* dwChunkId */
        ...
        if(   stream->codec_type == AVMEDIA_TYPE_VIDEO
           ...
           && s->streams[i]->sample_aspect_ratio.den>0){
            int vprp= (int)ff_start_tag(pb, "vprp");
            AVRational dar = av_mul_q(s->streams[i]->sample_aspect_ratio,
                                      av_rational(stream->width, stream->height));
            int num, den;
    ...
    if (pb->seekable) {
        ...
        ffio_wfourcc(pb, (const uint8_t *)"odml");
        ffio_wfourcc(pb, (const uint8_t *)"dmlh");
        avio_wl32(pb, 248);
        for (i = 0; i < 248; i+= 4)
             avio_wl32(pb, 0);
    ...
    ffio_wfourcc(pb, (const uint8_t *)"movi");
...
static int avi_write_ix(AVFormatContext *s)
{
    ...
    for (i=0;i<s->nb_streams;i++) {
        AVIStream *avist= (AVIStream *)s->streams[i]->priv_data;
         ...
         ix = avio_tell(pb);
         ffio_wfourcc(pb, (const uint8_t *)ix_tag);     /* ix?? */
         ...
         avio_wl32(pb, avist->indexes.entry);
                                      /* nEntriesInUse */
         ffio_wfourcc(pb, (const uint8_t *)tag);        /* dwChunkId */
         ...
         ffio_wfourcc(pb, (const uint8_t *)"indx");            /* enabling this entry */
         avio_skip(pb, 8);
         avio_wl32(pb, avi->riff_id);         /* nEntriesInUse */
         avio_skip(pb, 16*avi->riff_id);
         avio_wl64(pb, ix);                   /* qwOffset */
         avio_wl32(pb, (unsigned int)(pos - ix));             /* dwSize */
...
static int avi_write_idx1(AVFormatContext *s)
{
    ...
    if (pb->seekable) {
        ...
        for(i=0; i<s->nb_streams; i++){
            avist= (AVIStream *)s->streams[i]->priv_data;
            avist->entry=0;
        }
        ...
        do {
            ...
            if (!empty) {
                avist= (AVIStream *)s->streams[stream_id]->priv_data;
...
static int avi_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    AVIStream *avist= (AVIStream *)s->streams[stream_index]->priv_data;
...
//        av_log(s, AV_LOG_DEBUG, "dup %" PRId64 " %d\n", pkt->dts, avist->packet_count);
    ...
    if (s->pb->seekable) {
        ...
        if (idx->ents_allocated <= idx->entry) {
            idx->cluster = (AVIIentry **)av_realloc_f(idx->cluster, sizeof(void*), cl+1);
            if (!idx->cluster)
                return -1;
            idx->cluster[cl] = (AVIIentry *)av_malloc(AVI_INDEX_CLUSTER_SIZE*sizeof(AVIIentry));
            if (!idx->cluster[cl])
                return -1;
        ...
        idx->cluster[cl][id].pos = (unsigned int)(avio_tell(pb) - avi->movi_list);
...
static int avi_write_trailer(AVFormatContext *s)
{
    ...
    if (pb->seekable){
        if (avi->riff_id == 1) {
            ...
            ffio_wfourcc(pb, (const uint8_t *)"LIST"); /* Making this AVI OpenDML one */
            avio_skip(pb, 16);
...
static const AVCodecTag* const _ff_avi_muxer[] = {ff_codec_bmp_tags, ff_codec_wav_tags, 0};
