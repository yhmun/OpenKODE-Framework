...
int ff_get_wav_header(AVIOContext *pb, AVCodecContext *codec, int size)
{
    ...
    if (size >= 18) {  /* We're obviously dealing with WAVEFORMATEX */
        ...
        if (cbSize >= 22 && id == 0xfffe) { /* WAVEFORMATEXTENSIBLE */
			...
			{
				const uint8_t _t[] = {FF_MEDIASUBTYPE_BASE_GUID};
            if (!memcmp(subformat + 4, _t, 12)) {
        ...
        if (cbSize > 0) {
            ...
            codec->extradata = (uint8_t *)av_mallocz(codec->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!codec->extradata)
...
int ff_read_riff_info(AVFormatContext *s, int64_t size)
{
    ...
    while ((cur = avio_tell(pb)) >= 0 && cur <= end - 8 /* = tag + size */) {
        ...
        value = (char *)av_malloc(chunk_size + 1);
        if (!value) {
