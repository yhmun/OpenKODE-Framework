...
static void rv34_output_intra(RV34DecContext *r, int8_t *intra_types, int cbp)
{
    ...
    for(k = 0; k < 2; k++){
        ...
        for(j = 0; j < 2; j++){
            int* acache = (int *)(r->avail_cache + 6 + j*4);
...
static int rv34_decoder_alloc(RV34DecContext *r)
{
    ...
    r->cbp_chroma       = (uint8_t *)av_malloc(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->cbp_chroma));
    r->cbp_luma         = (uint16_t *)av_malloc(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->cbp_luma));
    r->deblock_coefs    = (int *)av_malloc(r->s.mb_stride * r->s.mb_height *
                                    sizeof(*r->deblock_coefs));
    r->intra_types_hist = (int8_t *)av_malloc(r->intra_types_stride * 4 * 2 *
                                    sizeof(*r->intra_types_hist));
    r->mb_type          = (int *)av_mallocz(r->s.mb_stride * r->s.mb_height *
                                     sizeof(*r->mb_type));
...
static int rv34_decode_slice(RV34DecContext *r, int end, const uint8_t* buf, int buf_size)
{
    ...
    if ((s->mb_x == 0 && s->mb_y == 0) || s->current_picture_ptr==NULL) {
        ...
        if (!r->tmp_b_block_base) {
            ...
            r->tmp_b_block_base = (uint8_t *)av_malloc(s->linesize * 48);
            for (i = 0; i < 2; i++)
...
{
    RV34DecContext *r = (RV34DecContext *)avctx->priv_data;
    MpegEncContext *s = &r->s;
    int ret;
...
{
    RV34DecContext *r = (RV34DecContext *)dst->priv_data, *r1 = (RV34DecContext *)src->priv_data;
