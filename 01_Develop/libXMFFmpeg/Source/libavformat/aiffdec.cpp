...
{
    uint8_t *str = (uint8_t *)av_malloc(size+1);
    ...
    if (str) {
        ...
        size += (size&1)-res;
        str[res] = 0;
        av_dict_set(&s->metadata, key, (const char *)str, AV_METADATA_DONT_STRDUP_VAL);
    }else
        size+= size&1;
                             ...
                             int size, unsigned version)
{
    ...
    exp = avio_rb16(pb);
    val = avio_rb64(pb);
    sample_rate = ldexp((double)val, exp - 16383 - 63);
...
static const AVCodecTag* const _ff_aiff_demuxer[] = {ff_codec_aiff_tags, 0};
