...
zzip_file_open(ZZIP_DIR * dir, zzip_char_t * name, int o_mode)
{
    ...
    if (! dir)
        return NULL;
    if (! dir->fd || dir->fd == -1)
        { dir->errcode = KD_EBADF; return NULL; }
    if (! hdr)
        { dir->errcode = KD_ENOENT; return NULL; }
...
zzip_read(ZZIP_FILE * fp, void *buf, zzip_size_t len)
{
    ...
    else
    {
        ...
        if (v == -1)
            { kdSetError ( zzip_errno(fp->dir->errcode) ); }
        return v;
    }
}
...
#   endif
	...
	case '5': case '6': case '7': case '8': case '9':
        ...
        case 'r': o_flags |= mode[1] == '+' ? KD_O_RDWR : KD_O_RDONLY; break;
        case 'w': o_flags |= mode[1] == '+' ? KD_O_RDWR : KD_O_WRONLY;
                  o_flags |= KD_O_TRUNC; break;
//        case 'b': o_flags |= KD_O_BINARY; break;
//        case 'f': o_flags |= KD_O_NOCTTY; break;
        case 'i': o_modes |= ZZIP_CASELESS; break;
        case '*': o_modes |= ZZIP_NOPATHS; break;
        case 'x': o_flags |= KD_O_EXCL; break;
//        case 's': o_flags |= KD_O_SYNC; break;
//        case 'n': o_flags |= KD_O_NONBLOCK; break;
	case 'o': o_modes &=~ 07;
                    ...
                    zzip_strings_t * ext, zzip_plugin_io_t io)
{
  ...
  try_zzip:
    ...
    if (o_flags & (KD_O_CREAT | KD_O_WRONLY))
        { kdSetError ( KD_EINVAL ); return 0; }
    if (o_flags & (KD_O_RDWR))
        { o_flags ^= KD_O_RDWR; o_flags |= KD_O_RDONLY; }
    ...
    /* see if we can open a file that is a zip file */
    {
        ...
        if (filename_len >= PATH_MAX)
            { kdSetError ( KD_ENAMETOOLONG ); return 0; }
        ...
        if (stream && stream->dir && stream->dir->realname)
        {
                ...
                filename[len] == '/' && filename[len + 1])
            {
                ...
                if (! fp)
                    { kdSetError ( zzip_errno (stream->dir->errcode) ); }
                return fp;
            }
        }
        ...
        while ((p = strrchr(basename, '/')))
        {
            ...
            *p = '\0';
            ...
            fd = __zzip_try_open(basename, o_flags | KD_O_RDONLY ,//| KD_O_BINARY,
                                 ext, io);
            if (fd == -1)
                { continue; }
            ...
            if (e)
                { kdSetError ( zzip_errno(e) ); io->fd.close(fd); return 0; }

            /* (p - basename) is the lenghtof zzip_dir part of the filename */
            fp = zzip_file_open(dir, filename + (p - basename) + 1, o_modes);
            if (! fp)
                { kdSetError ( zzip_errno(dir->errcode) ); }
            else
                { if (! dir->realname) dir->realname = kdStrdup (basename); }
...
zzip_seek32(ZZIP_FILE * fp, long offset, int whence)
{
    ...
    {
        zzip_off_t off = zzip_seek(fp, offset, whence);
        if (off >= 0) {
            register long off32 = off;
            if (off32 == off) return off32;
            kdSetError ( EOVERFLOW );
        }
        return -1;
    }
}
