...
#include "scripting/javascript/bindings/extension/jsb_cocos2dx_extension_manual.h"
...
private:
    ...
    bool callJSDelegate(TableView* table, KDint32 idx, std::string jsFunctionName, jsval& retVal)
    {
        js_proxy_t * p = jsb_get_native_proxy(table);
        if (!p) return false;
