...
{
    AVIOContext *pb = s->pb;
    CaffContext *caf  = (CaffContext *)s->priv_data;
    AVStream *st;
    int flags;
...
#define ALAC_NEW_KUKI 24
        if (size == ALAC_NEW_KUKI) {
            st->codec->extradata = (uint8_t *)av_mallocz(ALAC_HEADER + FF_INPUT_BUFFER_PADDING_SIZE);
...
static const AVCodecTag* _ff_caf_demuxer[] = {ff_codec_caf_tags, 0};
