 ...
 *
 *      File            main.cpp
 *      Description     M3G Sample - 13_Background
 *      Author          Y.H Mun
 * 
 * --------------------------------------------------------------------------
 *      
...
static KDint                            l_nCropX = 0;
static KDbool                           l_bRight = KD_TRUE;
...
static KDvoid xmEventCreate ( KDvoid )
{
    ...
    M3GImage2D*  pImage = dynamic_cast<M3GImage2D*> ( M3GLoader::load ( "/res/image/music-is.png" ) [ 0 ] );
    M3GBackground*  pBackground  = new M3GBackground ( );
    pBackground->setColor ( 0xff3f8f1f );
    pBackground->setImage ( pImage );
    pBackground->setCrop  ( 0, 0, 512, 512 );
    pBackground->setImageMode ( M3GBackground::REPEAT, M3GBackground::BORDER );

    M3GCamera*  pCamera = new M3GCamera ( );
    pCamera->translate ( 0, 0, 20 );
...
static KDvoid xmEventUpdate ( KDuint msec )
{
    ...
    KDint  nWidth = pBackground->getImage ( )->getWidth ( );

    if ( l_bRight )
    {
    	l_nCropX += 10;

    	if ( l_nCropX > nWidth - 512 )
    	{
    		l_nCropX = nWidth - 512;
    		l_bRight = KD_FALSE;
    	}
    ...
    pBackground->setCrop ( l_nCropX, 0, 512, 512 );
}
