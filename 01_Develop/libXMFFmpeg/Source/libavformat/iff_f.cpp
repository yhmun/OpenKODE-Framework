...
{
    uint8_t *buf = ((data_size + 1) == 0) ? NULL : (uint8_t *)av_malloc(data_size + 1);
    ...
    buf[data_size] = 0;
    av_dict_set(&s->metadata, tag, (const char *)buf, AV_DICT_DONT_STRDUP_VAL);
    return 0;
}
...
{
    IffDemuxContext *iff = (IffDemuxContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    AVStream *st;
    uint8_t *buf;
    ...
    while(!url_feof(pb)) {
        ...
        case ID_VHDR:
            ...
            if (data_size >= 16) {
                avio_skip(pb, 1);
                iff->svx8_compression = (svx8_compression_type)avio_r8(pb);
            }
            break;
        ...
        case ID_CMAP:
            ...
            st->codec->extradata      = (uint8_t *)av_malloc(data_size + IFF_EXTRA_VIDEO_SIZE + FF_INPUT_BUFFER_PADDING_SIZE);
        ...
        case ID_BMHD:
            iff->bitmap_compression = (bitmap_compression_type)-1;
            st->codec->codec_type            = (AVMediaType)AVMEDIA_TYPE_VIDEO;
            if (data_size <= 8)
                return AVERROR_INVALIDDATA;
            ...
            if (data_size >= 10)
                masking                      = avio_r8(pb);
            if (data_size >= 11)
                iff->bitmap_compression      = (bitmap_compression_type)avio_r8(pb);
            if (data_size >= 14) {
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (!st->codec->extradata) {
            ...
            st->codec->extradata      = (uint8_t *)av_malloc(IFF_EXTRA_VIDEO_SIZE + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!st->codec->extradata)
                return AVERROR(ENOMEM);
        }
