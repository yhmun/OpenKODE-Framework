...
#include <XMZZip/lib.h>           /* archive handling */
                            ...
                            zzip_plugin_io_t io)
{
    ...
    hdr0 = (struct zzip_dir_hdr *) kdMalloc((KDsize)zz_rootsize);
    if (! hdr0)
        return ZZIP_DIRSIZE;
    hdr = hdr0;
    __debug_dir_hdr(hdr);
    ...
    if (USE_MMAP && io->fd.sys)
    {
        ...
        if (fd_map == MAP_FAILED)
        {
            NOTE2("map failed: Error %d", kdGetError());//strerror(errno));
            fd_map = 0;
        } else
        {
                ...
                zzip_strings_t * ext, zzip_plugin_io_t io)
{
    ...
    if (len + 4 >= PATH_MAX)
        { kdSetError ( KD_ENAMETOOLONG ); return -1; }
                     ...
                     zzip_strings_t * ext, zzip_plugin_io_t io)
{
    ...
    fd = io->fd.open(filename, KD_O_RDONLY);// | O_BINARY);
    if (fd != -1)
    {
        return zzip_dir_fdopen_ext_io(fd, e, ext, io);
    } else
    {
        fd = __zzip_try_open(filename, KD_O_RDONLY, ext, io);// | O_BINARY, ext, io);
        if (fd != -1)
        {
            return zzip_dir_fdopen_ext_io(fd, e, ext, io);
        } else
