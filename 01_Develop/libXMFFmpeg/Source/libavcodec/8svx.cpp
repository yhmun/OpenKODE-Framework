...
{
    EightSvxContext *esc = (EightSvxContext *)avctx->priv_data;
    ...
    if (!esc->samples && avpkt) {
        ...
        if (!(esc->samples = (uint8_t *)av_malloc(esc->samples_size)))
        ...
        if (avctx->codec->id == CODEC_ID_8SVX_FIB || avctx->codec->id == CODEC_ID_8SVX_EXP) {
            ...
            if (!(deinterleaved_samples = (uint8_t *)av_mallocz(n)))
                return AVERROR(ENOMEM);
            p = deinterleaved_samples;

            /* the uncompressed starting value is contained in the first byte */
            if (avctx->channels == 2) {
                delta_decode((int8_t *)deinterleaved_samples      , buf+1, buf_size/2-1, buf[0], esc->table);
                buf += buf_size/2;
                delta_decode((int8_t *)(deinterleaved_samples+n/2-1), buf+1, buf_size/2-1, buf[0], esc->table);
            } else
                delta_decode((int8_t *)deinterleaved_samples      , buf+1, buf_size-1  , buf[0], esc->table);
        } else {
