                           ...
                           const char *vendor_string, const unsigned count)
{
    ...
    bytestream_put_buffer(p, (const uint8_t *)vendor_string, strlen(vendor_string));
    if (*m) {
        AVDictionaryEntry *tag = NULL;
        ...
        while ((tag = av_dict_get(*m, "", tag, AV_DICT_IGNORE_SUFFIX))) {
            ...
            bytestream_put_buffer(p, (const uint8_t *)tag->key, len1);
            bytestream_put_byte(p, '=');
            bytestream_put_buffer(p, (const uint8_t *)tag->value, len2);
        }
    } else
