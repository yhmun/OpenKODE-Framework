...
{"chunked_post" , "use chunked transfer-encoding for posts", OFFSET(chunked_post), AV_OPT_TYPE_INT, 1, NULL, 0, 1, E },
{"headers"      , "custom HTTP headers, can override built in default headers", OFFSET(headers), AV_OPT_TYPE_STRING, 0, NULL, 0, 0, D|E },
{"user-agent"   , "override User-Agent header", OFFSET(user_agent), AV_OPT_TYPE_STRING, 0, NULL, 0, 0, DEC},
{NULL}
};
...
static int http_open_cnx(URLContext *h)
{
 ...
 redo:
    ...
    ff_url_join(hoststr, sizeof(hoststr), 0, 0, hostname, port, 0);
    ...
    ff_url_join(buf, sizeof(buf), lower_proto, 0, hostname, port, 0);
...
static av_always_inline int has_header(const char *str, const char *header)
{
...
static int http_write(URLContext *h, const uint8_t *buf, int size)
{
    ...
    if (size > 0) {
        ...
        if ((ret = ffurl_write(s->hd, (const unsigned char *)temp, strlen(temp))) < 0 ||
