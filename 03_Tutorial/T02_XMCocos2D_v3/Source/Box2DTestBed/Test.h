...
#define    kdRand_LIMIT    32767

/// kdRandom number in range [-1,1]
inline float32 kdRandomFloat()
{
    float32 r = (float32)(std::rand() & (kdRand_LIMIT));
    r /= kdRand_LIMIT;
    r = 2.0f * r - 1.0f;
    return r;
}

/// kdRandom floating point number in range [lo, hi]
inline float32 kdRandomFloat(float32 lo, float32 hi)
{
    float32 r = (float32)(std::rand() & (kdRand_LIMIT));
    r /= kdRand_LIMIT;
    r = (hi - lo) * r + lo;
...
public:
    ...
    cocos2d::extension::Box2DDebugDraw m_debugDraw;
    int32 m_textLine;
    b2World* m_world;
    b2Body* m_bomb;
