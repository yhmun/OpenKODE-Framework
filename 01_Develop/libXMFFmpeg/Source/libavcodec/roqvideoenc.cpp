...
static void create_cel_evals(RoqContext *enc, RoqTempdata *tempData)
{
    ...
    tempData->cel_evals = (CelEvaluation *)av_malloc(enc->width*enc->height/64 * sizeof(CelEvaluation));
...
static void motion_search(RoqContext *enc, int blocksize)
{
    ...
    for (i=0; i<enc->height; i+=blocksize)
        for (j=0; j<enc->width; j+=blocksize) {
			motion_vect _m = { 0, 0 };
            lowestdiff = eval_motion_dist(enc, j, i, _m,
                                          blocksize);
            bestpick.d[0] = 0;
            bestpick.d[1] = 0;
                              ...
                              int size, int cbsize)
{
    ...
    int *buf;
    int *codebook = (int *)av_malloc(6*c_size*cbsize*sizeof(int));
    int *closest_cb;

    if (size == 4)
        closest_cb = (int *)av_malloc(6*c_size*inputCount*sizeof(int));
    else
...
static void generate_new_codebooks(RoqContext *enc, RoqTempdata *tempData)
{
    ...
    uint8_t mb2[3*4];
    roq_cell *results4 = (roq_cell *)av_malloc(sizeof(roq_cell)*MAX_CBS_4x4*4);
    uint8_t *yuvClusters= (uint8_t *)av_malloc(sizeof(int)*max*6*4);
    int *points = (int *)av_malloc(max*6*4*sizeof(int));
    int bias;
    ...
    tempData->closest_cb2 = (int *)av_malloc(max*4*sizeof(int));
...
{
    RoqContext *enc = (RoqContext *)avctx->priv_data;
    ...
    enc->tmpData      = (RoqTempdata *)av_malloc(sizeof(RoqTempdata));

    enc->this_motion4 = (motion_vect *)
        av_mallocz((enc->width*enc->height/16)*sizeof(motion_vect));

    enc->last_motion4 = (motion_vect *)
        av_malloc ((enc->width*enc->height/16)*sizeof(motion_vect));

    enc->this_motion8 = (motion_vect *)
        av_mallocz((enc->width*enc->height/64)*sizeof(motion_vect));

    enc->last_motion8 = (motion_vect *)
...
static const AVRational _ff_roq_encoder[] = {{30,1}, {0,0}};
static const enum PixelFormat __ff_roq_encoder[] = {PIX_FMT_YUV444P, PIX_FMT_NONE};
AVCodec ff_roq_encoder = {
	...
	/*.next					= */	0,
	/*.flush				= */	0,
	/*.supported_framerates	= */	_ff_roq_encoder,
	/*.pix_fmts				= */	__ff_roq_encoder,
