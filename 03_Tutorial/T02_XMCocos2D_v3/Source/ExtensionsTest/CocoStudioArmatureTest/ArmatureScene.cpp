...
void TestColliderDetector::onFrameEvent(Bone *bone, const char *evt, int originFrameIndex, int currentFrameIndex)
{
    ...
    */
    ...
    bullet->runAction(MoveBy::create(1.5f, Point(350, 0)));
}
...
void TestColliderDetector::initWorld()
{
    ...
    body = cpBodyNew(1.0f, KD_INFINITY);
    cpSpaceAddBody(space, body);
    armature2->setCPBody(body);

    ColliderFilter filter = ColliderFilter(eEnemyTag, 0);
...
void TestArmatureNesting2::onEnter()
{
    ...
    LabelTTF* label = LabelTTF::create("Change Mount", "Arial", 20);
