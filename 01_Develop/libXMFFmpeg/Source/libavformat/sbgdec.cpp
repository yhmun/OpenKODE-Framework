...
static void *alloc_array_elem(void **array, KDsize elsize,
...
static av_always_inline int is_space(char c)
{
    return c == ' '  || c == '\t' || c == '\r';
}

static av_always_inline int scale_double(void *log, double d, double m, int *r)
{
    m *= d * SBG_SCALE;
    ...
    while (lex_char(p, '-') && lex_wsword(p, &ostr)) {
        for (; ostr.s < ostr.e; ostr.s++) {
            ...
            switch (opt) {
                ...
                case 'm':
                    FORWARD_ERROR(parse_optarg(p, opt, &oarg));
                    tptr = (char *)av_malloc(oarg.e - oarg.s + 1);
                    if (!tptr)
...
static int parse_synth_channel(struct sbg_parser *p)
{
    ...
    synth = (struct sbg_script_synth *)alloc_array_elem((void **)&p->scs.synth, sizeof(*synth),
...
static int parse_named_def(struct sbg_parser *p)
{
    ...
    def = (sbg_script_definition *)alloc_array_elem((void **)&p->scs.def, sizeof(*def),
...
fail:
    ...
    if (log && *sp.err_msg) {
        ...
        const char *ectx = (const char *)av_x_if_null(memchr(ctx, '\n', sp.end - sp.cursor),
                                        sp.end);
        int lctx = ectx - ctx;
        const char *quote = "\"";
...
static int read_whole_file(AVIOContext *io, int max_size, char **rbuf)
{
    ...
    while (1) {
        if (bufsize - size < 1024) {
            ...
            buf = (char *)av_realloc_f(buf, bufsize, 1);
            if (!buf) {
                size = AVERROR(ENOMEM);
                goto fail;
            }
        }
        r = avio_read(io, (unsigned char *)buf, bufsize - size - 1);
        if (r == AVERROR_EOF)
            break;
        if (r < 0)
            goto fail;
                       ...
                       int64_t t0, struct sbg_script_tseq *tseq)
{
    ...
    if (def->type == 'B') {
        ...
        ev = (sbg_script_event *)alloc_array_elem((void **)&s->events, sizeof(*ev),
                        ...
                        int64_t ts2, int32_t f2, int32_t a2)
{
    ...
    i = (ws_interval *)alloc_array_elem((void **)&inter->inter, sizeof(*i),
                               ...
                               struct sbg_script_event *ev2)
{
    ...
    enum sbg_fade_type type = (enum sbg_fade_type)(ev1->fade.slide | (ev1->fade.out & ev2->fade.in));
                            ...
                            struct ws_intervals *inter)
{
    ...
    edata = (uint8_t *)av_malloc(edata_size);
    if (!edata)
        return AVERROR(ENOMEM);
    avc->extradata = edata;
...
static av_cold int sbg_read_probe(AVProbeData *p)
{
    ...
    r = parse_script(NULL, (char *)p->buf, p->buf_size, &script);
...
{
    struct sbg_demuxer *sbg = (struct sbg_demuxer *)avf->priv_data;
    int r;
    char *buf = NULL;
    struct sbg_script script = { 0 };
    AVStream *st;
