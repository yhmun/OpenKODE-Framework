...
static void svq1_write_header(SVQ1Context *s, int frame_type)
{
    ...
    if (frame_type == AV_PICTURE_TYPE_I) {
        ...
        i= ff_match_2uint16((const uint16_t (*)[2])ff_svq1_frame_KDsizeable, FF_ARRAY_ELEMS(ff_svq1_frame_KDsizeable), s->frame_width, s->frame_height);
        put_bits(&s->pb, 3, i);
...
//        s->m.unrestricted_mv= 1;
        ...
        if(!s->motion_val8[plane]){
            s->motion_val8 [plane]= (int16_t (*)[2])av_mallocz((s->m.b8_stride*block_height*2 + 2)*2*sizeof(int16_t));
            s->motion_val16[plane]= (int16_t (*)[2])av_mallocz((s->m.mb_stride*(block_height + 2) + 1)*2*sizeof(int16_t));
        }
...
{
    SVQ1Context * const s = (SVQ1Context *const)avctx->priv_data;
    ...
    s->mb_type        = (uint16_t *)av_mallocz((s->y_block_width+1)*s->y_block_height*sizeof(int16_t));
    s->dummy          = (uint32_t *)av_mallocz((s->y_block_width+1)*s->y_block_height*sizeof(int32_t));
    ...
    int buf_size, void *data)
{
    ...
    if(!s->current_picture.data[0]){
        ...
        s->scratchbuf = (uint8_t *)av_malloc(s->current_picture.linesize[0] * 16 * 2);
    }
...
static const enum PixelFormat _ff_svq1_encoder[] = {PIX_FMT_YUV410P, PIX_FMT_NONE};
