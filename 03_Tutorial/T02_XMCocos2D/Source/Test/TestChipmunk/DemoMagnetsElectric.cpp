...
#define COU_MKS     8.987551787e9f // Some physical constants
#define MAG_MKS     1e-7f
...
static KDvoid CoulombForce ( ForceData* data )
{
	data->F = cpvmult ( cpvnormalize ( data->relp ), COU_MKS / data->r[1] );
}

// forces between magnets
static KDvoid MagDipoleForce ( ForceData* data )
{
	...
	Fr   = ( 2.0e0f * cpfcos ( alpha ) * cpfcos ( beta ) - cpfsin ( alpha ) * cpfsin ( beta ) );
	Fphi = cpfsin ( alpha + beta );
	// kdLogMessagefKHR ( "%g %g %g %g %g", phi, alpha, beta, Fphi );
	
	// Cartesian coordinates
	data->F = cpv ( Fr * cpfcos ( phi ) - Fphi * cpfsin ( phi ), Fr * cpfsin ( phi ) + Fphi * cpfcos ( phi ) );
	data->F = cpvmult ( data->F,  -3.e0f * MAG_MKS / ( data->r[1] * data->r[1] ) );
}

static KDvoid MagDipoleTorque ( ForceData* data )
{
	...
	data->T = ( MAG_MKS / data->r[2] ) * (  3.0e0f * cpfcos ( alpha ) * cpfsin ( beta ) + cpfsin ( alpha - beta ) );
}
// ******* // 
...
static KDvoid FillForceData ( Sing* source, KDint inds, Sing* obs, KDint indo, ForceData* data )
{
...
static KDvoid LRangeForceApply ( cpBody* a, cpBody* b )
{
	...
	for ( KDint i = 0; i < aux->Nsing; i++ )
	{
		for ( int j=0; j<aux2->Nsing; j++ )
		{
			if ( !kdStrcmp ( aux->type[i], aux2->type[j] ) )
			{
				//kdLogMessagefKHR ( "%s %s", aux->type[i], aux2->type[j] );
...
static KDvoid ChargedBodyUpdatePositionVerlet ( cpBody* body, cpFloat dt )
{
	...
	cpBodySetAngle ( body, body->a + ( body->w + body->w_bias ) * dt + 0.5f * body->t * body->i_inv * dt * dt );

	// Update position of the singularities
	aux = ( Sing* ) body->data;
	for ( KDint i = 0; i < aux->Nsing; i++ )
	{
        aux->Gpos[i] = cpvadd ( body->p, cpvrotate ( cpv ( aux->position[i].x, aux->position[i].y ), body->rot ) );
...
static KDvoid ChargedBodyUpdateVelocityVerlet ( cpBody* body, cpVect gravity, cpFloat damping, cpFloat dt )
{
...
static KDvoid make_mag ( cpVect p, cpFloat ang, cpFloat mag )
{
	KDint nverts = 6;
	cpVect verts[] = 
	{
	...
	magnet->Nsing = 1; 
	magnet->value[0] = mag;
	kdSprintfKHR ( magnet->type[0], "magdipole" ); 
...
static KDvoid make_charged ( cpVect p, cpFloat chg )
{
	KDint nverts = 4;
	cpVect verts[] =
	{
	...
	charge->Nsing = 1; 
	charge->value[0] = chg;
	kdSprintfKHR ( charge->type[0], "electrical" ); 
...
void make_mix ( cpVect p, cpFloat ang, cpFloat mag,cpFloat chg )
{
	...
	mix->value[0] = mag;
	mix->value[1] = chg;
	kdSprintfKHR ( mix->type[0], "magdipole" );
	kdSprintfKHR ( mix->type[1], "electrical" );  
...
/*	shape = cpSegmentShapeNew ( staticBody, cpv ( -320, -240 ), cpv ( -320, 240 ), 0.0f );
	...
	cpSpaceAddCollisionPairFunc ( space, 0, 1, KD_NULL, NULL );
*/		
	
	kdSrand ( (KDuint) kdTime ( KD_NULL ) );
    cpVect  p;
	cpFloat ang;
	
	// Create magnets
	for ( KDint i = 0; i < NMAG; i++ )
	{
		p.x = ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * WIDTH / 2.0f;
		p.y = ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * HEIGHT / 2.0f;
		ang = ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * KD_PI_F;
		make_mag ( p, ang, 1.0e7f );
	}
	
	// Create charged objects
	for ( KDint i = 0; i < NCHG; i++ )
	{
		p.x= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * WIDTH / 2.0f;
		p.y= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * HEIGHT / 2.0f;
		ang= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1.0e0f ) * KD_PI_F;
		make_charged ( p, 1.0e-3f * cpfpow ( -1.0f, ( cpFloat ) ( i % 2 ) ) );
	}
		
	// Create charged magnets objects
	for ( KDint i = 0; i < NMIX; i++ )
	{
		p.x= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1 ) * WIDTH / 2.0f;
		p.y= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1 ) * HEIGHT / 2.0f;
		ang= ( 2.0e0f * kdRand ( ) / ( KD_RAND_MAX ) - 1 ) * KD_PI_F;
		make_mix ( p, ang, 1.0e7f * cpfpow ( -1.0f, ( cpFloat ) ( i % 2 ) ),  1.0e-3f * cpfpow ( -1.0f, ( cpFloat ) ( i % 2 ) ) );
	}
