...
{
    Mpeg1Context *s = (Mpeg1Context *)avctx->priv_data;
...
{
    Mpeg1Context *ctx = (Mpeg1Context *)avctx->priv_data, *ctx_from = (Mpeg1Context *)avctx_from->priv_data;
...
{
    Mpeg1Context *s1 = (Mpeg1Context *)avctx->priv_data;
            ...
            if (s->aspect_ratio_info > 1) {
                AVRational dar =
                    av_mul_q(av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],
                                      av_rational(s1->pan_scan.width, s1->pan_scan.height)),
                             av_rational(s->width, s->height));

                // we ignore the spec here and guess a bit as reality does not match the spec, see for example
                // res_change_ffmpeg_aspect.ts and sequence-display-aspect.mpg
                // issue1613, 621, 562
                if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
                   (av_cmp_q(dar, av_rational(4, 3)) && av_cmp_q(dar, av_rational(16, 9)))) {
...
static void mpeg_decode_sequence_display_extension(Mpeg1Context *s1)
{
    ...
    if (color_description) {
        s->avctx->color_primaries = (AVColorPrimaries)get_bits(&s->gb, 8);
        s->avctx->color_trc       = (AVColorTransferCharacteristic)get_bits(&s->gb, 8);
        s->avctx->colorspace      = (AVColorSpace)get_bits(&s->gb, 8);
    }
    w = get_bits(&s->gb, 14);
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    if (avctx->hwaccel) {
        ...
        int start_code = -1;
        buf_end = avpriv_mpv_find_start_code(buf_start + 2, *buf + buf_size, (uint32_t *)&start_code);
