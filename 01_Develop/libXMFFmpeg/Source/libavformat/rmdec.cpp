...
static av_always_inline void get_strl(AVIOContext *pb, char *buf, int buf_size, int len)
{
    int i;
    char *q, r;
...
{
    RMStream *rms = (RMStream *)av_mallocz(sizeof(RMStream));
    rms->curpic_num = -1;
    return rms;
}
                                     ...
                                     AVStream *st, RMStream *ast, int read_all)
{
    ...
    if (version == 3) {
        ...
        if (version == 5) {
            ...
            avio_read(pb, (unsigned char *)buf, 4);
            buf[4] = 0;
        } else {
...
{
    RMDemuxContext *rm = (RMDemuxContext *)s->priv_data;
    AVStream *st;
    ...
    return rm_read_audio_stream_info(s, s->pb, st, (RMStream *)st->priv_data, 1);
}
...
static int rm_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        case MKTAG('M', 'D', 'P', 'R'):
            ...
            st->priv_data = ff_rm_alloc_rmstream();
            if (ff_rm_read_mdpr_codecdata(s, s->pb, st, (RMStream *)st->priv_data,
                                          avio_rb32(pb)) < 0)
                return -1;
            break;
        case MKTAG('D', 'A', 'T', 'A'):
...
static int rm_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        if (rm->audio_pkt_cnt) {
            ...
            ff_rm_retrieve_cache(s, s->pb, st, (RMStream *)st->priv_data, pkt);
            flags = 0;
        } else {
            if (rm->old_format) {
                RMStream *ast;
            ...
            res = ff_rm_parse_packet (s, s->pb, st, (RMStream *)st->priv_data, len, pkt,
                                      &seq, flags, timestamp);
...
static int rm_read_close(AVFormatContext *s)
{
    ...
    for (i=0;i<s->nb_streams;i++)
        ff_rm_free_rmstream((RMStream *)s->streams[i]->priv_data);
