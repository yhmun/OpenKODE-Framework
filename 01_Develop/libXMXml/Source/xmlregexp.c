...
{
    const char *regexp = KD_NULL;
    if (ctxt != KD_NULL) {
        regexp = (const char *) ctxt->string;
	ctxt->error = XML_ERR_NO_MEMORY;
    }
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_REGEXP,
...
xmlRegexpErrCompile(xmlRegParserCtxtPtr ctxt, const char *extra)
{
    ...
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_REGEXP,
		    XML_REGEXP_COMPILE_ERROR, XML_ERR_FATAL, KD_NULL, 0, extra,
		    regexp, KD_NULL, idx, 0,
		    "failed to compile: %s\n", extra);
}
 ...
 *
 * Returns the new regexp or KD_NULL in case of error
 */
static xmlRegexpPtr
xmlRegEpxFromParse(xmlRegParserCtxtPtr ctxt) {
    ...
    kdMemset (ret, 0, sizeof(xmlRegexp));
	...
	if (stateRemap == KD_NULL) {
	    xmlRegexpErrMemory(ctxt, "compiling regexp");
	    xmlFree(ret);
	    return(KD_NULL);
	}
	for (i = 0;i < ret->nbStates;i++) {
	    if (ret->states[i] != KD_NULL) {
	...
	kdMemset (transitions, 0, (nbstates + 1) * (nbatoms + 1) * sizeof(int));
	...
	for (i = 0;i < ret->nbStates;i++) {
		...
		trans = &(state->trans[j]);
		if ((trans->to == -1) || (trans->atom == KD_NULL))
		    continue;
                atomno = stringRemap[trans->atom->no];
		if ((trans->atom->data != KD_NULL) && (transdata == KD_NULL)) {
		    transdata = (void **) xmlMalloc(nbstates * nbatoms *
			                            sizeof(void *));
		    if (transdata != KD_NULL)
			kdMemset (transdata, 0,
 ...
 *
 * Returns the new context or KD_NULL in case of error
 */
...
xmlRegNewParserCtxt(const xmlChar *string) {
    ...
    if (ret == KD_NULL)
	return(KD_NULL);
    kdMemset (ret, 0, sizeof(xmlRegParserCtxt));
    if (string != KD_NULL)
 ...
 *
 * Returns the new range or KD_NULL in case of error
 */
 ...
 * Copy a regexp range
 *
 * Returns the new copy or KD_NULL in case of error.
 */
static xmlRegRangePtr
 ...
 * Allocate a new atom
 *
 * Returns the new atom or KD_NULL in case of error
 */
...
xmlRegNewAtom(xmlRegParserCtxtPtr ctxt, xmlRegAtomType type) {
    ...
    kdMemset (ret, 0, sizeof(xmlRegAtom));
    ret->type = type;
...
xmlRegFreeAtom(xmlRegAtomPtr atom) {
    ...
    if (atom->ranges != KD_NULL)
	xmlFree(atom->ranges);
    if ((atom->type == XML_REGEXP_STRING) && (atom->valuep != KD_NULL))
	xmlFree(atom->valuep);
    if ((atom->type == XML_REGEXP_STRING) && (atom->valuep2 != KD_NULL))
	xmlFree(atom->valuep2);
    if ((atom->type == XML_REGEXP_BLOCK_NAME) && (atom->valuep != KD_NULL))
	xmlFree(atom->valuep);
    xmlFree(atom);
}
...
xmlRegCopyAtom(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    ...
    if (atom->nbRanges > 0) {
        ...
        ret->ranges = (xmlRegRangePtr *) xmlMalloc(sizeof(xmlRegRangePtr) *
	                                           atom->nbRanges);
	if (ret->ranges == KD_NULL) {
	    xmlRegexpErrMemory(ctxt, "copying atom");
	    goto error;
	}
	for (i = 0;i < atom->nbRanges;i++) {
	    ret->ranges[i] = xmlRegCopyRange(ctxt, atom->ranges[i]);
	    if (ret->ranges[i] == KD_NULL)
	        goto error;
	    ret->nbRanges = i + 1;
	}
    }
...
xmlRegNewState(xmlRegParserCtxtPtr ctxt) {
    ...
    kdMemset (ret, 0, sizeof(xmlRegState));
...
xmlRegFreeState(xmlRegStatePtr state) {
    ...
    if (state->trans != KD_NULL)
	xmlFree(state->trans);
    if (state->transTo != KD_NULL)
...
xmlRegFreeParserCtxt(xmlRegParserCtxtPtr ctxt) {
    ...
    if (ctxt->atoms != KD_NULL) {
	for (i = 0;i < ctxt->nbAtoms;i++)
	    xmlRegFreeAtom(ctxt->atoms[i]);
	xmlFree(ctxt->atoms);
    }
    if (ctxt->counters != KD_NULL)
...
static void
xmlRegPrintAtomType(KDFile *output, xmlRegAtomType type) {
    switch (type) {
        case XML_REGEXP_EPSILON:
	    kdFprintfKHR(output, "epsilon "); break;
        case XML_REGEXP_CHARVAL:
	    kdFprintfKHR(output, "charval "); break;
        case XML_REGEXP_RANGES:
	    kdFprintfKHR(output, "ranges "); break;
        case XML_REGEXP_SUBREG:
	    kdFprintfKHR(output, "subexpr "); break;
        case XML_REGEXP_STRING:
	    kdFprintfKHR(output, "string "); break;
        case XML_REGEXP_ANYCHAR:
	    kdFprintfKHR(output, "anychar "); break;
        case XML_REGEXP_ANYSPACE:
	    kdFprintfKHR(output, "anyspace "); break;
        case XML_REGEXP_NOTSPACE:
	    kdFprintfKHR(output, "notspace "); break;
        case XML_REGEXP_INITNAME:
	    kdFprintfKHR(output, "initname "); break;
        case XML_REGEXP_NOTINITNAME:
	    kdFprintfKHR(output, "notinitname "); break;
        case XML_REGEXP_NAMECHAR:
	    kdFprintfKHR(output, "namechar "); break;
        case XML_REGEXP_NOTNAMECHAR:
	    kdFprintfKHR(output, "notnamechar "); break;
        case XML_REGEXP_DECIMAL:
	    kdFprintfKHR(output, "decimal "); break;
        case XML_REGEXP_NOTDECIMAL:
	    kdFprintfKHR(output, "notdecimal "); break;
        case XML_REGEXP_REALCHAR:
	    kdFprintfKHR(output, "realchar "); break;
        case XML_REGEXP_NOTREALCHAR:
	    kdFprintfKHR(output, "notrealchar "); break;
        case XML_REGEXP_LETTER:
            kdFprintfKHR(output, "LETTER "); break;
        case XML_REGEXP_LETTER_UPPERCASE:
            kdFprintfKHR(output, "LETTER_UPPERCASE "); break;
        case XML_REGEXP_LETTER_LOWERCASE:
            kdFprintfKHR(output, "LETTER_LOWERCASE "); break;
        case XML_REGEXP_LETTER_TITLECASE:
            kdFprintfKHR(output, "LETTER_TITLECASE "); break;
        case XML_REGEXP_LETTER_MODIFIER:
            kdFprintfKHR(output, "LETTER_MODIFIER "); break;
        case XML_REGEXP_LETTER_OTHERS:
            kdFprintfKHR(output, "LETTER_OTHERS "); break;
        case XML_REGEXP_MARK:
            kdFprintfKHR(output, "MARK "); break;
        case XML_REGEXP_MARK_NONSPACING:
            kdFprintfKHR(output, "MARK_NONSPACING "); break;
        case XML_REGEXP_MARK_SPACECOMBINING:
            kdFprintfKHR(output, "MARK_SPACECOMBINING "); break;
        case XML_REGEXP_MARK_ENCLOSING:
            kdFprintfKHR(output, "MARK_ENCLOSING "); break;
        case XML_REGEXP_NUMBER:
            kdFprintfKHR(output, "NUMBER "); break;
        case XML_REGEXP_NUMBER_DECIMAL:
            kdFprintfKHR(output, "NUMBER_DECIMAL "); break;
        case XML_REGEXP_NUMBER_LETTER:
            kdFprintfKHR(output, "NUMBER_LETTER "); break;
        case XML_REGEXP_NUMBER_OTHERS:
            kdFprintfKHR(output, "NUMBER_OTHERS "); break;
        case XML_REGEXP_PUNCT:
            kdFprintfKHR(output, "PUNCT "); break;
        case XML_REGEXP_PUNCT_CONNECTOR:
            kdFprintfKHR(output, "PUNCT_CONNECTOR "); break;
        case XML_REGEXP_PUNCT_DASH:
            kdFprintfKHR(output, "PUNCT_DASH "); break;
        case XML_REGEXP_PUNCT_OPEN:
            kdFprintfKHR(output, "PUNCT_OPEN "); break;
        case XML_REGEXP_PUNCT_CLOSE:
            kdFprintfKHR(output, "PUNCT_CLOSE "); break;
        case XML_REGEXP_PUNCT_INITQUOTE:
            kdFprintfKHR(output, "PUNCT_INITQUOTE "); break;
        case XML_REGEXP_PUNCT_FINQUOTE:
            kdFprintfKHR(output, "PUNCT_FINQUOTE "); break;
        case XML_REGEXP_PUNCT_OTHERS:
            kdFprintfKHR(output, "PUNCT_OTHERS "); break;
        case XML_REGEXP_SEPAR:
            kdFprintfKHR(output, "SEPAR "); break;
        case XML_REGEXP_SEPAR_SPACE:
            kdFprintfKHR(output, "SEPAR_SPACE "); break;
        case XML_REGEXP_SEPAR_LINE:
            kdFprintfKHR(output, "SEPAR_LINE "); break;
        case XML_REGEXP_SEPAR_PARA:
            kdFprintfKHR(output, "SEPAR_PARA "); break;
        case XML_REGEXP_SYMBOL:
            kdFprintfKHR(output, "SYMBOL "); break;
        case XML_REGEXP_SYMBOL_MATH:
            kdFprintfKHR(output, "SYMBOL_MATH "); break;
        case XML_REGEXP_SYMBOL_CURRENCY:
            kdFprintfKHR(output, "SYMBOL_CURRENCY "); break;
        case XML_REGEXP_SYMBOL_MODIFIER:
            kdFprintfKHR(output, "SYMBOL_MODIFIER "); break;
        case XML_REGEXP_SYMBOL_OTHERS:
            kdFprintfKHR(output, "SYMBOL_OTHERS "); break;
        case XML_REGEXP_OTHER:
            kdFprintfKHR(output, "OTHER "); break;
        case XML_REGEXP_OTHER_CONTROL:
            kdFprintfKHR(output, "OTHER_CONTROL "); break;
        case XML_REGEXP_OTHER_FORMAT:
            kdFprintfKHR(output, "OTHER_FORMAT "); break;
        case XML_REGEXP_OTHER_PRIVATE:
            kdFprintfKHR(output, "OTHER_PRIVATE "); break;
        case XML_REGEXP_OTHER_NA:
            kdFprintfKHR(output, "OTHER_NA "); break;
        case XML_REGEXP_BLOCK_NAME:
	    kdFprintfKHR(output, "BLOCK "); break;
    }
}

static void
xmlRegPrintQuantType(KDFile *output, xmlRegQuantType type) {
    switch (type) {
        case XML_REGEXP_QUANT_EPSILON:
	    kdFprintfKHR(output, "epsilon "); break;
        case XML_REGEXP_QUANT_ONCE:
	    kdFprintfKHR(output, "once "); break;
        case XML_REGEXP_QUANT_OPT:
	    kdFprintfKHR(output, "? "); break;
        case XML_REGEXP_QUANT_MULT:
	    kdFprintfKHR(output, "* "); break;
        case XML_REGEXP_QUANT_PLUS:
	    kdFprintfKHR(output, "+ "); break;
	case XML_REGEXP_QUANT_RANGE:
	    kdFprintfKHR(output, "range "); break;
	case XML_REGEXP_QUANT_ONCEONLY:
	    kdFprintfKHR(output, "onceonly "); break;
	case XML_REGEXP_QUANT_ALL:
	    kdFprintfKHR(output, "all "); break;
    }
}
static void
xmlRegPrintRange(KDFile *output, xmlRegRangePtr range) {
    kdFprintfKHR(output, "  range: ");
    if (range->neg)
	kdFprintfKHR(output, "negative ");
    xmlRegPrintAtomType(output, range->type);
    kdFprintfKHR(output, "%c - %c\n", range->start, range->end);
}

static void
xmlRegPrintAtom(KDFile *output, xmlRegAtomPtr atom) {
    kdFprintfKHR(output, " atom: ");
    if (atom == KD_NULL) {
	kdFprintfKHR(output, "KD_NULL\n");
	return;
    }
    if (atom->neg)
        kdFprintfKHR(output, "not ");
    xmlRegPrintAtomType(output, atom->type);
    xmlRegPrintQuantType(output, atom->quant);
    if (atom->quant == XML_REGEXP_QUANT_RANGE)
	kdFprintfKHR(output, "%d-%d ", atom->min, atom->max);
    if (atom->type == XML_REGEXP_STRING)
	kdFprintfKHR(output, "'%s' ", (char *) atom->valuep);
    if (atom->type == XML_REGEXP_CHARVAL)
	kdFprintfKHR(output, "char %c\n", atom->codepoint);
    else if (atom->type == XML_REGEXP_RANGES) {
	int i;
	kdFprintfKHR(output, "%d entries\n", atom->nbRanges);
	for (i = 0; i < atom->nbRanges;i++)
	    xmlRegPrintRange(output, atom->ranges[i]);
    } else if (atom->type == XML_REGEXP_SUBREG) {
	kdFprintfKHR(output, "start %d end %d\n", atom->start->no, atom->stop->no);
    } else {
	kdFprintfKHR(output, "\n");
    }
}
...
static void
xmlRegPrintTrans(KDFile *output, xmlRegTransPtr trans) {
    kdFprintfKHR(output, "  trans: ");
    if (trans == KD_NULL) {
	kdFprintfKHR(output, "KD_NULL\n");
	return;
    }
    if (trans->to < 0) {
	kdFprintfKHR(output, "removed\n");
	return;
    }
    if (trans->nd != 0) {
	if (trans->nd == 2)
	    kdFprintfKHR(output, "last not determinist, ");
	else
	    kdFprintfKHR(output, "not determinist, ");
    }
    if (trans->counter >= 0) {
	kdFprintfKHR(output, "counted %d, ", trans->counter);
    }
    if (trans->count == REGEXP_ALL_COUNTER) {
	kdFprintfKHR(output, "all transition, ");
    } else if (trans->count >= 0) {
	kdFprintfKHR(output, "count based %d, ", trans->count);
    }
    if (trans->atom == KD_NULL) {
	kdFprintfKHR(output, "epsilon to %d\n", trans->to);
	return;
    }
    if (trans->atom->type == XML_REGEXP_CHARVAL)
	kdFprintfKHR(output, "char %c ", trans->atom->codepoint);
    kdFprintfKHR(output, "atom %d, to %d\n", trans->atom->no, trans->to);
}
    
static void
xmlRegPrintState(KDFile *output, xmlRegStatePtr state) {
    int i;

    kdFprintfKHR(output, " state: ");
    if (state == KD_NULL) {
	kdFprintfKHR(output, "KD_NULL\n");
	return;
    }
	...
	kdFprintfKHR(output, "FINAL ");
    
    kdFprintfKHR(output, "%d, %d transitions:\n", state->no, state->nbTrans);
...
static void
xmlRegPrintCtxt(KDFile *output, xmlRegParserCtxtPtr ctxt) {
    int i;

    kdFprintfKHR(output, " ctxt: ");
    if (ctxt == KD_NULL) {
	kdFprintfKHR(output, "KD_NULL\n");
	return;
    }
    kdFprintfKHR(output, "'%s' ", ctxt->string);
    if (ctxt->error)
	kdFprintfKHR(output, "error ");
    if (ctxt->neg)
	kdFprintfKHR(output, "neg ");
    kdFprintfKHR(output, "\n");
    kdFprintfKHR(output, "%d atoms:\n", ctxt->nbAtoms);
    for (i = 0;i < ctxt->nbAtoms; i++) {
	kdFprintfKHR(output, " %02d ", i);
	xmlRegPrintAtom(output, ctxt->atoms[i]);
    }
    if (ctxt->atom != KD_NULL) {
	kdFprintfKHR(output, "current atom:\n");
	xmlRegPrintAtom(output, ctxt->atom);
    }
    kdFprintfKHR(output, "%d states:", ctxt->nbStates);
    if (ctxt->start != KD_NULL)
	kdFprintfKHR(output, " start: %d", ctxt->start->no);
    if (ctxt->end != KD_NULL)
	kdFprintfKHR(output, " end: %d", ctxt->end->no);
    kdFprintfKHR(output, "\n");
    for (i = 0;i < ctxt->nbStates; i++) {
	xmlRegPrintState(output, ctxt->states[i]);
    }
    kdFprintfKHR(output, "%d counters:\n", ctxt->nbCounters);
    for (i = 0;i < ctxt->nbCounters; i++) {
	kdFprintfKHR(output, " %d: min %d max %d\n", i, ctxt->counters[i].min,
		                                ctxt->counters[i].max);
    }
}
#endif
...
xmlRegAtomAddRange(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom,
    ...
    if (atom == KD_NULL) {
	ERROR("add range: atom is KD_NULL");
	return;
    }
...
xmlRegAtomPush(xmlRegParserCtxtPtr ctxt, xmlRegAtomPtr atom) {
    if (atom == KD_NULL) {
	ERROR("atom push: atom is KD_NULL");
	return(-1);
    }
    if (ctxt->maxAtoms == 0) {
	ctxt->maxAtoms = 4;
...
xmlRegStateAddTrans(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr state,
    ...
    if (state == KD_NULL) {
	ERROR("add state: state is KD_NULL");
	return;
    }
    if (target == KD_NULL) {
	ERROR("add state: target is KD_NULL");
	return;
    }
    /*
 ...
 * @from:  the from state
 * @to:  the target state or KD_NULL for building a new one
 * @lax:
 *
 */
static void
xmlFAGenerateAllTransition(xmlRegParserCtxtPtr ctxt,
    ...
    if (lax)
	xmlRegStateAddTrans(ctxt, from, KD_NULL, to, -1, REGEXP_ALL_LAX_COUNTER);
    else
	xmlRegStateAddTrans(ctxt, from, KD_NULL, to, -1, REGEXP_ALL_COUNTER);
}
...
xmlFAGenerateEpsilonTransition(xmlRegParserCtxtPtr ctxt,
    ...
    xmlRegStateAddTrans(ctxt, from, KD_NULL, to, -1, -1);
}
...
xmlFAGenerateCountedEpsilonTransition(xmlRegParserCtxtPtr ctxt,
    ...
    xmlRegStateAddTrans(ctxt, from, KD_NULL, to, counter, -1);
}
...
xmlFAGenerateCountedTransition(xmlRegParserCtxtPtr ctxt,
    ...
    xmlRegStateAddTrans(ctxt, from, KD_NULL, to, -1, counter);
}
...
xmlFAGenerateTransitions(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr from,
    ...
    if (atom == KD_NULL) {
	ERROR("genrate transition: atom == KD_NULL");
	return(-1);
    }
...
#ifdef DV
	} else if ((to == KD_NULL) && (atom->quant != XML_REGEXP_QUANT_RANGE) && 
	...
	switch (atom->quant) {
		 ...
		 */
		if ((atom->min == 0) && (atom->start0 == KD_NULL)) {
		    xmlRegAtomPtr copy;
		    /*
		    ...
		    if (xmlFAGenerateTransitions(ctxt, atom->start, KD_NULL, copy)
		        < 0)
			return(-1);
		    inter = ctxt->state;
...
xmlFAReduceEpsilonTransitions(xmlRegParserCtxtPtr ctxt, int fromnr,
    ...
    for (transnr = 0;transnr < to->nbTrans;transnr++) {
	    ...
	    continue;
	if (to->trans[transnr].atom == KD_NULL) {
	    /*
		...
		if (to->trans[transnr].count >= 0) {
		    ...
		    xmlRegStateAddTrans(ctxt, from, KD_NULL,
					ctxt->states[newto], 
...
xmlFAEliminateEpsilonTransitions(xmlRegParserCtxtPtr ctxt) {
    ...
    for (statenr = 0;statenr < ctxt->nbStates;statenr++) {
	state = ctxt->states[statenr];
	if ((state != KD_NULL) && (state->type == XML_REGEXP_UNREACH_STATE)) {
#ifdef DEBUG_REGEXP_GRAPH
	    printf("Removed unreachable state %d\n", statenr);
#endif
	    xmlRegFreeState(state);
	    ctxt->states[statenr] = KD_NULL;
	}
    }
	...
	for (transnr = 0;transnr < state->nbTrans;transnr++) {
	    if ((state->trans[transnr].atom == KD_NULL) &&
	...
	for (transnr = 0;transnr < state->nbTrans;transnr++) {
		...
		((state->trans[transnr].atom != KD_NULL) ||
		 (state->trans[transnr].count >= 0))) {
		int newto = state->trans[transnr].to;

		if (ctxt->states[newto] == KD_NULL)
		    continue;
	...
	if (target == KD_NULL) {
		...
		state = ctxt->states[statenr];
		if ((state != KD_NULL) && (state->reached ==
			XML_REGEXP_MARK_START)) {
		    target = state;
		    break;
		}
    ...
    for (statenr = 0;statenr < ctxt->nbStates;statenr++) {
	state = ctxt->states[statenr];
	if ((state != KD_NULL) && (state->reached == XML_REGEXP_MARK_NORMAL)) {
#ifdef DEBUG_REGEXP_GRAPH
	    printf("Removed unreachable state %d\n", statenr);
#endif
	    xmlRegFreeState(state);
...
xmlFAEqualAtoms(xmlRegAtomPtr atom1, xmlRegAtomPtr atom2, int deep) {
    ...
    if (atom1 == atom2)
	return(1);
    if ((atom1 == KD_NULL) || (atom2 == KD_NULL))
	return(0);
...
xmlRegCheckCharacter(xmlRegAtomPtr atom, int codepoint) {
    ...
    if ((atom == KD_NULL) || (!IS_CHAR(codepoint)))
	return(-1);
    ...
    switch (atom->type) {
        ...
        case XML_REGEXP_STRING:
	    kdLogMessage ("TODO: XML_REGEXP_STRING\n");
	    return(-1);
        case XML_REGEXP_ANYCHAR:
        case XML_REGEXP_ANYSPACE:
        case XML_REGEXP_NOTSPACE:
...
xmlFARegExecSave(xmlRegExecCtxtPtr exec) {
	...
	if (exec->rollbacks == KD_NULL) {
	    xmlRegexpErrMemory(KD_NULL, "saving regexp");
	    exec->maxRollbacks = 0;
	    return;
	}
	kdMemset (exec->rollbacks, 0,
	...
	kdMemset (tmp, 0, (exec->maxRollbacks - len) * sizeof(xmlRegExecRollback));
    }
    exec->rollbacks[exec->nbRollbacks].state = exec->state;
    exec->rollbacks[exec->nbRollbacks].index = exec->index;
    exec->rollbacks[exec->nbRollbacks].nextbranch = exec->transno + 1;
    if (exec->comp->nbCounters > 0) {
	if (exec->rollbacks[exec->nbRollbacks].counts == KD_NULL) {
	...
	kdMemcpy (exec->rollbacks[exec->nbRollbacks].counts, exec->counts,
...
xmlFARegExecRollBack(xmlRegExecCtxtPtr exec) {
    ...
    if (exec->comp->nbCounters > 0) {
	if (exec->rollbacks[exec->nbRollbacks].counts == KD_NULL) {
	    kdPrintf("exec save: allocation failed");
	    exec->status = -6;
	    return;
	}
	kdMemcpy (exec->counts, exec->rollbacks[exec->nbRollbacks].counts,
	       exec->comp->nbCounters * sizeof(int));
    }
...
xmlFARegExec(xmlRegexpPtr comp, const xmlChar *content) {
    ...
    exec->maxRollbacks = 0;
    exec->nbRollbacks = 0;
    exec->rollbacks = KD_NULL;
    exec->status = 0;
    exec->comp = comp;
    exec->state = comp->states[0];
    exec->transno = 0;
    exec->transcount = 0;
    exec->inputStack = KD_NULL;
    exec->inputStackMax = 0;
    if (comp->nbCounters > 0) {
	exec->counts = (int *) xmlMalloc(comp->nbCounters * sizeof(int));
	if (exec->counts == KD_NULL) {
	    xmlRegexpErrMemory(KD_NULL, "running regexp");
	    return(-1);
	}
        kdMemset (exec->counts, 0, comp->nbCounters * sizeof(int));
    } else
	exec->counts = KD_NULL;
    while ((exec->status == 0) &&
	 ...
	 */
	len = 1;
	if ((exec->inputString[exec->index] == 0) && (exec->counts == KD_NULL)) {
	    /*
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		kdPrintf("epsilon transition left at runtime\n");
		exec->status = -2;
		break;
...
error:
	...
	if (exec->counts != KD_NULL) {
		...
		if (exec->rollbacks[i].counts != KD_NULL)
...
xmlRegNewExecCtxt(xmlRegexpPtr comp, xmlRegExecCallbacks callback, void *data) {
    ...
    if (comp == KD_NULL)
	return(KD_NULL);
    if ((comp->compact == KD_NULL) && (comp->states == KD_NULL))
        return(KD_NULL);
    exec = (xmlRegExecCtxtPtr) xmlMalloc(sizeof(xmlRegExecCtxt));
    if (exec == KD_NULL) {
	xmlRegexpErrMemory(KD_NULL, "creating execution context");
	return(KD_NULL);
    }
    kdMemset (exec, 0, sizeof(xmlRegExecCtxt));
    exec->inputString = KD_NULL;
    exec->index = 0;
    exec->determinist = 1;
    exec->maxRollbacks = 0;
    exec->nbRollbacks = 0;
	...
	if (exec->counts == KD_NULL) {
        ...
        kdMemset (exec->counts, 0, comp->nbCounters * sizeof(int) * 2);
	exec->errCounts = &exec->counts[comp->nbCounters];
    } else {
	exec->counts = KD_NULL;
	exec->errCounts = KD_NULL;
    }
    exec->inputStackMax = 0;
    exec->inputStackNr = 0;
    exec->inputStack = KD_NULL;
    exec->errStateNo = -1;
    exec->errString = KD_NULL;
    exec->nbPush = 0;
    return(exec);
}
...
xmlRegFreeExecCtxt(xmlRegExecCtxtPtr exec) {
	...
	for (i = 0;i < exec->inputStackNr;i++) {
	    if (exec->inputStack[i].value != KD_NULL)
		xmlFree(exec->inputStack[i].value);
	}
	xmlFree(exec->inputStack);
    }
    if (exec->errString != KD_NULL)
...
xmlFARegExecSaveInputString(xmlRegExecCtxtPtr exec, const xmlChar *value,
	...
	if (exec->inputStack == KD_NULL) {
	    xmlRegexpErrMemory(KD_NULL, "pushing input string");
	    exec->inputStackMax = 0;
	    return;
	}
    ...
    exec->inputStackNr++;
    exec->inputStack[exec->inputStackNr].value = KD_NULL;
    exec->inputStack[exec->inputStackNr].data = KD_NULL;
}
...
xmlRegStrEqualWildcard(const xmlChar *expStr, const xmlChar *valStr) {
    ...
    if (expStr == KD_NULL) return(0);
    if (valStr == KD_NULL) return(0);
    do {
	/*
...
xmlRegCompactPushString(xmlRegExecCtxtPtr exec,
    ...
    if ((comp == KD_NULL) || (comp->compact == KD_NULL) || (comp->stringMap == KD_NULL))
	return(-1);
	...
	if ((target > 0) && (target <= comp->nbstates)) {
		...
		exec->index = target;		
		if ((exec->callback != KD_NULL) && (comp->transdata != KD_NULL)) {
		    exec->callback((xmlRegExecCtxt*)exec->data, value,
 ...
 * @exec: a regexp execution context or KD_NULL to indicate the end
 * @value: a string token input
 * @data: data associated to the token to reuse in callbacks
...
xmlRegExecPushStringInternal(xmlRegExecCtxtPtr exec, const xmlChar *value,
    ...
    if (exec == KD_NULL)
	return(-1);
    if (exec->comp == KD_NULL)
	return(-1);
    if (exec->status != 0)
	return(exec->status);

    if (exec->comp->compact != KD_NULL)
     ...
     */
    if ((value != KD_NULL) && (exec->inputStackNr > 0)) {
	 ...
	 */
	if ((value == KD_NULL) && (exec->counts == KD_NULL))
	    goto rollback;
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		if ((value == KD_NULL) && (final)) {
			...
			if ((count < counter->max) && 
		            (t->atom != KD_NULL) &&
			    (xmlStrEqual(value, (const xmlChar*) t->atom->valuep))) {
			    ret = 0;
			    break;
			}
		...
		kdLogMessagef("epsilon transition left at runtime\n");
		exec->status = -2;
		break;
	    } else if (value != KD_NULL) {
		ret = xmlRegStrEqualWildcard( (const xmlChar*)atom->valuep, value);
		if (atom->neg) {
		    ret = !ret;
		    if (!compound)
		        ret = 0;
		...
		if ((ret == 1) && (atom->min > 0) && (atom->max > 0)) {
			...
			ret = xmlStrEqual(value, (const xmlChar*) atom->valuep);
			exec->transcount++;
		    } while (ret == 1);
		    if (exec->transcount < atom->min)
			ret = 0;
	    ...
	    if (ret == 1) {
		if ((exec->callback != KD_NULL) && (atom != KD_NULL) &&
			(data != KD_NULL)) {
		    exec->callback((xmlRegExecCtxt*)exec->data, (const xmlChar*)atom->valuep,
			           atom->data, data);
		}
		if (exec->state->nbTrans > exec->transno + 1) {
		    if (exec->inputStackNr <= 0) {
		...
		printf("entering state %d\n", trans->to);
#endif
                if ((exec->comp->states[trans->to] != KD_NULL) &&
		    ...
		    kdMemcpy (exec->errCounts, exec->counts,
			   exec->comp->nbCounters * sizeof(int));
		}
...
rollback:
	     ...
	     */
	    if ((progress) && (exec->state != KD_NULL) &&
...
xmlRegExecPushString2(xmlRegExecCtxtPtr exec, const xmlChar *value,
    ...
    lenn = kdStrlen ((char *) value2);
    lenp = kdStrlen ((char *) value);
    ...
    kdMemcpy (&str[0], value, lenp);
    str[lenp] = XML_REG_STRING_SEPARATOR;
    kdMemcpy (&str[lenp + 1], value2, lenn);
    str[lenn + lenp + 1] = 0;
...
xmlRegExecGetValues(xmlRegExecCtxtPtr exec, int err,
    ...
    if ((exec == KD_NULL) || (nbval == KD_NULL) || (nbneg == KD_NULL) || 
        (values == KD_NULL) || (*nbval <= 0))
        return(-1);

    maxval = *nbval;
    *nbval = 0;
    *nbneg = 0;
    if ((exec->comp != KD_NULL) && (exec->comp->compact != KD_NULL)) {
        xmlRegexpPtr comp;
	int target, i, state;
	...
	if (err) {
	    if (exec->errState == KD_NULL) return(-1);
	    state = exec->errState;
	} else {
	    if (exec->state == KD_NULL) return(-1);
	    state = exec->state;
	}
	for (transno = 0;
		...
		continue;
	    atom = trans->atom;
	    if ((atom == KD_NULL) || (atom->valuep == KD_NULL))
		continue;
	    if (trans->count == REGEXP_ALL_LAX_COUNTER) {
		...
		xmlRegCounterPtr counter = KD_NULL;
		int count;
		...
		if (exec->comp != KD_NULL)
		    counter = &exec->comp->counters[trans->counter];
		if ((counter == KD_NULL) || (count < counter->max)) {
		    if (atom->neg)
...
xmlRegExecPushChar(xmlRegExecCtxtPtr exec, int UCS) {
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		kdFprintfKHR(stderr, "epsilon transition left at runtime\n");
		exec->status = -2;
		break;
	    } else if (exec->inputString[exec->index] != 0) {
                codepoint = CUR_SCHAR(&(exec->inputString[exec->index]), len);
...
xmlFAParseCharClassEsc(xmlRegParserCtxtPtr ctxt) {
	...
	if (ctxt->atom == KD_NULL) {
	    ...
	    xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
			       XML_REGEXP_ANYCHAR, 0, 0, KD_NULL);
	}
	NEXT;
	return;
    }
	...
	if (ctxt->atom == KD_NULL) {
	    ...
	    xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
			       XML_REGEXP_CHARVAL, cur, cur, KD_NULL);
	}
	NEXT;
...
xmlFAParseCharRange(xmlRegParserCtxtPtr ctxt) {
    ...
    if ((cur != '-') || (NXT(1) == ']')) {
        xmlRegAtomAddRange(ctxt, ctxt->atom, ctxt->neg,
		              XML_REGEXP_CHARVAL, start, end, KD_NULL);
	return;
    }
    NEXT;
    cur = CUR;
...
xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
	 ...
	 */
	xmlFAGenerateEpsilonTransition(ctxt, ctxt->state, KD_NULL);
	start0 = ctxt->state;
	xmlFAGenerateEpsilonTransition(ctxt, ctxt->state, KD_NULL);
	start = ctxt->state;
	oldend = ctxt->end;
...
xmlFAParseBranch(xmlRegParserCtxtPtr ctxt, xmlRegStatePtr to) {
	...
	if (xmlFAGenerateTransitions(ctxt, previous, 
	        (CUR=='|' || CUR==')') ? to : KD_NULL, ctxt->atom) < 0)
	    return(-1);
	previous = ctxt->state;
	ctxt->atom = KD_NULL;
    }
 ...
 */
void
xmlRegexpPrint(KDFile *output, xmlRegexpPtr regexp) {
    int i;

    if (output == KD_NULL)
        return;
    kdFprintfKHR(output, " regexp: ");
    if (regexp == KD_NULL) {
	kdFprintfKHR(output, "KD_NULL\n");
	return;
    }
    kdFprintfKHR(output, "'%s' ", regexp->string);
    kdFprintfKHR(output, "\n");
    kdFprintfKHR(output, "%d atoms:\n", regexp->nbAtoms);
    ...
    kdFprintfKHR(output, "%d states:", regexp->nbStates);
    kdFprintfKHR(output, "\n");
 ...
 *
 * Returns the compiled expression or KD_NULL in case of error
 */
xmlRegexpPtr
...
xmlRegexpExec(xmlRegexpPtr comp, const xmlChar *content) {
    if ((comp == KD_NULL) || (content == KD_NULL))
	return(-1);
...
xmlRegFreeRegexp(xmlRegexpPtr regexp) {
    ...
    if (regexp->string != KD_NULL)
	xmlFree(regexp->string);
    if (regexp->states != KD_NULL) {
	for (i = 0;i < regexp->nbStates;i++)
	    xmlRegFreeState(regexp->states[i]);
	xmlFree(regexp->states);
    }
    if (regexp->atoms != KD_NULL) {
	for (i = 0;i < regexp->nbAtoms;i++)
	    xmlRegFreeAtom(regexp->atoms[i]);
	xmlFree(regexp->atoms);
    }
    if (regexp->counters != KD_NULL)
	xmlFree(regexp->counters);
    if (regexp->compact != KD_NULL)
	xmlFree(regexp->compact);
    if (regexp->transdata != KD_NULL)
	xmlFree(regexp->transdata);
    if (regexp->stringMap != KD_NULL) {
 ...
 *
 * Returns the new object or KD_NULL in case of failure
 */
xmlAutomataPtr
xmlNewAutomata(void) {
    xmlAutomataPtr ctxt;

    ctxt = xmlRegNewParserCtxt(KD_NULL);
    if (ctxt == KD_NULL)
	return(KD_NULL);
 ...
 * @to: the target point of the transition or KD_NULL
 * @token: the input string associated to that transition
 * @data: data passed to the callback function if the transition is activated
 *
 * If @to is KD_NULL, this creates first a new target state in the automata
 * and then adds a transition from the @from state to the target state
 * activated by the value of @token
 *
 * Returns the target state or KD_NULL in case of error
 */
xmlAutomataStatePtr
xmlAutomataNewTransition(xmlAutomataPtr am, xmlAutomataStatePtr from,
    ...
    if ((am == KD_NULL) || (from == KD_NULL) || (token == KD_NULL))
	return(KD_NULL);
...
xmlAutomataNewTransition2(xmlAutomataPtr am, xmlAutomataStatePtr from,
	...
	return(KD_NULL);
    atom->data = data;
    if ((token2 == KD_NULL) || (*token2 == 0)) {
	atom->valuep = xmlStrdup(token);
    } else {
	int lenn, lenp;
	xmlChar *str;

	lenn = kdStrlen ((char *) token2);
	lenp = kdStrlen ((char *) token);
	...
	kdMemcpy (&str[0], token, lenp);
	str[lenp] = '|';
	kdMemcpy (&str[lenp + 1], token2, lenn);
	str[lenn + lenp + 1] = 0;
 ...
 * Note that if @token2 is not KD_NULL, then (X, KD_NULL) won't match to follow
...
xmlAutomataNewNegTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    ...
    kdSnprintfKHR ((char *) err_msg, 199, "not %s", (const char *) atom->valuep);
    err_msg[199] = 0;
 ...
 *
 * Returns the new state or KD_NULL in case of error
 */
xmlAutomataStatePtr
...
xmlAutomataNewCountedTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    ...
    if ((am == KD_NULL) || (from == KD_NULL) || (counter < 0))
	return(KD_NULL);
 ...
 *
 * Returns the compiled regexp or KD_NULL in case of error
 */
xmlRegexpPtr          
xmlAutomataCompile(xmlAutomataPtr am) {
    xmlRegexpPtr ret;

    if ((am == KD_NULL) || (am->error != 0)) return(KD_NULL);
 ...
 *
 * Returns the context or KD_NULL in case of error
 */
xmlExpCtxtPtr
xmlExpNewCtxt(int maxNodes, xmlDictPtr dict) {
    ...
    if (ret == KD_NULL)
        return(KD_NULL);
    kdMemset (ret, 0, sizeof(xmlExpCtxt));
    ret->size = size;
    ret->nbElems = 0;
    ret->maxNodes = maxNodes;
    ret->table = (xmlExpNode**) xmlMalloc(size * sizeof(xmlExpNodePtr));
    if (ret->table == KD_NULL) {
        xmlFree(ret);
	return(KD_NULL);
    }
    kdMemset (ret->table, 0, size * sizeof(xmlExpNodePtr));
    if (dict == KD_NULL) {
...
static xmlExpNode forbiddenExpNode = {
    XML_EXP_FORBID, 0, 0, 0, 0, KD_NULL, KD_NULL, {{ 0, 0}}
};
xmlExpNodePtr forbiddenExp = &forbiddenExpNode;
static xmlExpNode emptyExpNode = {
    XML_EXP_EMPTY, 1, 0, 0, 0, KD_NULL, KD_NULL, {{ 0, 0}}
};
...
xmlExpNewNode(xmlExpCtxtPtr ctxt, xmlExpNodeType type) {
    ...
    if (ret == KD_NULL)
        return(KD_NULL);
    kdMemset (ret, 0, sizeof(xmlExpNode));
    ret->type = type;
    ret->next = KD_NULL;
    ctxt->nb_nodes++;
    ctxt->nb_cons++;
 ...
 *
 * Returns the pointer or KD_NULL in case of error
 */
static xmlExpNodePtr
xmlExpHashGetEntry(xmlExpCtxtPtr ctxt, xmlExpNodeType type,
    ...
    key = kbase % ctxt->size;
    if (ctxt->table[key] != KD_NULL) {
	for (insert = ctxt->table[key]; insert != KD_NULL;
	     insert = insert->next) {
	    if ((insert->key == kbase) &&
	        (insert->type == type)) {
		if (type == XML_EXP_ATOM) {
...
xmlExpFree(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp) {
    if ((exp == KD_NULL) || (exp == forbiddenExp) || (exp == emptyExp))
        return;
    exp->ref--;
    if (exp->ref == 0) {
        unsigned short key;
 ...
 *
 * Returns the node or KD_NULL in case of error
 */
xmlExpNodePtr
xmlExpNewAtom(xmlExpCtxtPtr ctxt, const xmlChar *name, int len) {
    ...
    if (name == KD_NULL)
        return(KD_NULL);
    return(xmlExpHashGetEntry(ctxt, XML_EXP_ATOM, KD_NULL, KD_NULL, name, 0, 0));
}
 ...
 * this is true even in case of failure (unless ctxt == KD_NULL).
 *
 * Returns the node or KD_NULL in case of error
 */
xmlExpNodePtr
xmlExpNewOr(xmlExpCtxtPtr ctxt, xmlExpNodePtr left, xmlExpNodePtr right) {
    if (ctxt == KD_NULL)
        return(KD_NULL);
    if ((left == KD_NULL) || (right == KD_NULL)) {
        xmlExpFree(ctxt, left);
        xmlExpFree(ctxt, right);
        return(KD_NULL);
    }
    return(xmlExpHashGetEntry(ctxt, XML_EXP_OR, left, right, KD_NULL, 0, 0));
}
...
xmlExpNewSeq(xmlExpCtxtPtr ctxt, xmlExpNodePtr left, xmlExpNodePtr right) {
    ...
    return(xmlExpHashGetEntry(ctxt, XML_EXP_SEQ, left, right, KD_NULL, 0, 0));
}
...
xmlExpGetLanguage(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, 
    ...
    if ((ctxt == KD_NULL) || (exp == KD_NULL) || (langList == KD_NULL) || (len <= 0))
        return(-1);
...
xmlExpGetStart(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, 
    ...
    if ((ctxt == KD_NULL) || (exp == KD_NULL) || (tokList == KD_NULL) || (len <= 0))
        return(-1);
...
xmlExpStringDeriveInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, const xmlChar *str)
{
	...
	case XML_EXP_COUNT: {
		...
		min = 0;
	    exp->exp_left->ref++;
	    tmp = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, exp->exp_left, KD_NULL,
				     KD_NULL, min, max);
	    if (ret == emptyExp) {
#ifdef DEBUG_DERIV
 ...
 *
 * Returns the resulting expression or KD_NULL in case of internal error
 */
xmlExpNodePtr
xmlExpStringDerive(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp,
    ...
    if ((exp == KD_NULL) || (ctxt == KD_NULL) || (str == KD_NULL)) {
        return(KD_NULL);
    }
    /*
...
xmlExpDivide(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub,
    ...
    if (mult != KD_NULL) *mult = KD_NULL;
    if (remain != KD_NULL) *remain = KD_NULL;
...
xmlExpExpDeriveInt(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub) {
	...
	if ((ret == KD_NULL) || (ret == forbiddenExp)) {
	    xmlExpFree(ctxt, tmp);
	    return(ret);
	}
	return(xmlExpHashGetEntry(ctxt, XML_EXP_OR, tmp, ret, KD_NULL, 0, 0));
    }
    ...
    switch (exp->type) {
		...
		if ((ret != forbiddenExp) && (ret != KD_NULL)) {
#ifdef DEBUG_DERIV
		...
		if (ret != forbiddenExp) {
		    ...
		    tmp2 = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT,
				      sub->exp_left, KD_NULL, KD_NULL, min, max);
		    if (tmp2 == KD_NULL) {
		        xmlExpFree(ctxt, tmp);
			return(KD_NULL);
		    }
	    ...
	    return(xmlExpHashGetEntry(ctxt, XML_EXP_OR, ret, tmp, KD_NULL, 0, 0));
        case XML_EXP_COUNT: {
	    int min, max;
	...
	if ((tmp == KD_NULL) || (tmp == forbiddenExp)) {
	    xmlExpFree(ctxt, ret);
	    xmlFree((xmlChar **) tab);
	    return(tmp);
	}
	tmp2 = xmlExpStringDeriveInt(ctxt, sub, tab[i]);
	if ((tmp2 == KD_NULL) || (tmp2 == forbiddenExp)) {
	    xmlExpFree(ctxt, tmp);
	    xmlExpFree(ctxt, ret);
	    xmlFree((xmlChar **) tab);
	    return(tmp);
	...
	if ((tmp3 == KD_NULL) || (tmp3 == forbiddenExp)) {
	    xmlExpFree(ctxt, ret);
	    xmlFree((xmlChar **) tab);
	    return(tmp3);
	}
	...
	if (ret == KD_NULL)
	    ...
	    ret = xmlExpHashGetEntry(ctxt, XML_EXP_OR, ret, tmp3, KD_NULL, 0, 0);
	    if (ret == KD_NULL) {
		xmlFree((xmlChar **) tab);
	        return(KD_NULL);
	    }
 ...
 * infinite languages.
 *
 * Returns the resulting expression or KD_NULL in case of internal error, the
 *         result must be freed
 */
xmlExpNodePtr
xmlExpExpDerive(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub) {
    if ((exp == KD_NULL) || (ctxt == KD_NULL) || (sub == KD_NULL))
        return(KD_NULL);
...
xmlExpSubsume(xmlExpCtxtPtr ctxt, xmlExpNodePtr exp, xmlExpNodePtr sub) {
    ...
    if (tmp == emptyExp)
	return(1);
    if ((tmp != KD_NULL) && (IS_NILLABLE(tmp))) {
        xmlExpFree(ctxt, tmp);
        return(1);
    }
    xmlExpFree(ctxt, tmp);
...
xmlExpParseOr(xmlExpCtxtPtr ctxt) {
    ...
    if (val == KD_NULL)
        return(KD_NULL);
    ret = xmlExpHashGetEntry(ctxt, XML_EXP_ATOM, KD_NULL, KD_NULL, val, 0, 0);
    if (ret == KD_NULL)
        return(KD_NULL);
    SKIP_BLANKS
parse_quantifier:
	...
	if (CUR != '}') {
        ...
        NEXT
	ret = xmlExpHashGetEntry(ctxt, XML_EXP_COUNT, ret, KD_NULL, KD_NULL,
	                         min, max);
	SKIP_BLANKS
    } else if (CUR == '?') {
        NEXT
...
xmlExpParseSeq(xmlExpCtxtPtr ctxt) {
	...
	ret = xmlExpHashGetEntry(ctxt, XML_EXP_OR, ret, right, KD_NULL, 0, 0);
	if (ret == KD_NULL)
	    return(KD_NULL);
    }
    return(ret);
...
xmlExpParseExpr(xmlExpCtxtPtr ctxt) {
	...
	ret = xmlExpHashGetEntry(ctxt, XML_EXP_SEQ, ret, right, KD_NULL, 0, 0);
	if (ret == KD_NULL)
	    return(KD_NULL);
    }
    return(ret);
 ...
 *
 * Returns a new expression or KD_NULL in case of failure
 */
xmlExpNodePtr
...
xmlExpDumpInt(xmlBufferPtr buf, xmlExpNodePtr expr, int glob) {
    ...
    switch (expr->type) {
		...
		rep[1] = 0;
	        ...
	        kdSnprintfKHR (rep, 39, "{%d}", expr->exp_min);
	    } else if (expr->exp_max < 0) {
	        kdSnprintfKHR (rep, 39, "{%d,inf}", expr->exp_min);
	    } else {
	        kdSnprintfKHR (rep, 39, "{%d,%d}", expr->exp_min, expr->exp_max);
	    }
	    rep[39] = 0;
