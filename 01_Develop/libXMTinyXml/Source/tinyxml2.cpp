...
char* StrPair::ParseText( char* p, const char* endTag, int strFlags )
{
    ...
    char  endChar = *endTag;
    KDsize length = kdStrlen( endTag );

    // Inner loop of text parsing.
    while ( *p ) {
        if ( *p == endChar && kdStrncmp( p, endTag, length ) == 0 ) {
                    ...
                    *q++ = LF;
                    ...
                    if ( *(p+1) == '#' ) {
                        ...
                        for(; i<NUM_ENTITIES; ++i ) {
                            if (    kdStrncmp( p+1, entities[i].pattern, entities[i].length ) == 0
...
XMLError XMLDocument::LoadFile( KDFile* fp )
{
    Clear();

    kdFseek( fp, 0, KD_SEEK_END );
    KDsize size = kdFtell( fp );
    ...
    KDsize read = kdFread( _charBuffer, 1, size, fp );
    if ( read != size ) {
...
XMLError XMLDocument::SaveFile( KDFile* fp, bool compact )
{
...
XMLError XMLDocument::Parse( const char* p, KDsize len )
{
	const char* start = p;
    Clear();
...
{
    XMLPrinter	stdStreamer ( KD_NULL );
    if ( !streamer ) 
	{
...
void XMLDocument::PrintError() const
{
    if ( _errorID ) {
        ...
        kdPrintf( "XMLDocument error id=%d str1=%s str2=%s\n",
                _errorID, buf1, buf2 );
    }
}


XMLPrinter::XMLPrinter( KDFile* file, bool compact ) :
...
#else
*/
        int len = kdVsnprintf( 0, 0, format, va );
        // Close out and re-start the va-args
        va_end( va );
        va_start( va, format );
        char* p = _buffer.PushArr( len ) - 1;
        kdVsnprintf( p, len+1, format, va );
//#endif
    }
    va_end( va );
}
