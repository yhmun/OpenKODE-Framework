...
{
    ASSContext *ass = (ASSContext *)s->priv_data;
...
{
    return (int)(get_pts(*a) - get_pts(*b));
}

static int read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    unsigned int i;
	int len, header_remaining;
    ASSContext *ass = (ASSContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    AVStream *st;
    int allocated[2]={0};
    ...
    while(!url_feof(pb)){
        ...
        len = ff_get_line(pb, (char *)line, sizeof(line));
        ...
        p = (uint8_t *)av_fast_realloc(*(dst[i]), (unsigned int *)&allocated[i], pos[i]+MAX_LINESIZE);
        if(!p)
            goto fail;
        *(dst[i])= p;
    ...
    ass->event= (uint8_t **)av_malloc(ass->event_count * sizeof(*ass->event));
    p= ass->event_buffer;
                      ...
                      int64_t min_ts, int64_t ts, int64_t max_ts, int flags)
{
    ...
    if (flags & AVSEEK_FLAG_BYTE) {
        ...
        ass->event_index = (unsigned int) ts;
    } else {
        unsigned int i;
		int idx = -1;
