...
static const AVOption options[] = {
    { "oggpagesize", "Set preferred Ogg page size.",
      offsetof(OGGContext, pref_size), AV_OPT_TYPE_INT, 0, NULL, 0, MAX_PAGE_SIZE, AV_OPT_FLAG_ENCODING_PARAM},
    { NULL },
};
...
static int ogg_compare_granule(AVFormatContext *s, OGGPage *next, OGGPage *page)
{
    ...
    next_granule = av_rescale_q(ogg_granule_to_timestamp((OGGStreamContext *)st2->priv_data, next->granule),
                                st2->time_base, AV_TIME_BASE_Q);
    cur_granule  = av_rescale_q(ogg_granule_to_timestamp((OGGStreamContext *)st->priv_data, page->granule),
...
{
    OGGContext *ogg = (OGGContext *)s->priv_data;
    OGGPageList **p = &ogg->page_list;
    OGGPageList *l = (OGGPageList *)av_mallocz(sizeof(*l));
...
{
    OGGStreamContext *oggstream = (OGGStreamContext *)st->priv_data;
    OGGContext *ogg = (OGGContext *)s->priv_data;
                                  ...
                                  AVDictionary **m)
{
    ...
    oggstream->header[0] = (uint8_t *)av_mallocz(51); // per ogg flac specs
    p = oggstream->header[0];
    if (!p)
        return AVERROR(ENOMEM);
    bytestream_put_byte(&p, 0x7F);
    bytestream_put_buffer(&p, (const uint8_t *)"FLAC", 4);
                                   ...
                                   AVDictionary **m)
{
    ...
    p = (uint8_t *)av_mallocz(SPEEX_HEADER_SIZE);
    if (!p)
        return AVERROR(ENOMEM);
...
static int ogg_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        oggstream = (OGGStreamContext *)av_mallocz(sizeof(*oggstream));
        ...
        if (!(st->codec->flags & CODEC_FLAG_BITEXACT))
            do {
                ...
                for (j = 0; j < i; j++) {
                    OGGStreamContext *sc = (OGGStreamContext *)s->streams[j]->priv_data;
        ...
        if (st->codec->codec_id == CODEC_ID_FLAC) {
            ...
            bytestream_put_buffer(&p, (const uint8_t *)cstr, 6);
    ...
    for (j = 0; j < s->nb_streams; j++) {
        OGGStreamContext *oggstream = (OGGStreamContext *)s->streams[j]->priv_data;
...
static void ogg_write_pages(AVFormatContext *s, int flush)
{
    ...
    for (p = ogg->page_list; p; ) {
        OGGStreamContext *oggstream = (OGGStreamContext *)
...
static int ogg_write_trailer(AVFormatContext *s)
{
    ...
    /* flush current page */
    for (i = 0; i < s->nb_streams; i++)
        ogg_buffer_page(s, (OGGStreamContext *)s->streams[i]->priv_data);
