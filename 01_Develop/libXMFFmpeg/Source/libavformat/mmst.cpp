 ...
 * Copyright (c) 2007 Bj?n Axelsson
...
static void mms_put_utf16(MMSContext *mms, const uint8_t *src)
{
    ...
    len = avio_put_str16le(&bic, (const char *)src);
...
static int send_protocol_select(MMSTContext *mmst)
{
    ...
    mms_put_utf16(mms, (const uint8_t *)data_string);
    return send_command_packet(mmst);
}
...
static int send_media_file_request(MMSTContext *mmst)
{
    ...
    bytestream_put_le32(&mms->write_out_ptr, 0);
    bytestream_put_le32(&mms->write_out_ptr, 0);
    mms_put_utf16(mms, (const uint8_t *)(mmst->path + 1)); // +1 for skip "/"
...
{
    int read_result;
    MMSSCPacketType packet_type= SC_PKT_CANCEL;
    MMSContext *mms = &mmst->mms;
    for(;;) {
        ...
        if(AV_RL32(mms->in_buffer + 4)==0xb00bface) {
            ...
            if(read_result != 4) {
                av_log(NULL, AV_LOG_ERROR,
                       ...
                       read_result < 0 ? strerror(read_result) :
                           "The server closed the connection");
                return (MMSSCPacketType)(read_result < 0 ? read_result : AVERROR(EIO));
            }
            ...
            if (length_remaining < 0
                ...
                av_log(NULL, AV_LOG_ERROR,
                       ...
                       length_remaining, sizeof(mms->in_buffer) - 12);
                return (MMSSCPacketType)AVERROR_INVALIDDATA;
            }
            ...
            if(packet_id_type == mmst->header_packet_id) {
                ...
                if(!mms->header_parsed) {
                    ...
                    if (!p) {
                        ...
                        return (MMSSCPacketType)AVERROR(ENOMEM);
                    }
                    mms->asf_header = (uint8_t *)p;
