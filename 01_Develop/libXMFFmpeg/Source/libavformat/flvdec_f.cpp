    ...
    switch(flv_codecid) {
        ...
        case FLV_CODECID_VP6A  :
            ...
            if(vcodec->extradata_size != 1) {
                ...
                vcodec->extradata = (uint8_t *)av_malloc(1);
            }
...
static int parse_keyframes_index(AVFormatContext *s, AVIOContext *ioc, AVStream *vstream, int64_t max_pos) {
    ...
    while (avio_tell(ioc) < max_pos - 2 && amf_get_string(ioc, str_val, sizeof(str_val)) > 0) {
        ...
        if (!(*current_array = (int64_t *)av_mallocz(sizeof(**current_array) * arraylen))) {
            ret = AVERROR(ENOMEM);
            goto finish;
        }
...
static int amf_parse_object(AVFormatContext *s, AVStream *astream, AVStream *vstream, const char *key, int64_t max_pos, int depth) {
    ...
    amf_type = (AMFDataType)avio_r8(ioc);
                               ...
                               int size)
{
    ...
    flv->new_extradata[stream] = (uint8_t *)av_mallocz(size + FF_INPUT_BUFFER_PADDING_SIZE);
