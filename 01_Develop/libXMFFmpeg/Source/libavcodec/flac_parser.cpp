                               ...
                               uint8_t** wrap_buf, int* allocated_size)
{
    ...
    tmp_buf = (uint8_t *)av_fast_realloc(*wrap_buf, (unsigned int *)allocated_size, len);
...
static int find_headers_search_validate(FLACParseContext *fpc, int offset)
{
    ...
    if (frame_header_is_valid(fpc->avctx, header_buf, &fi)) {
        ...
        *end_handle = (FLACHeaderMarker *)av_mallocz(sizeof(FLACHeaderMarker));
        if (!*end_handle) {
            av_log(fpc->avctx, AV_LOG_ERROR,
        ...
        (*end_handle)->link_penalty = (int *)av_malloc(sizeof(int) *
...
{
    FLACParseContext *fpc = (FLACParseContext *)s->priv_data;
    FLACHeaderMarker *curr;
    int nb_headers;
...
{
    FLACParseContext *fpc = (FLACParseContext *)c->priv_data;
