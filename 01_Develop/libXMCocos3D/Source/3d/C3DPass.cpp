...
C3DPass::C3DPass(const char* id, C3DTechnique* technique, C3DEffect* effect) :
    _id(id ? id : ""), _technique(technique), _effect(effect), _vaBinding(NULL), _nMaxDirLight(0), _nMaxPointLight(0), _nMaxSpotLight(0), _nMaxShadowMap(0)
{
    assert(technique);
...
void C3DPass::setupParametersFromEffect()
{
    ...
    for ( KDuint i = 0; i < _effect->getUniformCount(); i++)
    {
...
bool C3DPass::load(C3DElementNode* node)
{
	...
	std::string vshPath = m==NULL ? "" : m;
	assert(vshPath.c_str());

	m = node->getElement("fragmentShader");
    std::string fshPath = m==NULL ? "" : m;	
	...
	std::string uniqueId = C3DEffectManager::generateID(vshPath,fshPath,defines,elementNode);
    
	C3DEffect* effect = static_cast<C3DEffect*>(C3DEffectManager::getInstance()->getResource(uniqueId));
	if(effect == NULL)
	{
		effect = static_cast<C3DEffect*>(C3DEffectManager::getInstance()->createResource(uniqueId,elementNode));			
	}		
	 ...
	 if (_defines.c_str())
    {
        ...
        if (ptr)
        {
           this->_nMaxDirLight = kdAtoi(ptr + 11);
        }
        ptr = strstr(_defines.c_str(), "MAXPOINTLIGHT");
        if (ptr)
        {
            this->_nMaxPointLight = kdAtoi(ptr + 13);
        }
        ptr = strstr(_defines.c_str(), "MAXSPOTLIGHT");
        if (ptr)
        {
            this->_nMaxSpotLight = kdAtoi(ptr + 12);
        }
		ptr = strstr(_defines.c_str(), "MAXANIMLIGHT");
		if (ptr)
		{
			this->_nMaxAnimLight = kdAtoi(ptr + 12);
		}
