...
#include "XMFFmpeg/libavutil/lzo.h" // for av_memcpy_backptr
...
{
    DfaContext *s = (DfaContext *)avctx->priv_data;

    avctx->pix_fmt = PIX_FMT_PAL8;

    s->frame_buf = (uint8_t *)av_mallocz(avctx->width * avctx->height + AV_LZO_OUTPUT_PADDING);
    if (!s->frame_buf)
