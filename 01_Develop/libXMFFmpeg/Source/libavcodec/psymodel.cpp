...
extern FFPsyModel ff_aac_psy_model;
                        ...
                        int num_groups, const uint8_t *group_map)
{
    ...
    ctx->avctx = avctx;
    ctx->ch        = (FFPsyChannel *)av_mallocz(sizeof(ctx->ch[0]) * avctx->channels * 2);
    ctx->group     = (FFPsyChannelGroup *)av_mallocz(sizeof(ctx->group[0]) * num_groups);
    ctx->bands     = (uint8_t **)av_malloc (sizeof(ctx->bands[0])     * num_lens);
    ctx->num_bands = (int *)av_malloc (sizeof(ctx->num_bands[0]) * num_lens);
...
av_cold struct FFPsyPreprocessContext* ff_psy_preprocess_init(AVCodecContext *avctx)
{
    ...
    ctx        = (FFPsyPreprocessContext *)av_mallocz(sizeof(FFPsyPreprocessContext));
    ctx->avctx = avctx;
    ...
    if (ctx->fcoeffs) {
        ctx->fstate = (FFIIRFilterState **)av_mallocz(sizeof(ctx->fstate[0]) * avctx->channels);
