...
{
    MMFContext *mmf = (MMFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    int64_t pos;
    int rate;
    ...
    ffio_wfourcc(pb, (const uint8_t *)"MMMD");
    avio_wb32(pb, 0);
    ...
    avio_w8(pb, 0); /* status */
    avio_w8(pb, 0); /* counts */
    avio_write(pb, (const unsigned char *)"VN:libavcodec,", sizeof("VN:libavcodec,") -1); /* metadata ("ST:songtitle,VN:version,...") */
    end_tag_be(pb, pos);

    avio_write(pb, (const unsigned char *)"ATR\x00", 4);
    avio_wb32(pb, 0);
    ...
    ffio_wfourcc(pb, (const uint8_t *)"Atsq");
    avio_wb32(pb, 16);
    mmf->atsqpos = avio_tell(pb);
    /* Will be filled on close */
    avio_write(pb, (const unsigned char *)"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 16);
...
static int mmf_write_trailer(AVFormatContext *s)
{
    ...
    if (s->pb->seekable) {
        ...
        avio_write(pb, (const unsigned char *)"\xff\x00", 2); /* nop */

        /* "end of sequence" */
        avio_write(pb, (const unsigned char *)"\x00\x00\x00\x00", 4);
