                                          ...
                                          AVCodecContext *avctx)
{
    ...
    for (p = 0; p < 3; p++) {
        ...
        ctx->planes[p].buffers[0] = (uint8_t *)av_malloc(!p ? luma_size : chroma_size);
        ctx->planes[p].buffers[1] = (uint8_t *)av_malloc(!p ? luma_size : chroma_size);
                        ...
                        int32_t strip_width)
{
    ...
    ctx->mc_vectors  = (const int8_t *)(num_vectors ? data : 0);
...
{
    Indeo3DecodeContext *ctx = (Indeo3DecodeContext *)avctx->priv_data;
