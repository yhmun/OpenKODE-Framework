...
//    int len;
	...
	kdPrintf("%s", prompt);
	/*
    if (!kdFgets(line_read, 500, stdin))
        return(NULL);
    line_read[500] = 0;
    len = kdStrlen(line_read);
    ret = (char *) kdMalloc(len + 1);
    if (ret != NULL) {
	kdMemcpy (ret, line_read, len + 1);
	*/
//    }
    return(ret);
#endif
...
static void usershell(void) {
	...
	if (!strcmp(command, "public")) {
	    if (nbargs != 1) {
		kdPrintf("public requires 1 arguments\n");
	    } else {
		ans = xmlCatalogResolvePublic((const xmlChar *) argv[0]);
		if (ans == NULL) {
		    kdPrintf("No entry for PUBLIC %s\n", argv[0]);
		} else {
		    kdPrintf("%s\n", (char *) ans);
		    xmlFree(ans);
		}
	    ...
	    if (nbargs != 1) {
		kdPrintf("system requires 1 arguments\n");
	    } else {
		ans = xmlCatalogResolveSystem((const xmlChar *) argv[0]);
		if (ans == NULL) {
		    kdPrintf("No entry for SYSTEM %s\n", argv[0]);
		} else {
		    kdPrintf("%s\n", (char *) ans);
		    xmlFree(ans);
		}
		...
		if ((nbargs != 3) && (nbargs != 2)) {
		    kdPrintf("add requires 2 or 3 arguments\n");
		} else {
		    if (argv[2] == NULL)
	    ...
	    if (nbargs != 1) {
		kdPrintf("del requires 1\n");
	    } else {
		ret = xmlCatalogRemove(BAD_CAST argv[0]);
		if (ret <= 0)
		    kdPrintf("del command failed\n");

	    }
	} else if (!strcmp(command, "resolve")) {
	    if (nbargs != 2) {
		kdPrintf("resolve requires 2 arguments\n");
	    } else {
		ans = xmlCatalogResolve(BAD_CAST argv[0],
			                BAD_CAST argv[1]);
		if (ans == NULL) {
		    kdPrintf("Resolver failed to find an answer\n");
		} else {
		    kdPrintf("%s\n", (char *) ans);
		    xmlFree(ans);
		}
	    ...
	    if (nbargs != 0) {
		kdPrintf("dump has no arguments\n");
	    } else {
		xmlCatalogDump(stdout);
	    }
	} else if (!strcmp(command, "debug")) {
	    if (nbargs != 0) {
		kdPrintf("debug has no arguments\n");
	    } else {
		verbose++;
	    ...
	    if (nbargs != 0) {
		kdPrintf("quiet has no arguments\n");
	    } else {
		if (verbose > 0)
		    verbose--;
		xmlCatalogSetDebug(verbose);
		...
		kdPrintf("Unrecognized command %s\n", command);
	    }
	    kdPrintf("Commands available:\n");
	    kdPrintf("\tpublic PublicID: make a PUBLIC identifier lookup\n");
	    kdPrintf("\tsystem SystemID: make a SYSTEM identifier lookup\n");
	    kdPrintf("\tresolve PublicID SystemID: do a full resolver lookup\n");
	    kdPrintf("\tadd 'type' 'orig' 'replace' : add an entry\n");
	    kdPrintf("\tdel 'values' : remove values\n");
	    kdPrintf("\tdump: print the current catalog state\n");
	    kdPrintf("\tdebug: increase the verbosity level\n");
	    kdPrintf("\tquiet: decrease the verbosity level\n");
	    kdPrintf("\texit:  quit the shell\n");
	} 
	kdFree(cmdline); /* not xmlFree here ! */
    }
}
...
int main(int argc, char **argv) {
	...
	if ((!strcmp(argv[i], "-verbose")) ||
	    ...
	    kdFprintf(stderr, "Unknown option %s\n", argv[i]);
	    usage(argv[0]);
	    return(1);
	}
    }
	...
	if ((!strcmp(argv[i], "-add")) ||
		...
		kdFprintf(stderr, "No catalog entry specified to remove from\n");
		usage (argv[0]);
		return(1);
	    }
	...
	for (i = 1; i < argc ; i++) {
		...
		if (noout) {
			...
			out = kdFopen(argv[i + 1], "w");
			if (out == NULL) {
			    kdFprintf(stderr, "could not open %s for saving\n",
				    argv[i + 1]);
			    exit_value = 2;
			    noout = 0;
			} else {
			...
			if (xmlCatalogIsEmpty(super)) {
			    kdRemove(XML_SGML_DEFAULT_CATALOG);
			} else {
			    out = kdFopen(XML_SGML_DEFAULT_CATALOG, "w");
			    if (out == NULL) {
				fprintf(stderr,
	...
	if (noout && filename && *filename) {
	    ...
	    out = kdFopen(filename, "w");
	    if (out == NULL) {
		kdPrintf ( "could not open %s for saving\n", filename );
		exit_value = 2;
		noout = 0;
	    } else {
		xmlCatalogDump(out);
