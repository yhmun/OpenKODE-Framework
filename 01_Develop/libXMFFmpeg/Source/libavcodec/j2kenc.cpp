...
static int init_tiles(J2kEncoderContext *s)
{
    ...
    s->tile = (J2kTile *)av_malloc(s->numXtiles * s->numYtiles * sizeof(J2kTile));
    if (!s->tile)
    ...
    for (tileno = 0, tiley = 0; tiley < s->numYtiles; tiley++)
        for (tilex = 0; tilex < s->numXtiles; tilex++, tileno++){
            ...
            tile->comp = (J2kComponent *)av_malloc(s->ncomponents * sizeof(J2kComponent));
            if (!tile->comp)
                return AVERROR(ENOMEM);
            for (compno = 0; compno < s->ncomponents; compno++){
                J2kComponent *comp = tile->comp + compno;
...
{
    int tileno, ret;
    J2kEncoderContext *s = (J2kEncoderContext *)avctx->priv_data;
...
static const enum PixelFormat _ff_jpeg2000_encoder[] = {PIX_FMT_RGB24, PIX_FMT_YUV444P, PIX_FMT_GRAY8,
/*                              PIX_FMT_YUV420P,
