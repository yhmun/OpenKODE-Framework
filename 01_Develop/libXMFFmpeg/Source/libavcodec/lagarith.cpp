...
static int lag_read_prob_header(lag_rac *rac, GetBitContext *gb)
{
    ...
    uint32_t prob, cumulative_target;
    uint32_t cumul_prob = 0;
    uint32_t scaled_cumul_prob = 0;
                            ...
                            void *data, int *data_size, AVPacket *avpkt)
{
    ...
    LagarithContext *l = (LagarithContext *)avctx->priv_data;
    ...
    case FRAME_ARITH_RGBA:
        ...
        if (!l->rgb_planes) {
            ...
            l->rgb_planes = (uint8_t *)av_malloc(l->rgb_stride * avctx->height * 4);
            if (!l->rgb_planes) {
    ...
    case FRAME_ARITH_RGB24:
        ...
        if (!l->rgb_planes) {
            l->rgb_stride = FFALIGN(avctx->width, 16);
            l->rgb_planes = (uint8_t *)av_malloc(l->rgb_stride * avctx->height * 3);
            if (!l->rgb_planes) {
                av_log(avctx, AV_LOG_ERROR, "cannot allocate temporary buffer\n");
                return AVERROR(ENOMEM);
            }
