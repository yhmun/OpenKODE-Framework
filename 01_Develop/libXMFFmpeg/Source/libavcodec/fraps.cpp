...
{
    FrapsContext * const s = (FrapsContext *const)avctx->priv_data;
...
static int huff_cmp(const void *va, const void *vb){
    const Node *a = (const Node *)va, *b = (const Node *)vb;
                        ...
                        AVPacket *avpkt)
{
    ...
    case 4:
        ...
        for(i = 0; i < planes; i++){
            is_chroma = !!i;
            av_fast_padded_malloc(&s->tmpbuf, (unsigned int *)&s->tmpbuf_size, offs[i + 1] - offs[i] - 1024);
            if (!s->tmpbuf)
