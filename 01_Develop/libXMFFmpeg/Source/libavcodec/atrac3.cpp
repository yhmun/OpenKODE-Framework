...
static av_cold int atrac3_decode_init(AVCodecContext *avctx)
{
    ...
    if ((q->decoded_bytes_buffer = (uint8_t *)av_mallocz((avctx->block_align+(4-avctx->block_align%4) + FF_INPUT_BUFFER_PADDING_SIZE)))  == NULL)
    ...
    q->pUnits = (channel_unit *)av_mallocz(sizeof(channel_unit)*q->channels);
    if (!q->pUnits) {
    ...
    if (avctx->channels > 1 || avctx->sample_fmt == AV_SAMPLE_FMT_S16) {
        q->outSamples[0] = (float *)av_mallocz(SAMPLES_PER_FRAME * avctx->channels * sizeof(*q->outSamples[0]));
