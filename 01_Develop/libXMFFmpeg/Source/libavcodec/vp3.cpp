...
{
    Vp3DecodeContext *s = (Vp3DecodeContext *)avctx->priv_data;
...
static av_cold int allocate_tables(AVCodecContext *avctx)
{
    ...
    s->superblock_coding = (unsigned char *)av_malloc(s->superblock_count);
    s->all_fragments = (Vp3Fragment *)av_malloc(s->fragment_count * sizeof(Vp3Fragment));
    s->coded_fragment_list[0] = (int *)av_malloc(s->fragment_count * sizeof(int));
    s->dct_tokens_base = (int16_t *)av_malloc(64*s->fragment_count * sizeof(*s->dct_tokens_base));
    s->motion_val[0] = (int8_t (*)[2])av_malloc(y_fragment_count * sizeof(*s->motion_val[0]));
    s->motion_val[1] = (int8_t (*)[2])av_malloc(c_fragment_count * sizeof(*s->motion_val[1]));

    /* work out the block mapping tables */
    s->superblock_fragments = (int *)av_malloc(s->superblock_count * 16 * sizeof(int));
    s->macroblock_coding = (unsigned char *)av_malloc(s->macroblock_count + 1);
...
{
    Vp3DecodeContext *s = (Vp3DecodeContext *)dst->priv_data, *s1 = (Vp3DecodeContext *)src->priv_data;
                            ...
                            AVPacket *avpkt)
{
    ...
    if (!s->edge_emu_buffer)
        s->edge_emu_buffer = (uint8_t *)av_malloc(9*FFABS(s->current_frame.linesize[0]));
