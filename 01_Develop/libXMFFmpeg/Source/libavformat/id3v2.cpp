...
const char ff_id3v2_4_tags[][5] = {
   "TDEN", "TDOR", "TDRC", "TDRL", "TDTG", "TIPL", "TMCL", "TMOO",
   "TPRO", "TSOA", "TSOP", "TSOT", "TSST",
   { 0 },
};

const char ff_id3v2_3_tags[][5] = {
...
static void read_ttag(AVFormatContext *s, AVIOContext *pb, int taglen, const char *key)
{
    ...
    if (!(strcmp(key, "TCON") && strcmp(key, "TCO"))
        ...
        av_freep(&dst);
        dst = (uint8_t*)ff_id3v1_genre_str[genre];
    ...
    if (dst)
        av_dict_set(&s->metadata, key, (const char *)dst, dict_flags);
}
...
static void read_geobtag(AVFormatContext *s, AVIOContext *pb, int taglen, char *tag, ID3v2ExtraMeta **extra_meta)
{
    ...
    if (taglen) {
        ...
        geob_data->data = (uint8_t *)av_malloc(taglen);
        if (!geob_data->data) {
    ...
    while (len >= taghdrlen) {
        ...
        /* check for text tag or supported special meta tag */
            ...
            if (unsync || tunsync || tcomp) {
                ...
                av_fast_malloc(&buffer, (unsigned int *)&buffer_size, dlen);
                if (!buffer) {
                ...
                if (tcomp) {
                    ...
                    av_fast_malloc(&compressed_buffer, (unsigned int *)&compressed_buffer_size, tlen);
                    ...
                    err = uncompress(buffer, (z_uLongf *)&dlen, compressed_buffer, n);
                    if (err != Z_OK) {
