...
#include "XMBullet/BulletCollision/Gimpact/btGenericPoolAllocator.h"



/// *************** btGenericMemoryPool ******************///////////

KDsize btGenericMemoryPool::allocate_from_free_nodes(KDsize num_elements)
{
	KDsize ptr = BT_UINT_MAX;

	if(m_free_nodes_count == 0) return BT_UINT_MAX;
	// find an avaliable free node with the correct size
	KDsize revindex = m_free_nodes_count;
	...
	KDsize  finalsize = m_allocated_sizes[ptr];
...
KDsize btGenericMemoryPool::allocate_from_pool(KDsize num_elements)
{
	if(m_allocated_count+num_elements>m_max_element_count) return BT_UINT_MAX;

	KDsize ptr = m_allocated_count;
...
void btGenericMemoryPool::init_pool(KDsize element_size, KDsize element_count)
{
	m_allocated_count = 0;
	...
	m_free_nodes = (KDsize *) btAlignedAlloc(sizeof(KDsize)*m_max_element_count,16);
	m_allocated_sizes = (KDsize *) btAlignedAlloc(sizeof(KDsize)*m_max_element_count,16);

	for (KDsize i = 0;i< m_max_element_count;i++ )
	{
...
*/
void * btGenericMemoryPool::allocate(KDsize size_bytes)
{

	KDsize module = size_bytes%m_element_size;
	KDsize element_count = size_bytes/m_element_size;
	if(module>0) element_count++;

	KDsize alloc_pos = allocate_from_free_nodes(element_count);
...
bool btGenericMemoryPool::freeMemory(void * pointer)
{
	...
	KDsize offset = KDsize(pointer_pos - pool_pos);
...
void * btGenericPoolAllocator::failback_alloc(KDsize size_bytes)
{
...
\param size_bytes size in bytes of the buffer
*/
void * btGenericPoolAllocator::allocate(KDsize size_bytes)
{
	void * ptr = NULL;
