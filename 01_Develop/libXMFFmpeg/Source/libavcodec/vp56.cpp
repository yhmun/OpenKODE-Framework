                                 ...
                                 VP56mb prev_type, int ctx)
{
    ...
    if (vp56_rac_get_prob(c, mb_type_model[0]))
        ...
        return (VP56mb)vp56_rac_get_tree(c, vp56_pmbt_tree, mb_type_model);
}
...
{
    VP56Context *s = (VP56Context *)avctx->priv_data;
    ...
    s->qscale_table = (int8_t *)av_realloc(s->qscale_table, s->mb_width);
    s->above_blocks = (VP56RefDc *)av_realloc(s->above_blocks,
                                 (4*s->mb_width+6) * sizeof(*s->above_blocks));
    s->macroblocks = (VP56Macroblock *)av_realloc(s->macroblocks,
                                s->mb_width*s->mb_height*sizeof(*s->macroblocks));
    av_free(s->edge_emu_buffer_alloc);
    s->edge_emu_buffer_alloc = (uint8_t *)av_malloc(16*stride);
