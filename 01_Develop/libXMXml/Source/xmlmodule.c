...
{
    const char *name = KD_NULL;

    if (module != KD_NULL) {
        name = (const char *) module->name;
    }

    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
 ...
 *
 * Returns a handle for the module or KD_NULL in case of error
 */
xmlModulePtr
xmlModuleOpen(const char *name, int options ATTRIBUTE_UNUSED)
{
    ...
    if (module == KD_NULL) {
        xmlModuleErrMemory(KD_NULL, "creating module");
        return (KD_NULL);
    }

    kdMemset(module, 0, sizeof(xmlModule));
/*
    module->handle = xmlModulePlatformOpen(name);

    if (module->handle == KD_NULL) {
        xmlFree(module);
        __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
                        XML_MODULE_OPEN, XML_ERR_FATAL, KD_NULL, 0, 0,
                        name, KD_NULL, 0, 0, "failed to open %s\n", name);
        return(KD_NULL);
    }
*/
...
xmlModuleSymbol(xmlModulePtr module, const char *name, void **symbol)
{
    ...
    if ((KD_NULL == module) || (symbol == KD_NULL)) {
        __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
                        XML_MODULE_OPEN, XML_ERR_FATAL, KD_NULL, 0, 0,
                        KD_NULL, KD_NULL, 0, 0, "null parameter\n");
        return rc;
    }
/*
...
xmlModuleClose(xmlModulePtr module)
{
    ...
    if (KD_NULL == module) {
        __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
                        XML_MODULE_CLOSE, XML_ERR_FATAL, KD_NULL, 0, 0,
                        KD_NULL, KD_NULL, 0, 0, "null module pointer\n");
        return -1;
    }
/*
    ...
    if (rc != 0) {
        __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
                        XML_MODULE_CLOSE, XML_ERR_FATAL, KD_NULL, 0, 0,
                        (const char *) module->name, KD_NULL, 0, 0,
                        "failed to close: %s\n", module->name);
        return -2;
    }
...
xmlModuleFree(xmlModulePtr module)
{
    if (KD_NULL == module) {
        __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_MODULE,
                        XML_MODULE_CLOSE, XML_ERR_FATAL, KD_NULL, 0, KD_NULL,
                        KD_NULL, KD_NULL, 0, 0, "null module pointer\n");
        return -1;
    }
