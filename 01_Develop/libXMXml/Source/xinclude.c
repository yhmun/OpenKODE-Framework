                     ...
                     const char *extra)
{
    if (ctxt != KD_NULL)
	ctxt->nbErrors++;
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, ctxt, node, XML_FROM_XINCLUDE,
               ...
               const char *msg, const xmlChar *extra)
{
    ...
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, ctxt, node, XML_FROM_XINCLUDE,
		    ...
		    (const char *) extra, KD_NULL, KD_NULL, 0, 0,
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, ctxt, node, XML_FROM_XINCLUDE,
                    error, XML_ERR_WARNING, KD_NULL, 0,
		    (const char *) extra, KD_NULL, KD_NULL, 0, 0,
		    msg, (const char *) extra);
}
#endif
 ...
 *
 * Returns the value (to be freed) or KD_NULL if not found
 */
static xmlChar *
...
xmlXIncludeNewRef(xmlXIncludeCtxtPtr ctxt, const xmlChar *URI,
    ...
    kdMemset (ret, 0, sizeof(xmlXIncludeRef));
    if (URI == KD_NULL)
	ret->URI = KD_NULL;
    else
	ret->URI = xmlStrdup(URI);
    ret->fragment = KD_NULL;
    ret->ref = ref;
    ret->doc = KD_NULL;
    ret->count = 0;
    ret->xml = 0;
...
xmlXIncludeNewContext(xmlDocPtr doc) {
    ...
    if (ret == KD_NULL) {
	xmlXIncludeErrMemory(KD_NULL, (xmlNodePtr) doc,
	                     "creating XInclude context");
	return(KD_NULL);
    }
    kdMemset (ret, 0, sizeof(xmlXIncludeCtxt));
    ret->doc = doc;
    ret->incNr = 0;
    ret->incBase = 0;
    ret->incMax = 0;
	           ...
	           const xmlChar *value)
{
	...
	xmlXIncludeErr(ctxt, KD_NULL, XML_XINCLUDE_RECURSION,
	               "detected a recursion in %s\n", value);
	return(-1);
    }
    if (ctxt->urlTab == KD_NULL) {
	ctxt->urlMax = 4;
	ctxt->urlNr = 0;
	ctxt->urlTab = (xmlChar * *) xmlMalloc(
		        ctxt->urlMax * sizeof(ctxt->urlTab[0]));
        if (ctxt->urlTab == KD_NULL) {
	    xmlXIncludeErrMemory(ctxt, KD_NULL, "adding URL");
            return (-1);
        }
    }
...
xmlXIncludeURLPop(xmlXIncludeCtxtPtr ctxt)
{
    ...
    ctxt->urlTab[ctxt->urlNr] = KD_NULL;
    if (ret != KD_NULL)
	xmlFree(ret);
}
...
xmlXIncludeFreeContext(xmlXIncludeCtxtPtr ctxt) {
	...
	if (ctxt->incTab[i] != KD_NULL)
	    xmlXIncludeFreeRef(ctxt->incTab[i]);
    }
    if (ctxt->txturlTab != KD_NULL) {
	for (i = 0;i < ctxt->txtNr;i++) {
	    if (ctxt->txturlTab[i] != KD_NULL)
		xmlFree(ctxt->txturlTab[i]);
	}
    }
    if (ctxt->incTab != KD_NULL)
	xmlFree(ctxt->incTab);
    if (ctxt->txtTab != KD_NULL)
	xmlFree(ctxt->txtTab);
    if (ctxt->txturlTab != KD_NULL)
	xmlFree(ctxt->txturlTab);
    if (ctxt->base != KD_NULL) {
        xmlFree(ctxt->base);
    }
    xmlFree(ctxt);
}
...
xmlXIncludeParseFile(xmlXIncludeCtxtPtr ctxt, const char *URL) {
    ...
    if (pctxt == KD_NULL) {
	xmlXIncludeErrMemory(ctxt, KD_NULL, "cannot allocate parser context");
	return(KD_NULL);
    }
     ...
     */
    if ((ctxt->doc != KD_NULL) && (ctxt->doc->dict != KD_NULL)) {
       if (pctxt->dict != KD_NULL)
    ...
    inputStream = xmlLoadExternalEntity(URL, KD_NULL, pctxt);
...
xmlXIncludeAddTxt(xmlXIncludeCtxtPtr ctxt, xmlNodePtr txt, const xmlURL url) {
	...
	ctxt->txtMax = 4;
        ...
        if (ctxt->txtTab == KD_NULL) {
	    xmlXIncludeErrMemory(ctxt, KD_NULL, "processing text");
	    return;
	}
...
xmlXIncludeCopyNode(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
	            xmlDocPtr source, xmlNodePtr elem) {
    xmlNodePtr result = KD_NULL;

    if ((ctxt == KD_NULL) || (target == KD_NULL) || (source == KD_NULL) ||
	(elem == KD_NULL))
	return(KD_NULL);
...
xmlXIncludeCopyNodeList(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
	                xmlDocPtr source, xmlNodePtr elem) {
    xmlNodePtr cur, res, result = KD_NULL, last = KD_NULL;
 ...
 * @no:  the child number
 *
 * Returns the @n'th element child of @cur or KD_NULL
 */
static xmlNodePtr
 ...
 *
 * Returns an xmlNodePtr list or KD_NULL.
...
xmlXIncludeCopyRange(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
    ...
    xmlNodePtr list = KD_NULL, last = KD_NULL, listParent = KD_NULL;
    xmlNodePtr tmp, tmp2;
...
xmlXIncludeCopyXPointer(xmlXIncludeCtxtPtr ctxt, xmlDocPtr target,
    ...
    xmlNodePtr list = KD_NULL, last = KD_NULL;
    int i;
...
xmlXIncludeMergeEntity(xmlEntityPtr ent, xmlXIncludeMergeDataPtr data,
	...
	if (prev != KD_NULL) {
	    ...
	    if ((ent->SystemID != KD_NULL) && (prev->SystemID != KD_NULL)) {
		if (!xmlStrEqual(ent->SystemID, prev->SystemID))
		    goto error;
	    } else if ((ent->ExternalID != KD_NULL) &&
		       (prev->ExternalID != KD_NULL)) {
		if (!xmlStrEqual(ent->ExternalID, prev->ExternalID))
		    goto error;
	    } else if ((ent->content != KD_NULL) && (prev->content != KD_NULL)) {
...
xmlXIncludeMergeEntities(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc,
    ...
    if ((from == KD_NULL) || (from->intSubset == KD_NULL))
	return(0);
	...
	if (cur == KD_NULL)
	    return(-1);
        target = xmlCreateIntSubset(doc, cur->name, KD_NULL, KD_NULL);
	if (target == KD_NULL)
	    return(-1);
    }

    source = from->intSubset;
    if ((source != KD_NULL) && (source->entities != KD_NULL)) {
...
xmlXIncludeLoadDoc(xmlXIncludeCtxtPtr ctxt, const xmlChar *url, int nr) {
    ...
    if ((ctxt->incTab != KD_NULL) && (ctxt->incTab[nr] != KD_NULL) &&
        (ctxt->incTab[nr]->fragment != KD_NULL)) {
	if (fragment != KD_NULL) xmlFree(fragment);
     ...
     */
    if ((URL[0] == 0) || (URL[0] == '#') ||
	((ctxt->doc != KD_NULL) && (xmlStrEqual(URL, ctxt->doc->URL)))) {
	doc = KD_NULL;
        goto loaded;
    }
    ...
    if (fragment != KD_NULL) {	/* if this is an XPointer eval */
...
loaded:
	...
	if (doc == KD_NULL) {
	    ...
	    xptrctxt = xmlXPtrNewContext(doc, KD_NULL, KD_NULL);
	}
	if (xptrctxt == KD_NULL) {
	    xmlXIncludeErr(ctxt, ctxt->incTab[nr]->ref, 
     ...
     */
    if ((doc != KD_NULL) && (URL != KD_NULL) && (xmlStrchr(URL, (xmlChar) '/')) &&
	...
	if (base == KD_NULL) {
	    ...
	    if (curBase == KD_NULL) {	/* Error return */
	        xmlXIncludeErr(ctxt, ctxt->incTab[nr]->ref, 
	               XML_XINCLUDE_HREF_URI,
	...
	if (base != KD_NULL) {	/* Adjustment may be needed */
    ...
    if ((nr < ctxt->incNr) && (ctxt->incTab[nr]->doc != KD_NULL) &&
	(ctxt->incTab[nr]->count <= 1)) {
#ifdef DEBUG_XINCLUDE
        printf("freeing %s\n", ctxt->incTab[nr]->doc->URL);
#endif
	xmlFreeDoc(ctxt->incTab[nr]->doc);
	ctxt->incTab[nr]->doc = KD_NULL;
    }
    xmlFree(URL);
    return(0);
}
...
xmlXIncludeLoadTxt(xmlXIncludeCtxtPtr ctxt, const xmlChar *url, int nr) {
     ...
     */
    if ((ctxt->incTab[nr] != KD_NULL) && (ctxt->incTab[nr]->ref != KD_NULL)) {
	...
	for (i = 0;i < len;) {
	    ...
	    cur = xmlStringCurrentChar(KD_NULL, &content[i], &l);
	    if (!IS_CHAR(cur)) {
		xmlXIncludeErr(ctxt, ctxt->incTab[nr]->ref,
...
xmlXIncludeLoadFallback(xmlXIncludeCtxtPtr ctxt, xmlNodePtr fallback, int nr) {
    ...
    if ((fallback == KD_NULL) || (ctxt == KD_NULL))
	return(-1);
    if (fallback->children != KD_NULL) {
	/*
 ...
 *
 * Returns the result list or KD_NULL in case of error
 */
static xmlNodePtr
...
xmlXIncludeIncludeNode(xmlXIncludeCtxtPtr ctxt, int nr) {
     ...
     */
    if ((ctxt->incTab[nr]->inc == KD_NULL) &&
	(ctxt->incTab[nr]->xptr != KD_NULL)) {
	ctxt->incTab[nr]->inc =
	...
	end = xmlNewDocNode(cur->doc, cur->ns, cur->name, KD_NULL);
	if (end == KD_NULL) {
	    xmlXIncludeErr(ctxt, ctxt->incTab[nr]->ref,
...
xmlXIncludeDoProcess(xmlXIncludeCtxtPtr ctxt, xmlDocPtr doc, xmlNodePtr tree) {
    ...
    while ((cur != KD_NULL) && (cur != tree->parent)) {
	...
	if ((cur->children != KD_NULL) &&
	    ...
	    do {
		cur = cur->parent;
		if ((cur == KD_NULL) || (cur == tree->parent))
		    break; /* do */
		if (cur->next != KD_NULL) {
		    cur = cur->next;
		    if (xmlXIncludeTestNode(ctxt, cur))
     ...
     * so the check for inc!=KD_NULL || xptr!=KD_NULL was put in.  This may
     * give some other problems in the future, but for now it seems to
     * work ok.
     *
     */
    for (i = ctxt->incBase;i < ctxt->incNr; i++) {
	if ((ctxt->incTab[i]->inc != KD_NULL) ||
		(ctxt->incTab[i]->xptr != KD_NULL) ||
...
xmlXIncludeProcessTreeFlagsData(xmlNodePtr tree, int flags, void *data) {
    ...
    if ((tree == KD_NULL) || (tree->doc == KD_NULL))
        return(-1);
...
xmlXIncludeProcessFlags(xmlDocPtr doc, int flags) {
    return xmlXIncludeProcessFlagsData(doc, flags, KD_NULL);
}
...
xmlXIncludeProcessNode(xmlXIncludeCtxtPtr ctxt, xmlNodePtr node) {
    ...
    if ((node == KD_NULL) || (node->doc == KD_NULL) || (ctxt == KD_NULL))
	return(-1);
