...
int tolua_cocos2d_BezierBy_create(lua_State* tolua_S)
{
    ...
    if (argc == 2)
    {
        ...
        config.tControlPoint1 = arr[0];
        config.tControlPoint2 = arr[1];
        config.tEndPosition = arr[2];
        free(arr);
...
static int tolua_cocos2dx_SpriteBatchNode_getDescendants(lua_State* tolua_S)
{
    ...
    if (argc == 0)
    {
        ...
        for (; iter != ret.end(); ++iter)
        {
            ...
            toluafix_pushusertype_ccobject(tolua_S, (*iter)->m_uID, &((*iter)->m_nLuaID), (void*)(*iter),"Sprite");
            lua_rawset(tolua_S, -3);
            (*iter)->retain();
            ++indexTable;
        }
