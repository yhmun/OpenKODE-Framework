 ...
 * @X:  the error code
 *
 * Macro to raise an XPath error and return KD_NULL.
 */
#define XP_ERRORKD_NULL(X)							\
    { xmlXPathErr(ctxt, X); return(KD_NULL); }
...
xmlXPathErrMemory(xmlXPathContextPtr ctxt, const char *extra)
{
	...
	if (ctxt->error != KD_NULL)
	    ctxt->error(ctxt->userData, &ctxt->lastError);
    } else {
        if (extra)
            __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
                            KD_NULL, KD_NULL, XML_FROM_XPATH,
                            XML_ERR_NO_MEMORY, XML_ERR_FATAL, KD_NULL, 0,
...
{
    if (ctxt == KD_NULL)
	xmlXPathErrMemory(KD_NULL, extra);
    else {
...
xmlXPathErr(xmlXPathParserContextPtr ctxt, int error)
{
    ...
    ctxt->error = error;
    if (ctxt->context == KD_NULL) {
	__xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
			KD_NULL, KD_NULL, XML_FROM_XPATH,
			error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK,
			XML_ERR_ERROR, KD_NULL, 0,
			(const char *) ctxt->base, KD_NULL, KD_NULL,
    ...
    if (ctxt->context->error != KD_NULL) {
	ctxt->context->error(ctxt->context->userData,
	                     &ctxt->context->lastError);
    } else {
	__xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
			KD_NULL, ctxt->context->debugNode, XML_FROM_XPATH,
			error + XML_XPATH_EXPRESSION_OK - XPATH_EXPRESSION_OK,
			XML_ERR_ERROR, KD_NULL, 0,
			(const char *) ctxt->base, KD_NULL, KD_NULL,
			ctxt->cur - ctxt->base, 0,
 ...
 *
 * Returns a xsltPointerList structure or KD_NULL in case of an error.
 */
...
xmlPointerListCreate(int initialSize)
{
    ...
    if (initialSize > 0) {
	xmlPointerListAddSize(ret, KD_NULL, initialSize);
	ret->number = 0;
    }
    return (ret);
}
 ...
 *
 * Returns the newly allocated xmlXPathCompExprPtr or KD_NULL in case of error
 */
...
xmlXPathNewCompExpr(void) {
    ...
    if (cur == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "allocating component\n");
	return(KD_NULL);
    }
    ...
    if (cur->steps == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "allocating steps\n");
	xmlFree(cur);
	return(KD_NULL);
    }
...
xmlXPathFreeCompExpr(xmlXPathCompExprPtr comp)
{
    ...
    if (comp == KD_NULL)
        return;
    if (comp->dict == KD_NULL) {
	for (i = 0; i < comp->nbStep; i++) {
	    op = &comp->steps[i];
	    if (op->value4 != KD_NULL) {
		if (op->op == XPATH_OP_VALUE)
		    xmlXPathFreeObject( (xmlXPathObjectPtr) op->value4);
		else
		    xmlFree(op->value4);
	    }
	    if (op->value5 != KD_NULL)
		xmlFree(op->value5);
	}
    } else {
	for (i = 0; i < comp->nbStep; i++) {
    ...
    if (comp->steps != KD_NULL) {
        xmlFree(comp->steps);
    }
#ifdef DEBUG_EVAL_COUNTS
    if (comp->string != KD_NULL) {
        xmlFree(comp->string);
    }
#endif
#ifdef XPATH_STREAMING
    if (comp->stream != KD_NULL) {
        xmlFreePatternList(comp->stream);
    }
#endif
    if (comp->expr != KD_NULL) {
        xmlFree(comp->expr);
    }
...
xmlXPathCompExprAdd(xmlXPathCompExprPtr comp, int ch1, int ch2,
   ...
   int value2, int value3, void *value4, void *value5) {
	...
	if (real == KD_NULL) {
	    comp->maxStep /= 2;
	    xmlXPathErrMemory(KD_NULL, "adding step\n");
	    return(-1);
	}
	comp->steps = real;
    }
    ...
    if ((comp->dict != KD_NULL) &&
	 ...
	 (op == XPATH_OP_COLLECT))) {
	    ...
	    comp->steps[comp->nbStep].value4 = (xmlChar *)
	        (void *)xmlDictLookup(comp->dict, (const xmlChar*) value4, -1);
	    xmlFree(value4);
	} else
	    comp->steps[comp->nbStep].value4 = KD_NULL;
        if (value5 != KD_NULL) {
	    comp->steps[comp->nbStep].value5 = (xmlChar *)
	        (void *)xmlDictLookup(comp->dict, (const xmlChar*) value5, -1);
	    xmlFree(value5);
	} else
	    comp->steps[comp->nbStep].value5 = KD_NULL;
    } else {
	comp->steps[comp->nbStep].value4 = value4;
	comp->steps[comp->nbStep].value5 = value5;
    }
    comp->steps[comp->nbStep].cache = KD_NULL;
    return(comp->nbStep++);
}
...
xmlXPathCompExprAdd(ctxt->comp, -1, -1, (op), (val), (val2), 0 ,KD_NULL ,KD_NULL)

#define PUSH_UNARY_EXPR(op, ch, val, val2)				\
xmlXPathCompExprAdd(ctxt->comp, (ch), -1, (op), (val), (val2), 0 ,KD_NULL ,KD_NULL)

#define PUSH_BINARY_EXPR(op, ch1, ch2, val, val2)			\
xmlXPathCompExprAdd(ctxt->comp, (ch1), (ch2), (op),			\
			(val), (val2), 0 ,KD_NULL ,KD_NULL)
...
#define XP_HAS_CACHE(c) ((c != KD_NULL) && ((c)->cache != KD_NULL))
...
#ifdef LIBXML_DEBUG_ENABLED
static void
xmlXPathDebugDumpNode(KDFile *output, xmlNodePtr cur, int depth) {
    int i;
    char shift[100];
	...
	fprintf(output, "Node is KD_NULL !\n");
	return;
...
static void
xmlXPathDebugDumpNodeList(KDFile *output, xmlNodePtr cur, int depth) {
    xmlNodePtr tmp;
    int i;
    char shift[100];
...
static void
xmlXPathDebugDumpNodeSet(KDFile *output, xmlNodeSetPtr cur, int depth) {
    int i;
    char shift[100];
    ...
    if (cur == KD_NULL) {
	fprintf(output, "%s", shift);
	fprintf(output, "NodeSet is KD_NULL !\n");
	return;
...
static void
xmlXPathDebugDumpValueTree(KDFile *output, xmlNodeSetPtr cur, int depth) {
    int i;
    char shift[100];
    ...
    if ((cur == KD_NULL) || (cur->nodeNr == 0) || (cur->nodeTab[0] == KD_NULL)) {
	fprintf(output, "%s", shift);
	fprintf(output, "Value Tree is KD_NULL !\n");
	return;
...
static void
xmlXPathDebugDumpLocationSet(KDFile *output, xmlLocationSetPtr cur, int depth) {
    int i;
    char shift[100];
    ...
    if (cur == KD_NULL) {
	fprintf(output, "%s", shift);
	fprintf(output, "LocationSet is KD_NULL !\n");
	return;
 ...
 */
void
xmlXPathDebugDumpObject(KDFile *output, xmlXPathObjectPtr cur, int depth) {
    int i;
    char shift[100];

    if (output == KD_NULL) return;
    ...
    if (cur == KD_NULL) {
        fprintf(output, "Object is empty (KD_NULL)\n");
	return;
    }
    switch(cur->type) {
...
static void
xmlXPathDebugDumpStepOp(KDFile *output, xmlXPathCompExprPtr comp,
    ...
    fprintf(output, "%s", shift);
    if (op == KD_NULL) {
	fprintf(output, "Step is KD_NULL\n");
	return;
    }
    switch (op->op) {
        case XPATH_OP_END:
 ...
 */
void
xmlXPathDebugDumpCompExpr(KDFile *output, xmlXPathCompExprPtr comp,
	                  int depth) {
    int i;
    char shift[100];

    if ((output == KD_NULL) || (comp == KD_NULL)) return;
...
xmlXPathNewCache(void)
{
    ...
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating object cache\n");
	return(KD_NULL);
    }
...
xmlXPathCacheFreeObjectList(xmlPointerListPtr list)
{
	...
	*/
	if (obj->nodesetval != KD_NULL) {
	    if (obj->nodesetval->nodeTab != KD_NULL)
...
{
    if ((ctxt != KD_NULL) && (ctxt->cache != KD_NULL)) {
	xmlXPathContextCachePtr cache =
	    (xmlXPathContextCachePtr) ctxt->cache;

	if ((cache->miscObjs != KD_NULL) &&
...
#endif
	    return(ret);
	} else if ((cache->miscObjs != KD_NULL) &&
	    (cache->miscObjs->number != 0))
	{
	    xmlXPathObjectPtr ret;
	    /*
...
{
    if ((ctxt != KD_NULL) && (ctxt->cache)) {
	xmlXPathContextCachePtr cache = (xmlXPathContextCachePtr) ctxt->cache;

	if ((cache->nodesetObjs != KD_NULL) &&
...
xmlXPathCacheObjectCopy(xmlXPathContextPtr ctxt, xmlXPathObjectPtr val)
{
	...
	switch (val->type) {
		...
		return(xmlXPathCacheWrapNodeSet(ctxt,
		    xmlXPathNodeSetMerge(KD_NULL, val->nodesetval)));
	    case XPATH_STRING:
...
valuePop(xmlXPathParserContextPtr ctxt)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->valueNr <= 0))
        return (KD_NULL);
    ctxt->valueNr--;
    if (ctxt->valueNr > 0)
    ...
    ctxt->valueTab[ctxt->valueNr] = KD_NULL;
    return (ret);
}
/**
 * valuePush:
...
{
    if ((ctxt == KD_NULL) || (value == KD_NULL)) return(-1);
...
xmlXPathPopNodeSet (xmlXPathParserContextPtr ctxt) {
    ...
    if (obj->boolval && obj->user != KD_NULL)
        xmlFreeNodeList((xmlNodePtr) obj->user);
#endif
    obj->nodesetval = KD_NULL;
    xmlXPathReleaseObject(ctxt->context, obj);
    return(ret);
}
...
xmlXPathPopExternal (xmlXPathParserContextPtr ctxt) {
    ...
    if ((ctxt == KD_NULL) || (ctxt->value == KD_NULL)) {
	xmlXPathSetError(ctxt, XPATH_INVALID_OPERAND);
	return(KD_NULL);
    }
...
xmlXPathFormatNumber(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("Infinity"))
	    kdSnprintfKHR (buffer, buffersize, "Infinity");
	break;
    case -1:
	if (buffersize > (int)sizeof("-Infinity"))
	    kdSnprintfKHR (buffer, buffersize, "-Infinity");
	...
	if (xmlXPathIsNaN(number)) {
		...
		kdSnprintfKHR (buffer, buffersize, "NaN");
	} else if (number == 0 && xmlXPathGetSign(number) != 0) {
	    kdSnprintfKHR (buffer, buffersize, "0");
	    ...
	    */
		...
		size = kdSnprintfKHR (work, sizeof(work),"%*.*e",
		...
		size = kdSnprintfKHR (work, sizeof(work), "%0.*f",
	    ...
	    size = kdStrlen(work) + 1;
	    if (size > buffersize) {
		work[buffersize - 1] = 0;
		size = buffersize;
	    }
	    kdMemmove(buffer, work, size);
	}
	break;
    }
}
...
xmlXPathCmpNodes(xmlNodePtr node1, xmlNodePtr node2) {
    int depth1, depth2;
    int attr1 = 0, attr2 = 0;
    xmlNodePtr attrNode1 = KD_NULL, attrNode2 = KD_NULL;
    xmlNodePtr cur, root;

    if ((node1 == KD_NULL) || (node2 == KD_NULL))
	return(-2);
    /*
     ...
     */
    for (depth2 = 0, cur = node2;cur->parent != KD_NULL;cur = cur->parent) {
	if (cur == node1)
	    return(1);
	depth2++;
    }
    root = cur;
    for (depth1 = 0, cur = node1;cur->parent != KD_NULL;cur = cur->parent) {
	if (cur == node2)
	    return(-1);
	depth1++;
    }
    ...
    for (cur = node1->next;cur != KD_NULL;cur = cur->next)
	if (cur == node2)
	    return(1);
...
xmlXPathCmpNodesExt(xmlNodePtr node1, xmlNodePtr node2) {
    ...
    xmlNodePtr miscNode1 = KD_NULL, miscNode2 = KD_NULL;
    xmlNodePtr cur, root;
    long l1, l2;
	...
	case XML_PI_NODE: {
	    ...
	    */
	    ...
	    if ((node1 == KD_NULL) || (node1->type != XML_ELEMENT_NODE) ||
		(0 <= (long) node1->content)) {
		/*
	...
	case XML_PI_NODE: {
	    ...
	    if ((node2 == KD_NULL) || (node2->type != XML_ELEMENT_NODE) ||
		(0 <= (long) node1->content))
	    {
		node2 = miscNode2;
		precedence2 = 0;
...
xmlXPathNodeSetDupNs(xmlNodePtr node, xmlNsPtr ns) {
	...
	return(KD_NULL);
    if ((node == KD_NULL) || (node->type == XML_NAMESPACE_DECL))
    ...
    if (cur == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "duplicating namespace\n");
	return(KD_NULL);
    }
...
xmlXPathNodeSetFreeNs(xmlNsPtr ns) {
    ...
    if ((ns->next != KD_NULL) && (ns->next->type != XML_NAMESPACE_DECL)) {
	if (ns->href != KD_NULL)
 ...
 * @val:  an initial xmlNodePtr, or KD_NULL
 *
...
xmlXPathNodeSetCreate(xmlNodePtr val) {
    ...
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating nodeset\n");
	return(KD_NULL);
    }
...
xmlXPathNodeSetContains (xmlNodeSetPtr cur, xmlNodePtr val) {
    ...
    if ((cur == KD_NULL) || (val == KD_NULL)) return(0);
    if (val->type == XML_NAMESPACE_DECL) {
	for (i = 0; i < cur->nodeNr; i++) {
		...
		if (ns1 == ns2)
		    return(1);
		if ((ns1->next != KD_NULL) && (ns2->next == ns1->next) &&
...
xmlXPathNodeSetAddNs(xmlNodeSetPtr cur, xmlNodePtr node, xmlNsPtr ns) {
    ...
    if ((cur == KD_NULL) || (ns == KD_NULL) || (node == KD_NULL) ||
	...
	if (cur->nodeTab == KD_NULL) {
	    xmlXPathErrMemory(KD_NULL, "growing nodeset\n");
	    return;
	}
...
xmlXPathNodeSetAdd(xmlNodeSetPtr cur, xmlNodePtr val) {
    ...
    if ((cur == KD_NULL) || (val == KD_NULL)) return;
 ...
 * @val1:  the first NodeSet or KD_NULL
 * @val2:  the second NodeSet
 *
 * Merges two nodesets, all nodes from @val2 are added to @val1
 * if @val1 is KD_NULL, a new set is created and copied from @val2
 *
 * Returns @val1 once extended or KD_NULL in case of error.
 */
xmlNodeSetPtr
xmlXPathNodeSetMerge(xmlNodeSetPtr val1, xmlNodeSetPtr val2) {
    ...
    if (val2 == KD_NULL) return(val1);
    if (val1 == KD_NULL) {
	val1 = xmlXPathNodeSetCreate(KD_NULL);
    if (val1 == KD_NULL)
        return (KD_NULL);
#if 0
	/*
	...
	*/
	...
	if (val1->nodeMax == 0) {
	    val1->nodeTab = (xmlNodePtr *) xmlMalloc(XML_NODESET_DEFAULT *
						    sizeof(xmlNodePtr));
	    if (val1->nodeTab == KD_NULL) {
	        xmlXPathErrMemory(KD_NULL, "merging nodeset\n");
		return(KD_NULL);
	    }
 ...
 * @set1:  the first NodeSet or KD_NULL
 * @set2:  the second NodeSet
 * @hasSet2NsNodes: 1 if set2 contains namespaces nodes
 *
 * Merges two nodesets, all nodes from @set2 are added to @set1
 * if @set1 is KD_NULL, a new set is created and copied from @set2.
 * Checks for duplicate nodes. Clears set2.
 *
 * Returns @set1 once extended or KD_NULL in case of error.
 */
static xmlNodeSetPtr
xmlXPathNodeSetMergeAndClear(xmlNodeSetPtr set1, xmlNodeSetPtr set2,
			     int hasNullEntries)
{
    if ((set1 == KD_NULL) && (hasNullEntries == 0)) {
	/*
	...
	*/
	...
	if (set1 == KD_NULL)
            set1 = xmlXPathNodeSetCreate(KD_NULL);
        if (set1 == KD_NULL)
            return (KD_NULL);
...
xmlXPathNodeSetDel(xmlNodeSetPtr cur, xmlNodePtr val) {
    ...
    cur->nodeTab[cur->nodeNr] = KD_NULL;
}
...
xmlXPathFreeNodeSet(xmlNodeSetPtr obj) {
    if (obj == KD_NULL) return;
    if (obj->nodeTab != KD_NULL) {
	int i;

	/* @@ with_ns to check whether namespace nodes should be looked at @@ */
	for (i = 0;i < obj->nodeNr;i++)
	    if ((obj->nodeTab[i] != KD_NULL) &&
...
{
    if ((set == KD_NULL) || (set->nodeNr <= 0) || (pos >= set->nodeNr))
	return;
    else if ((hasNsNodes)) {
	int i;
	xmlNodePtr node;
 ...
 */
void
xmlGenericErrorContextNodeSet(KDFile *output, xmlNodeSetPtr obj) {
    int i;

    if (output == KD_NULL) output = xmlGenericErrorContext;
    if (obj == KD_NULL)  {
        fprintf(output, "NodeSet == KD_NULL !\n");
	return;
    }
    if (obj->nodeNr == 0) {
    ...
    if (obj->nodeTab == KD_NULL) {
	fprintf(output, " nodeTab == KD_NULL !\n");
	return;
    }
    for (i = 0; i < obj->nodeNr; i++) {
        if (obj->nodeTab[i] == KD_NULL) {
	    fprintf(output, " KD_NULL !\n");
	    return;
        }
	if ((obj->nodeTab[i]->type == XML_DOCUMENT_NODE) ||
	    (obj->nodeTab[i]->type == XML_HTML_DOCUMENT_NODE))
	    fprintf(output, " /");
	else if (obj->nodeTab[i]->name == KD_NULL)
...
xmlXPathNewNodeSet(xmlNodePtr val) {
    ...
    /* @@ with_ns to check whether namespace nodes should be looked at @@ */
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_NODESET);
#endif
    return(ret);
}
...
xmlXPathNewValueTree(xmlNodePtr val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating result value tree\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ...
    ret->nodesetval = xmlXPathNodeSetCreate(val);
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_XSLT_TREE);
#endif
    return(ret);
}
...
xmlXPathNewNodeSetList(xmlNodeSetPtr val)
{
    ...
    if (val == KD_NULL)
        ret = KD_NULL;
    else if (val->nodeTab == KD_NULL)
        ret = xmlXPathNewNodeSet(KD_NULL);
    else {
...
xmlXPathWrapNodeSet(xmlNodeSetPtr val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating node set object\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ret->type = XPATH_NODESET;
...
xmlXPathFreeNodeSetList(xmlXPathObjectPtr obj) {
    if (obj == KD_NULL) return;
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageReleased(KD_NULL, obj->type);
#endif
    xmlFree(obj);
}
...
xmlXPathDifference (xmlNodeSetPtr nodes1, xmlNodeSetPtr nodes2) {
    ...
    ret = xmlXPathNodeSetCreate(KD_NULL);
...
xmlXPathIntersection (xmlNodeSetPtr nodes1, xmlNodeSetPtr nodes2) {
    xmlNodeSetPtr ret = xmlXPathNodeSetCreate(KD_NULL);
    int i, l1;
    xmlNodePtr cur;
...
xmlXPathDistinctSorted (xmlNodeSetPtr nodes) {
	...
	if (xmlHashLookup(hash, strval) == KD_NULL) {
 ...
 *         @nodes if @node is KD_NULL or an empty node-set if @nodes
 ...
 *         in document order, @nodes1 if @nodes2 is KD_NULL or empty or
...
xmlXPathLeading (xmlNodeSetPtr nodes1, xmlNodeSetPtr nodes2) {
	...
	return(nodes1);
    if (xmlXPathNodeSetIsEmpty(nodes1))
	return(xmlXPathNodeSetCreate(KD_NULL));
 ...
 * @f:  the function implementation or KD_NULL
 *
 * Register a new function. If @f is KD_NULL it unregisters the function
 *
...
xmlXPathRegisterFunc(xmlXPathContextPtr ctxt, const xmlChar *name,
		     xmlXPathFunction f) {
    return(xmlXPathRegisterFuncNS(ctxt, name, KD_NULL, f));
}
...
xmlXPathRegisterFuncNS(xmlXPathContextPtr ctxt, const xmlChar *name,
    ...
    if (ctxt->funcHash == KD_NULL)
	ctxt->funcHash = xmlHashCreate(0);
    if (ctxt->funcHash == KD_NULL)
	return(-1);
    if (f == KD_NULL)
        return(xmlHashRemoveEntry2(ctxt->funcHash, name, ns_uri, KD_NULL));
 ...
 * function.
 *
 * Returns the xmlXPathFunction or KD_NULL if not found
 */
xmlXPathFunction
xmlXPathFunctionLookup(xmlXPathContextPtr ctxt, const xmlChar *name) {
    ...
    if (ctxt->funcLookupFunc != KD_NULL) {
	xmlXPathFunction ret;
	xmlXPathFuncLookupFunc f;

	f = ctxt->funcLookupFunc;
	ret = f(ctxt->funcLookupData, name, KD_NULL);
	if (ret != KD_NULL)
	    return(ret);
    }
    return(xmlXPathFunctionLookupNS(ctxt, name, KD_NULL));
}
...
xmlXPathRegisteredFuncsCleanup(xmlXPathContextPtr ctxt) {
    ...
    xmlHashFree(ctxt->funcHash, KD_NULL);
    ctxt->funcHash = KD_NULL;
}
 ...
 * @value:  the variable value or KD_NULL
 *
 * Register a new variable value. If @value is KD_NULL it unregisters
 * the variable
 *
 * Returns 0 in case of success, -1 in case of error
 */
...
xmlXPathRegisterVariable(xmlXPathContextPtr ctxt, const xmlChar *name,
    ...
    return(xmlXPathRegisterVariableNS(ctxt, name, KD_NULL, value));
}
 ...
 * variable value.
 *
 * Returns a copy of the value or KD_NULL if not found
 */
xmlXPathObjectPtr
xmlXPathVariableLookup(xmlXPathContextPtr ctxt, const xmlChar *name) {
    ...
    if (ctxt->varLookupFunc != KD_NULL) {
	xmlXPathObjectPtr ret;

	ret = ((xmlXPathVariableLookupFunc)ctxt->varLookupFunc)
	        (ctxt->varLookupData, name, KD_NULL);
	return(ret);
    }
    return(xmlXPathVariableLookupNS(ctxt, name, KD_NULL));
}
 ...
 * variable value.
 *
 * Returns the a copy of the value or KD_NULL if not found
 */
xmlXPathObjectPtr
...
xmlXPathRegisteredVariablesCleanup(xmlXPathContextPtr ctxt) {
    ...
    ctxt->varHash = KD_NULL;
}

/**
 * xmlXPathRegisterNs:
 * @ctxt:  the XPath context
 * @prefix:  the namespace prefix cannot be KD_NULL or empty string
 * @ns_uri:  the namespace name
 *
 * Register a new namespace. If @ns_uri is KD_NULL it unregisters
 * the namespace
 *
 * Returns 0 in case of success, -1 in case of error
 */
 ...
 *
 * Returns the value or KD_NULL if not found
 */
const xmlChar *
xmlXPathNsLookup(xmlXPathContextPtr ctxt, const xmlChar *prefix) {
    ...
    if (ctxt->namespaces != KD_NULL) {
	int i;

	for (i = 0;i < ctxt->nsNr;i++) {
	    if ((ctxt->namespaces[i] != KD_NULL) &&
...
xmlXPathNewFloat(double val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating float object\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ret->type = XPATH_NUMBER;
    ret->floatval = val;
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_NUMBER);
#endif
    return(ret);
}
...
xmlXPathNewBoolean(int val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating boolean object\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ret->type = XPATH_BOOLEAN;
    ret->boolval = (val != 0);
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_BOOLEAN);
#endif
    return(ret);
}
...
xmlXPathNewString(const xmlChar *val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating string object\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ret->type = XPATH_STRING;
	...
	ret->stringval = xmlStrdup((const xmlChar *)"");
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_STRING);
#endif
    return(ret);
}
...
xmlXPathWrapExternal (void *val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating user object\n");
	return(KD_NULL);
    }
    memset(ret, 0 , (size_t) sizeof(xmlXPathObject));
    ...
    ret->user = val;
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, XPATH_USERS);
#endif
    return(ret);
}
...
xmlXPathObjectCopy(xmlXPathObjectPtr val) {
    ...
    ret = (xmlXPathObjectPtr) xmlMalloc(sizeof(xmlXPathObject));
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "copying object\n");
	return(KD_NULL);
    }
    memcpy(ret, val , (size_t) sizeof(xmlXPathObject));
#ifdef XP_DEBUG_OBJ_USAGE
    xmlXPathDebugObjUsageRequested(KD_NULL, val->type);
#endif
    switch (val->type) {
	case XPATH_BOOLEAN:
	case XPATH_NUMBER:
...
*/
	    if ((val->nodesetval != KD_NULL) &&
		(val->nodesetval->nodeTab != KD_NULL)) {
		xmlNodePtr cur, tmp;
		xmlDocPtr top;

		ret->boolval = 1;
		top =  xmlNewDoc(KD_NULL);
		top->name = (char *)
		...
		ret->nodesetval = xmlXPathNodeSetCreate(KD_NULL);
	    /* Deallocate the copied tree value */
	    break;
#endif
	case XPATH_NODESET:
	    ret->nodesetval = xmlXPathNodeSetMerge(KD_NULL, val->nodesetval);
...
#ifdef LIBXML_XPTR_ENABLED
	{
	    ...
	    ret->user = (void *) xmlXPtrLocationSetMerge(KD_NULL, loc);
	    break;
	}
#endif
        case XPATH_USERS:
...
xmlXPathReleaseObject(xmlXPathContextPtr ctxt, xmlXPathObjectPtr obj)
{
#define XP_CACHE_ADD(sl, o) if (sl == KD_NULL) { \
	sl = xmlPointerListCreate(10); if (sl == KD_NULL) goto free_obj; } \
    if (xmlPointerListAddSize(sl, obj, 0) == -1) goto free_obj;

#define XP_CACHE_WANTS(sl, n) ((sl == KD_NULL) || ((sl)->number < n))
...
xmlChar *ret;
    if ((ret = xmlNodeGetContent(node)) == KD_NULL)
...
xmlXPathCastNodeSetToString (xmlNodeSetPtr ns) {
    if ((ns == KD_NULL) || (ns->nodeNr == 0) || (ns->nodeTab == KD_NULL))
 ...
 * Converts an existing object to its string() equivalent
 *
 * Returns the allocated string value of the object, KD_NULL in case of error.
...
xmlXPathCastStringToBoolean (const xmlChar *val) {
    if ((val == KD_NULL) || (xmlStrlen(val) == 0))
	return(0);
    return(1);
}
...
xmlXPathNewContext(xmlDocPtr doc) {
    ...
    if (ret == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "creating context\n");
	return(KD_NULL);
    }
...
#define CHECK_CONTEXT(ctxt)						\
    if ((ctxt == KD_NULL) || (ctxt->doc == KD_NULL) ||			\
        (ctxt->doc->children == KD_NULL)) {				\
...
xmlXPathNewParserContext(const xmlChar *str, xmlXPathContextPtr ctxt) {
    ...
    if ((ctxt != KD_NULL) && (ctxt->dict != KD_NULL)) {
...
xmlXPathFreeParserContext(xmlXPathParserContextPtr ctxt) {
    if (ctxt->valueTab != KD_NULL) {
        xmlFree(ctxt->valueTab);
    }
    if (ctxt->comp != KD_NULL) {
#ifdef XPATH_STREAMING
	if (ctxt->comp->stream != KD_NULL) {
	    xmlFreePatternList(ctxt->comp->stream);
	    ctxt->comp->stream = KD_NULL;
	}
#endif
...
xmlXPathNodeValHash(xmlNodePtr node) {
    int len = 2;
    const xmlChar * string = KD_NULL;
    xmlNodePtr tmp = KD_NULL;
    unsigned int ret = 0;
	 ...
	 * Skip to next node
	 */
	if ((tmp->children != KD_NULL) && (tmp->type != XML_DTD_NODE)) {
...
xmlXPathCompareNodeSets(int inf, int strict,
    ...
    if ((ns1 == KD_NULL) || (ns1->nodeNr <= 0)) {
	xmlXPathFreeObject(arg1);
	xmlXPathFreeObject(arg2);
	return(0);
    }
    if ((ns2 == KD_NULL) || (ns2->nodeNr <= 0)) {
	xmlXPathFreeObject(arg1);
	xmlXPathFreeObject(arg2);
	return(0);
    }
    ...
    if (values2 == KD_NULL) {
        xmlXPathErrMemory(KD_NULL, "comparing nodesets\n");
	xmlXPathFreeObject(arg1);
	xmlXPathFreeObject(arg2);
	return(0);
    }
...
xmlXPathEqualNodeSetString(xmlXPathObjectPtr arg, const xmlChar * str, int neq)
{
    ...
    if ((str == KD_NULL) || (arg == KD_NULL) ||
        ((arg->type != XPATH_NODESET) && (arg->type != XPATH_XSLT_TREE)))
        return (0);
    ns = arg->nodesetval;
    /*
     * A KD_NULL nodeset compared with a string is always false
    ...
    for (i = 0; i < ns->nodeNr; i++) {
        if (xmlXPathNodeValHash(ns->nodeTab[i]) == hash) {
            ...
            if ((str2 != KD_NULL) && (xmlStrEqual(str, str2))) {
                xmlFree(str2);
		if (neq)
		    continue;
                return (1);
	    } else if ((str2 == KD_NULL) && (xmlStrEqual(str, BAD_CAST ""))) {
		if (neq)
		    continue;
                return (1);
            } else if (neq) {
...
xmlXPathEqualNodeSets(xmlXPathObjectPtr arg1, xmlXPathObjectPtr arg2, int neq) {
	...
	for (j = 0;j < ns2->nodeNr;j++) {
		...
		if (values1[i] == KD_NULL)
		    values1[i] = xmlNodeGetContent(ns1->nodeTab[i]);
		if (values2[j] == KD_NULL)
...
xmlXPathEqualValues(xmlXPathParserContextPtr ctxt) {
    ...
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return(0);
    arg2 = valuePop(ctxt);
    arg1 = valuePop(ctxt);
    if ((arg1 == KD_NULL) || (arg2 == KD_NULL)) {
	if (arg1 != KD_NULL)
	    xmlXPathReleaseObject(ctxt->context, arg1);
	else
	    xmlXPathReleaseObject(ctxt->context, arg2);
...
xmlXPathValueFlipSign(xmlXPathParserContextPtr ctxt) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return;
    CAST_TO_NUMBER;
 ...
 * Initially it must be called with KD_NULL, and it indicates
 * termination on the axis by returning KD_NULL.
 */
...
xmlXPathNextSelf(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return(KD_NULL);
    if (cur == KD_NULL)
...
xmlXPathNextChild(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return(KD_NULL);
    if (cur == KD_NULL) {
	if (ctxt->context->node == KD_NULL) return(KD_NULL);
...
next_sibling:
	    if ((cur == KD_NULL) || (cur == contextNode))
		return(KD_NULL);
	    if (cur->next != KD_NULL) {
		cur = cur->next;
	    } else {
...
xmlXPathNextDescendant(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    ...
    if (cur == ctxt->context->node) return(KD_NULL);
...
xmlXPathNextParent(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
	...
	switch (ctxt->context->node->type) {
	    ...
	    case XML_XINCLUDE_END:
	    case XML_ENTITY_DECL:
		if (ctxt->context->node->parent == KD_NULL)
...
xmlXPathNextPrecedingSibling(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    ...
    if ((cur->prev != KD_NULL) && (cur->prev->type == XML_DTD_NODE)) {
	cur = cur->prev;
	if (cur == KD_NULL)
	    return(ctxt->context->node->prev);
    }
...
xmlXPathNextFollowing(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return(KD_NULL);
    if ((cur != KD_NULL) && (cur->type  != XML_ATTRIBUTE_NODE) &&
        (cur->type != XML_NAMESPACE_DECL) && (cur->children != KD_NULL))
        return(cur->children);
    ...
    if (cur == KD_NULL) return(KD_NULL) ; /* ERROR */
    if (cur->next != KD_NULL) return(cur->next) ;
    do {
        cur = cur->parent;
        if (cur == KD_NULL) break;
        if (cur == (xmlNodePtr) ctxt->context->doc) return(KD_NULL);
        if (cur->next != KD_NULL) return(cur->next);
    } while (cur != KD_NULL);
    return(cur);
}
...
xmlXPathIsAncestor(xmlNodePtr ancestor, xmlNodePtr node) {
    if ((ancestor == KD_NULL) || (node == KD_NULL)) return(0);
    /* nodes need to be in the same document */
    if (ancestor->doc != node->doc) return(0);
    /* avoid searching if ancestor or node is the root node */
    if (ancestor == (xmlNodePtr) node->doc) return(1);
    if (node == (xmlNodePtr) ancestor->doc) return(0);
    while (node->parent != KD_NULL) {
...
xmlXPathNextPreceding(xmlXPathParserContextPtr ctxt, xmlNodePtr cur)
{
    ...
    do {
        if (cur->prev != KD_NULL) {
            for (cur = cur->prev; cur->last != KD_NULL; cur = cur->last) ;
            return (cur);
        }
...
xmlXPathNextNamespace(xmlXPathParserContextPtr ctxt, xmlNodePtr cur) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL)) return(KD_NULL);
    if (ctxt->context->node->type != XML_ELEMENT_NODE) return(KD_NULL);
    if (ctxt->context->tmpNsList == KD_NULL && cur != (xmlNodePtr) xmlXPathXMLNamespace) {
        if (ctxt->context->tmpNsList != KD_NULL)
	    xmlFree(ctxt->context->tmpNsList);
	ctxt->context->tmpNsList =
	    xmlGetNsList(ctxt->context->doc, ctxt->context->node);
	ctxt->context->tmpNsNr = 0;
	if (ctxt->context->tmpNsList != KD_NULL) {
	    while (ctxt->context->tmpNsList[ctxt->context->tmpNsNr] != KD_NULL) {
	...
	if (ctxt->context->tmpNsList != KD_NULL)
	    xmlFree(ctxt->context->tmpNsList);
	ctxt->context->tmpNsList = KD_NULL;
	return(KD_NULL);
    }
}
...
xmlXPathRoot(xmlXPathParserContextPtr ctxt) {
    if ((ctxt == KD_NULL) || (ctxt->context == KD_NULL))
	return;
...
xmlXPathCountFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    if ((cur == KD_NULL) || (cur->nodesetval == KD_NULL))
	...
	if ((cur->nodesetval->nodeNr != 1) ||
	    (cur->nodesetval->nodeTab == KD_NULL)) {
	    valuePush(ctxt, xmlXPathCacheNewFloat(ctxt->context, (double) 0));
	} else {
	    xmlNodePtr tmp;
	    int i = 0;
...
xmlXPathGetElementsByIds (xmlDocPtr doc, const xmlChar *ids) {
    ...
    xmlChar *ID;
    xmlAttrPtr attr;
    xmlNodePtr elem = KD_NULL;

    if (ids == KD_NULL) return(KD_NULL);
...
xmlXPathIdFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    CHECK_ARITY(1);
    obj = valuePop(ctxt);
    if (obj == KD_NULL) XP_ERROR(XPATH_INVALID_OPERAND);
    if ((obj->type == XPATH_NODESET) || (obj->type == XPATH_XSLT_TREE)) {
	xmlNodeSetPtr ns;
	int i;
...
xmlXPathLocalNameFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    if ((cur->nodesetval == KD_NULL) || (cur->nodesetval->nodeNr == 0)) {
...
xmlXPathNamespaceURIFunction(xmlXPathParserContextPtr ctxt, int nargs) {
	...
	case XML_ELEMENT_NODE:
	case XML_ATTRIBUTE_NODE:
	    if (cur->nodesetval->nodeTab[i]->ns == KD_NULL)
		valuePush(ctxt, xmlXPathCacheNewCString(ctxt->context, ""));
	    else
		valuePush(ctxt, xmlXPathCacheNewString(ctxt->context,
...
xmlXPathNameFunction(xmlXPathParserContextPtr ctxt, int nargs)
{
    ...
    if ((cur->nodesetval == KD_NULL) || (cur->nodesetval->nodeNr == 0)) {
		...
		else if ((cur->nodesetval->nodeTab[i]->ns == KD_NULL) ||
                         (cur->nodesetval->nodeTab[i]->ns->prefix == KD_NULL)) {
		    valuePush(ctxt,
		        xmlXPathCacheNewString(ctxt->context,
			    cur->nodesetval->nodeTab[i]->name));
		} else {
...
xmlXPathStringFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    CHECK_ARITY(1);
    cur = valuePop(ctxt);
    if (cur == KD_NULL) XP_ERROR(XPATH_INVALID_OPERAND);
...
xmlXPathConcatFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    CAST_TO_STRING;
    cur = valuePop(ctxt);
    if ((cur == KD_NULL) || (cur->type != XPATH_STRING)) {
	xmlXPathReleaseObject(ctxt->context, cur);
	return;
    }
    nargs--;
	...
	if ((newobj == KD_NULL) || (newobj->type != XPATH_STRING)) {
...
xmlXPathContainsFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    if ((hay == KD_NULL) || (hay->type != XPATH_STRING)) {
...
xmlXPathNormalizeFunction(xmlXPathParserContextPtr ctxt, int nargs) {
  xmlXPathObjectPtr obj = KD_NULL;
  xmlChar *source = KD_NULL;
  xmlBufferPtr target;
  xmlChar blank;
...
xmlXPathLangFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    xmlXPathObjectPtr val = KD_NULL;
    const xmlChar *theLang = KD_NULL;
    const xmlChar *lang;
    int ret = 0;
    int i;
    ...
    if ((theLang != KD_NULL) && (lang != KD_NULL)) {
        for (i = 0;lang[i] != 0;i++)
	    if (kdToupper(lang[i]) != kdToupper(theLang[i]))
	        goto not_equal;
 ...
 *
 * Returns the namespace name or KD_NULL
 */
...
xmlXPathParseNCName(xmlXPathParserContextPtr ctxt) {
    ...
    if ((ctxt == KD_NULL) || (ctxt->cur == KD_NULL)) return(KD_NULL);
    /*
...
xmlXPathParseNameComplex(xmlXPathParserContextPtr ctxt, int qualified) {
	...
	if (len >= XML_MAX_NAMELEN) {
	    ...
	    if (buffer == KD_NULL) {
		XP_ERRORKD_NULL(XPATH_MEMORY_ERROR);
	    }
...
//    double mult = 1;
    ...
    PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_NUMBER, 0, 0,
                   xmlXPathCacheNewFloat(ctxt->context, ret), KD_NULL);
}
 ...
 *                    | "'" [^']* "'"
 *
 * Returns the value found or KD_NULL in case of error
 */
static xmlChar *
xmlXPathParseLiteral(xmlXPathParserContextPtr ctxt) {
	...
	while ((IS_CHAR_CH(CUR)) && (CUR != '"'))
	    NEXT;
	if (!IS_CHAR_CH(CUR)) {
	    XP_ERRORKD_NULL(XPATH_UNFINISHED_LITERAL_ERROR);
	} else {
	...
	XP_ERRORKD_NULL(XPATH_START_LITERAL_ERROR);
    }
    return(ret);
}
...
xmlXPathCompLiteral(xmlXPathParserContextPtr ctxt) {
    ...
    if (ret == KD_NULL) return;
    PUSH_LONG_EXPR(XPATH_OP_VALUE, XPATH_STRING, 0, 0,
	           xmlXPathCacheNewString(ctxt->context, ret), KD_NULL);
    xmlFree(ret);
}
...
xmlXPathCompVariableReference(xmlXPathParserContextPtr ctxt) {
    ...
    PUSH_LONG_EXPR(XPATH_OP_VARIABLE, 0, 0, 0,
	           name, prefix);
    SKIP_BLANKS;
    if ((ctxt->context != KD_NULL) && (ctxt->context->flags & XML_XPATH_NOVAR)) {
...
xmlXPathCompPathExpr(xmlXPathParserContextPtr ctxt) {
    ...
    xmlChar *name = KD_NULL; /* we may have to preparse a name to find out */
	...
	if ((name != KD_NULL) && (xmlStrstr(name, (xmlChar *) "::") != KD_NULL)) {
#ifdef DEBUG_STEP
	    xmlGenericError(xmlGenericErrorContext,
		    "PathExpr: Axis\n");
#endif
	...
	if ((CUR == '/') && (NXT(1) == '/')) {
	    ...
	    PUSH_LONG_EXPR(XPATH_OP_COLLECT, AXIS_DESCENDANT_OR_SELF,
		    NODE_TEST_TYPE, NODE_TYPE_NODE, KD_NULL, KD_NULL);
...
xmlXPathCompNodeTest(xmlXPathParserContextPtr ctxt, xmlXPathTestVal *test,
    ...
    if ((test == KD_NULL) || (type == KD_NULL) || (prefix == KD_NULL)) {
	STRANGE;
	return(KD_NULL);
    }
	...
	*test = NODE_TEST_ALL;
    ...
    if (name == KD_NULL) {
	XP_ERRORKD_NULL(XPATH_EXPR_ERROR);
    }
	...
	*test = NODE_TEST_TYPE;
	...
	if (CUR != ')') {
	    if (name != KD_NULL)
		xmlFree(name);
	    XP_ERRORKD_NULL(XPATH_UNCLOSED_ERROR);
	}
	NEXT;
	return(name);
    }
...
xmlXPathCompStep(xmlXPathParserContextPtr ctxt) {
	...
	if (ctxt->xptr) {
	    name = xmlXPathParseNCName(ctxt);
	    if ((name != KD_NULL) && (xmlStrEqual(name, BAD_CAST "range-to"))) {
                op2 = ctxt->comp->last;
		xmlFree(name);
		SKIP_BLANKS;
		if (CUR != '(') {
	...
	if (test == 0)
        ...
        if ((prefix != KD_NULL) && (ctxt->context != KD_NULL) &&
	    (ctxt->context->flags & XML_XPATH_CHECKNS)) {
	    if (xmlXPathNsLookup(ctxt->context, prefix) == KD_NULL) {
	...
	else if (ctxt->value->nodesetval == KD_NULL)
			    ...
			    int hasNsNodes)
{
	...
	*/
	...
	xmlXPathObjectPtr contextObj = KD_NULL, exprRes = KD_NULL;
				      ...
				      int hasNsNodes)
{
	...
	*/
	...
	xmlXPathStepOpPtr exprOp;
	xmlXPathObjectPtr contextObj = KD_NULL, exprRes = KD_NULL;
	xmlNodePtr oldContextNode, contextNode = KD_NULL;
	xmlXPathContextPtr xpctxt = ctxt->context;
...
#define XP_TEST_HIT_NS \
    ...
    */
    ...
    xmlXPathTraversalFunction next = KD_NULL;
    /* compound axis traversal */
    xmlXPathTraversalFunctionExt outerNext = KD_NULL;
    ...
    */
	...
	(obj->nodesetval != KD_NULL) ? obj->nodesetval->nodeNr : 0);
#endif
    ...
    if ((contextSeq == KD_NULL) || (contextSeq->nodeNr <= 0)) {
	xmlXPathReleaseObject(xpctxt, obj);
        valuePush(ctxt, xmlXPathCacheWrapNodeSet(xpctxt, KD_NULL));
        return(0);
    }
    /*
	...
	*/
    ...
    breakOnFirstHit = ((toBool) && (predOp == KD_NULL)) ? 1 : 0;
    /*
    * Axis traversal -----------------------------------------------------
    */
    /*
    ...
    outSeq = KD_NULL;
    seq = KD_NULL;
    outerContextNode = KD_NULL;
    contextNode = KD_NULL;
    contextIdx = 0;


    while ((contextIdx < contextSeq->nodeNr) || (contextNode != KD_NULL)) {
	if (outerNext != KD_NULL) {
	    /*
		...
		*/
		...
		contextNode = outerNext(KD_NULL, outerContextNode);
	    } else
	...
	if (seq == KD_NULL) {
	    seq = xmlXPathNodeSetCreate(KD_NULL);
	    if (seq == KD_NULL) {
		total = 0;
		goto error;
	    }
	...
	*/
	...
	hasNsNodes = 0;
	    ...
	    */
            if ((first != KD_NULL) && (*first != KD_NULL)) {
		if (*first == cur)
		    break;
		if (((total % 256) == 0) &&
#ifdef XP_OPTIMIZED_NON_ELEM_COMPARISON
	    ...
	    switch (test) {
		    ...
		    */
			...
			if ( (xmlElementType) type == XML_NAMESPACE_DECL)
			    XP_TEST_HIT_NS
			else
			    XP_TEST_HIT
		    ...
		    break;
                case NODE_TEST_PI:
                    if ((cur->type == XML_PI_NODE) &&
                        ((name == KD_NULL) || xmlStrEqual(name, cur->name)))
		    {
			XP_TEST_HIT
                    }
                    break;
					      ...
					      attr->ns->href)))
					{
                        ...
                        case XML_NAMESPACE_DECL:
                            if (cur->type == XML_NAMESPACE_DECL) {
                                ...
                                if ((ns->prefix != KD_NULL) && (name != KD_NULL)
...
apply_predicates: /* --------------------------------------------------- */
	...
	* Apply predicates.
	*/
        if ((predOp != KD_NULL) && (seq->nodeNr > 0)) {
	    /*
		...
		*/
		if (outSeq == KD_NULL) {
			...
			* the sequence will contained KD_NULLed entries.
			*/
			outSeq = mergeAndClear(KD_NULL, seq, 1);
		    } else {
			outSeq = seq;
			seq = KD_NULL;
		    }
...
error:
    ...
    */
    if (outSeq == KD_NULL) {
	if ((seq != KD_NULL) && (seq->nodeNr == 0))
	    outSeq = seq;
	else
	    outSeq = xmlXPathNodeSetCreate(KD_NULL);
        /* XXX what if xmlXPathNodeSetCreate returned KD_NULL here? */
    }
    if ((seq != KD_NULL) && (seq != outSeq)) {
    ...
    switch (op->op) {
		...
		*/
		...
		CHECK_ERROR0;
                ...
                total += xmlXPathNodeCollectAndTest(ctxt, op, first, KD_NULL, 0);
                return (total);
            }
        case XPATH_OP_VALUE:
            valuePush(ctxt,
                       ...
                       xmlNodePtr * last)
{
    ...
    switch (op->op) {
		...
		CHECK_ERROR0;
                ...
                total += xmlXPathNodeCollectAndTest(ctxt, op, KD_NULL, last, 0);
                return (total);
            }
        case XPATH_OP_VALUE:
            valuePush(ctxt,
			      ...
			      xmlXPathStepOpPtr op, xmlNodePtr * first)
{
    ...
    */
	...
	if ((f != -1) &&
	    ...
	    (comp->steps[f].value5 == KD_NULL) &&
	    (comp->steps[f].value == 0) &&
	    (comp->steps[f].value4 != KD_NULL) &&
	    (xmlStrEqual
	    ...
	    */
		...
		(ctxt->value->type == XPATH_NODESET) &&
		(ctxt->value->nodesetval != KD_NULL) &&
		(ctxt->value->nodesetval->nodeTab != KD_NULL) &&
		(ctxt->value->nodesetval->nodeNr > 1)) {
    ...
    */
	...
	xmlXPathObjectPtr tmp = KD_NULL;
	xmlLocationSetPtr newlocset = KD_NULL;
	...
	*/
	...
	if ((oldlocset == KD_NULL) || (oldlocset->locNr == 0)) {
	...
	newlocset = xmlXPtrLocationSetCreate(KD_NULL);
    ...
    */
    ...
    if ((oldset == KD_NULL) || (oldset->nodeNr == 0)) {
	ctxt->context->contextSize = 0;
	ctxt->context->proximityPosition = 0;
	...
	* key() evaluation are attempted on the predicate
	*/
	newset = xmlXPathNodeSetCreate(KD_NULL);
        /* XXX what if xmlXPathNodeSetCreate returned KD_NULL? */
...
xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op)
{
    ...
    switch (op->op) {
	    ...
	    CHECK_ERROR0;
            xmlXPathBooleanFunction(ctxt, 1);
            if ((ctxt->value == KD_NULL) || (ctxt->value->boolval == 0))
                return (total);
            arg2 = valuePop(ctxt);
	    ctxt->context->doc = bakd;
	    ctxt->context->node = bak;
	    ...
	    CHECK_ERROR0;
            xmlXPathBooleanFunction(ctxt, 1);
            if ((ctxt->value == KD_NULL) || (ctxt->value->boolval == 1))
                return (total);
            arg2 = valuePop(ctxt);
	    ctxt->context->doc = bakd;
	    ctxt->context->node = bak;
		...
		CHECK_ERROR0;
                ...
                total += xmlXPathNodeCollectAndTest(ctxt, op, KD_NULL, KD_NULL, 0);
                return (total);
            }
        case XPATH_OP_VALUE:
            valuePush(ctxt,
		...
		for (i = 0; i < op->value; i++)
		    if (ctxt->valueTab[(ctxt->valueNr - 1) - i] == KD_NULL) {
			xmlGenericError(xmlGenericErrorContext,
				"xmlXPathCompOpEval: parameter error\n");
			ctxt->error = XPATH_INVALID_OPERAND;
			return (total);
		...
		xmlDocPtr oldDoc;
		    ...
		    */
		    ...
		    (comp->steps[op->ch1].op == XPATH_OP_SORT) &&
                    ...
                    if ((val != KD_NULL) && (val->type == XPATH_NUMBER) &&
                        (val->floatval == 1.0)) {
                        xmlNodePtr first = KD_NULL;
...
xmlXPathCompOpEvalToBoolean(xmlXPathParserContextPtr ctxt,
			    ...
			    int isPredicate)
{
    xmlXPathObjectPtr resObj = KD_NULL;
...
start:
	...
	case XPATH_OP_COLLECT:
		...
		return(-1);
            ...
            xmlXPathNodeCollectAndTest(ctxt, op, KD_NULL, KD_NULL, 1);
	    if (ctxt->error != XPATH_EXPRESSION_OK)
		return(-1);
		      ...
		      xmlXPathObjectPtr *resultSeq, int toBool)
{
    ...
    int ret, depth;
    int eval_all_nodes;
    xmlNodePtr cur = KD_NULL, limit = KD_NULL;
    xmlStreamCtxtPtr patstream = KD_NULL;

    int nb_nodes = 0;

    if ((ctxt == KD_NULL) || (comp == KD_NULL))
        return(-1);
    ...
    if (! toBool) {
	if (resultSeq == KD_NULL)
	    return(-1);
	*resultSeq = xmlXPathCacheNewNodeSet(ctxt, KD_NULL);
	if (*resultSeq == KD_NULL)
	    return(-1);
    }
	...
	*/
    ...
    if (from_root) {
	ret = xmlStreamPush(patstream, KD_NULL, KD_NULL);
	if (ret < 0) {
	} else if (ret == 1) {
	    if (toBool)
		goto return_1;
	...
	switch (cur->type) {
		...
		if (cur->type == XML_ELEMENT_NODE) {
		    ret = xmlStreamPush(patstream, cur->name,
				(cur->ns ? cur->ns->href : KD_NULL));
		} else if (eval_all_nodes)
		    ret = xmlStreamPushNode(patstream, KD_NULL, KD_NULL, cur->type);
		else
		    break;
		...
		if ((cur->children == KD_NULL) || (depth >= max_depth)) {
...
xmlXPathRunEval(xmlXPathParserContextPtr ctxt, int toBool)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->comp == KD_NULL))
	return(-1);
...
xmlXPathEvalPredicate(xmlXPathContextPtr ctxt, xmlXPathObjectPtr res) {
    if ((ctxt == KD_NULL) || (res == KD_NULL)) return(0);
    switch (res->type) {
        case XPATH_BOOLEAN:
	    return(res->boolval);
        case XPATH_NUMBER:
        ...
        case XPATH_NODESET:
        case XPATH_XSLT_TREE:
	    if (res->nodesetval == KD_NULL)
		return(0);
	    return(res->nodesetval->nodeNr != 0);
        case XPATH_STRING:
	    return((res->stringval != KD_NULL) &&
...
xmlXPathEvaluatePredicateResult(xmlXPathParserContextPtr ctxt,
    ...
    switch (res->type) {
	    ...
	    return(res->nodesetval->nodeNr != 0);
        case XPATH_STRING:
	    return((res->stringval != KD_NULL) && (res->stringval[0] != 0));
#ifdef LIBXML_XPTR_ENABLED
	case XPATH_LOCATIONSET:{
 ...
 *
 * Returns the compiled expression or KD_NULL if failed to compile.
 */
static xmlXPathCompExprPtr
xmlXPathTryStreamCompile(xmlXPathContextPtr ctxt, const xmlChar *str) {
    ...
    const xmlChar **namespaces = KD_NULL;
    xmlNsPtr ns;
    int i, j;
	...
	tmp = xmlStrchr(str, ':');
	if ((tmp != KD_NULL) &&
	    ((ctxt == KD_NULL) || (ctxt->nsNr == 0) || (tmp[1] == ':')))
	    return(KD_NULL);
	...
	if ((stream != KD_NULL) && (xmlPatternStreamable(stream) == 1)) {
 ...
 *
 * Returns the xmlXPathCompExprPtr resulting from the compilation or KD_NULL.
...
xmlXPathCompile(const xmlChar *str) {
    return(xmlXPathCtxtCompile(KD_NULL, str));
}
 ...
 * @ctxt:  the XPath context
 * @resObj: the resulting XPath object or KD_NULL
 * @toBool: 1 if only a boolean result is requested
 *
 * Evaluate the Precompiled XPath expression in the given context.
 * The caller has to free @resObj.
 *
 * Returns the xmlXPathObjectPtr resulting from the evaluation or KD_NULL.
 *         the caller has to free the object.
 */
static int
...
#ifdef DEBUG_EVAL_COUNTS
    comp->nb++;
    if ((comp->string != KD_NULL) && (comp->nb > 100)) {
...
xmlXPathCompiledEvalToBoolean(xmlXPathCompExprPtr comp,
			      xmlXPathContextPtr ctxt)
{
    return(xmlXPathCompiledEvalInternal(comp, ctxt, KD_NULL, 1));
}
...
xmlXPathEval(const xmlChar *str, xmlXPathContextPtr ctx) {
    xmlXPathParserContextPtr ctxt;
    xmlXPathObjectPtr res, tmp, init = KD_NULL;
    int stack = 0;
	...
	res = KD_NULL;
    } else if ((*ctxt->cur != 0) && (ctxt->comp != KD_NULL)
#ifdef XPATH_STREAMING
            && (ctxt->comp->stream == KD_NULL)
#endif
	      ) {
