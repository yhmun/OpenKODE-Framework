...
#include "XMFFmpeg/libavutil/eval.h"
...
{
    AVClass *_class = *(AVClass**)obj;
    if (!last && _class->option[0].name) return _class->option;
...
static int read_number(const AVOption *o, void *dst, double *num, int *den, int64_t *intnum)
{
    ...
    case AV_OPT_TYPE_RATIONAL:  *intnum = ((AVRational*)dst)->num;
                                *den    = ((AVRational*)dst)->den;
                                                        return 0;
    case AV_OPT_TYPE_CONST:     *num    = o->dbl; return 0;
    }
...
static int write_number(void *obj, const AVOption *o, void *dst, double num, int den, int64_t intnum)
{
    ...
    case AV_OPT_TYPE_RATIONAL:
        if ((int)num == num) *(AVRational*)dst= av_rational(num*intnum, den);
    ...
    for (;;) {
...

        {
            ...
            if (o_named && o_named->type == AV_OPT_TYPE_CONST)
                d = o_named->dbl;
            else if (!strcmp(buf, "default")) d = o->dbl;
...
int av_opt_set(void *obj, const char *name, const char *val, int search_flags)
{
    ...
    switch (o->type) {
    case AV_OPT_TYPE_STRING:   return set_string(obj, o, val, (uint8_t **)dst);
    case AV_OPT_TYPE_BINARY:   return set_string_binary(obj, o, val, (uint8_t **)dst);
    case AV_OPT_TYPE_FLAGS:
...
const char *av_get_string(void *obj, const char *name, const AVOption **o_out, char *buf, int buf_len)
{
    ...
    case AV_OPT_TYPE_CONST:     snprintf(buf, buf_len, "%f" , o->dbl);break;
    case AV_OPT_TYPE_STRING:    return (const char *)(*(void**)dst);
    case AV_OPT_TYPE_BINARY:
        len = *(int*)(((uint8_t *)dst) + sizeof(uint8_t *));
        if (len >= (buf_len + 1)/2) return NULL;
        bin = *(uint8_t**)dst;
        for (i = 0; i < len; i++) snprintf((char*)(buf + i*2), 3, "%02X", bin[i]);
        break;
    default: return NULL;
    }
    return buf;
...
int av_opt_get(void *obj, const char *name, int search_flags, uint8_t **out_val)
{
    ...
    case AV_OPT_TYPE_CONST:     ret = snprintf(buf, sizeof(buf), "%f" ,     o->dbl);break;
...
AVRational av_get_q(void *obj, const char *name, const AVOption **o_out)
{
    ...
    if (get_number(obj, name, o_out, &num, &den, &intnum, 0) < 0)
        return av_rational(0, 0);
    if (num == 1.0 && (int)intnum == intnum)
        return av_rational(intnum, den);
    else
...
int av_opt_get_q(void *obj, const char *name, int search_flags, AVRational *out_val)
{
    ...
    if (num == 1.0 && (int)intnum == intnum)
        *out_val = av_rational(intnum, den);
    else
...
void av_opt_set_defaults2(void *s, int mask, int flags)
{
    ...
    while ((opt = av_opt_next(s, opt)) != NULL) {
        ...
        switch (opt->type) {
            ...
            break;
            case AV_OPT_TYPE_INT64:
                if ((double)(opt->dbl+0.6) == opt->dbl)
                    av_log(s, AV_LOG_DEBUG, "loss of precision in default of %s\n", opt->name);
                av_opt_set_int(s, opt->name, opt->dbl, 0);
            break;
            case AV_OPT_TYPE_DOUBLE:
            case AV_OPT_TYPE_FLOAT: {
                double val;
            ...
            case AV_OPT_TYPE_RATIONAL: {
                AVRational val;
                val = av_d2q(opt->dbl, INT_MAX);
                av_opt_set_q(s, opt->name, val, 0);
            }
            break;
            case AV_OPT_TYPE_STRING:
                av_opt_set(s, opt->name, opt->str, 0);
                break;
            case AV_OPT_TYPE_BINARY:
...
{
    const AVOption *opt= av_opt_find2((void*)&avclass, name, NULL, 0, AV_OPT_SEARCH_FAKE_OBJ, NULL);
    if(!opt)
        return NULL;
