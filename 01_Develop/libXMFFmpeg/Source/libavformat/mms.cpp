...
int ff_mms_read_header(MMSContext *mms, uint8_t *buf, const int size)
{
    ...
    KDsizeo_copy = FFMIN(size, remaining_size);
    pos = (char *)(mms->asf_header + mms->asf_header_read_size);
    memcpy(buf, pos, KDsizeo_copy);
    if (mms->asf_header_read_size == mms->asf_header_size) {
        av_freep(&mms->asf_header); // which contains asf header
    }
    mms->asf_header_read_size += KDsizeo_copy;
    return KDsizeo_copy;
}
    ...
    while(end - p >= sizeof(ff_asf_guid) + 8) {
        ...
        if (!memcmp(p, ff_asf_file_header, sizeof(ff_asf_guid))) {
            ...
            if (mms->stream_num < MMS_MAX_STREAMS &&
                ...
                mms->streams = (MMSStream *)av_fast_realloc(mms->streams,
