...
namespace cocos3d
{
    ...
    C3DTexture::C3DTexture() : _handle(0), _mipmapped(false), _width(0), _height(0), /*_cached(false), */_texture(NULL)
    {
    }
	...
	C3DTexture* C3DTexture::create(int width, int height, C3DTexture::Format fmt, const void* data, bool generateMipmaps)
	{
		...
		texture2D->initWithData(data, nDataLen, format, width, height, Size(width, height));
    ...
    {
        if (_texture != NULL && !_texture->hasMipmaps() )
        {
