...
xmlSAX2ErrMemory(xmlParserCtxtPtr ctxt, const char *msg) {
    if (ctxt != KD_NULL) {
	if ((ctxt->sax != KD_NULL) && (ctxt->sax->error != KD_NULL))
               ...
               const char *msg, const xmlChar *str1)
{
    ...
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, ctxt, KD_NULL, XML_FROM_PARSER, error,
		    ...
		    (const char *) str1, KD_NULL,
		    KD_NULL, 0, 0, msg, str1);
}
...
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    if ((ctx == KD_NULL) || (ctxt->input == KD_NULL)) return(KD_NULL);
...
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    if ((ctx == KD_NULL) || (ctxt->input == KD_NULL)) return(0);
...
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    if ((ctx == KD_NULL) || (ctxt->myDoc == KD_NULL)) return(0);
...
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    if ((ctxt == KD_NULL) || (ctxt->myDoc == KD_NULL)) return(0);
    return(ctxt->myDoc->intSubset != KD_NULL);
}
...
xmlSAX2HasExternalSubset(void *ctx)
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr) ctx;
    if ((ctxt == KD_NULL) || (ctxt->myDoc == KD_NULL)) return(0);
    return(ctxt->myDoc->extSubset != KD_NULL);
}
	       ...
	       const xmlChar *ExternalID, const xmlChar *SystemID)
{
    ...
    xmlGenericError(xmlGenericErrorContext,
	    "SAX.xmlSAX2ExternalSubset(%s, %s, %s)\n",
            name, ExternalID, SystemID);
#endif
    if (((ExternalID != KD_NULL) || (SystemID != KD_NULL)) &&
 ...
 *
 * Returns the xmlParserInputPtr if inlined or KD_NULL for DOM behaviour.
 */
xmlParserInputPtr
xmlSAX2ResolveEntity(void *ctx, const xmlChar *publicId, const xmlChar *systemId)
{
    ...
    xmlChar *URI;
    const char *base = KD_NULL;

    if (ctx == KD_NULL) return(KD_NULL);
...
xmlSAX2GetEntity(void *ctx, const xmlChar *name)
{
    ...
    if ((ctxt->myDoc != KD_NULL) && (ctxt->myDoc->standalone == 1)) {
	if (ctxt->inSubset == 2) {
          ...
          const xmlChar *publicId, const xmlChar *systemId, xmlChar *content)
{
	...
	ent = xmlAddDocEntity(ctxt->myDoc, name, type, publicId,
		              systemId, content);
	if ((ent == KD_NULL) && (ctxt->pedantic))
	    xmlWarnMsg(ctxt, XML_WAR_ENTITY_REDEFINED,
	     "Entity(%s) already defined in the internal subset\n",
	               name);
	if ((ent != KD_NULL) && (ent->URI == KD_NULL) && (systemId != KD_NULL)) {
	    xmlChar *URI;
	    const char *base = KD_NULL;
	...
	ent = xmlAddDtdEntity(ctxt->myDoc, name, type, publicId,
		              systemId, content);
	if ((ent == KD_NULL) && (ctxt->pedantic) &&
	    (ctxt->sax != KD_NULL) && (ctxt->sax->warning != KD_NULL))
	      ...
	      xmlEnumerationPtr tree)
{
    ...
    xmlChar *name = KD_NULL, *prefix = KD_NULL;

    if ((ctxt == KD_NULL) || (ctxt->myDoc == KD_NULL))
        return;
	...
	ctxt->valid = 0;
    if ((attr != KD_NULL) && (ctxt->validate) && (ctxt->wellFormed) &&
        (ctxt->myDoc->intSubset != KD_NULL))
...
xmlSAX2StartDocument(void *ctx)
{
	...
	if (ctxt->myDoc == KD_NULL)
	    ctxt->myDoc = htmlNewDocNoDtD(KD_NULL, KD_NULL);
	...
	if (doc != KD_NULL) {
		...
		doc->encoding = KD_NULL;
	    doc->standalone = ctxt->standalone;
	} else {
	    xmlSAX2ErrMemory(ctxt, "xmlSAX2StartDocument");
	    return;
	}
	if ((ctxt->dictNames) && (doc != KD_NULL)) {
	    doc->dict = ctxt->dict;
	    xmlDictReference(doc->dict);
	}
    }
    if ((ctxt->myDoc != KD_NULL) && (ctxt->myDoc->URL == KD_NULL) &&
	(ctxt->input != KD_NULL) && (ctxt->input->filename != KD_NULL)) {
	ctxt->myDoc->URL = xmlPathToURI((const xmlChar *)ctxt->input->filename);
	if (ctxt->myDoc->URL == KD_NULL)
	    xmlSAX2ErrMemory(ctxt, "xmlSAX2StartDocument");
    }
}
...
xmlSAX2EndDocument(void *ctx)
{
     ...
     */
    if ((ctxt->encoding != KD_NULL) && (ctxt->myDoc != KD_NULL) &&
	(ctxt->myDoc->encoding == KD_NULL)) {
	ctxt->myDoc->encoding = ctxt->encoding;
	ctxt->encoding = KD_NULL;
    }
    if ((ctxt->inputTab != KD_NULL) &&
        (ctxt->inputNr > 0) && (ctxt->inputTab[0] != KD_NULL) &&
        (ctxt->inputTab[0]->encoding != KD_NULL) && (ctxt->myDoc != KD_NULL) &&
	(ctxt->myDoc->encoding == KD_NULL)) {
	ctxt->myDoc->encoding = xmlStrdup(ctxt->inputTab[0]->encoding);
    }
    if ((ctxt->charset != XML_CHAR_ENCODING_NONE) && (ctxt->myDoc != KD_NULL) &&
             ...
             const xmlChar *value, const xmlChar *prefix ATTRIBUTE_UNUSED)
{
...
#ifdef LIBXML_HTML_ENABLED
    if ((ctxt->html) &&
        (value == KD_NULL) && (htmlIsBooleanAttr(fullname))) {
	...
	if (val[0] != 0) {
	    ...
	    if (uri == KD_NULL) {
		if ((ctxt->sax != KD_NULL) && (ctxt->sax->warning != KD_NULL))
		    ctxt->sax->warning(ctxt->userData, 
	...
	nsret = xmlNewNs(ctxt->node, val, KD_NULL);
	 ...
	 */
        if (nsret != KD_NULL && ctxt->validate && ctxt->wellFormed &&
	    ctxt->myDoc && ctxt->myDoc->intSubset)
    ...
    if ((!ctxt->html) &&
	(ns != KD_NULL) && (ns[0] == 'x') && (ns[1] == 'm') && (ns[2] == 'l') &&
    ...
    if (ns != KD_NULL) {
    	_namespace = xmlSearchNs(ctxt->myDoc, ctxt->node, ns);

	if (_namespace == KD_NULL) {
            ...
            while (prop != KD_NULL) {
                if (prop->ns != KD_NULL) {
                    if ((xmlStrEqual(name, prop->name)) &&
                        ((_namespace == prop->ns) ||
                         (xmlStrEqual(_namespace->href, prop->ns->href)))) {
                            xmlNsErrMsg(ctxt, XML_ERR_ATTRIBUTE_REDEFINED,
                                    "Attribute %s in %s redefined\n",
                                             name, _namespace->href);
                        ctxt->wellFormed = 0;
    ...
    ret = xmlNewNsPropEatName(ctxt->node, _namespace, name, KD_NULL);
...
xmlSAX2StartElement(void *ctx, const xmlChar *fullname, const xmlChar **atts)
{
    ...
    if ((ctx == KD_NULL) || (fullname == KD_NULL) || (ctxt->myDoc == KD_NULL)) return;
    parent = ctxt->node;
#ifdef DEBUG_SAX
    xmlGenericError(xmlGenericErrorContext,
     ...
     */
    if (ctxt->validate && (ctxt->myDoc->extSubset == KD_NULL) && 
        ((ctxt->myDoc->intSubset == KD_NULL) ||
	 ((ctxt->myDoc->intSubset->notations == KD_NULL) && 
	  (ctxt->myDoc->intSubset->elements == KD_NULL) &&
	  (ctxt->myDoc->intSubset->attributes == KD_NULL) && 
	  (ctxt->myDoc->intSubset->entities == KD_NULL)))) {
	xmlErrValid(ctxt, XML_ERR_NO_DTD,
	  "Validation failed: no DTD found !", KD_NULL, KD_NULL);
	ctxt->validate = 0;
    }
     ...
     */
    ret = xmlNewDocNodeEatName(ctxt->myDoc, KD_NULL, name, KD_NULL);
    if (ret == KD_NULL) {
        if (prefix != KD_NULL)
	    xmlFree(prefix);
	xmlSAX2ErrMemory(ctxt, "xmlSAX2StartElement");
    ...
    if (ctxt->myDoc->children == KD_NULL) {
#ifdef DEBUG_SAX_TREE
	xmlGenericError(xmlGenericErrorContext, "Setting %s as root\n", name);
#endif
        xmlAddChild((xmlNodePtr) ctxt->myDoc, (xmlNodePtr) ret);
    } else if (parent == KD_NULL) {
	...
	value = atts[i++];
	if (!ctxt->html) {
	    while ((att != KD_NULL) && (value != KD_NULL)) {
    ...
    if ((ns == KD_NULL) && (parent != KD_NULL))
	ns = xmlSearchNs(ctxt->myDoc, parent, prefix);
    if ((prefix != KD_NULL) && (ns == KD_NULL)) {
	ns = xmlNewNs(ret, KD_NULL, prefix);
     ...
     * is unbound on a parent we simply kee it KD_NULL
     */
    if ((ns != KD_NULL) && (ns->href != KD_NULL) &&
	((ns->href[0] != 0) || (ns->prefix != KD_NULL)))
	xmlSetNs(ret, ns);
	...
	if (ctxt->html) {
	    while (att != KD_NULL) {
		xmlSAX2AttributeInternal(ctxt, att, value, KD_NULL);
		att = atts[i++];
		value = atts[i++];
	    }
	} else {
...
#ifdef DEBUG_SAX
    if (name == KD_NULL)
        xmlGenericError(xmlGenericErrorContext, "SAX.xmlSAX2EndElement(KD_NULL)\n");
    else
	xmlGenericError(xmlGenericErrorContext, "SAX.xmlSAX2EndElement(%s)\n", name);
#endif
    
    /* Capture end position and add node */
    if (cur != KD_NULL && ctxt->record_info) {
 ...
 *
 * Returns the newly allocated string or KD_NULL if not needed or error
 */
static xmlNodePtr
xmlSAX2TextNode(xmlParserCtxtPtr ctxt, const xmlChar *str, int len) {
    xmlNodePtr ret;
    const xmlChar *intern = KD_NULL;
...
{
    xmlAttrPtr ret;
    xmlNsPtr _namespace = KD_NULL;
    xmlChar *dup = KD_NULL;

    /*
     * Note: if prefix == KD_NULL, the attribute is not in the default namespace
     */
    if (prefix != KD_NULL)
    	_namespace = xmlSearchNs(ctxt->myDoc, ctxt->node, prefix);
	...
	if (ctxt->node->properties == KD_NULL) {
	    ctxt->node->properties = ret;
	} else {
	    xmlAttrPtr prev = ctxt->node->properties;

	    while (prev->next != KD_NULL) prev = prev->next;
	    prev->next = ret;
	    ret->prev = prev;
	}
	...
	if (ctxt->dictNames)
	    ret = xmlNewNsPropEatName(ctxt->node, _namespace,
	                              (xmlChar *) localname, KD_NULL);
	else
	    ret = xmlNewNsProp(ctxt->node, _namespace, localname, KD_NULL);
	if (ret == KD_NULL) {
		      ...
		      const xmlChar **attributes)
{
	...
	if (ctxt->dictNames)
	    ret = xmlNewDocNodeEatName(ctxt->myDoc, KD_NULL, 
	                               (xmlChar *) localname, KD_NULL);
	else
	    ret = xmlNewDocNode(ctxt->myDoc, KD_NULL, localname, KD_NULL);
	if (ret == KD_NULL) {
	    xmlSAX2ErrMemory(ctxt, "xmlSAX2StartElementNs");
	    return;
	}
    ...
    if ((ctxt->myDoc->children == KD_NULL) || (parent == KD_NULL)) {
        xmlAddChild((xmlNodePtr) ctxt->myDoc, (xmlNodePtr) ret);
    }
    /*
     ...
     * Note that, if prefix is KD_NULL, this searches for the default Ns
     */
    if ((URI != KD_NULL) && (ret->ns == KD_NULL)) {
        ret->ns = xmlSearchNs(ctxt->myDoc, parent, prefix);
	if ((ret->ns == KD_NULL) && (xmlStrEqual(prefix, BAD_CAST "xml"))) {
		    ...
		    const xmlChar * URI ATTRIBUTE_UNUSED)
{
    ...
    cur = ctxt->node;
    /* Capture end position and add node */
    if ((ctxt->record_info) && (cur != KD_NULL)) {
        node_info.end_pos = ctxt->input->cur - ctxt->input->base;
        node_info.end_line = ctxt->input->line;
        node_info.node = cur;
        xmlParserAddNodeInfo(ctxt, &node_info);
...
#endif
    if (xmlAddChild(ctxt->node, ret) == KD_NULL) {
        xmlFreeNode(ret);
    }
}
...
xmlSAX2Characters(void *ctx, const xmlChar *ch, int len)
{
	...
	int coalesceText = (lastChild != KD_NULL) &&
	...
	if ((coalesceText) && (ctxt->nodemem != 0)) {
		...
		lastChild->properties = KD_NULL;
	    } else if ((ctxt->nodemem == ctxt->nodelen + 1) &&
	               (xmlDictOwns(ctxt->dict, lastChild->content))) {
		lastChild->content = xmlStrdup(lastChild->content);
	    }
            if (((KDsize)ctxt->nodelen + (KDsize)len > XML_MAX_TEXT_LENGTH) &&
                ((ctxt->options & XML_PARSE_HUGE) == 0)) {
                xmlSAX2ErrMemory(ctxt, "xmlSAX2Characters: huge text node");
                return;
            }
	    if ((KDsize)ctxt->nodelen > SIZE_T_MAX - (KDsize)len || 
	        (KDsize)ctxt->nodemem + (KDsize)len > SIZE_T_MAX / 2) {
	    ...
	    kdMemcpy (&lastChild->content[ctxt->nodelen], ch, len);
	    ctxt->nodelen += len;
...
{
    if (hdlr == KD_NULL) return(-1);
    if (version == 2) {
	hdlr->startElement = KD_NULL;
	hdlr->endElement = KD_NULL;
...
{
    if ((hdlr == KD_NULL) || (hdlr->initialized != 0))
	return;
