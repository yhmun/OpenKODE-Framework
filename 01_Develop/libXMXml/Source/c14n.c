...
#define 	xmlC14NIsVisible( ctx, node, parent ) \
     (((ctx)->is_visible_callback != KD_NULL) ? \
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_C14N,
		    XML_ERR_NO_MEMORY, XML_ERR_ERROR, KD_NULL, 0, extra,
		    KD_NULL, KD_NULL, 0, 0,
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_C14N,
		    XML_ERR_INTERNAL_ERROR, XML_ERR_ERROR, KD_NULL, 0, extra,
		    KD_NULL, KD_NULL, 0, 0,
		    "Invalid parameter : %s\n", extra);
}
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_C14N,
		    XML_C14N_INVALID_NODE, XML_ERR_ERROR, KD_NULL, 0, extra,
		    KD_NULL, KD_NULL, 0, 0,
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_C14N,
		    XML_C14N_UNKNOW_NODE, XML_ERR_ERROR, KD_NULL, 0, extra,
		    KD_NULL, KD_NULL, 0, 0,
...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_C14N,
		    XML_C14N_RELATIVE_NAMESPACE, XML_ERR_ERROR, KD_NULL, 0, KD_NULL,
		    KD_NULL, KD_NULL, 0, 0,
           ...
           const char * msg)
{
    ...
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
		    ...
		    KD_NULL, KD_NULL, KD_NULL, 0, 0, "%s", msg);
}
...
xmlC14NIsNodeInNodeset(xmlNodeSetPtr nodes, xmlNodePtr node, xmlNodePtr parent) {
    if((nodes != KD_NULL) && (node != KD_NULL)) {
	if(node->type != XML_NAMESPACE_DECL) {
	    return(xmlXPathNodeSetContains(nodes, node));
	} else {
	    xmlNs ns;
	    
	    kdMemcpy (&ns, node, sizeof(ns)); 
	    
	    /* this is a libxml hack! check xpath.c for details */
	    if((parent != KD_NULL) && (parent->type == XML_ATTRIBUTE_NODE)) {
...
xmlC14NVisibleNsStackCreate(void) {
    ...
    kdMemset (ret, 0 , (KDsize) sizeof(xmlC14NVisibleNsStack));
    return(ret);
}
...
xmlC14NVisibleNsStackDestroy(xmlC14NVisibleNsStackPtr cur) {
    ...
    if(cur->nsTab != KD_NULL) {
	kdMemset (cur->nsTab, 0, cur->nsMax * sizeof(xmlNsPtr));
	xmlFree(cur->nsTab);
    }
    if(cur->nodeTab != KD_NULL) {
	kdMemset (cur->nodeTab, 0, cur->nsMax * sizeof(xmlNodePtr));
	xmlFree(cur->nodeTab);
    }
    kdMemset (cur, 0, sizeof(xmlC14NVisibleNsStack));
    xmlFree(cur);
...
xmlC14NVisibleNsStackAdd(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns, xmlNodePtr node) {
    if((cur == KD_NULL) || 
       ((cur->nsTab == KD_NULL) && (cur->nodeTab != KD_NULL)) ||
       ((cur->nsTab != KD_NULL) && (cur->nodeTab == KD_NULL))) {
        xmlC14NErrParam("adding namespace to stack");
	return;
    }

    if ((cur->nsTab == KD_NULL) && (cur->nodeTab == KD_NULL)) {
	...
	kdMemset (cur->nsTab, 0 , XML_NAMESPACES_DEFAULT * sizeof(xmlNsPtr));
	kdMemset (cur->nodeTab, 0 , XML_NAMESPACES_DEFAULT * sizeof(xmlNodePtr));
...
xmlC14NStrEqual(const xmlChar *str1, const xmlChar *str2) {
    ...
    if (str1 == KD_NULL) return((*str2) == '\0');
    if (str2 == KD_NULL) return((*str1) == '\0');
    do {
...
xmlC14NVisibleNsStackFind(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns)
{
     ...
     */
    prefix = ((ns == KD_NULL) || (ns->prefix == KD_NULL)) ? BAD_CAST "" : ns->prefix;
    href = ((ns == KD_NULL) || (ns->href == KD_NULL)) ? BAD_CAST "" : ns->href;
    has_empty_ns = (xmlC14NStrEqual(prefix, KD_NULL) && xmlC14NStrEqual(href, KD_NULL));
	...
	int start = (has_empty_ns) ? 0 : cur->nsPrevStart;
	    ...
	    if(xmlC14NStrEqual(prefix, (ns1 != KD_NULL) ? ns1->prefix : KD_NULL)) {
		return(xmlC14NStrEqual(href, (ns1 != KD_NULL) ? ns1->href : KD_NULL));
	    }
        }
    }
    return(has_empty_ns);
...
xmlExcC14NVisibleNsStackFind(xmlC14NVisibleNsStackPtr cur, xmlNsPtr ns, xmlC14NCtxPtr ctx) {
	...
	int start = 0;
	    ...
	    if(xmlC14NStrEqual(prefix, (ns1 != KD_NULL) ? ns1->prefix : KD_NULL)) {
		if(xmlC14NStrEqual(href, (ns1 != KD_NULL) ? ns1->href : KD_NULL)) {
...
xmlC14NProcessNamespacesAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur, int visible)
{
    ...
    if ((ctx == KD_NULL) || (cur == KD_NULL) || (cur->type != XML_ELEMENT_NODE)) {
     ...
     */
    list = xmlListCreate(KD_NULL, (xmlListDataCompare) xmlC14NNsCompare);
    if (list == KD_NULL) {
    ...
    for(n = cur; n != KD_NULL; n = n->parent) {
	for(ns = n->nsDef; ns != KD_NULL; ns = ns->next) {
    ...
    if(visible && !has_empty_ns) {
        ...
        kdMemset (&ns_default, 0, sizeof(ns_default));
...
xmlExcC14NProcessNamespacesAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur, int visible)
{
     ...
     */
    if(ctx->inclusive_ns_prefixes != KD_NULL) {
	xmlChar *prefix; 
	int i;
	
	for (i = 0; ctx->inclusive_ns_prefixes[i] != KD_NULL; ++i) {
	    ...
	    if((ns != KD_NULL) && !xmlC14NIsXmlNs(ns) && xmlC14NIsVisible(ctx, ns, cur)) {
		already_rendered = xmlC14NVisibleNsStackFind(ctx->ns_rendered, ns);
		if(visible) {
    	    	    xmlC14NVisibleNsStackAdd(ctx->ns_rendered, ns, cur);
		}
	...
	ns = cur->ns;
        ...
        ns = xmlSearchNs(cur->doc, cur, KD_NULL);
	has_visibly_utilized_empty_ns = 1;
    }
    if((ns != KD_NULL) && !xmlC14NIsXmlNs(ns)) {
    ...
    /* add attributes */
    for(attr = cur->properties; attr != KD_NULL; attr = attr->next) {
        /* 
         * we need to check that attribute is visible and has non
         * default namespace (XML Namespaces: "default namespaces 
    	 * do not apply directly to attributes")	 
         */
	if((attr->ns != KD_NULL) && !xmlC14NIsXmlNs(attr->ns) && xmlC14NIsVisible(ctx, attr, cur)) {
	...
	} else if((attr->ns != KD_NULL) && (xmlStrlen(attr->ns->prefix) == 0) && (xmlStrlen(attr->ns->href) == 0)) {
	    has_visibly_utilized_empty_ns = 1;
	}
    }
...
xmlC14NAttrsCompare(xmlAttrPtr attr1, xmlAttrPtr attr2)
{
    ...
    if (attr2->ns == KD_NULL)
        return (1);
    if (attr1->ns->prefix == KD_NULL)
        return (-1);
    if (attr2->ns->prefix == KD_NULL)
        return (1);
...
xmlC14NPrintAttrs(const xmlAttrPtr attr, xmlC14NCtxPtr ctx)
{
    ...
    if ((attr == KD_NULL) || (ctx == KD_NULL)) {
        xmlC14NErrParam("writing attributes");
        return (0);
    }

    xmlOutputBufferWriteString(ctx->buf, " ");
    if (attr->ns != KD_NULL && xmlStrlen(attr->ns->prefix) > 0) {
        xmlOutputBufferWriteString(ctx->buf,
    ...
    /* todo: should we log an error if value==KD_NULL ? */
    if (value != KD_NULL) {
 ...
 * 
 * Returns a pointer to the attribute node (if found) or KD_NULL otherwise.
 */
static xmlAttrPtr
xmlC14NFindHiddenParentAttr(xmlC14NCtxPtr ctx, xmlNodePtr cur, const xmlChar * name, const xmlChar * ns)
{
    xmlAttrPtr res;
    while((cur != KD_NULL) && (!xmlC14NIsVisible(ctx, cur, cur->parent))) {
 ...
 *
 * Returns the newly created attribute or KD_NULL
 */
static xmlAttrPtr
...
{    
    ...
    if ((ctx == KD_NULL) || (xml_base_attr == KD_NULL) || (xml_base_attr->parent == KD_NULL)) {
    ...
    if((res == KD_NULL) || xmlStrEqual(res, BAD_CAST "")) {
        xmlFree(res);
        return (KD_NULL);
    }

    /* create and return the new attribute node */
    attr = xmlNewNsProp(KD_NULL, xml_base_attr->ns, BAD_CAST "base", res);
    if(attr == KD_NULL) {
        xmlFree(res);
...
xmlC14NProcessAttrsAxis(xmlC14NCtxPtr ctx, xmlNodePtr cur, int parent_visible)
{
    xmlAttrPtr attr;
    xmlListPtr list;    
    xmlAttrPtr attrs_to_delete = KD_NULL;
    
    /* special processing for 1.1 spec */
    xmlAttrPtr xml_base_attr = KD_NULL;
    xmlAttrPtr xml_lang_attr = KD_NULL;
    xmlAttrPtr xml_space_attr = KD_NULL;
     ...
     */
    list = xmlListCreate(KD_NULL, (xmlListDataCompare) xmlC14NAttrsCompare);
    if (list == KD_NULL) {
    ...
    case XML_C14N_1_0:
         ...
         */
        if (parent_visible && (cur->parent != KD_NULL) && 
            ...
            while (tmp != KD_NULL) {
                ...
                while (attr != KD_NULL) {
                    if (xmlC14NIsXmlAttr(attr) != 0) {
                        if (xmlListSearch(list, attr) == KD_NULL) {
                            xmlListInsert(list, attr);
                        }
                    }
                    attr = attr->next;
    ...
    case XML_C14N_1_1:
        ...
        while (attr != KD_NULL) {
            ...
            if ((!parent_visible) || (xmlC14NIsXmlAttr(attr) == 0)) {
                ...
                if((!matched) && (xml_lang_attr == KD_NULL) && xmlStrEqual(attr->name, BAD_CAST "lang")) {
                    xml_lang_attr = attr;
                    matched = 1;
                } 
                if((!matched) && (xml_space_attr == KD_NULL) && xmlStrEqual(attr->name, BAD_CAST "space")) {
                    xml_space_attr = attr;
                    matched = 1;
                }

                /* check for base attr */
                if((!matched) && (xml_base_attr == KD_NULL) && xmlStrEqual(attr->name, BAD_CAST "base")) {
                    xml_base_attr = attr;
                    matched = 1;
                }
...
xmlC14NProcessElementNode(xmlC14NCtxPtr ctx, xmlNodePtr cur, int visible)
{
    ...
    if (visible) {	
        ...
        if ((cur->ns != KD_NULL) && (xmlStrlen(cur->ns->prefix) > 0)) {
            xmlOutputBufferWriteString(ctx->buf,
...
xmlC14NProcessNode(xmlC14NCtxPtr ctx, xmlNodePtr cur)
{
    ...
    switch (cur->type) {
        ...
        case XML_TEXT_NODE:
            ...
            if ((visible) && (cur->content != KD_NULL)) {
                xmlChar *buffer;
        ...
        case XML_PI_NODE:
            ...
            if (visible) {
                ...
                xmlOutputBufferWriteString(ctx->buf,
                                           (const char *) cur->name);
                if ((cur->content != KD_NULL) && (*(cur->content) != '\0')) {
                    xmlChar *buffer;
...
xmlC14NProcessNodeList(xmlC14NCtxPtr ctx, xmlNodePtr cur)
{
    ...
    for (ret = 0; cur != KD_NULL && ret >= 0; cur = cur->next) {
 ...
 *			ended with a KD_NULL or KD_NULL if there is no
 *			inclusive namespaces (only for ` 
 *			canonicalization)
 * @with_comments: 	include comments in the result (!=0) or not (==0)
 * @buf: 		the output buffer to store canonical XML; this 
 *			buffer MUST have encoder==KD_NULL because C14N requires
 *			UTF-8 output
 *  		
 * Creates new C14N context object to store C14N parameters.
 *
 * Returns pointer to newly created object (success) or KD_NULL (fail)
 */
...
{
    xmlC14NCtxPtr ctx = KD_NULL;

    if ((doc == KD_NULL) || (buf == KD_NULL)) {
     ...
     */
    if (buf->encoder != KD_NULL) {
        xmlC14NErr(ctx, (xmlNodePtr) doc, XML_C14N_REQUIRES_UTF8,
"xmlC14NNewCtx: output buffer encoder != KD_NULL but C14N requires UTF8 output\n");
        return (KD_NULL);
    }
    ...
    kdMemset (ctx, 0, sizeof(xmlC14NCtx));
...
xmlC14NExecute(xmlDocPtr doc, xmlC14NIsVisibleCallback is_visible_callback,
     ...
     */
    if (buf->encoder != KD_NULL) {
        xmlC14NErr(KD_NULL, (xmlNodePtr) doc, XML_C14N_REQUIRES_UTF8,
"xmlC14NExecute: output buffer encoder != KD_NULL but C14N requires UTF8 output\n");
        return (-1);
    }
    ...
    ctx = xmlC14NNewCtx(doc, is_visible_callback, user_data, 
	            c14n_mode, inclusive_ns_prefixes,
                    with_comments, buf);
    if (ctx == KD_NULL) {
        xmlC14NErr(KD_NULL, (xmlNodePtr) doc, XML_C14N_CREATE_CTXT,
 ...
 *      		or KD_NULL if all document nodes should be included
 * @mode:		the c14n mode (see @xmlC14NMode)
 * @inclusive_ns_prefixes: the list of inclusive namespace prefixes 
 *			ended with a KD_NULL or KD_NULL if there is no
 *			inclusive namespaces (only for exclusive 
                     ...
                     int with_comments, xmlChar ** doc_txt_ptr)
{
    ...
    *doc_txt_ptr = KD_NULL;
     ...
     */
    buf = xmlAllocOutputBuffer(KD_NULL);
    if (buf == KD_NULL) {
               ...
               int with_comments, const char *filename, int compression)
{
     ...
     */
    buf = xmlOutputBufferCreateFilename(filename, KD_NULL, compression);
    if (buf == KD_NULL) {
