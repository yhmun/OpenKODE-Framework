...
#define IS_KD_NULLABLE		(1 << 0)
#define IS_NOT_KD_NULLABLE		(1 << 1)
...
xmlRngPErrMemory(xmlRelaxNGParserCtxtPtr ctxt, const char *extra)
{
    ...
    if (extra)
        __xmlRaiseError(schannel, channel, data,
                        KD_NULL, KD_NULL, XML_FROM_RELAXNGP,
...
xmlRngVErrMemory(xmlRelaxNGValidCtxtPtr ctxt, const char *extra)
{
    ...
    if (extra)
        __xmlRaiseError(schannel, channel, data,
                        KD_NULL, KD_NULL, XML_FROM_RELAXNGV,
                        XML_ERR_NO_MEMORY, XML_ERR_FATAL, KD_NULL, 0, extra,
                        KD_NULL, KD_NULL, 0, 0,
                        "Memory allocation failed : %s\n", extra);
    else
           ...
           const char *msg, const xmlChar * str1, const xmlChar * str2)
{
    ...
    __xmlRaiseError(schannel, channel, data,
                    KD_NULL, node, XML_FROM_RELAXNGP,
           ...
           const char *msg, const xmlChar * str1, const xmlChar * str2)
{
    ...
    __xmlRaiseError(schannel, channel, data,
                    KD_NULL, node, XML_FROM_RELAXNGV,
                    error, XML_ERR_ERROR, KD_NULL, 0,
                    (const char *) str1, (const char *) str2, KD_NULL, 0, 0,
                    msg, str1, str2);
}
...
struct _xmlRelaxNGTypeLibrary {
    const xmlChar *_namespace;   /* the datatypeLibrary value */
...
xmlRelaxNGFreeDocument(xmlRelaxNGDocumentPtr docu)
{
    ...
    if (docu->href != KD_NULL)
        xmlFree(docu->href);
    if (docu->doc != KD_NULL)
        xmlFreeDoc(docu->doc);
    if (docu->schema != KD_NULL)
...
xmlRelaxNGFreeInclude(xmlRelaxNGIncludePtr incl)
{
    ...
    if (incl->href != KD_NULL)
        xmlFree(incl->href);
    if (incl->doc != KD_NULL)
        xmlFreeDoc(incl->doc);
    if (incl->schema != KD_NULL)
...
xmlRelaxNGNewRelaxNG(xmlRelaxNGParserCtxtPtr ctxt)
{
    ...
    if (ret == KD_NULL) {
        xmlRngPErrMemory(ctxt, KD_NULL);
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlRelaxNG));
...
xmlRelaxNGFreeInnerSchema(xmlRelaxNGPtr schema)
{
    ...
    if (schema->doc != KD_NULL)
        xmlFreeDoc(schema->doc);
    if (schema->defTab != KD_NULL) {
        int i;
...
xmlRelaxNGFree(xmlRelaxNGPtr schema)
{
    ...
    if (schema->topgrammar != KD_NULL)
        xmlRelaxNGFreeGrammar(schema->topgrammar);
    if (schema->doc != KD_NULL)
        xmlFreeDoc(schema->doc);
    if (schema->documents != KD_NULL)
...
xmlRelaxNGNewGrammar(xmlRelaxNGParserCtxtPtr ctxt)
{
    ...
    kdMemset(ret, 0, sizeof(xmlRelaxNGGrammar));
...
xmlRelaxNGFreeGrammar(xmlRelaxNGGrammarPtr grammar)
{
    ...
    if (grammar->children != KD_NULL) {
        xmlRelaxNGFreeGrammar(grammar->children);
    }
    if (grammar->next != KD_NULL) {
        xmlRelaxNGFreeGrammar(grammar->next);
    }
    if (grammar->refs != KD_NULL) {
        xmlHashFree(grammar->refs, KD_NULL);
    }
    if (grammar->defs != KD_NULL) {
        xmlHashFree(grammar->defs, KD_NULL);
    }
...
xmlRelaxNGNewDefine(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    kdMemset(ret, 0, sizeof(xmlRelaxNGDefine));
...
xmlRelaxNGFreePartition(xmlRelaxNGPartitionPtr partitions)
{
    ...
    if (partitions != KD_NULL) {
        if (partitions->groups != KD_NULL) {
            for (j = 0; j < partitions->nbgroups; j++) {
                group = partitions->groups[j];
                if (group != KD_NULL) {
                    if (group->defs != KD_NULL)
                        xmlFree(group->defs);
                    if (group->attrs != KD_NULL)
                        xmlFree(group->attrs);
                    xmlFree(group);
                }
            }
        ...
        if (partitions->triage != KD_NULL) {
            xmlHashFree(partitions->triage, KD_NULL);
        }
        xmlFree(partitions);
    }
}
...
xmlRelaxNGFreeDefine(xmlRelaxNGDefinePtr define)
{
    ...
    if ((define->type == XML_RELAXNG_VALUE) && (define->attrs != KD_NULL)) {
        xmlRelaxNGTypeLibraryPtr lib;

        lib = (xmlRelaxNGTypeLibraryPtr) define->data;
        if ((lib != KD_NULL) && (lib->freef != KD_NULL))
            lib->freef(lib->data, (void *) define->attrs);
    }
    if ((define->data != KD_NULL) && (define->type == XML_RELAXNG_INTERLEAVE))
        xmlRelaxNGFreePartition((xmlRelaxNGPartitionPtr) define->data);
    if ((define->data != KD_NULL) && (define->type == XML_RELAXNG_CHOICE))
        xmlHashFree((xmlHashTablePtr) define->data, KD_NULL);
    if (define->name != KD_NULL)
        xmlFree(define->name);
    if (define->ns != KD_NULL)
        xmlFree(define->ns);
    if (define->value != KD_NULL)
        xmlFree(define->value);
    if (define->contModel != KD_NULL)
...
xmlRelaxNGNewStates(xmlRelaxNGValidCtxtPtr ctxt, int size)
{
    ...
    if ((ctxt != KD_NULL) &&
        (ctxt->freeStates != KD_NULL) && (ctxt->freeStatesNr > 0)) {
        ctxt->freeStatesNr--;
 ...
 * @ctxt:  a Relax-NG validation context
 * @node:  the current node or KD_NULL for the document
 *
...
xmlRelaxNGNewValidState(xmlRelaxNGValidCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    if ((ctxt->freeState != KD_NULL) && (ctxt->freeState->nbState > 0)) {
        ...
        kdMemset(ret, 0, sizeof(xmlRelaxNGValidState));
    }
    ret->value = KD_NULL;
    ret->endvalue = KD_NULL;
    if (node == KD_NULL) {
    ...
    if (nbAttrs > 0) {
        ...
        ret->nbAttrs = nbAttrs;
        if (nbAttrs < MAX_ATTR) {
            kdMemcpy (ret->attrs, attrs, sizeof(xmlAttrPtr) * nbAttrs);
        } else {
            attr = node->properties;
            nbAttrs = 0;
            while (attr != KD_NULL) {
                         ...
                         xmlRelaxNGValidStatePtr state)
{
    ...
    kdMemcpy (ret, state, sizeof(xmlRelaxNGValidState));
    ret->attrs = attrs;
    ...
    if (state->nbAttrs > 0) {
        ...
        kdMemcpy (ret->attrs, state->attrs,
                          ...
                          xmlRelaxNGValidStatePtr state2)
{
    ...
    if ((state1 == KD_NULL) || (state2 == KD_NULL))
        return (0);
    if (state1 == state2)
        return (1);
                         ...
                         xmlRelaxNGValidStatePtr state)
{
    ...
    if ((ctxt != KD_NULL) && (ctxt->freeState == KD_NULL)) {
        ctxt->freeState = xmlRelaxNGNewStates(ctxt, 40);
    }
    if ((ctxt == KD_NULL) || (ctxt->freeState == KD_NULL)) {
        if (state->attrs != KD_NULL)
            xmlFree(state->attrs);
        xmlFree(state);
    } else {
...
xmlRelaxNGIncludePop(xmlRelaxNGParserCtxtPtr ctxt)
{
    ...
    ctxt->incTab[ctxt->incNr] = KD_NULL;
    return (ret);
}
                         ...
                         xmlNodePtr target, const xmlChar * name)
{
    ...
    while (tmp != KD_NULL) {
        tmp2 = tmp->next;
        if ((name == KD_NULL) && (IS_RELAXNG(tmp, "start"))) {
            found = 1;
            xmlUnlinkNode(tmp);
            xmlFreeNode(tmp);
        } else if ((name != KD_NULL) && (IS_RELAXNG(tmp, "define"))) {
            ...
            xmlChar *href = KD_NULL;
            xmlRelaxNGDocumentPtr inc = (xmlRelaxNGDocumentPtr) tmp->psvi;

            if ((inc != KD_NULL) && (inc->doc != KD_NULL) &&
                (inc->doc->children != KD_NULL)) {
                      ...
                      xmlNodePtr node, const xmlChar * ns)
{
    ...
    for (i = 0; i < ctxt->incNr; i++) {
        if (xmlStrEqual(ctxt->incTab[i]->href, URL)) {
            xmlRngPErr(ctxt, KD_NULL, XML_RNGP_INCLUDE_RECURSE,
     ...
     * load the document
     */
    doc = xmlReadFile((const char *) URL,KD_NULL,0);
    if (doc == KD_NULL) {
    ...
    kdMemset(ret, 0, sizeof(xmlRelaxNGInclude));
    ret->doc = doc;
    ...
    if (ns != KD_NULL) {
        ...
        if (root != KD_NULL) {
            if (xmlHasProp(root, BAD_CAST "ns") == KD_NULL) {
    ...
    while (cur != KD_NULL) {
        if (IS_RELAXNG(cur, "start")) {
            ...
            found =
                xmlRelaxNGRemoveRedefine(ctxt, URL, root->children, KD_NULL);
            if (!found) {
                         ...
                         const xmlChar * arg2, int dup)
{
    ...
    if ((ctxt->err != KD_NULL) && (ctxt->state != KD_NULL) &&
...
xmlRelaxNGDocumentPop(xmlRelaxNGParserCtxtPtr ctxt)
{
    ...
    ctxt->docTab[ctxt->docNr] = KD_NULL;
    return (ret);
}
 ...
 * the content is preprocessed before being returned back to the caller.
 *
 * Returns the xmlRelaxNGDocumentPtr or KD_NULL in case of error
 */
static xmlRelaxNGDocumentPtr
xmlRelaxNGLoadExternalRef(xmlRelaxNGParserCtxtPtr ctxt,
                          const xmlChar * URL, const xmlChar * ns)
{
    xmlRelaxNGDocumentPtr ret = KD_NULL;
    xmlDocPtr doc;
    xmlNodePtr root;
    int i;
    ...
    for (i = 0; i < ctxt->docNr; i++) {
        if (xmlStrEqual(ctxt->docTab[i]->href, URL)) {
            xmlRngPErr(ctxt, KD_NULL, XML_RNGP_EXTERNALREF_RECURSE,
    ...
    doc = xmlReadFile((const char *) URL,KD_NULL,0);
    if (doc == KD_NULL) {
        xmlRngPErr(ctxt, KD_NULL, XML_RNGP_PARSE_ERROR,
                   "xmlRelaxNG: could not load %s\n", URL, KD_NULL);
        return (KD_NULL);
    }
    ...
    kdMemset(ret, 0, sizeof(xmlRelaxNGDocument));
    ret->doc = doc;
    ret->href = xmlStrdup(URL);
...
#define VALID_ERR(a) xmlRelaxNGAddValidError(ctxt, a, KD_NULL, KD_NULL, 0);
#define VALID_ERR2(a, b) xmlRelaxNGAddValidError(ctxt, a, b, KD_NULL, 0);
#define VALID_ERR3(a, b, c) xmlRelaxNGAddValidError(ctxt, a, b, c, 0);
#define VALID_ERR2P(a, b) xmlRelaxNGAddValidError(ctxt, a, b, KD_NULL, 1);
                         ...
                         const xmlChar * arg2)
{
    ...
    switch (err) {
        ...
        case XML_RELAXNG_ERR_TYPE:
            kdSnprintfKHR (msg, 1000, "failed to validate type %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_TYPEVAL:
            kdSnprintfKHR (msg, 1000, "Type %s doesn't allow value '%s'\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_DUPID:
            kdSnprintfKHR (msg, 1000, "ID %s redefined\n", arg1);
            break;
        case XML_RELAXNG_ERR_TYPECMP:
            kdSnprintfKHR (msg, 1000, "failed to compare type %s\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_INTERNAL:
            kdSnprintfKHR (msg, 1000, "Internal error: %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_LISTEXTRA:
            kdSnprintfKHR (msg, 1000, "Extra data in list: %s\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_INTEREXTRA:
            kdSnprintfKHR (msg, 1000, "Extra element %s in interleave\n", arg1);
            break;
        case XML_RELAXNG_ERR_ELEMNAME:
            kdSnprintfKHR (msg, 1000, "Expecting element %s, got %s\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_ELEMNONS:
            kdSnprintfKHR (msg, 1000, "Expecting a namespace for element %s\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_ELEMWRONGNS:
            kdSnprintfKHR (msg, 1000,
                     "Element %s has wrong namespace: expecting %s\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_ELEMWRONG:
            kdSnprintfKHR (msg, 1000, "Did not expect element %s there\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_ELEMEXTRANS:
            kdSnprintfKHR (msg, 1000, "Expecting no namespace for element %s\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_ELEMNOTEMPTY:
            kdSnprintfKHR (msg, 1000, "Expecting element %s to be empty\n", arg1);
            break;
        case XML_RELAXNG_ERR_NOELEM:
            kdSnprintfKHR (msg, 1000, "Expecting an element %s, got nothing\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_NOTELEM:
            return (xmlCharStrdup("Expecting an element got text\n"));
        case XML_RELAXNG_ERR_ATTRVALID:
            kdSnprintfKHR (msg, 1000, "Element %s failed to validate attributes\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_CONTENTVALID:
            kdSnprintfKHR (msg, 1000, "Element %s failed to validate content\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_EXTRACONTENT:
            kdSnprintfKHR (msg, 1000, "Element %s has extra content: %s\n",
                     arg1, arg2);
            break;
        case XML_RELAXNG_ERR_INVALIDATTR:
            kdSnprintfKHR (msg, 1000, "Invalid attribute %s for element %s\n",
                     arg1, arg2);
            break;
        case XML_RELAXNG_ERR_LACKDATA:
            kdSnprintfKHR (msg, 1000, "Datatype element %s contains no data\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_DATAELEM:
            kdSnprintfKHR (msg, 1000, "Datatype element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_VALELEM:
            kdSnprintfKHR (msg, 1000, "Value element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_LISTELEM:
            kdSnprintfKHR (msg, 1000, "List element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_DATATYPE:
            kdSnprintfKHR (msg, 1000, "Error validating datatype %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_VALUE:
            kdSnprintfKHR (msg, 1000, "Error validating value %s\n", arg1);
            break;
    ...
    if (msg[0] == 0) {
        kdSnprintfKHR (msg, 1000, "Unknown error code %d\n", err);
    }
    msg[1000 - 1] = 0;
                         ...
                         const xmlChar * arg2)
{
    ...
    if (ctxt->errNo == XML_RELAXNG_OK)
        ctxt->errNo = err;
    xmlRngVErr(ctxt, (child == KD_NULL ? node : child), err,
                           ...
                           void *value)
{
    ...
    if ((type == KD_NULL) || (strval == KD_NULL))
        return (-1);
    typ = xmlSchemaGetPredefinedType(type,
                                     BAD_CAST
                                     "http://www.w3.org/2001/XMLSchema");
    ...
    facet->value = val;
    ret = xmlSchemaCheckFacet(facet, typ, KD_NULL, type);
    if (ret != 0) {
        xmlSchemaFreeFacet(facet);
        return (-1);
    }
                            ...
                            const xmlChar * value2, xmlNodePtr ctxt2)
{
    int ret;
    xmlSchemaTypePtr typ;
    xmlSchemaValPtr res1 = KD_NULL, res2 = KD_NULL;

    if ((type == KD_NULL) || (value1 == KD_NULL) || (value2 == KD_NULL))
        return (-1);
    typ = xmlSchemaGetPredefinedType(type,
                                     BAD_CAST
                                     "http://www.w3.org/2001/XMLSchema");
                             ...
                             xmlNodePtr ctxt2 ATTRIBUTE_UNUSED)
{
    ...
    if (xmlStrEqual(type, BAD_CAST "string")) {
        ...
        if (!xmlStrEqual(value1, value2)) {
             ...
             */
            nval = xmlRelaxNGNormalize(KD_NULL, value1);
            nvalue = xmlRelaxNGNormalize(KD_NULL, value2);

            if ((nval == KD_NULL) || (nvalue == KD_NULL))
                ret = -1;
...
static xmlHashTablePtr xmlRelaxNGRegisteredTypes = KD_NULL;
...
xmlRelaxNGFreeTypeLibrary(xmlRelaxNGTypeLibraryPtr lib,
                          const xmlChar * _namespace ATTRIBUTE_UNUSED)
{
    if (lib == KD_NULL)
        return;
    if (lib->_namespace != KD_NULL)
        xmlFree((xmlChar *) lib->_namespace);
    xmlFree(lib);
}
 ...
 */
static int
xmlRelaxNGRegisterTypeLibrary(const xmlChar * _namespace, void *data,
                              ...
                              xmlRelaxNGTypeFree freef)
{
    ...
    if ((xmlRelaxNGRegisteredTypes == KD_NULL) || (_namespace == KD_NULL) ||
        (check == KD_NULL) || (comp == KD_NULL))
        return (-1);
    if (xmlHashLookup(xmlRelaxNGRegisteredTypes, _namespace) != KD_NULL) {
        xmlGenericError(xmlGenericErrorContext,
    ...
    if (lib == KD_NULL) {
        xmlRngVErrMemory(KD_NULL, "adding types library\n");
        return (-1);
    }
    kdMemset(lib, 0, sizeof(xmlRelaxNGTypeLibrary));
    lib->_namespace = xmlStrdup(_namespace);
    lib->data = data;
    lib->have = have;
    lib->comp = comp;
    lib->check = check;
    lib->facet = facet;
    lib->freef = freef;
    ret = xmlHashAddEntry(xmlRelaxNGRegisteredTypes, _namespace, lib);
    if (ret < 0) {
        xmlGenericError(xmlGenericErrorContext,
                        "Relax-NG types library failed to register '%s'\n",
                        _namespace);
        xmlRelaxNGFreeTypeLibrary(lib, _namespace);
        return (-1);
    }
    return (0);
}
...
xmlRelaxNGInitTypes(void)
{
    ...
    if (xmlRelaxNGRegisteredTypes == KD_NULL) {
        xmlGenericError(xmlGenericErrorContext,
    ...
    xmlRelaxNGRegisterTypeLibrary(BAD_CAST
                                  ...
                                  KD_NULL, xmlRelaxNGSchemaTypeHave,
                                  xmlRelaxNGSchemaTypeCheck,
                                  xmlRelaxNGSchemaTypeCompare,
                                  xmlRelaxNGSchemaFacetCheck,
                                  xmlRelaxNGSchemaFreeValue);
    xmlRelaxNGRegisterTypeLibrary(xmlRelaxNGNs, KD_NULL,
                                  xmlRelaxNGDefaultTypeHave,
                                  xmlRelaxNGDefaultTypeCheck,
                                  xmlRelaxNGDefaultTypeCompare, KD_NULL,
                                  KD_NULL);
...
xmlRelaxNGIsCompileable(xmlRelaxNGDefinePtr def)
{
    ...
    switch (def->type) {
		 ...
		 */
             ...
             */
            if ((def->nameClass != KD_NULL) || (def->name == KD_NULL))
                ret = 0;
            else
                ret = 1;
            return (ret);
...
xmlRelaxNGCompile(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def)
{
    ...
    switch (def->type) {
        case XML_RELAXNG_START:
            ...
            break;
        case XML_RELAXNG_ELEMENT:
            if ((ctxt->am != KD_NULL) && (def->name != KD_NULL)) {
        ...
        case XML_RELAXNG_ZEROORMORE:{
                ...
                ctxt->state =
                    xmlAutomataNewEpsilon(ctxt->am, ctxt->state, KD_NULL);
                oldstate = ctxt->state;
                list = def->content;
                while (list != KD_NULL) {
                    xmlRelaxNGCompile(ctxt, list);
                ...
                ctxt->state =
                    xmlAutomataNewEpsilon(ctxt->am, oldstate, KD_NULL);
                break;
            }
        case XML_RELAXNG_ONEORMORE:{
                xmlAutomataStatePtr oldstate;
        ...
        case XML_RELAXNG_CHOICE:{
                xmlAutomataStatePtr target = KD_NULL;
                xmlAutomataStatePtr oldstate = ctxt->state;
        ...
        case XML_RELAXNG_VALUE:
            ...
            kdPrintf("RNG internal error trying to compile %s\n",
                    xmlRelaxNGDefName(def));
            break;
    }
    return (ret);
...
xmlRelaxNGIsNullable(xmlRelaxNGDefinePtr define)
{
    ...
    if (define->dflags & IS_KD_NULLABLE)
        return (1);
    if (define->dflags & IS_NOT_KD_NULLABLE)
        return (0);
    switch (define->type) {
        case XML_RELAXNG_EMPTY:
        case XML_RELAXNG_TEXT:
    ...
    if (ret == 0)
        define->dflags |= IS_NOT_KD_NULLABLE;
    if (ret == 1)
        define->dflags |= IS_KD_NULLABLE;
    return (ret);
}
 ...
 *
 * Returns the datatypeLibary value or KD_NULL if not found
 */
static xmlChar *
 ...
 *
 * Returns the definition pointer or KD_NULL in case of error
 */
static xmlRelaxNGDefinePtr
xmlRelaxNGParseValue(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    xmlRelaxNGDefinePtr def = KD_NULL;
    xmlRelaxNGTypeLibraryPtr lib = KD_NULL;
    xmlChar *type;
    xmlChar *library;
    int success = 0;
    ...
    if (node->children == KD_NULL) {
        ...
        if (def->value == KD_NULL) {
            ...
            if (success != 1) {
                ...
                if (val != KD_NULL)
                    def->attrs = (xmlRelaxNGDefine*) val;
            }
        }
    }
    return (def);
...
{
    xmlRelaxNGDefinePtr def = KD_NULL, except_;
    xmlRelaxNGDefinePtr param, lastparam = KD_NULL;
    xmlRelaxNGTypeLibraryPtr lib;
    xmlChar *type;
    xmlChar *library;
    xmlNodePtr content;
    ...
    if ((content != KD_NULL)
        ...
        xmlNodePtr child;
        xmlRelaxNGDefinePtr tmp2, last = KD_NULL;

        except_ = xmlRelaxNGNewDefine(ctxt, node);
        if (except_ == KD_NULL) {
            return (def);
        }
        except_->type = XML_RELAXNG_EXCEPT;
                             ...
                             xmlRelaxNGDefinePtr def2)
{
    ...
    kdMemset(&ctxt, 0, sizeof(xmlRelaxNGValidCtxt));
 ...
 *
 * Returns a list of elements or KD_NULL if none was found.
 */
static xmlRelaxNGDefinePtr *
xmlRelaxNGGetElements(xmlRelaxNGParserCtxtPtr ctxt,
                      xmlRelaxNGDefinePtr def, int eora)
{
    xmlRelaxNGDefinePtr *ret = KD_NULL, parent, cur, tmp;
    int len = 0;
    int max = 0;
    ...
    while (cur != KD_NULL) {
        if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
	        ...
	        xmlRelaxNGDefinePtr *temp;
                ...
                max *= 2;
                temp = (xmlRelaxNGDefine**) xmlRealloc(ret,
                                 ...
                                 xmlRelaxNGDefinePtr def)
{
    ...
    xmlHashTablePtr triage = KD_NULL;
    int is_triable = 1;

    if ((def == KD_NULL) || (def->type != XML_RELAXNG_CHOICE))
        return;
    ...
    while (cur != KD_NULL) {
        ...
        if ((list[i] == KD_NULL) || (list[i][0] == KD_NULL)) {
            is_triable = 0;
            ...
            tmp = list[i];
            while ((*tmp != KD_NULL) && (is_triable == 1)) {
                if ((*tmp)->type == XML_RELAXNG_TEXT) {
                    res = xmlHashAddEntry2(triage,
                                           BAD_CAST "#text", KD_NULL,
                                           (void *) cur);
                    if (res != 0)
                        is_triable = -1;
                } else if (((*tmp)->type == XML_RELAXNG_ELEMENT) &&
                           ((*tmp)->name != KD_NULL)) {
                    if (((*tmp)->ns == KD_NULL) || ((*tmp)->ns[0] == 0))
                        res = xmlHashAddEntry2(triage,
                                               (*tmp)->name, KD_NULL,
                                               (void *) cur);
                    else
    ...
    if (is_triable == 1) {
        ...
        def->data = triage;
    } else if (triage != KD_NULL) {
        xmlHashFree(triage, KD_NULL);
    }
                             ...
                             xmlChar * name ATTRIBUTE_UNUSED)
{
    ...
    xmlRelaxNGPartitionPtr partitions = KD_NULL;
    xmlRelaxNGInterleaveGroupPtr *groups = KD_NULL;
    xmlRelaxNGInterleaveGroupPtr group;
    int i, j, ret, res;
    int nbgroups = 0;
    int nbchild = 0;
    ...
    if (partitions == KD_NULL)
        goto error;
    kdMemset(partitions, 0, sizeof(xmlRelaxNGPartition));
...
{
    xmlRelaxNGDefinePtr def = KD_NULL;
    xmlRelaxNGDefinePtr last = KD_NULL, cur;
    xmlNodePtr child;
    ...
    if (ctxt->interleaves == KD_NULL) {
        ...
        kdSnprintfKHR (name, 32, "interleave%d", ctxt->nbInterleaves++);
...
xmlRelaxNGParseInclude(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    incl = (xmlRelaxNGIncludePtr) node->psvi;
    if (incl == KD_NULL) {
...
xmlRelaxNGParseImportRef(void *payload, void *data, xmlChar *name) {
    ...
    if (tmp < 0) {
        ...
        if (prev == KD_NULL) {
            if (def->name != KD_NULL) {
                xmlRngPErr(ctxt, KD_NULL, XML_RNGP_REF_CREATE_FAILED,
...
xmlRelaxNGParseImportRefs(xmlRelaxNGParserCtxtPtr ctxt,
    ...
    if ((ctxt == KD_NULL) || (grammar == KD_NULL) || (ctxt->grammar == KD_NULL))
        return(-1);
    if (grammar->refs == KD_NULL)
        return(0);
    if (ctxt->grammar->refs == KD_NULL)
 ...
 *
 * Returns the xmlRelaxNGDefinePtr or KD_NULL in case of error
 */
static xmlRelaxNGDefinePtr
xmlRelaxNGProcessExternalRef(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    docu = (xmlRelaxNGDocumentPtr) node->psvi;
    if (docu != KD_NULL) {
        def = xmlRelaxNGNewDefine(ctxt, node);
        if (def == KD_NULL)
            return (KD_NULL);
        ...
        if (docu->content == KD_NULL) {
            ...
            if (ns == KD_NULL) {
                tmp = node;
                while ((tmp != KD_NULL) && (tmp->type == XML_ELEMENT_NODE)) {
            ...
            ctxt->flags = oldflags;
            if ((docu->schema != KD_NULL) &&
                (docu->schema->topgrammar != KD_NULL)) {
 ...
 *
 * Returns the definition pointer or KD_NULL in case of error or if no
    ...
    if (IS_RELAXNG(node, "element")) {
        ...
        if (node->children == KD_NULL) {
            ...
            if (def != KD_NULL) {
                ...
                if ((def->content != KD_NULL) && (def->content->next != KD_NULL)) {
...
{
    xmlRelaxNGDefinePtr ret, cur, last = KD_NULL;
    xmlNodePtr child;
    ...
    while (child != KD_NULL) {
        ...
        if (xmlRelaxNGParseNameClass(ctxt, child, cur) != KD_NULL) {
            if (last == KD_NULL) {
                ret->content = cur;
            } else {
                last->next = cur;
                         ...
                         xmlRelaxNGDefinePtr def)
{
    ...
    if (IS_RELAXNG(node, "name")) {
        ...
        xmlNodePtr child;
        xmlRelaxNGDefinePtr last = KD_NULL;
        ...
        xmlRngPErr(ctxt, node, XML_RNGP_CHOICE_CONTENT,
                   ...
                   (node == KD_NULL ? (const xmlChar *) "nothing" : node->name),
		   KD_NULL);
        return (KD_NULL);
    }
    if (ret != def) {
        if (def->nameClass == KD_NULL) {
            def->nameClass = ret;
        } else {
            tmp = def->nameClass;
            while (tmp->next != KD_NULL) {
                tmp = tmp->next;
            }
            tmp->next = ret;
        }
...
xmlRelaxNGParsePatterns(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes,
                        int group)
{
    xmlRelaxNGDefinePtr def = KD_NULL, last = KD_NULL, cur, parent;
...
{
    int ret = 0;
    xmlRelaxNGDefinePtr def = KD_NULL, last;
                         ...
                         const xmlChar * name)
{
    ...
    if (grammar->defs != KD_NULL) {
        def = (xmlRelaxNGDefine*) xmlHashLookup(grammar->defs, name);
        if (def != KD_NULL) {
            cur = ref;
            while (cur != KD_NULL) {
                cur->content = def;
                       ...
                       xmlRelaxNGParserCtxtPtr ctxt, const xmlChar * name)
{
    ...
    if (choiceOrInterleave == 0) {
        ...
        if (ctxt->interleaves == KD_NULL) {
            ...
            kdSnprintfKHR (tmpname, 32, "interleave%d", ctxt->nbInterleaves++);
            if (xmlHashAddEntry(ctxt->interleaves, BAD_CAST tmpname, cur) <
                0) {
                xmlRngPErr(ctxt, define->node, XML_RNGP_INTERLEAVE_CREATE_FAILED,
                           "Failed to add %s to hash table\n",
			   (const xmlChar *) tmpname, KD_NULL);
            }
        }
    }
}
                       ...
                       xmlRelaxNGGrammarPtr grammar)
{
    ...
    if ((starts == KD_NULL) || (starts->next == KD_NULL))
        return;
    cur = starts;
    while (cur != KD_NULL) {
        if ((cur->node == KD_NULL) || (cur->node->parent == KD_NULL) ||
...
{
    xmlRelaxNGDefinePtr prev = KD_NULL;
    ...
    while (cur != KD_NULL) {
        if ((cur->type == XML_RELAXNG_REF) ||
            ...
            if ((parent != KD_NULL) && (parent->type == XML_RELAXNG_CHOICE)) {
            ...
            if (cur->content != KD_NULL)
                xmlRelaxNGSimplify(ctxt, cur->content, cur);
            if ((cur->type != XML_RELAXNG_VALUE) && (cur->attrs != KD_NULL))
                xmlRelaxNGSimplify(ctxt, cur->attrs, cur);
            if (cur->nameClass != KD_NULL)
            ...
            if (cur->type == XML_RELAXNG_ELEMENT) {
                ...
                pre = cur->content;
                while ((pre != KD_NULL) && (pre->next != KD_NULL)) {
                    tmp = pre->next;
            ...
            if ((cur->type == XML_RELAXNG_GROUP) ||
                ...
                if (cur->content == KD_NULL)
                    cur->type = XML_RELAXNG_EMPTY;
                else if (cur->content->next == KD_NULL) {
                    if ((parent == KD_NULL) && (prev == KD_NULL)) {
                        cur->type = XML_RELAXNG_NOOP;
                    } else if (prev == KD_NULL) {
            ...
            if (cur->content == KD_NULL) {
                if (cur->type == XML_RELAXNG_PARENTREF)
                    ...
                    xmlRngPErr(ctxt, cur->node, XML_RNGP_REF_NO_DEF,
                               ...
                               (cur->name ? cur->name: BAD_CAST "null"), KD_NULL);
            }
            if (cur->depth > -4) {
                cur->depth = -4;
...
xmlRelaxNGParseGrammar(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr nodes)
{
    ...
    } else if (ctxt->grammar->start == KD_NULL) {
...
xmlRelaxNGParseDocument(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    ctxt->define = olddefine;
    if (schema->topgrammar->start != KD_NULL) {
        xmlRelaxNGCheckCycles(ctxt, schema->topgrammar->start, 0);
        if ((ctxt->flags & XML_RELAXNG_IN_EXTERNALREF) == 0) {
            xmlRelaxNGSimplify(ctxt, schema->topgrammar->start, KD_NULL);
            while ((schema->topgrammar->start != KD_NULL) &&
                   (schema->topgrammar->start->type == XML_RELAXNG_NOOP) &&
                   (schema->topgrammar->start->next != KD_NULL))
...
xmlRelaxNGNewParserCtxt(const char *URL)
{
    ...
    if (ret == KD_NULL) {
        xmlRngPErrMemory(KD_NULL, "building parser\n");
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlRelaxNGParserCtxt));
...
xmlRelaxNGFreeParserCtxt(xmlRelaxNGParserCtxtPtr ctxt)
{
    ...
    if (ctxt->interleaves != KD_NULL)
        xmlHashFree(ctxt->interleaves, KD_NULL);
    if (ctxt->documents != KD_NULL)
        xmlRelaxNGFreeDocumentList(ctxt->documents);
    if (ctxt->includes != KD_NULL)
    ...
    if ((ctxt->document != KD_NULL) && (ctxt->freedoc))
        ...
        if (cur->type == XML_ELEMENT_NODE) {
		    ...
		    xmlFreeURI(uri);
                           ...
                           (xmlStrEqual(cur->name, BAD_CAST "attribute")))
                {
                     ...
                     * and attribute elements
                     */
                    if (xmlHasProp(cur, BAD_CAST "ns") == KD_NULL) {
                        xmlNodePtr node;
                        xmlChar *ns = KD_NULL;
                ...
                if (xmlStrEqual(cur->name, BAD_CAST "div")) {
                    ...
                    if (ns != KD_NULL)
                        xmlFree(ns);
		    /*
		     * Since we are about to delete cur, if it's nsDef is non-KD_NULL we
 ...
 * Include and externalRef lookups.
 *
 * Returns the cleaned up document or KD_NULL in case of error
 */
static xmlDocPtr
...
xmlRelaxNGParse(xmlRelaxNGParserCtxtPtr ctxt)
{
     ...
     */
    if (ctxt->URL != KD_NULL) {
        doc = xmlReadFile((const char *) ctxt->URL,KD_NULL,0);
        if (doc == KD_NULL) {
            xmlRngPErr(ctxt, KD_NULL, XML_RNGP_PARSE_ERROR,
    ...
    } else if (ctxt->buffer != KD_NULL) {
        doc = xmlReadMemory(ctxt->buffer, ctxt->size,KD_NULL,KD_NULL,0);
        if (doc == KD_NULL) {
            xmlRngPErr(ctxt, KD_NULL, XML_RNGP_PARSE_ERROR,
    ...
    } else if (ctxt->document != KD_NULL) {
        doc = ctxt->document;
    } else {
        xmlRngPErr(ctxt, KD_NULL, XML_RNGP_EMPTY,
    ...
    if (root == KD_NULL) {
        xmlRngPErr(ctxt, (xmlNodePtr) doc,
	           XML_RNGP_EMPTY, "xmlRelaxNGParse: %s is empty\n",
                   (ctxt->URL ? ctxt->URL : BAD_CAST "schemas"), KD_NULL);
    ...
    /*
     * if there was a parsing error return KD_NULL
     */
    if (ctxt->nbErrors > 0) {
        xmlRelaxNGFree(ret);
        ctxt->document = KD_NULL;
     ...
     */
    if ((ret->topgrammar != KD_NULL) && (ret->topgrammar->start != KD_NULL)) {
        if (ret->topgrammar->start->type != XML_RELAXNG_START) {
            xmlRelaxNGDefinePtr def;

            def = xmlRelaxNGNewDefine(ctxt, KD_NULL);
            if (def != KD_NULL) {
    ...
    ctxt->documents = KD_NULL;

    ret->includes = ctxt->includes;
    ctxt->includes = KD_NULL;
 ...
 * 									*
 ************************************************************************/
static void xmlRelaxNGDumpDefine(KDFile * output,
                                 xmlRelaxNGDefinePtr define);
 ...
 */
static void
xmlRelaxNGDumpDefines(KDFile * output, xmlRelaxNGDefinePtr defines)
{
    while (defines != KD_NULL) {
 ...
 */
static void
xmlRelaxNGDumpDefine(KDFile * output, xmlRelaxNGDefinePtr define)
{
    if (define == KD_NULL)
        return;
    switch (define->type) {
        case XML_RELAXNG_EMPTY:
            kdFprintfKHR (output, "<empty/>\n");
            break;
        case XML_RELAXNG_NOT_ALLOWED:
            kdFprintfKHR (output, "<notAllowed/>\n");
            break;
        case XML_RELAXNG_TEXT:
            kdFprintfKHR (output, "<text/>\n");
            break;
        case XML_RELAXNG_ELEMENT:
            kdFprintfKHR (output, "<element>\n");
            if (define->name != KD_NULL) {
                kdFprintfKHR (output, "<name");
                if (define->ns != KD_NULL)
                    kdFprintfKHR (output, " ns=\"%s\"", define->ns);
                kdFprintfKHR (output, ">%s</name>\n", define->name);
            }
            ...
            break;
        case XML_RELAXNG_LIST:
            kdFprintfKHR (output, "<list>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</list>\n");
            break;
        case XML_RELAXNG_ONEORMORE:
            kdFprintfKHR (output, "<oneOrMore>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</oneOrMore>\n");
            break;
        case XML_RELAXNG_ZEROORMORE:
            kdFprintfKHR (output, "<zeroOrMore>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</zeroOrMore>\n");
            break;
        case XML_RELAXNG_CHOICE:
            kdFprintfKHR (output, "<choice>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</choice>\n");
            break;
        case XML_RELAXNG_GROUP:
            kdFprintfKHR (output, "<group>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</group>\n");
            break;
        case XML_RELAXNG_INTERLEAVE:
            kdFprintfKHR (output, "<interleave>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</interleave>\n");
            break;
        case XML_RELAXNG_OPTIONAL:
            kdFprintfKHR (output, "<optional>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</optional>\n");
            break;
        case XML_RELAXNG_ATTRIBUTE:
            kdFprintfKHR (output, "<attribute>\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</attribute>\n");
            break;
        case XML_RELAXNG_DEF:
            kdFprintfKHR (output, "<define");
            if (define->name != KD_NULL)
                kdFprintfKHR (output, " name=\"%s\"", define->name);
            kdFprintfKHR (output, ">\n");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</define>\n");
            break;
        case XML_RELAXNG_REF:
            kdFprintfKHR (output, "<ref");
            if (define->name != KD_NULL)
                kdFprintfKHR (output, " name=\"%s\"", define->name);
            kdFprintfKHR (output, ">\n");
            xmlRelaxNGDumpDefines(output, define->content);
        ...
        case XML_RELAXNG_PARENTREF:
            ...
            break;
        case XML_RELAXNG_EXTERNALREF:
            kdFprintfKHR (output, "<externalRef>");
            xmlRelaxNGDumpDefines(output, define->content);
            kdFprintfKHR (output, "</externalRef>\n");
            break;
        case XML_RELAXNG_DATATYPE:
 ...
 */
static void
xmlRelaxNGDumpGrammar(KDFile * output, xmlRelaxNGGrammarPtr grammar, int top)
{
    if (grammar == KD_NULL)
        return;

    kdFprintfKHR (output, "<grammar");
    if (top)
        kdFprintfKHR (output, " xmlns=\"http://relaxng.org/ns/structure/1.0\"");
    switch (grammar->combine) {
        case XML_RELAXNG_COMBINE_UNDEFINED:
            break;
        case XML_RELAXNG_COMBINE_CHOICE:
            kdFprintfKHR (output, " combine=\"choice\"");
            break;
        case XML_RELAXNG_COMBINE_INTERLEAVE:
            kdFprintfKHR (output, " combine=\"interleave\"");
            break;
        default:
            kdFprintfKHR (output, " <!-- invalid combine value -->");
    }
    kdFprintfKHR (output, ">\n");
    if (grammar->start == KD_NULL) {
        kdFprintfKHR (output, " <!-- grammar had no start -->");
    } else {
        kdFprintfKHR (output, "<start>\n");
 ...
 */
void
xmlRelaxNGDump(KDFile * output, xmlRelaxNGPtr schema)
{
    if (output == KD_NULL)
        return;
    if (schema == KD_NULL) {
        kdFprintfKHR (output, "RelaxNG empty or failed to compile\n");
        return;
    }
    kdFprintfKHR (output, "RelaxNG: ");
    if (schema->doc == KD_NULL) {
        kdFprintfKHR (output, "no document\n");
    } else if (schema->doc->URL != KD_NULL) {
        kdFprintfKHR (output, "%s\n", schema->doc->URL);
    } else {
        kdFprintfKHR (output, "\n");
    }
    if (schema->topgrammar == KD_NULL) {
        kdFprintfKHR (output, "RelaxNG has no top grammar\n");
        return;
    }
 ...
 */
void
xmlRelaxNGDumpTree(KDFile * output, xmlRelaxNGPtr schema)
{
    if (output == KD_NULL)
        return;
    if (schema == KD_NULL) {
...
#endif
    if (ctxt == KD_NULL) {
        kdPrintf("callback on %s missing context\n", token);
        return;
    }
    if (define == KD_NULL) {
        if (token[0] == '#')
            return;
        kdPrintf("callback on %s missing define\n", token);
        if ((ctxt != KD_NULL) && (ctxt->errNo == XML_RELAXNG_OK))
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    }
    if ((ctxt == KD_NULL) || (define == KD_NULL)) {
        kdPrintf("callback on %s missing info\n", token);
        if ((ctxt != KD_NULL) && (ctxt->errNo == XML_RELAXNG_OK))
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    } else if (define->type != XML_RELAXNG_ELEMENT) {
        kdPrintf("callback on %s define is not element\n", token);
        if (ctxt->errNo == XML_RELAXNG_OK)
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    }
                                  ...
                                  xmlRegexpPtr regexp, xmlNodePtr content)
{
    ...
    if ((ctxt == KD_NULL) || (regexp == KD_NULL))
        return (-1);
    oldperr = ctxt->perr;
 ...
 *
 * Returns the exec or KD_NULL if empty
 */
...
xmlRelaxNGElemPop(xmlRelaxNGValidCtxtPtr ctxt)
{
    ...
    ctxt->elemTab[ctxt->elemNr] = KD_NULL;
    if (ctxt->elemNr > 0)
                              ...
                              xmlNodePtr elem)
{
    ...
    if (ctxt->elem == 0) {
        ...
        if ((grammar == KD_NULL) || (grammar->start == KD_NULL)) {
            VALID_ERR(XML_RELAXNG_ERR_NOGRAMMAR);
            return (-1);
        }
                            ...
                            const xmlChar * data, int len ATTRIBUTE_UNUSED)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->elem == KD_NULL) || (data == KD_NULL))
        return (-1);
                             ...
                             xmlNodePtr elem)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->elem == KD_NULL) || (elem == KD_NULL))
        return (-1);
#ifdef DEBUG_PROGRESSIVE
                              ...
                              xmlNodePtr elem)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->pdef == KD_NULL) || (elem == KD_NULL))
        return (-1);
#ifdef DEBUG_PROGRESSIVE
 ...
 *
 * Returns the new sibling or KD_NULL in case of error.
 */
static xmlNodePtr
 ...
 *
 * Returns the new string or KD_NULL in case of error.
 */
static xmlChar *
                           ...
                           xmlRelaxNGDefinePtr define, xmlNodePtr node)
{
    ...
    if ((define == KD_NULL) || (define->data == KD_NULL)) {
        return (-1);
    }
    lib = (xmlRelaxNGTypeLibraryPtr) define->data;
    if (lib->check != KD_NULL) {
        if ((define->attrs != KD_NULL) &&
            (define->attrs->type == XML_RELAXNG_PARAM)) {
            ret =
                lib->check(lib->data, define->name, value, &result, node);
        } else {
            ret = lib->check(lib->data, define->name, value, KD_NULL, node);
        }
    } else
        ret = -1;
    if (ret < 0) {
        VALID_ERR2(XML_RELAXNG_ERR_TYPE, define->name);
        if ((result != KD_NULL) && (lib != KD_NULL) && (lib->freef != KD_NULL))
    ...
    cur = define->attrs;
    while ((ret == 0) && (cur != KD_NULL) && (cur->type == XML_RELAXNG_PARAM)) {
        if (lib->facet != KD_NULL) {
...
xmlRelaxNGNextValue(xmlRelaxNGValidCtxtPtr ctxt)
{
    ...
    if ((cur == KD_NULL) || (ctxt->state->endvalue == KD_NULL)) {
        ctxt->state->value = KD_NULL;
        ctxt->state->endvalue = KD_NULL;
        return (0);
    }
    while (*cur != 0)
                        ...
                        xmlRelaxNGDefinePtr define)
{
    ...
    switch (define->type) {
        ...
        case XML_RELAXNG_VALUE:{
                if (!xmlStrEqual(value, define->value)) {
                    if (define->name != KD_NULL) {
                        ...
                        lib = (xmlRelaxNGTypeLibraryPtr) define->data;
                        if ((lib != KD_NULL) && (lib->comp != KD_NULL)) {
        ...
        case XML_RELAXNG_LIST:{
                ...
                if ((ret == 0) && (ctxt->state->value != KD_NULL) &&
        ...
        case XML_RELAXNG_ZEROORMORE:{
                ...
                cur = ctxt->state->value;
                temp = KD_NULL;
                while ((cur != KD_NULL) && (cur != ctxt->state->endvalue) &&
                       (temp != cur)) {
                    temp = cur;
                    ret =
                            ...
                            xmlRelaxNGDefinePtr define)
{
    ...
    if (define->name != KD_NULL) {
        for (i = 0; i < ctxt->state->nbAttrs; i++) {
            ...
            if ((tmp != KD_NULL) && (xmlStrEqual(define->name, tmp->name))) {
                if ((((define->ns == KD_NULL) || (define->ns[0] == 0)) &&
                     (tmp->ns == KD_NULL)) ||
                    ((tmp->ns != KD_NULL) &&
                                ...
                                xmlRelaxNGDefinePtr defines)
{
    ...
    while (cur != KD_NULL) {
        if (cur->type != XML_RELAXNG_ATTRIBUTE) {
            if ((ctxt->state != KD_NULL) || (ctxt->states != KD_NULL)) {
 ...
 * @node:  the node
 * @list:  a KD_NULL terminated array of definitions
 *
...
xmlRelaxNGNodeMatchesList(xmlNodePtr node, xmlRelaxNGDefinePtr * list)
{
    ...
    while (cur != KD_NULL) {
        if ((node->type == XML_ELEMENT_NODE) &&
            (cur->type == XML_RELAXNG_ELEMENT)) {
            tmp = xmlRelaxNGElementMatch(KD_NULL, cur, node);
            if (tmp == 1)
                return (1);
                             ...
                             xmlRelaxNGDefinePtr define)
{
    ...
    xmlRelaxNGInterleaveGroupPtr group = KD_NULL;
    xmlNodePtr cur, start, last = KD_NULL, lastchg = KD_NULL, lastelem;
    xmlNodePtr *list = KD_NULL, *lasts = KD_NULL;

    if (define->data != KD_NULL) {
    ...
    kdMemset(list, 0, nbgroups * sizeof(xmlNodePtr));
    lasts = (xmlNodePtr *) xmlMalloc(nbgroups * sizeof(xmlNodePtr));
    if (lasts == KD_NULL) {
        xmlRngVErrMemory(ctxt, "validating\n");
        return (-1);
    }
    kdMemset(lasts, 0, nbgroups * sizeof(xmlNodePtr));
    ...
    for (i = 0; i < nbgroups; i++) {
		...
		if (best != -1) {
		    ...
		    ctxt->states->tabState[best] = KD_NULL;
		} else {
		    oldstate =
			ctxt->states->tabState[ctxt->states->nbState - 1];
                    ctxt->states->tabState[ctxt->states->nbState - 1] = KD_NULL;
                    ctxt->states->nbState--;
		}
            }
            for (j = 0; j < ctxt->states->nbState ; j++) {
                                 ...
                                 xmlRelaxNGDefinePtr defines)
{
    ...
    if (defines == KD_NULL) {
        VALID_ERR2(XML_RELAXNG_ERR_INTERNAL,
                   BAD_CAST "KD_NULL definition list");
        return (-1);
    }
    while (defines != KD_NULL) {
        if ((ctxt->state != KD_NULL) || (ctxt->states != KD_NULL)) {
                       ...
                       xmlRelaxNGDefinePtr define, xmlNodePtr elem)
{
    ...
    } else if ((elem->ns != KD_NULL) && (define->ns != KD_NULL) &&
               (define->name == KD_NULL)) {
        VALID_ERR2(XML_RELAXNG_ERR_ELEMEXTRANS, elem->name);
        return (0);
    } else if ((elem->ns != KD_NULL) && (define->name != KD_NULL)) {
...
xmlRelaxNGBestState(xmlRelaxNGValidCtxtPtr ctxt)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->states == KD_NULL) ||
                        ...
                        xmlRelaxNGDefinePtr define)
{
    ...
    xmlRelaxNGValidStatePtr oldstate = KD_NULL, state;
    ...
    if ((node != KD_NULL) && (node->name != KD_NULL))
    ...
    switch (define->type) {
			...
			ctxt->state->seq = nseq;
                ...
                if (ctxt->states != KD_NULL) {
                    ...
                    for (i = 0; i < ctxt->states->nbState; i++) {
                        ...
                        ctxt->states->tabState[i] = KD_NULL;
                    }
                    xmlRelaxNGFreeStates(ctxt, ctxt->states);
                    ctxt->flags = oldflags;
                    ctxt->states = KD_NULL;
        ...
        case XML_RELAXNG_ZEROORMORE:{
                int progress;
                xmlRelaxNGStatesPtr states = KD_NULL, res = KD_NULL;
                int base, j;
        ...
        case XML_RELAXNG_CHOICE:{
                xmlRelaxNGDefinePtr list = KD_NULL;
                xmlRelaxNGStatesPtr states = KD_NULL;

                node = xmlRelaxNGSkipIgnored(ctxt, node);

                errNr = ctxt->errNr;
                if ((define->dflags & IS_TRIABLE) && (define->data != KD_NULL) &&
		    (node != KD_NULL)) {
		    /*
		     * node == KD_NULL can't be optimized since IS_TRIABLE
		     ...
		     */
                    ...
                    if ((node->type == XML_TEXT_NODE) ||
                        (node->type == XML_CDATA_SECTION_NODE)) {
                        list = (xmlRelaxNGDefine*)
                            xmlHashLookup2(triage, BAD_CAST "#text", KD_NULL);
                    } else if (node->type == XML_ELEMENT_NODE) {
                        if (node->ns != KD_NULL) {
                            list = (xmlRelaxNGDefine*) xmlHashLookup2(triage, node->name,
                                                  node->ns->href);
                            if (list == KD_NULL)
                                list = (xmlRelaxNGDefine*)
                            ...
                            list = (xmlRelaxNGDefine*)
                                xmlHashLookup2(triage, node->name, KD_NULL);
                        if (list == KD_NULL)
                            list = (xmlRelaxNGDefine*)
                                xmlHashLookup2(triage, BAD_CAST "#any",
                                               KD_NULL);
                             ...
                             xmlRelaxNGDefinePtr define)
{
    ...
    if ((ctxt->states == KD_NULL) || (ctxt->states->nbState == 1)) {
        if (ctxt->states != KD_NULL) {
...
xmlRelaxNGValidateDocument(xmlRelaxNGValidCtxtPtr ctxt, xmlDocPtr doc)
{
    ...
    if ((ctxt == KD_NULL) || (ctxt->schema == KD_NULL) || (doc == KD_NULL))
        return (-1);
    ...
    state = xmlRelaxNGNewValidState(ctxt, KD_NULL);
    ctxt->state = state;
    ret = xmlRelaxNGValidateDefinition(ctxt, grammar->start);
    if ((ctxt->state != KD_NULL) && (state->seq != KD_NULL)) {
        state = ctxt->state;
        node = state->seq;
        node = xmlRelaxNGSkipIgnored(ctxt, node);
        if (node != KD_NULL) {
...
xmlRelaxNGNewValidCtxt(xmlRelaxNGPtr schema)
{
    ...
    if (ret == KD_NULL) {
        xmlRngVErrMemory(KD_NULL, "building context\n");
        return (KD_NULL);
    }
    kdMemset(ret, 0, sizeof(xmlRelaxNGValidCtxt));
    ret->schema = schema;
    ret->error = xmlGenericError;
    ret->userData = xmlGenericErrorContext;
    ret->errNr = 0;
    ...
    ret->states = KD_NULL;
    ret->freeState = KD_NULL;
    ret->freeStates = KD_NULL;
...
xmlRelaxNGFreeValidCtxt(xmlRelaxNGValidCtxtPtr ctxt)
{
    ...
    if (ctxt == KD_NULL)
        return;
    if (ctxt->states != KD_NULL)
        xmlRelaxNGFreeStates(KD_NULL, ctxt->states);
    if (ctxt->freeState != KD_NULL) {
        for (k = 0; k < ctxt->freeState->nbState; k++) {
            xmlRelaxNGFreeValidState(KD_NULL, ctxt->freeState->tabState[k]);
        }
        xmlRelaxNGFreeStates(KD_NULL, ctxt->freeState);
    }
    if (ctxt->freeStates != KD_NULL) {
        for (k = 0; k < ctxt->freeStatesNr; k++) {
            xmlRelaxNGFreeStates(KD_NULL, ctxt->freeStates[k]);
        }
