...
htmlIsBooleanAttr(const xmlChar *name)
{
    ...
    while (htmlBooleanAttrs[i] != KD_NULL) {
 ...
 * Dump an HTML node, recursive behaviour,children are printed too.
 *
 * TODO: if encoding == KD_NULL try to save in the doc encoding
 *
 * returns: the number of byte written or -1 in case of failure.
 */
int
htmlNodeDumpFileFormat(KDFile *out, xmlDocPtr doc,
 ...
 */
void
htmlNodeDumpFile(KDFile *out, xmlDocPtr doc, xmlNodePtr cur) {
    htmlNodeDumpFileFormat(out, doc, cur, KD_NULL, 1);
}
...
htmlDtdDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
    ...
    if (cur == KD_NULL) {
	htmlSaveErr(XML_SAVE_NO_DOCTYPE, (xmlNodePtr) doc, KD_NULL);
	return;
    }
...
htmlAttrDumpOutput(xmlOutputBufferPtr buf, xmlDocPtr doc, xmlAttrPtr cur,
    ...
    xmlOutputBufferWriteString(buf, (const char *)cur->name);
    if ((cur->children != KD_NULL) && (!htmlIsBooleanAttr(cur->name))) {
	value = xmlNodeListGetString(doc, cur->children, 0);
	if (value) {
	    xmlOutputBufferWriteString(buf, "=");
	    if ((cur->ns == KD_NULL) && (cur->parent != KD_NULL) &&
		(cur->parent->ns == KD_NULL) &&
...
htmlNodeDumpFormatOutput(xmlOutputBufferPtr buf, xmlDocPtr doc,
    ...
    if ((info != KD_NULL) && (info->empty)) {
        xmlOutputBufferWriteString(buf, ">");
	if ((format) && (!info->isinline) && (cur->next != KD_NULL)) {
	...
	(cur->children == KD_NULL)) {
        if ((info != KD_NULL) && (info->saveEndTag != 0) &&
	...
	if ((format) && (cur->next != KD_NULL) &&
            (info != KD_NULL) && (!info->isinline)) {
	    if ((cur->next->type != HTML_TEXT_NODE) &&
		(cur->next->type != HTML_ENTITY_REF_NODE) &&
		(cur->parent != KD_NULL) &&
		(cur->parent->name != KD_NULL) &&
    ...
    if (cur->children != KD_NULL) {
        if ((format) && (info != KD_NULL) && (!info->isinline) &&
...
htmlDocContentDumpFormatOutput(xmlOutputBufferPtr buf, xmlDocPtr cur,
    ...
    if (cur->intSubset != KD_NULL) {
        htmlDtdDumpOutput(buf, cur, KD_NULL);
    }
    if (cur->children != KD_NULL) {
 ...
 */
int
htmlDocDump(KDFile *f, xmlDocPtr cur) {
    xmlOutputBufferPtr buf;
    xmlCharEncodingHandlerPtr handler = KD_NULL;
    const char *encoding;
    int ret;
    ...
    htmlDocContentDumpOutput(buf, cur, KD_NULL);
...
htmlSaveFile(const char *filename, xmlDocPtr cur) {
    ...
    if ((cur == KD_NULL) || (filename == KD_NULL))
        return(-1);
