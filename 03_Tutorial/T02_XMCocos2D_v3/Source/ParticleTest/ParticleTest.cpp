...
void AddAndDeleteParticleSystems::onEnter()
{
    ...
    for (int i = 0; i<6; i++) {
        ...
        unsigned int kdRandZ = kdRand() % 100; 
        _batchNode->addChild(particleSystem, kdRandZ, -1);
...
void AddAndDeleteParticleSystems::removeSystem(float dt)
{
    ...
    if (nChildrenCount > 0) 
    {
        CCLOG("remove kdRandom system");
        unsigned int ukdRand = kdRand() % (nChildrenCount - 1);
        _batchNode->removeChild((Node*)_batchNode->getChildren()->getObjectAtIndex(ukdRand), true);
        ...
        particleSystem->setPosition(Point(kdRand() % 300 ,kdRand() % 400));
