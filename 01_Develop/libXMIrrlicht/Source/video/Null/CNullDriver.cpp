...
#include "../../scene/CMeshManipulator.h"
...
IImage* CNullDriver::createImageFromFile(io::IReadFile* file)
{
	...
	video::ECOLOR_FORMAT	eFormat = ECF_UNKNOWN;
	IImage*					pIImage = 0;
	core::dimension2d<u32>  tDim;

	KDFile*			pFile	= file->getFile ( );
	...
	tDim.Width	= kdGetImageIntATX ( pImage, KD_IMAGE_WIDTH_ATX        );	
	tDim.Height	= kdGetImageIntATX ( pImage, KD_IMAGE_HEIGHT_ATX       );
	nFormat		= kdGetImageIntATX ( pImage, KD_IMAGE_FORMAT_ATX       );
	nAlpha		= kdGetImageIntATX ( pImage, KD_IMAGE_ALPHA_ATX        );	
	nBpp		= kdGetImageIntATX ( pImage, KD_IMAGE_BITSPERPIXEL_ATX );	
	...
	switch ( nFormat )
	{
        ...
        case KD_IMAGE_FORMAT_LUMINANCE_ATX :
			...
			eFormat	= video::ECF_R5G6B5;
            nFormat = KD_IMAGE_FORMAT_RGB565_ATX;
            
            break;
            
        case KD_IMAGE_FORMAT_LUMALPHA_ATX :
            
            eFormat = ECF_A1R5G5B5;
			nFormat = KD_IMAGE_FORMAT_RGBA5551_ATX;
	...
	if ( pPixels )
	{
		pIImage = new CImage ( eFormat, tDim, pPixels, false );
		if ( pIImage )
		{
			pIImage->unlock ( );
		}
	...
	//KDFile*  pFile = kdFmemopen ( file->g
	/*
	s32 i;
