...
{
    SWFContext *swf = (SWFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
...
static av_always_inline void max_nbits(int *nbits_ptr, int val)
{
    int n;
...
static int swf_write_header(AVFormatContext *s)
{
    ...
    avio_write(pb, (const unsigned char *)"FWS", 3);
                           ...
                           AVCodecContext *enc, const uint8_t *buf, int size)
{
    ...
    if (enc->codec_id == CODEC_ID_VP6F ||
        ...
        if (swf->video_frame_number == 0) {
            ...
            avio_write(pb, (const unsigned char *)"video", 5);
            avio_w8(pb, 0x00);
            put_swf_end_tag(s);
        } else {
    ...
    if (swf->audio_enc && av_fifo_size(swf->audio_fifo)) {
        ...
        av_fifo_generic_read(swf->audio_fifo, pb, frame_size, (void (*)(void *,void *,int))avio_write);
        put_swf_end_tag(s);
