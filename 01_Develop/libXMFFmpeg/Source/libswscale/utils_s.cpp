...
static const FormatEntry format_entries[PIX_FMT_NB] = {
	...
	/*[PIX_FMT_XVMC_MPEG2_MC]	= */	{ 0 , 0 },
	/*[PIX_FMT_XVMC_MPEG2_IDCT]	= */	{ 0 , 0 },
	...
	/*[PIX_FMT_VDPAU_H264]		= */	{ 0 , 0 },
	/*[PIX_FMT_VDPAU_MPEG1]		= */	{ 0 , 0 },
	/*[PIX_FMT_VDPAU_MPEG2]		= */	{ 0 , 0 },
	/*[PIX_FMT_VDPAU_WMV3]		= */	{ 0 , 0 },
	/*[PIX_FMT_VDPAU_VC1]		= */	{ 0 , 0 },
	...
	/*[PIX_FMT_VAAPI_MOCO]		= */	{ 0 , 0 },
	/*[PIX_FMT_VAAPI_IDCT]		= */	{ 0 , 0 },
	/*[PIX_FMT_VAAPI_VLD]		= */	{ 0 , 0 },
	...
	/*[PIX_FMT_VDPAU_MPEG4]		= */	{ 0 , 0 },
	/*[PIX_FMT_DXVA2_VLD]		= */	{ 0 , 0 },
                      ...
                      SwsVector *srcFilter, SwsVector *dstFilter, double param[2])
{
    ...
    FF_ALLOC_OR_GOTO(int32_t *, NULL, *filterPos, (dstW+3)*sizeof(**filterPos), fail);

    if (FFABS(xInc - 0x10000) <10) { // unscaled
        int i;
        filterSize= 1;
        FF_ALLOCZ_OR_GOTO(int64_t *, NULL, filter, dstW*sizeof(*filter)*filterSize, fail);
        ...
        int64_t xDstInSrc;
        filterSize= 1;
        FF_ALLOC_OR_GOTO(int64_t *, NULL, filter, dstW*sizeof(*filter)*filterSize, fail);
...
/*                else if (flags & SWS_X) {
    ...
    */
    ...
    FF_ALLOCZ_OR_GOTO(int64_t *, NULL, filter2, filter2Size*dstW*sizeof(*filter2), fail);
    ...
    *outFilterSize= filterSize;
    ...
    FF_ALLOCZ_OR_GOTO(int16_t *, NULL, *outFilter, *outFilterSize*(dstW+3)*sizeof(int16_t), fail);
...
int sws_init_context(SwsContext *c, SwsFilter *srcFilter, SwsFilter *dstFilter)
{
    ...
    if (c->dstBpc == 16)
        dst_stride <<= 1;
    FF_ALLOC_OR_GOTO(uint8_t *, c, c->formatConvBuffer, FFALIGN(srcW*2+78, 16) * 2, fail);
...
// can't downscale !!!
    ...
    FF_ALLOC_OR_GOTO(int16_t **, c, c->lumPixBuf, c->vLumBufSize*2*sizeof(int16_t*), fail);
    FF_ALLOC_OR_GOTO(int16_t **, c, c->chrUPixBuf, c->vChrBufSize*2*sizeof(int16_t*), fail);
    FF_ALLOC_OR_GOTO(int16_t **, c, c->chrVPixBuf, c->vChrBufSize*2*sizeof(int16_t*), fail);
    if (CONFIG_SWSCALE_ALPHA && isALPHA(c->srcFormat) && isALPHA(c->dstFormat))
        FF_ALLOCZ_OR_GOTO(int16_t **, c, c->alpPixBuf, c->vLumBufSize*2*sizeof(int16_t*), fail);
    //Note we need at least one pixel more at the end because of the MMX code (just in case someone wanna replace the 4000/8000)
    /* align at 16 bytes for AltiVec */
    for (i=0; i<c->vLumBufSize; i++) {
        FF_ALLOCZ_OR_GOTO(int16_t *, c, c->lumPixBuf[i+c->vLumBufSize], dst_stride+16, fail);
    ...
    for (i=0; i<c->vChrBufSize; i++) {
        FF_ALLOC_OR_GOTO(int16_t *, c, c->chrUPixBuf[i+c->vChrBufSize], dst_stride*2+32, fail);
        c->chrUPixBuf[i] = c->chrUPixBuf[i+c->vChrBufSize];
        c->chrVPixBuf[i] = c->chrVPixBuf[i+c->vChrBufSize] = c->chrUPixBuf[i] + (dst_stride >> 1) + 8;
    }
    if (CONFIG_SWSCALE_ALPHA && c->alpPixBuf)
        for (i=0; i<c->vLumBufSize; i++) {
            FF_ALLOCZ_OR_GOTO(int16_t *, c, c->alpPixBuf[i+c->vLumBufSize], dst_stride+16, fail);
                           ...
                           SwsFilter *srcFilter, SwsFilter *dstFilter, const double *param)
{
    ...
    sws_setColorspaceDetails(c, (const int*)ff_yuv2rgb_coeffs[SWS_CS_DEFAULT], c->srcRange, (const int*)ff_yuv2rgb_coeffs[SWS_CS_DEFAULT] /* FIXME*/, c->dstRange, 0, 1<<16, 1<<16);
...
SwsFilter *sws_getDefaultFilter(float lumaGBlur, float chromaGBlur,
                                ...
                                int verbose)
{
    SwsFilter *filter= (SwsFilter *)av_malloc(sizeof(SwsFilter));
    if (!filter)
        return NULL;
...
{
    SwsVector *vec = (SwsVector *)av_malloc(sizeof(SwsVector));
    if (!vec)
        return NULL;
    vec->length = length;
    vec->coeff  = (double *)av_malloc(sizeof(double) * length);
    if (!vec->coeff)
        av_freep(&vec);
    return vec;
}
                                        ...
                                        SwsFilter *srcFilter, SwsFilter *dstFilter, const double *param)
{
    ...
    if (!context) {
        ...
        context->param[0]  = param[0];
        context->param[1]  = param[1];
        sws_setColorspaceDetails(context, (const int*)ff_yuv2rgb_coeffs[SWS_CS_DEFAULT], context->srcRange, (const int*)ff_yuv2rgb_coeffs[SWS_CS_DEFAULT] /* FIXME*/, context->dstRange, 0, 1<<16, 1<<16);
