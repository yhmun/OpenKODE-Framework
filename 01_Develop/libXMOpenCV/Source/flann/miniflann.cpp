...
template<typename IndexType> void saveIndex_(const Index* index0, const void* index, KDFile* fout)
{
...
template<typename Distance> void saveIndex(const Index* index0, const void* index, KDFile* fout)
{
...
{
    KDFile* fout = kdFopen(filename.c_str(), "wb");
    if (fout == NULL)
...
template<typename Distance, typename IndexType>
bool loadIndex_(Index* index0, void*& index, const Mat& data, KDFile* fin, const Distance& dist=Distance())
{
    typedef typename Distance::ElementType ElementType;
...
template<typename Distance>
bool loadIndex(Index* index0, void*& index, const Mat& data, KDFile* fin, const Distance& dist=Distance())
{
...
bool Index::load(InputArray _data, const std::string& filename)
{
    ...
    bool ok = true;
    release();
    KDFile* fin = kdFopen(filename.c_str(), "rb");
    if (fin == NULL)
        return false;
    ...
    if( (int)header.rows != data.rows || (int)header.cols != data.cols ||
        featureType != data.type() )
    {
        kdLogMessagefKHR ( "Reading FLANN index error: the saved data size (%d, %d) or type (%d) is different from the passed one (%d, %d), %d\n",
    ...
    {
        kdLogMessagefKHR ( "Reading FLANN index error: unsupported feature type %d for the index type %d\n", featureType, algo);
        kdFclose(fin);
        return false;
    }
    int idistType = 0;
    ...
    #endif
        default:
            kdLogMessagefKHR ( "Reading FLANN index error: unsupported distance type %d\n", distType);
            ok = false;
        }
    }
