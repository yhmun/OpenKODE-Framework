								...
								PVRTuint8 ui8Bpp)
{
	...
	for ( y=0; y < ui32WordHeight; y++)
	{
		for ( x=0; x < ui32WordWidth; x++)
		{
			...
			KDbool punchthroughAlpha = KD_FALSE;
			if (mod>10) {punchthroughAlpha = KD_TRUE; mod-=10;}
...
*************************************************************************/
static KDbool isPowerOf2( unsigned int input )
{
  unsigned int minus1;
						 ...
						 const PVRTuint8 ui8Bpp)
{
	...
	for (y=0; y < ui32WordHeight/2; y++)
	{
		for ( x=0; x < ui32WordWidth/2; x++)
		{
							...
							PVRTuint8 ui8Bpp)
{
...

	{
		...
		for( wordY=-1; wordY < i32NumYWords-1; wordY++)
		{
			// for each column of words
			for( wordX=-1; wordX < i32NumXWords-1; wordX++)
			{
				...
				unsigned char* pResultImage)
{
	...
	int XTrueDim = KD_MAX(XDim,((Do2bitMode==1)?16:8));
	int YTrueDim = KD_MAX(YDim,8);
	...
	retval = pvrtcDecompress((PVRTuint8*)pCompressedData,pDecompressedData,XTrueDim,YTrueDim,(Do2bitMode==1?2:4));
						 ...
						 const int* nMode)
{
	...
	{	// decompress into a buffer big enough to take the minimum size
		char* pTempBuffer =	(char*) kdMalloc ( KD_MAX ( *x, ETC_MIN_TEXWIDTH ) * KD_MAX ( *y, ETC_MIN_TEXHEIGHT ) * 4 );

		KDint  nX = KD_MAX ( *x, ETC_MIN_TEXWIDTH );
		KDint  nY = KD_MAX ( *y, ETC_MIN_TEXHEIGHT );

		i32read = ETCTextureDecompress ( pSrcData, &nX , &nY, pTempBuffer, nMode );

		for( i = 0; i < *y;i++ )
		{	// copy from larger temp buffer to output data
			kdMemcpy ( (char*)( pDestData ) + i * (*x) * 4, pTempBuffer + KD_MAX ( *x, ETC_MIN_TEXWIDTH ) * 4 * i, (*x) * 4 );
		}

		if(pTempBuffer) kdFree(pTempBuffer);
	}
