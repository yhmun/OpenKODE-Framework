                           ...
                           int alignment)
{
    ...
    avctx->subtitle_header = (uint8_t *)av_strdup(header);
                    ...
                    int ts_start, int ts_end, int raw)
{
    ...
    rects = (AVSubtitleRect **)av_realloc(sub->rects, (sub->num_rects+1) * sizeof(*sub->rects));
    if (!rects)
        return AVERROR(ENOMEM);
    sub->rects = rects;
    sub->end_display_time = FFMAX(sub->end_display_time, 10 * duration);
    rects[sub->num_rects]       = (AVSubtitleRect *)av_mallocz(sizeof(*rects[0]));
    rects[sub->num_rects]->type = SUBTITLE_ASS;
    rects[sub->num_rects]->ass  = (char *)av_malloc(len + dlen + 1);
