...
static av_always_inline int64_t bs_get_v(uint8_t **bs)
{
    int64_t v = 0;
    int br = 0;
    int c;
...
static av_always_inline int64_t gb_get_v(GetBitContext *gb)
{
    int64_t v = 0;
    int bits = 0;
...
{
    MPCContext *c = (MPCContext *)s->priv_data;
    int tag;
    ...
    if(!(buf = (uint8_t *)av_malloc(size + FF_INPUT_BUFFER_PADDING_SIZE)))
        return;
