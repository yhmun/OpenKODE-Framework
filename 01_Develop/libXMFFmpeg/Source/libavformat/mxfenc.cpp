...
{
    MXFContext *c = (MXFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    int i;
    ...
    for (i = 0; i < c->essence_container_count; i++) {
        MXFStreamContext *sc = (MXFStreamContext *)s->streams[i]->priv_data;
...
static void mxf_write_track(AVFormatContext *s, AVStream *st, enum MXFMetadataSetType type)
{
    MXFContext *mxf = (MXFContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    MXFStreamContext *sc = (MXFStreamContext *)st->priv_data;
    ...
    // write track uid
    mxf_write_local_tag(pb, 16, 0x3C0A);
    mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? Track : Track + TypeBottom), st->index);
    ...
    mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? Sequence: Sequence + TypeBottom), st->index);
}
...
static void mxf_write_sequence(AVFormatContext *s, AVStream *st, enum MXFMetadataSetType type)
{
    ...
    if (type == SourcePackage)
        component = (MXFMetadataSetType) ( component + TypeBottom );
...
static void mxf_write_timecode_component(AVFormatContext *s, AVStream *st, enum MXFMetadataSetType type)
{
    ...
    // UID
    mxf_write_local_tag(pb, 16, 0x3C0A);
    mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? TimecodeComponent :
...
static void mxf_write_structural_component(AVFormatContext *s, AVStream *st, enum MXFMetadataSetType type)
{
    ...
    // write uid
    mxf_write_local_tag(pb, 16, 0x3C0A);
    mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? SourceClip: SourceClip + TypeBottom), st->index);
...
static void mxf_write_multi_descriptor(AVFormatContext *s)
{
    ...
    if (mxf->essence_container_count > 1)
        ...
        MXFStreamContext *sc = (MXFStreamContext *)s->streams[0]->priv_data;
...
static void mxf_write_package(AVFormatContext *s, enum MXFMetadataSetType type)
{
    ...
    mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? Track :
                   Track + TypeBottom), -1); // timecode track
    for (i = 0; i < s->nb_streams; i++)
        mxf_write_uuid(pb, (MXFMetadataSetType)(type == MaterialPackage ? Track : Track + TypeBottom), i);
                                ...
                                const uint8_t *key, int write_metadata)
{
    ...
    if (!memcmp(key, body_partition_key, 16)) {
        mxf->body_partition_offset =
            (uint64_t *)av_realloc(mxf->body_partition_offset,
...
static int mxf_write_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        AVStream *st = s->streams[i];
        MXFStreamContext *sc = (MXFStreamContext *)av_mallocz(sizeof(*sc));
        if (!sc)
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (fabs(av_q2d(st->codec->time_base) - 1/25.0) < 0.0001) {
				...
				mxf->time_base.num = 1001;  
				mxf->time_base.den = 30000;
    ...
    mxf->timecode_track = (AVStream *)av_mallocz(sizeof(*mxf->timecode_track));
...
static int mxf_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (!mxf->edit_unit_byte_count && !(mxf->edit_units_count % EDIT_UNITS_PER_BODY)) {
        mxf->index_entries = (MXFIndexEntry *)av_realloc(mxf->index_entries,
...
{
    MXFStreamContext *sc  = (MXFStreamContext *)s->streams[pkt ->stream_index]->priv_data;
    MXFStreamContext *sc2 = (MXFStreamContext *)s->streams[next->stream_index]->priv_data;
...
static const AVOption _mxf_class[] = {
...
static const AVOption _mxf_d10_class[] = {
        {TIMECODE_OPT(MXFContext, AV_OPT_FLAG_ENCODING_PARAM)},
        {NULL}
    };
