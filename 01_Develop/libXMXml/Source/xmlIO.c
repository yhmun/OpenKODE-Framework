...
{
    __xmlSimpleError(XML_FROM_IO, XML_ERR_NO_MEMORY, KD_NULL, KD_NULL, extra);
}
...
__xmlIOErr(int domain, int code, const char *extra)
{
    ...
    __xmlSimpleError(domain, code, KD_NULL, IOerr[idx], extra);
}
...
__xmlLoaderErr(void *ctx, const char *msg, const char *filename)
{
    ...
    __xmlRaiseError(schannel, channel, data, ctxt, KD_NULL, XML_FROM_IO,
                    XML_IO_LOAD_ERROR, level, KD_NULL, 0,
		    filename, KD_NULL, KD_NULL, 0, 0,
		    msg, filename);
...
xmlCleanupInputCallbacks(void)
{
    ...
    for (i = xmlInputCallbackNr - 1; i >= 0; i--) {
        xmlInputCallbackTable[i].matchcallback = KD_NULL;
        xmlInputCallbackTable[i].opencallback = KD_NULL;
        xmlInputCallbackTable[i].readcallback = KD_NULL;
        xmlInputCallbackTable[i].closecallback = KD_NULL;
    }
...
xmlPopInputCallbacks(void)
{
    ...
    xmlInputCallbackNr--;
    xmlInputCallbackTable[xmlInputCallbackNr].matchcallback = KD_NULL;
    xmlInputCallbackTable[xmlInputCallbackNr].opencallback = KD_NULL;
    xmlInputCallbackTable[xmlInputCallbackNr].readcallback = KD_NULL;
    xmlInputCallbackTable[xmlInputCallbackNr].closecallback = KD_NULL;
...
xmlCleanupOutputCallbacks(void)
{
    ...
    for (i = xmlOutputCallbackNr - 1; i >= 0; i--) {
        xmlOutputCallbackTable[i].matchcallback = KD_NULL;
        xmlOutputCallbackTable[i].opencallback = KD_NULL;
        xmlOutputCallbackTable[i].writecallback = KD_NULL;
        xmlOutputCallbackTable[i].closecallback = KD_NULL;
    }
...
{
    return kdFopen ( path, mode ? "wb" : "rb" );
}
...
typedef KDFile* (* xmlWrapOpenFunc)(const char *f,int mode);
...
xmlFdWrite (void * context, const char * buffer, int len) 
{
    ...
    if (len > 0) {
	ret = kdWrite((int) (long) context, (KDvoid*) &buffer[0], len);
 ...
 * @filename:  the URI for matching
 *
 * input from KDFile *, supports compressed input
 * if @filename is " " then the standard input is used
 *
 * Returns an I/O context or KD_NULL in case of error
 */
static void *
xmlFileOpen_real (const char *filename) 
{
    const char *path = KD_NULL;
    KDFile *fd;

    if (filename == KD_NULL)
        return(KD_NULL);

    if (!kdStrcmp(filename, "-")) {
	fd = 0; //stdin;
	return((void *) fd);
    }
 ...
 *
 * Returns a handler or KD_NULL in case or failure
 */
void *
xmlFileOpen (const char *filename) {
    ...
    if (retval == KD_NULL) {
	unescaped = xmlURIUnescapeString(filename, 0, KD_NULL);
	if (unescaped != KD_NULL) {
...
xmlFileRead (void * context, char * buffer, int len) {
    int ret;
    if ((context == KD_NULL) || (buffer == KD_NULL))
        return(-1);
    ret = kdFread ( &buffer[0], 1,  len, (KDFile *) context );
...
xmlFileWrite (void * context, const char * buffer, int len) {
    ...
    if ((context == KD_NULL) || (buffer == KD_NULL))
        return(-1);
    items = kdFwrite(&buffer[0], len, 1, (KDFile *) context);
    if ((items == 0) && (kdFerror((KDFile *) context))) {
        xmlIOErr(0, "fwrite()");
	return(-1);
    }
    return(items * len);
...
/*
	...
	return(0);
*/
    ret = ( kdFclose ( (KDFile *) context ) == KD_EOF ) ? -1 : 0;
    if (ret < 0)
        xmlIOErr(0, "fclose()");
    return(ret);
}
...
xmlFileFlush (void * context)
{
    ...
    ret = ( kdFflush ( (KDFile *) context ) == KD_EOF ) ? -1 : 0;
    if (ret < 0)
        xmlIOErr(0, "fflush()");
    return(ret);
}
...
xmlGzfileOpenW (const char *filename, int compression) {
    ...
    kdSnprintf(mode, sizeof(mode), "wb%d", compression);
    if (!kdStrcmp(filename, "-")) {
...
xmlCreateZMemBuff( int compression ) {
    ...
    (void)kdMemset( buff, 0, sizeof( xmlZMemBuff ) );
    buff->size = INIT_HTTP_BUFF_SIZE;
    buff->zbuff = xmlMalloc( buff->size );
    if ( buff->zbuff == KD_NULL ) {
 ...
 */
static int
xmlZMemBuffExtend( xmlZMemBuffPtr buff, KDsize ext_amt ) {

    int			rc = -1;
    KDsize		new_size;
    KDsize		cur_used;

    unsigned char *	tmp_ptr = KD_NULL;
...
xmlZMemBuffGetContent( xmlZMemBuffPtr buff, char ** data_ref ) {
    ...
    if ( ( buff == KD_NULL ) || ( data_ref == KD_NULL ) )
	return ( -1 );
...
xmlIOHTTPOpen (const char *filename) {
    return(xmlNanoHTTPOpen(filename, KD_NULL));
}
...
xmlIOHTTPOpenW(const char *post_uri, int compression)
{
    ...
    xmlIOHTTPWriteCtxtPtr ctxt = KD_NULL;
    ...
    (void) kdMemset(ctxt, 0, sizeof(xmlIOHTTPWriteCtxt));
...
#endif
    {
        ...
        ctxt->doc_buff = xmlAllocOutputBufferInternal(KD_NULL);
    }
...
xmlIOHTTPRead(void * context, char * buffer, int len) {
    if ((buffer == KD_NULL) || (len < 0)) return(-1);
...
xmlIOHTTPWrite( void * context, const char * buffer, int len ) {
    ...
    if ( ( ctxt == KD_NULL ) || ( ctxt->doc_buff == KD_NULL ) || ( buffer == KD_NULL ) )
	return ( -1 );
...
xmlIOHTTPCloseWrite( void * context, const char * http_mthd ) {
    ...
    char *			http_content = KD_NULL;
    char *			content_encoding = KD_NULL;
    char *			content_type = (char *) "text/xml";
    void *			http_ctxt = KD_NULL;

    if ( ( ctxt == KD_NULL ) || ( http_mthd == KD_NULL ) )
	return ( -1 );
	...
	if ( http_ctxt != KD_NULL ) {
#ifdef DEBUG_HTTP
	    /*  If testing/debugging - dump reply with request content  */

	    KDFile *	tst_file = KD_NULL;
	    char	buffer[ 4096 ];
	    char *	dump_name = KD_NULL;
	    int		avail;
	    ...
	    */
	    ...
	    dump_name = tempnam( KD_NULL, "lxml" );
	    if ( dump_name != KD_NULL ) {
 ...
 *
 * Returns the new parser input or KD_NULL
 */
...
xmlAllocParserInputBuffer(xmlCharEncoding enc) {
    ...
    kdMemset(ret, 0, (KDsize) sizeof(xmlParserInputBuffer));
    ...
    if (ret->encoder != KD_NULL)
        ret->raw = xmlBufferCreateSize(2 * xmlDefaultBufferSize);
    else
        ret->raw = KD_NULL;
    ret->readcallback = KD_NULL;
    ret->closecallback = KD_NULL;
    ret->context = KD_NULL;
    ret->compressed = -1;
    ret->rawconsumed = 0;
 ...
 * @encoder:  the encoding converter or KD_NULL
 *
 * Create a buffered parser output
 *
 * Returns the new parser output or KD_NULL
 */
xmlOutputBufferPtr
xmlAllocOutputBuffer(xmlCharEncodingHandlerPtr encoder) {
    ...
    kdMemset(ret, 0, (KDsize) sizeof(xmlOutputBuffer));
	 ...
	 */
	xmlCharEncOutFunc(encoder, ret->conv, KD_NULL);
    } else
        ret->conv = KD_NULL;
    ret->writecallback = KD_NULL;
    ret->closecallback = KD_NULL;
    ret->context = KD_NULL;
    ret->written = 0;
...
xmlFreeParserInputBuffer(xmlParserInputBufferPtr in) {
    ...
    if (in->encoder != KD_NULL) {
        xmlCharEncCloseFunc(in->encoder);
    }
    if (in->closecallback != KD_NULL) {
	in->closecallback(in->context);
    }
    if (in->buffer != KD_NULL) {
...
xmlOutputBufferClose(xmlOutputBufferPtr out)
{
    ...
    if (out == KD_NULL)
        return (-1);
    if (out->writecallback != KD_NULL)
        xmlOutputBufferFlush(out);
    if (out->closecallback != KD_NULL) {
    ...
    if (out->encoder != KD_NULL) {
        xmlCharEncCloseFunc(out->encoder);
    }
    if (out->buffer != KD_NULL) {
...
__xmlParserInputBufferCreateFilename(const char *URI, xmlCharEncoding enc) {
    xmlParserInputBufferPtr ret;
    int i = 0;
    void *context = KD_NULL;

    if (xmlInputCallbackInitialized == 0)
	xmlRegisterDefaultInputCallbacks();

    if (URI == KD_NULL) return(KD_NULL);
    ...
    if (context == KD_NULL) {
	for (i = xmlInputCallbackNr - 1;i >= 0;i--) {
	    if ((xmlInputCallbackTable[i].matchcallback != KD_NULL) &&
...
__xmlOutputBufferCreateFilename(const char *URI,
	...
	if ((puri->scheme == KD_NULL) ||
	    (xmlStrEqual(BAD_CAST puri->scheme, BAD_CAST "file")))
	    unescaped = xmlURIUnescapeString(URI, 0, KD_NULL);
	xmlFreeURI(puri);
    }
...
#endif
	for (i = xmlOutputCallbackNr - 1;i >= 0;i--) {
	    if ((xmlOutputCallbackTable[i].matchcallback != KD_NULL) &&
 ...
 *
 * Returns the new output or KD_NULL
 */
xmlOutputBufferPtr
 ...
 * @enc:  the charset encoding if known
 *
 * Create a buffered parser input for the progressive parsing of a KDFile *
 * buffered C I/O
 *
 * Returns the new parser input or KD_NULL
 */
xmlParserInputBufferPtr
xmlParserInputBufferCreateFile(KDFile *file, xmlCharEncoding enc) {
    xmlParserInputBufferPtr ret;

    if (xmlInputCallbackInitialized == 0)
	xmlRegisterDefaultInputCallbacks();

    if (file == KD_NULL) return(KD_NULL);
 ...
 * @encoder:  the encoding converter or KD_NULL
 *
 * Create a buffered output for the progressive saving to a KDFile *
 * buffered C I/O
 *
 * Returns the new parser output or KD_NULL
 */
xmlOutputBufferPtr
xmlOutputBufferCreateFile(KDFile *file, xmlCharEncodingHandlerPtr encoder) {
    xmlOutputBufferPtr ret;
...
xmlOutputBufferCreateBuffer(xmlBufferPtr buffer,
    ...
    if (buffer == KD_NULL) return(KD_NULL);

    ret = xmlOutputBufferCreateIO((xmlOutputWriteCallback)
                                  xmlBufferWrite,
                                  (xmlOutputCloseCallback)
                                  KD_NULL, (void *) buffer, encoder);
...
xmlParserInputBufferCreateMem(const char *mem, int size, xmlCharEncoding enc) {
    ...
    if (size <= 0) return(KD_NULL);
    if (mem == KD_NULL) return(KD_NULL);
...
xmlParserInputBufferCreateStatic(const char *mem, int size,
    ...
    kdMemset(ret, 0, (KDsize) sizeof(xmlParserInputBuffer));
    ret->buffer = xmlBufferCreateStatic((void *)mem, (KDsize) size);
    if (ret->buffer == KD_NULL) {
        xmlFree(ret);
	return(KD_NULL);
    }
...
xmlOutputBufferCreateIO(xmlOutputWriteCallback   iowrite,
    ...
    if (iowrite == KD_NULL) return(KD_NULL);
...
xmlParserInputBufferPush(xmlParserInputBufferPtr in,
    ...
    if (len < 0) return(0);
    if ((in == KD_NULL) || (in->error)) return(-1);
    if (in->encoder != KD_NULL) {
        unsigned int use;
	...
	if (nbchars < 0) {
	    xmlIOErr(XML_IO_ENCODER, KD_NULL);
...
xmlParserInputBufferGrow(xmlParserInputBufferPtr in, int len) {
    ...
    if (buffree <= 0) {
	xmlIOErr(XML_IO_BUFFER_FULL, KD_NULL);
     ...
     */
    if (in->readcallback != KD_NULL) {
	res = in->readcallback(in->context, &buffer[0], len);
	if (res <= 0)
	    in->readcallback = endOfInput;
    } else {
	xmlIOErr(XML_IO_NO_INPUT, KD_NULL);
...
xmlOutputBufferWrite(xmlOutputBufferPtr out, int len, const char *buf) {
    ...
    if ((out == KD_NULL) || (out->error)) return(-1);
    if (len < 0) return(0);
 ...
 * @escaping:  an optional escaping function (or KD_NULL)
 *
...
xmlOutputBufferWriteEscape(xmlOutputBufferPtr out, const xmlChar *str,
    ...
    if ((out == KD_NULL) || (out->error) || (str == KD_NULL) ||
        (out->buffer == KD_NULL) ||
	(out->buffer->alloc == XML_BUFFER_ALLOC_IMMUTABLE)) return(-1);
    len = kdStrlen ((const char *)str);
    if (len < 0) return(0);
    if (out->error) return(-1);
    if (escaping == KD_NULL) escaping = xmlEscapeContent;
...
xmlOutputBufferFlush(xmlOutputBufferPtr out) {
     ...
     * first handle encoding stuff.
     */
    if ((out->conv != KD_NULL) && (out->encoder != KD_NULL)) {
	/*
	 * convert as much as possible to the parser reading buffer.
	 */
     ...
     */
    if ((out->conv != KD_NULL) && (out->encoder != KD_NULL) &&
	(out->writecallback != KD_NULL)) {
	ret = out->writecallback(out->context,
	           (const char *)out->conv->content, out->conv->use);
	if (ret >= 0)
	    xmlBufferShrink(out->conv, ret);
    } else if (out->writecallback != KD_NULL) {
	ret = out->writecallback(out->context,
 ...
 *
 * Returns a new allocated string containing the directory, or KD_NULL.
 */
char *
xmlParserGetDirectory(const char *filename) {
    ...
    if (filename == KD_NULL) return(KD_NULL);
...
#   define IS_XMLPGD_SEP(ch) (ch=='/')
    ...
    kdStrncpy (dir, filename, 1023);
    dir[1023] = 0;
    cur = &dir[kdStrlen (dir)];
    while (cur > dir) {
 ...
 *
 * Returns the input or KD_NULL in case of HTTP error.
 */
xmlParserInputPtr
xmlCheckHTTPInput(xmlParserCtxtPtr ctxt, xmlParserInputPtr ret) {
#ifdef LIBXML_HTTP_ENABLED
    if ((ret != KD_NULL) && (ret->buf != KD_NULL) &&
        (ret->buf->readcallback == xmlIOHTTPRead) &&
        (ret->buf->context != KD_NULL)) {
        const char *encoding;
        const char *redir;
        const char *mime;
        int code;
		...
		__xmlLoaderErr(ctxt, "failed to load HTTP resource\n", KD_NULL);
                ...
                if (encoding != KD_NULL) {
                    ...
                    if (handler != KD_NULL) {
                        ...
                        __xmlErrEncoding(ctxt, XML_ERR_UNKNOWN_ENCODING,
                                         "Unknown encoding %s",
                                         BAD_CAST encoding, KD_NULL);
                    }
                    if (ret->encoding == KD_NULL)
            ...
            if (redir != KD_NULL) {
                ...
                if (ret->directory != KD_NULL) {
                    xmlFree((xmlChar *) ret->directory);
                    ret->directory = KD_NULL;
                }
                ret->filename =
 ...
 * @ctxt:  the context in which the entity is called or KD_NULL
 *
 * Resolves the URL and ID against the appropriate catalog.
 * This function is used by xmlDefaultExternalEntityLoader and
 * xmlNoNetExternalEntityLoader.
 *
 * Returns a new allocated URL, or KD_NULL.
 */
static xmlChar *
xmlResolveResourceFromCatalog(const char *URL, const char *ID,
                              xmlParserCtxtPtr ctxt) {
    xmlChar *resource = KD_NULL;
	 ...
	 * Do a local lookup
	 */
	if ((ctxt != KD_NULL) && (ctxt->catalogs != KD_NULL) &&
	...
	if ((resource == KD_NULL) && (URL != KD_NULL))
	    resource = xmlStrdup((const xmlChar *) URL);

	/*
	 * TODO: do an URI lookup on the reference
	 */
	if ((resource != KD_NULL) && (!xmlNoNetExists((const char *)resource))) {
	    xmlChar *tmp = KD_NULL;
 ...
 *
 * Returns a new allocated xmlParserInputPtr, or KD_NULL.
 */
static xmlParserInputPtr
xmlDefaultExternalEntityLoader(const char *URL, const char *ID,
                               xmlParserCtxtPtr ctxt)
{
    xmlParserInputPtr ret = KD_NULL;
    xmlChar *resource = KD_NULL;
...
#endif
    if ((ctxt != KD_NULL) && (ctxt->options & XML_PARSE_NONET)) {
    ...
    if ((resource != KD_NULL) && (resource != (xmlChar *) URL))
        xmlFree(resource);
    return (ret);
}
 ...
 *
 * Returns the xmlParserInputPtr or KD_NULL
 */
xmlParserInputPtr
xmlLoadExternalEntity(const char *URL, const char *ID,
                      xmlParserCtxtPtr ctxt) {
    if ((URL != KD_NULL) && (xmlNoNetExists(URL) == 0)) {
