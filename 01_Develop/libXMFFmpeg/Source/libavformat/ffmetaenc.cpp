...
static void write_tags(AVIOContext *s, AVDictionary *m)
{
    ...
    while ((t = av_dict_get(m, "", t, AV_DICT_IGNORE_SUFFIX))) {
        write_escape_str(s, (const uint8_t *)t->key);
        avio_w8(s, '=');
        write_escape_str(s, (const uint8_t *)t->value);
        avio_w8(s, '\n');
    }
}
...
{
    avio_write(s->pb, (const unsigned char *)ID_STRING, sizeof(ID_STRING) - 1);
...
static int write_trailer(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        avio_write(s->pb, (const unsigned char *)ID_STREAM, sizeof(ID_STREAM) - 1);
        avio_w8(s->pb, '\n');
