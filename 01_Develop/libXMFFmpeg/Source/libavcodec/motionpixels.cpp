...
{
    MotionPixelsContext *mp = (MotionPixelsContext *)avctx->priv_data;
    ...
    mp->changes_map = (uint8_t *)av_mallocz(avctx->width * h4);
    mp->offset_bits_len = av_log2(avctx->width * avctx->height) + 1;
    mp->vpt = (YuvPixel *)av_mallocz(avctx->height * sizeof(YuvPixel));
    mp->hpt = (YuvPixel *)av_mallocz(h4 * w4 / 16 * sizeof(YuvPixel));
                                 ...
                                 AVPacket *avpkt)
{
    ...
    av_fast_malloc(&mp->bswapbuf, (unsigned int *)&mp->bswapbuf_size, buf_size + FF_INPUT_BUFFER_PADDING_SIZE);
    if (!mp->bswapbuf)
