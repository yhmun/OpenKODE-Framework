...
{"top", "top field first", offsetof(RawVideoContext, tff), AV_OPT_TYPE_INT, -1, NULL, -1, 1, AV_OPT_FLAG_DECODING_PARAM|AV_OPT_FLAG_VIDEO_PARAM},
{NULL}
};
static const AVClass av_class = { "rawdec", NULL, options, LIBAVUTIL_VERSION_INT };
...
static av_cold int raw_init_decoder(AVCodecContext *avctx)
{
    ...
    if((avctx->bits_per_coded_sample == 4 || avctx->bits_per_coded_sample == 2) &&
       ...
       (!avctx->codec_tag || avctx->codec_tag == MKTAG('r','a','w',' '))){
        ...
        context->buffer = (unsigned char *)av_malloc(context->length);
        if (!context->buffer)
            return -1;
    } else {
