...
static int amr_write_header(AVFormatContext *s)
{
    ...
    {
        avio_write(pb, (const unsigned char *)AMR_header,   sizeof(AMR_header)   - 1); /* magic number */
    }
    else if(enc->codec_id == CODEC_ID_AMR_WB)
    {
        avio_write(pb, (const unsigned char *)AMRWB_header, sizeof(AMRWB_header) - 1); /* magic number */
    }
    else
    {
        return -1;
