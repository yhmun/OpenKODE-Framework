...
static const AVOption options[] = {
    {"smc-interval", "StreamMuxConfig interval.",
     offsetof(LATMContext, mod), AV_OPT_TYPE_INT, 0x0014, NULL, 0x0001, 0xffff, AV_OPT_FLAG_ENCODING_PARAM},
    {NULL},
};
...
{
    LATMContext *ctx = (LATMContext *)s->priv_data;
...
static int latm_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    buf = (uint8_t *)av_malloc(pkt->size+1024);
    if (!buf)
