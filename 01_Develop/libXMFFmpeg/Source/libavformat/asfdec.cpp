...
static int get_value(AVIOContext *pb, int type){
    switch(type){
        ...
        case 4: return (int)avio_rl64(pb);
...
static int asf_read_marker(AVFormatContext *s, int64_t size)
{
    ...
    for(i=0;i<count;i++){
        ...
        if ((ret = avio_get_str16le(pb, name_len * 2, name, sizeof(name))) < name_len)
            avio_skip(pb, name_len - ret);
        avpriv_new_chapter(s, i, av_rational(1, 10000000), pres_time, AV_NOPTS_VALUE, name );
    }
        ...
        if (asf_st->frag_offset == asf_st->pkt.size) {
            ...
            if (asf_st->ds_span > 1) {
              if(asf_st->pkt.size != asf_st->ds_packet_size * asf_st->ds_span){
                ...
                /* packet descrambling */
                uint8_t *newdata = (uint8_t *)av_malloc(asf_st->pkt.size + FF_INPUT_BUFFER_PADDING_SIZE);
                if (newdata) {
                    int offset = 0;
