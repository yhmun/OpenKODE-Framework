...
{\
    const type *data = (const type *)arg;\
    int i;\
    printf("    {\n");\
    for (i = 0; i < len; i++) {\
...
#define WRITE_ARRAY(prefix, type, name)                 \
    do {                                                \
        const KDsize array_size = FF_ARRAY_ELEMS(name); \
...
#define WRITE_2D_ARRAY(prefix, type, name)                              \
    do {                                                                \
        const KDsize array_size1 = FF_ARRAY_ELEMS(name);                \
        const KDsize array_size2 = FF_ARRAY_ELEMS(name[0]);             \
