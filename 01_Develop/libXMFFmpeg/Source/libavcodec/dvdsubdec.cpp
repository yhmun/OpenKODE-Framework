    ...
    the_end:
        if (offset1 >= 0) {
            ...
            if (w > 0 && h > 0) {
                ...
                sub_header->rects = (AVSubtitleRect **)av_mallocz(sizeof(*sub_header->rects));
                sub_header->rects[0] = (AVSubtitleRect *)av_mallocz(sizeof(AVSubtitleRect));
                ...
                sub_header->rects[0]->pict.data[1] = (uint8_t *)av_mallocz(AVPALETTE_SIZE);
                if (is_8bit) {
                    if (yuv_palette == 0)
                        goto fail;
...
static int dvdsub_init(AVCodecContext *avctx)
{
    ...
    dataorig = data = (char *)av_malloc(avctx->extradata_size+1);
    if (!data)
