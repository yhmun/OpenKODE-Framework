...
static int decode_picture_header(AVCodecContext *avctx, const uint8_t *buf, const int buf_size)
{
    ...
    if (ctx->slice_count != slice_count || !ctx->slices) {
        av_freep(&ctx->slices);
        ctx->slices = (SliceContext *)av_mallocz(slice_count * sizeof(*ctx->slices));
        if (!ctx->slices)
                              ...
                              const int16_t *qmat)
{
    ...
    for (i = 0; i < slice->mb_count; i++) {
        ctx->prodsp.idct_put((uint16_t *)(dst), dst_stride, block+(0<<6), qmat);
        ctx->prodsp.idct_put((uint16_t *)(dst+16), dst_stride, block+(1<<6), qmat);
        ctx->prodsp.idct_put((uint16_t *)(dst+8*dst_stride), dst_stride, block+(2<<6), qmat);
        ctx->prodsp.idct_put((uint16_t *)(dst+8*dst_stride+16), dst_stride, block+(3<<6), qmat);
        block += 4*64;
        dst += 32;
    }
}
                                ...
                                const int16_t *qmat, int log2_blocks_per_mb)
{
    ...
    for (i = 0; i < slice->mb_count; i++) {
        for (j = 0; j < log2_blocks_per_mb; j++) {
            ctx->prodsp.idct_put((uint16_t *)(dst),              dst_stride, block+(0<<6), qmat);
            ctx->prodsp.idct_put((uint16_t *)(dst+8*dst_stride), dst_stride, block+(1<<6), qmat);
            block += 2*64;
            dst += 16;
        }
    }
...
static int decode_picture(AVCodecContext *avctx)
{
	...
	int* threads_ret = (int*)av_malloc(sizeof(int)*ctx->slice_count);
