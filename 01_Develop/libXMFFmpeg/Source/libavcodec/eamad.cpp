...
{
    MadContext *t = (MadContext *)avctx->priv_data;
...
static inline int decode_block_intra(MadContext * t, DCTELEM * block)
{
    ...
    int16_t *quant_matrix = (int16_t *)s->intra_matrix;
                        ...
                        AVPacket *avpkt)
{
    ...
    bswap16_buf((uint16_t *)t->bitstream_buf, (const uint16_t*)buf, (buf_end-buf)/2);
    memset((uint8_t*)t->bitstream_buf + (buf_end-buf), 0, FF_INPUT_BUFFER_PADDING_SIZE);
    init_get_bits(&s->gb, (const uint8_t *)t->bitstream_buf, 8*(buf_end-buf));
