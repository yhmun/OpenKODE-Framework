                                 ...
                                 int type, int id, int *channels)
{
    if (che_pos[type][id]) {
        if (!ac->che[type][id]) {
            if (!(ac->che[type][id] = (ChannelElement *)av_mallocz(sizeof(ChannelElement))))
...
{
    AACContext *ac= (AACContext *)avctx->priv_data;
    int type, i, j;
                           ...
                           GetBitContext *gb)
{
    ...
    ics->window_sequence[0] = (WindowSequence)get_bits(gb, 2);
...
static int decode_cce(AACContext *ac, GetBitContext *gb, ChannelElement *che)
{
    ...
    coup->coupling_point = (CouplingPoint)(2 * get_bits1(gb));
    ...
    coup->coupling_point = (CouplingPoint)(coup->coupling_point + get_bits1(gb) || (coup->coupling_point >> 1));
                                       ...
                                       ChannelElement *cce, int index)
{
    ...
    const int len = 1024 << (int)(ac->m4ac.sbr == 1);
...
static void spectral_to_sample(AACContext *ac)
{
    ...
    for (type = 3; type >= 0; type--) {
        for (i = 0; i < MAX_ELEM_ID; i++) {
            ...
            if (che) {
                if (type <= TYPE_CPE)
                    apply_channel_coupling(ac, che, (RawDataBlockType)type, i, BEFORE_TNS, apply_dependent_coupling);
                if (ac->m4ac.object_type == AOT_AAC_LTP) {
                ...
                if (type <= TYPE_CPE)
                    apply_channel_coupling(ac, che, (RawDataBlockType)type, i, BETWEEN_TNS_AND_IMDCT, apply_dependent_coupling);
                ...
                if (type <= TYPE_CCE)
                    apply_channel_coupling(ac, che, (RawDataBlockType)type, i, AFTER_IMDCT, apply_independent_coupling);
            }
        }
    }
}
                                ...
                                int *got_frame_ptr, GetBitContext *gb)
{
    ...
    ac->tags_mapped = 0;
    // parse
    while ((elem_type = (RawDataBlockType)get_bits(gb, 3)) != TYPE_END) {
        elem_id = get_bits(gb, 4);

        if (elem_type < TYPE_DSE) {
            if (!ac->tags_mapped && elem_type == TYPE_CPE && ac->m4ac.chan_config==1) {
                enum ChannelPosition new_che_pos[4][MAX_ELEM_ID]= {AAC_CHANNEL_OFF};
                            ...
                            int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    if (new_extradata) {
        ...
        avctx->extradata = (uint8_t *)av_mallocz(new_extradata_size +
                                             ...
                                             GetBitContext *gb, int asclen)
{
    ...
    if (ac->m4ac.sample_rate != m4ac.sample_rate ||
        ...
        if (avctx->extradata_size < esize) {
            av_free(avctx->extradata);
            avctx->extradata = (uint8_t *)av_malloc(esize + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!avctx->extradata)
                return AVERROR(ENOMEM);
        }
...
static int latm_decode_frame(AVCodecContext *avctx, void *out,
                             int *got_frame_ptr, AVPacket *avpkt)
{
    struct LATMContext *latmctx = (struct LATMContext *)avctx->priv_data;
    int                 muxlength, err;
    GetBitContext       gb;
...
static const enum AVSampleFormat _ff_aac_decoder[] = { AV_SAMPLE_FMT_FLT, AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_NONE };
...
*/
static const enum AVSampleFormat _ff_aac_latm_decoder[] = {AV_SAMPLE_FMT_FLT, AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_NONE};
AVCodec ff_aac_latm_decoder = { 
	...
	/*.supported_samplerates= */	0,
	/*.sample_fmts			= */	_ff_aac_latm_decoder,
	/*.channel_layouts		= */	aac_channel_layout,
};
