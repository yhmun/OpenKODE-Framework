...
static av_always_inline uint16_t dv_audio_12to16(uint16_t sample)
{
...
static int dv_extract_video_info(DVDemuxContext *c, uint8_t* frame)
{
    ...
    if (c->sys) {
        ...
        avctx->bit_rate = av_rescale_q(c->sys->frame_size, av_rational(8,1),
                                       c->sys->time_base);
...
DVDemuxContext* avpriv_dv_init_demux(AVFormatContext *s)
{
    ...
    c = (DVDemuxContext *)av_mallocz(sizeof(DVDemuxContext));
    if (!c)
        return NULL;
...
void dv_offset_reset(DVDemuxContext *c, int64_t frame_offset)
{
    ...
    if (c->ach)
        c->abytes= av_rescale_q(c->frames, c->sys->time_base,
                                av_rational(8, c->ast[0]->codec->bit_rate));
    c->audio_pkt[0].size = c->audio_pkt[1].size = 0;
    c->audio_pkt[2].size = c->audio_pkt[3].size = 0;
}
...
static int dv_read_timecode(AVFormatContext *s) {
    ...
    uint8_t *partial_frame = (uint8_t *)av_mallocz(sizeof(*partial_frame) *
                                        partial_frame_size);

    RawDVContext *c = (RawDVContext *)s->priv_data;
                          ...
                          AVFormatParameters *ap)
{
    ...
    s->bit_rate = av_rescale_q(c->dv_demux->sys->frame_size, av_rational(8,1),
...
static int dv_read_seek(AVFormatContext *s, int stream_index,
                       int64_t timestamp, int flags)
{
    RawDVContext *r   = (RawDVContext *)s->priv_data;
