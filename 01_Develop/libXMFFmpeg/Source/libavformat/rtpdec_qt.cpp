...
#define MKTAG16(a,b) MKTAG(a,b,0,0)
            ...
            case MKTAG16('s','d'): {
                ...
                MOVContext *mc = (MOVContext *)av_mallocz(sizeof(*mc));
                if (!mc)
                    return AVERROR(ENOMEM);
                mc->fc = s;
                st->priv_data = msc = (MOVStreamContext *)av_mallocz(sizeof(MOVStreamContext));
                if (!msc) {
                    av_free(mc);
    ...
    case 3: /* one data packet spread over 1 or multiple RTP packets */
        if (qt->pkt.size > 0 && qt->timestamp == *timestamp) {
            qt->pkt.data = (uint8_t *)av_realloc(qt->pkt.data, qt->pkt.size + alen +
                                      FF_INPUT_BUFFER_PADDING_SIZE);
        } else {
            av_freep(&qt->pkt.data);
            av_init_packet(&qt->pkt);
            qt->pkt.data = (uint8_t *)av_malloc(alen + FF_INPUT_BUFFER_PADDING_SIZE);
            qt->pkt.size = 0;
    ...
    case 1: /* constant packet size, multiple packets per RTP packet */
        ...
        if (qt->remaining > 0) {
            av_freep(&qt->pkt.data);
            qt->pkt.data = (uint8_t *)av_malloc(qt->remaining * qt->bytes_per_frame);
            if (!qt->pkt.data) {
                av_free_packet(pkt);
                return AVERROR(ENOMEM);
            }
