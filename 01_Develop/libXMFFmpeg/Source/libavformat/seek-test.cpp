...
int main(int argc, char **argv)
{
    ...
    if (ret < 0) {
        kdLogMessagefKHR("cannot open %s\n", filename);
        return 1;
    }

    ret = avformat_find_stream_info(ic, NULL);
    if (ret < 0) {
        kdLogMessagefKHR("%s: could not find codec parameters\n", filename);
        return 1;
    }
    ...
    for(i=0; ; i++){
        ...
        if(stream_id>=0){
            ...
            timestamp= av_rescale_q(timestamp, _a, st->time_base);
        }
        ...
        ts_str(ts_buf, timestamp, stream_id < 0 ? _a : st->time_base);
