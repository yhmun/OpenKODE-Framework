...
#include "3d/C3DActionListener.h"
    ...
    if (_actionEvents)
    {
        ...
        if (_speed >= 0.0f)
        {
			...
			while (iter != _actionEvents->end())
			{
				...
				if(_elapsedTime == 0)
				{
					if( _elapsedTime>=((counter-1)*(KDint)_duration + (long)evt->_eventTime))
					{
				...
				else
				{
					if(currentTime == 0)
					{						
						if( _elapsedTime>=((counter-1)*(KDint)_duration + (long)evt->_eventTime)/* && evt->_finished == false*/)
						{
							evt->_listener->action();
							//evt->_finished = true;
						}						
					...
					{						
						if( _elapsedTime>=(counter*(KDint)_duration + (long)evt->_eventTime)/* && evt->_finished == false*/)
						{
							evt->_listener->action();
							//evt->_finished = true;
						}						
