...
#include "scripting/javascript/bindings/js_bindings_config.h"
#include "scripting/javascript/bindings/js_manual_conversions.h"
#include "scripting/javascript/bindings/cocos2d_specifics.hpp"
...
jsval longlong_to_jsval( JSContext *cx, long long number )
{
	...
	char chr[128];
	kdSnprintf(chr, sizeof(chr)-1, "%lld", number);
	JSString *ret_obj = JS_NewStringCopyZ(cx, chr);
	return STRING_TO_JSVAL(ret_obj);
...
jsval FontDefinition_to_jsval(JSContext* cx, const FontDefinition& t)
{
    ...
    ok &= JS_DefineProperty(cx, tmp, "fontName", std_string_to_jsval(cx, t.m_sFontName), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "fontSize", int32_to_jsval(cx, t.m_nFontSize), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "fontAlignmentH", int32_to_jsval(cx, (int32_t)t.m_eAlignment), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "fontAlignmentV", int32_to_jsval(cx, (int32_t)t.m_eVertAlignment), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "fontFillColor", cccolor3b_to_jsval(cx, t.m_tFontFillColor), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "fontDimensions", ccsize_to_jsval(cx, t.m_tDimensions), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    // Shadow
    ok &= JS_DefineProperty(cx, tmp, "shadowEnabled", BOOLEAN_TO_JSVAL(t.m_tShadow.m_bShadowEnabled), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "shadowOffset", ccsize_to_jsval(cx, t.m_tShadow.m_tShadowOffset), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "shadowBlur", DOUBLE_TO_JSVAL(t.m_tShadow.m_fShadowBlur), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "shadowOpacity", DOUBLE_TO_JSVAL(t.m_tShadow.m_fShadowOpacity), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    // Stroke
    ok &= JS_DefineProperty(cx, tmp, "strokeEnabled", BOOLEAN_TO_JSVAL(t.m_tStroke.m_bStrokeEnabled), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "strokeColor", cccolor3b_to_jsval(cx, t.m_tStroke.m_tStrokeColor), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
    
    ok &= JS_DefineProperty(cx, tmp, "strokeSize", DOUBLE_TO_JSVAL(t.m_tStroke.m_fStrokeSize), NULL, NULL, JSPROP_ENUMERATE | JSPROP_PERMANENT);
...
JSBool jsval_to_FontDefinition( JSContext *cx, jsval vp, FontDefinition *out )
{
    ...
    out->m_tShadow.m_bShadowEnabled = false;
    out->m_tStroke.m_bStrokeEnabled = false;
    
    // white text by default
    out->m_tFontFillColor = Color3B::WHITE;
    ...
    {
        out->m_sFontName  = fontName;
    }
    else
    {
        out->m_sFontName  = defautlFontName;
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_nFontSize  = fontSize;
    }
    else
    {
        out->m_nFontSize  = defaultFontSize;
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_eAlignment = (TextHAlignment)(int)fontAlign;
    }
    else
    {
        out->m_eAlignment  = defaultTextAlignment;
    }
    ...
    if ( hasProperty )
    {
        ...
        double fontAlign = 0.0;
        JS_ValueToNumber(cx, jsr, &fontAlign);
        out->m_eVertAlignment = (TextVAlignment)(int)fontAlign;
    }
    else
    {
        out->m_eVertAlignment  = defaultTextVAlignment;
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_tFontFillColor = getColorFromJSObject(cx, jsobjColor);
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_tDimensions = getSizeFromJSObject(cx, jsobjSize);
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_tShadow.m_bShadowEnabled  = ToBoolean(jsr);
        
        if ( out->m_tShadow.m_bShadowEnabled )
        {
            // default shadow values
            out->m_tShadow.m_tShadowOffset  = Size(5, 5);
            out->m_tShadow.m_fShadowBlur    = 1;
            out->m_tShadow.m_fShadowOpacity = 1;
            ...
            if ( hasProperty )
            {
                ...
                if (!JS_ValueToObject( cx, jsr, &jsobjShadowOffset ) )
                    return JS_FALSE;
                out->m_tShadow.m_tShadowOffset = getSizeFromJSObject(cx, jsobjShadowOffset);
            }
            ...
            if ( hasProperty )
            {
                ...
                out->m_tShadow.m_fShadowBlur = shadowBlur;
            }
            ...
            if ( hasProperty )
            {
                ...
                out->m_tShadow.m_fShadowOpacity = shadowOpacity;
            }
        }
    }
    ...
    if ( hasProperty )
    {
        ...
        out->m_tStroke.m_bStrokeEnabled  = ToBoolean(jsr);
        
        if ( out->m_tStroke.m_bStrokeEnabled )
        {
            // default stroke values
            out->m_tStroke.m_fStrokeSize  = 1;
            out->m_tStroke.m_tStrokeColor = Color3B::BLUE;
            ...
            if ( hasProperty )
            {
                ...
                if (!JS_ValueToObject( cx, jsr, &jsobjStrokeColor ) )
                    return JS_FALSE;
                out->m_tStroke.m_tStrokeColor = getColorFromJSObject(cx, jsobjStrokeColor);
            }
            ...
            if ( hasProperty )
            {
                ...
                out->m_tStroke.m_fStrokeSize = strokeSize;
            }
        }
    }
