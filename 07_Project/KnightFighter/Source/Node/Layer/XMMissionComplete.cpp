...
#include "XMMissionComplete.h"
#include "Node/Scene/XMReady.h"
#include "Node/Scene/XMMain.h"

XMMissionComplete::XMMissionComplete ( )
{
...
//	this->setPosition ( EGLView::getInstance ( )->getVisibleOrigin ( ) );
	...
	m_pUiBg = Sprite::create ( "Complete.jpg" );
	...
	Sprite*			pUiBtnNext = Sprite::create ( "button_next.png" );
...
KDvoid XMMissionComplete::makeRestart ( KDvoid )
{
	Director::getInstance ( )->replaceScene ( XMReady::create ( ) );
}

KDvoid XMMissionComplete::makeConfirm ( KDvoid )
{
	Director::getInstance ( )->replaceScene ( XMMain::create ( ) );
}

KDvoid XMMissionComplete::makeGoNextStage ( KDvoid )
{
	Director::getInstance ( )->replaceScene ( XMReady::create ( ) );
}


KDvoid XMMissionComplete::setTouchEnable ( )
{
	m_pEventListener = EventListenerTouchOneByOne::create ( );
	m_pEventListener->setSwallowTouches ( true );
	m_pEventListener->onTouchBegan		= CC_CALLBACK_2 ( XMMissionComplete::onTouchBegan, this );
	m_pEventListener->onTouchMoved		= CC_CALLBACK_2 ( XMMissionComplete::onTouchMoved, this );
	m_pEventListener->onTouchEnded		= CC_CALLBACK_2 ( XMMissionComplete::onTouchEnded, this );
	m_pEventListener->onTouchCancelled	= CC_CALLBACK_2 ( XMMissionComplete::onTouchCancelled, this );
	
	m_pEventDispatcher->addEventListenerWithSceneGraphPriority ( m_pEventListener, this );
}

KDbool XMMissionComplete::onTouchBegan ( Touch* pTouch, Event* pEvent )
{
	return true;
}

KDvoid XMMissionComplete::onTouchMoved ( Touch* pTouch, Event* pEvent )
{

}

KDvoid XMMissionComplete::onTouchEnded ( Touch* pTouch, Event* pEvent )
{
	...
	for ( std::vector<Sprite*>::iterator pIter = m_aUiSprite.begin ( ); pIter != m_aUiSprite.end ( ); pIter++ )
	{
		...
		KDbool		bContain	= tRect.containsPoint ( m_pUiBg->convertToNodeSpace ( tPoint ) );   // ÁÂÇÏ´Ü
...
KDvoid XMMissionComplete::onTouchCancelled	( Touch* pTouch, Event* pEvent )
{
