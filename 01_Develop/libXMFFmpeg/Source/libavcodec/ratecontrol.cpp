...
/*        "lastIQP",
    ...
    static double (* const func1[])(void *, double)={
        (double ( *const )(void *,double))bits2qp,
        (double ( *const )(void *,double))qp2bits,
        NULL
    };
...
//printf("q:%f max:%f min:%f size:%f index:%d bits:%f agr:%f\n", q,max_rate, min_rate, buffer_size, rcc->buffer_index, bits, s->avctx->rc_buffer_aggressivity);
    if(s->avctx->rc_qsquish==0.0 || qmin==qmax){
        ...
        double min2= log((double)qmin);
        double max2= log((double)qmax);
...
static void adaptive_quantization(MpegEncContext *s, double q){
    ...
    float bits_sum= 0.0;
    float cplx_sum= 0.0;
    float *cplx_tab = (float *)av_malloc(s->mb_num * sizeof(*cplx_tab));
    float *bits_tab = (float *)av_malloc(s->mb_num * sizeof(*bits_tab));
    ...
    for(i=0; i<s->mb_num; i++){
        ...
        float temp_cplx= sqrt((float)pic->mc_mb_var[mb_xy]); //FIXME merge in pow()
        float spat_cplx= sqrt((float)pic->mb_var[mb_xy]);
        ...
        if(factor<0.00001) factor= 0.00001f;
...
//printf("input_pic_num:%d pic_num:%d frame_rate:%d\n", s->input_picture_number, s->picture_number, s->frame_rate);
    ...
    if(picture_number>2 && !dry_run){
        ...
        update_predictor(&rcc->pred[s->last_pict_type], rcc->last_qscale, sqrt((float)last_var), s->frame_bits);
    }
...
//            av_log(NULL, AV_LOG_ERROR, "%" PRId64 " %" PRId64 " %" PRId64 " %d\n", s->current_picture_ptr->pts, s->user_specified_pts, dts_pic->pts, picture_number);
    ...
    if(br_compensation<=0.0) br_compensation=0.001f;
...
//printf("%f %f %f last:%d var:%d type:%d//\n", q, rce->new_qscale, br_compensation, s->frame_bits, var, pict_type);
        ...
        bits= predict_size(&rcc->pred[pict_type], rce->qscale, sqrt((float)var));
        if(pict_type== AV_PICTURE_TYPE_I){
            rce->i_count   = s->mb_num;
            rce->i_tex_bits= bits;
            rce->p_tex_bits= 0;
...
//printf("%f ", q);
    ...
    if(s->avctx->debug&FF_DEBUG_RC){
        ...
        av_get_picture_type_char((AVPictureType)pict_type), qmin, q, qmax, picture_number, (int)wanted_bits/1000, (int)s->total_bits/1000,
...
static int init_pass2(MpegEncContext *s)
{
    ...
    qscale= (double *)av_malloc(sizeof(double)*rcc->num_entries);
    blurred_qscale= (double *)av_malloc(sizeof(double)*rcc->num_entries);
    toobig = 0;
