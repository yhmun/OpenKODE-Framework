...
static int truemotion1_decode_header(TrueMotion1Context *s)
{
    ...
    if (s->w != s->avctx->width || s->h != s->avctx->height ||
        ...
        s->avctx->sample_aspect_ratio = av_rational( 1 << width_shift, 1 );
        s->avctx->pix_fmt = (PixelFormat)new_pix_fmt;
        avcodec_set_dimensions(s->avctx, s->w, s->h);
        av_fast_malloc(&s->vert_pred, (unsigned int *)&s->vert_pred_size, s->avctx->width * sizeof(unsigned int));
    }
