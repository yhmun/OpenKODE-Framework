...
#include "XMFFmpeg/libavutil/error.h"
#include "internal.h"
#include "url.h"
...
{
    av_md5_update((AVMD5 *)h->priv_data, buf, size);
    return size;
}
...
static int md5_close(URLContext *h)
{
    ...
    av_md5_final((AVMD5 *)h->priv_data, md5);
    for (i = 0; i < sizeof(md5); i++)
        snprintf((char*)(buf + i*2), 3, "%02x", md5[i]);
    buf[i*2] = '\n';
