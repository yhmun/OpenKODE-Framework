...
#  endif
    struct KDStat st;
    if (kdStat(filename, &st) || ! st.st_size)
        return 0;
    //___ int fd = kdOpen(filename, O_RDONLY | O_BINARY);
	___ int fd = kdOpen(filename, KD_O_RDONLY, 0);
    ...
    if (! buffer)
        return 0;
    if ((st.st_size == kdRead(fd, buffer, st.st_size)) &&
        (disk = zzip_disk_new()))
    {
        disk->buffer = buffer;
 ...
 */
zzip__new__ ZZIP_DISK *
zzip_disk_buffer(void *buffer, KDsize buflen) {
