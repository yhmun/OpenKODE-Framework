...
//FIXME remove extradata "rebuilding"
    ...
    smk->frm_size = (uint32_t *)av_malloc(smk->frames * 4);
    smk->frm_flags = (uint8_t *)av_malloc(smk->frames);
    ...
    st->codec->extradata = (uint8_t *)av_malloc(smk->treesize + 16);
    ...
    if(smk->curstream < 0) {
        ...
        for(i = 0; i < 7; i++) {
            if(flags & 1) {
                ...
                frame_size -= 4;
                smk->curstream++;
                tmpbuf = (uint8_t *)av_realloc(smk->bufs[smk->curstream], size);
                if (!tmpbuf)
