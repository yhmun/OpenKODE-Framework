...
int ff_h264_alloc_tables(H264Context *h){
    ...
    FF_ALLOCZ_OR_GOTO(int8_t (*)		, h->s.avctx, h->intra4x4_pred_mode, row_mb_num * 8  * sizeof(uint8_t), fail)

    FF_ALLOCZ_OR_GOTO(uint8_t (*)[48]	, h->s.avctx, h->non_zero_count    , big_mb_num * 48 * sizeof(uint8_t), fail)
    FF_ALLOCZ_OR_GOTO(uint16_t *		, h->s.avctx, h->slice_table_base  , (big_mb_num+s->mb_stride) * sizeof(*h->slice_table_base), fail)
    FF_ALLOCZ_OR_GOTO(uint16_t *		, h->s.avctx, h->cbp_table, big_mb_num * sizeof(uint16_t), fail)

    FF_ALLOCZ_OR_GOTO(uint8_t *			, h->s.avctx, h->chroma_pred_mode_table, big_mb_num * sizeof(uint8_t), fail)
    FF_ALLOCZ_OR_GOTO(uint8_t (*)[2]	, h->s.avctx, h->mvd_table[0], 16*row_mb_num * sizeof(uint8_t), fail);
    FF_ALLOCZ_OR_GOTO(uint8_t (*)[2]	, h->s.avctx, h->mvd_table[1], 16*row_mb_num * sizeof(uint8_t), fail);
    FF_ALLOCZ_OR_GOTO(uint8_t *			, h->s.avctx, h->direct_table, 4*big_mb_num * sizeof(uint8_t) , fail);
    FF_ALLOCZ_OR_GOTO(uint8_t *			, h->s.avctx, h->list_counts, big_mb_num * sizeof(uint8_t), fail)

    memset(h->slice_table_base, -1, (big_mb_num+s->mb_stride)  * sizeof(*h->slice_table_base));
    h->slice_table= h->slice_table_base + s->mb_stride*2 + 1;

    FF_ALLOCZ_OR_GOTO(uint32_t *		, h->s.avctx, h->mb2b_xy  , big_mb_num * sizeof(uint32_t), fail);
    FF_ALLOCZ_OR_GOTO(uint32_t *		, h->s.avctx, h->mb2br_xy , big_mb_num * sizeof(uint32_t), fail);
...
static int context_init(H264Context *h){
    FF_ALLOCZ_OR_GOTO(uint8_t (*)[96], h->s.avctx, h->top_borders[0], h->s.mb_width * 16*3 * sizeof(uint8_t)*2, fail)
    FF_ALLOCZ_OR_GOTO(uint8_t (*)[96], h->s.avctx, h->top_borders[1], h->s.mb_width * 16*3 * sizeof(uint8_t)*2, fail)
...
static void copy_picture_range(Picture **to, Picture **from, int count, MpegEncContext *new_base, MpegEncContext *old_base)
{
    ...
    for (i=0; i<count; i++){
        assert((IN_RANGE(from[i], (Picture *)old_base, sizeof(*old_base)) ||
                IN_RANGE(from[i], (Picture *)old_base->picture, sizeof(Picture) * old_base->picture_count) ||
                !from[i]));
...
static int decode_update_thread_context(AVCodecContext *dst, const AVCodecContext *src){
    H264Context *h= (H264Context *)dst->priv_data, *h1= (H264Context *)src->priv_data;
    ...
    if(!inited){
        ...
        h->s.obmc_scratchpad = (uint8_t *)av_malloc(16*6*s->linesize);
...
int ff_h264_frame_start(H264Context *h){
    ...
    for(i = 0; i < s->slice_context_count; i++)
        if(h->thread_context[i] && !h->thread_context[i]->s.obmc_scratchpad)
            h->thread_context[i]->s.obmc_scratchpad = (uint8_t *)av_malloc(16*6*s->linesize);
...
static int decode_slice_header(H264Context *h, H264Context *h0){
    ...
    if (!s->context_initialized) {
        ...
        if (!HAVE_THREADS || !(s->avctx->active_thread_type&FF_THREAD_SLICE)) {
            ...
            for(i = 1; i < s->slice_context_count; i++) {
                H264Context *c;
                c = h->thread_context[i] = (H264Context *)av_malloc(sizeof(H264Context));
    ...
    if(s->avctx->debug&FF_DEBUG_PICT_INFO){
        av_log(h->s.avctx, AV_LOG_DEBUG, "slice:%d %s mb:%d %c%s%s pps:%u frame:%d poc:%d/%d ref:%d/%d qp:%d loop:%d:%d:%d weight:%d%s %s\n",
               ...
               first_mb_in_slice,
               av_get_picture_type_char((AVPictureType)h->slice_type), h->slice_type_fixed ? " fix" : "", h->nal_unit_type == NAL_IDR_SLICE ? " IDR" : "",
               pps_id, h->frame_num,
                                                      ...
                                                      int left_xy[LEFT_MBS], int top_type, int left_type[LEFT_MBS], int mb_xy, int list)
{
    ...
    if(IS_INTER(mb_type) || IS_DIRECT(mb_type)){
        if(USES_LIST(top_type, list)){
            ...
            int (*ref2frm)[64] = (int (*)[64])(h->ref2frm[ h->slice_table[top_xy]&(MAX_SLICES-1) ][0] + (MB_MBAFF ? 20 : 2));
        ...
        if(!IS_INTERLACED(mb_type^left_type[LTOP])){
            if(USES_LIST(left_type[LTOP], list)){
                ...
                int (*ref2frm)[64] = (int (*)[64])(h->ref2frm[ h->slice_table[left_xy[LTOP]]&(MAX_SLICES-1) ][0] + (MB_MBAFF ? 20 : 2));
...
static int decode_slice(struct AVCodecContext *avctx, void *arg){
    H264Context *h = (H264Context *)(*(void**)arg);
    MpegEncContext * const s = &h->s;
...
static const AVOption h264_options[] = {
    {"is_avc", "is avc", offsetof(H264Context, is_avc), FF_OPT_TYPE_INT, 0, NULL, 0, 1, 0},
    {"nal_length_size", "nal_length_size", offsetof(H264Context, nal_length_size), FF_OPT_TYPE_INT, 0, NULL, 0, 4, 0},
    {NULL}
};
