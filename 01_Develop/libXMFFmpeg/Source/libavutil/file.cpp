...
#include "XMFFmpeg/libavutil/file.h"
...
int av_file_map(const char *filename, uint8_t **bufptr, KDsize *size,
                int log_offset, void *log_ctx)
{
    FileLogContext file_log_ctx = { &file_log_ctx_class, log_offset, log_ctx };
    int err, fd = open(filename, O_RDONLY, 0);
    struct KDStat st;
//    av_unused void *ptr;
    KDoff off_size;
    char errbuf[128];
...
void av_file_unmap(uint8_t *bufptr, KDsize size)
{
#if HAVE_MMAP
    munmap(bufptr, size);
#elif HAVE_MAPVIEWOFFILE
...
int av_tempfile(const char *prefix, char **filename, int log_offset, void *log_ctx) {
    ...
    KDsize len = strlen(prefix) + 12; /* room for "/tmp/" and "XXXXXX\0" */
