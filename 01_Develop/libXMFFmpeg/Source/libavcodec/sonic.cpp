...
{
    int i;
    int *state = (int *)av_mallocz(4* window_entries);
...
{
    SonicContext *s = (SonicContext *)avctx->priv_data;
    PutBitContext pb;
    int i, version = 0;
    ...
    // generate taps
    s->tap_quant = (int *)av_mallocz(4* s->num_taps);
    for (i = 0; i < s->num_taps; i++)
        s->tap_quant[i] = (int)(sqrt((double)(i+1)));
    ...
    s->tail = (int *)av_mallocz(4* s->num_taps*s->channels);
    if (!s->tail)
        return -1;
    s->tail_size = s->num_taps*s->channels;

    s->predictor_k = (int *)av_mallocz(4 * s->num_taps);
    if (!s->predictor_k)
        return -1;

    for (i = 0; i < s->channels; i++)
    {
        s->coded_samples[i] = (int *)av_mallocz(4* s->block_align);
        if (!s->coded_samples[i])
            return -1;
    }

    s->int_samples = (int *)av_mallocz(4* s->frame_size);

    s->window_size = ((2*s->tail_size)+s->frame_size);
    s->window = (int *)av_mallocz(4* s->window_size);
    if (!s->window)
        return -1;

    avctx->extradata = (uint8_t *)av_mallocz(16);
    if (!avctx->extradata)
        return -1;
...
//    avctx->frame_size = s->block_align;
    ...
    for (i = 0; i < s->channels; i++)
    {
        s->predictor_state[i] = (int *)av_mallocz(4* s->num_taps);
