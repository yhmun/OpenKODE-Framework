                          ...
                          uint32_t tag, enum ID3v2Encoding enc)
{
     ...
     * they're not */
    if (enc == ID3v2_ENCODING_UTF16BOM && string_is_ascii((const uint8_t *)str1) &&
        (!str2 || string_is_ascii((const uint8_t *)str2)))
...
static int id3v2_check_write_tag(AVFormatContext *s, AVDictionaryEntry *t, const char table[][5],
                   ...
                   const char *magic)
{
    ...
    while ((t = av_dict_get(s->metadata, "", t, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if ((ret = id3v2_check_write_tag(s, t, ff_id3v2_tags, (ID3v2Encoding)enc)) > 0) {
            totlen += ret;
            continue;
        }
        if ((ret = id3v2_check_write_tag(s, t, id3v2_version == 3 ?
                                               ff_id3v2_3_tags : ff_id3v2_4_tags, (ID3v2Encoding)enc)) > 0) {
            totlen += ret;
            continue;
        }

        /* unknown tag, write as TXXX frame */
        if ((ret = id3v2_put_ttag(s, t->key, t->value, MKBETAG('T', 'X', 'X', 'X'), (ID3v2Encoding)enc)) < 0)
            return ret;
        totlen += ret;
    }
