...
#include "XMBullet/LinearMath/btHashMap.h"
...
{
	if(kdFabsf(v.x())>1e-6 || kdFabsf(v.y())>1e-6 || kdFabsf(v.z())>1e-6)	return false;
	return true;
}
...
void	btConvexPolyhedron::initialize()
{
	...
	if(1)
	{
		...
		btScalar MinX = KD_FLT_MAX;
		btScalar MinY = KD_FLT_MAX;
		btScalar MinZ = KD_FLT_MAX;
		btScalar MaxX = -KD_FLT_MAX;
		btScalar MaxY = -KD_FLT_MAX;
		btScalar MaxZ = -KD_FLT_MAX;
...
//		const btScalar r = m_radius / sqrtf(2.0f);
		const btScalar r = m_radius / kdSqrtf(3.0f);
