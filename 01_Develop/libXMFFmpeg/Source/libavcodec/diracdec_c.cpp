...
static int alloc_sequence_buffers(DiracContext *s)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        s->plane[i].idwt_buf_base = (IDWTELEM *)av_mallocz((w+max_xblen)*h * sizeof(IDWTELEM));
        s->plane[i].idwt_tmp      = (IDWTELEM *)av_malloc((w+16) * sizeof(IDWTELEM));
    ...
    s->sbsplit  = (uint8_t *)av_malloc(sbwidth * sbheight);
    s->blmotion = (DiracBlock *)av_malloc(sbwidth * sbheight * 4 * sizeof(*s->blmotion));
    s->edge_emu_buffer_base = (uint8_t *)av_malloc((w+64)*MAX_BLOCKSIZE);

    s->mctmp     = (uint16_t *)av_malloc((w+64+MAX_BLOCKSIZE) * (h*MAX_BLOCKSIZE) * sizeof(*s->mctmp));
    s->mcscratch = (uint8_t *)av_malloc((w+64)*MAX_BLOCKSIZE);
...
{
    DiracContext *s = (DiracContext *)avctx->priv_data;
    s->avctx = avctx;
    s->frame_number = -1;
...
{
    DiracContext *s = (DiracContext *)avctx->priv_data;
    decode_subband_internal(s, (SubBand *)b, 1);
    return 0;
}
...
static void decode_component(DiracContext *s, int comp)
{
    ...
    for (level = 0; level < s->wavelet_depth; level++) {
        for (orientation = (dirac_subband)(!!level); orientation < 4; orientation = (dirac_subband)(orientation + 1)) {
...
{
    DiracContext *s = (DiracContext *)avctx->priv_data;
    struct lowdelay_slice *slice = (struct lowdelay_slice *)arg;
...
static void decode_lowdelay(DiracContext *s)
{
    ...
    slices = (struct lowdelay_slice *)av_mallocz(s->lowdelay.num_x * s->lowdelay.num_y * sizeof(struct lowdelay_slice));
...
static void interpolate_refplane(DiracContext *s, DiracFrame *ref, int plane, int width, int height)
{
    ...
    for (i = 1; i < 4; i++) {
        if (!ref->hpel_base[plane][i])
            ref->hpel_base[plane][i] = (uint8_t *)av_malloc((height+2*edge) * ref->avframe.linesize[plane] + 32);
...
static int dirac_decode_frame_internal(DiracContext *s)
{
    ...
    for (comp = 0; comp < 3; comp++) {
        ...
        if (ff_spatial_idwt_init2(&d, p->idwt_buf, p->idwt_width, p->idwt_height, p->idwt_stride,
                                  (dwt_type)(s->wavelet_idx+2), s->wavelet_depth, p->idwt_tmp))
            return -1;
...
static int dirac_decode_data_unit(AVCodecContext *avctx, const uint8_t *buf, int size)
{
    ...
    if (parse_code == pc_seq_header) {
        ...
        pic->avframe.pict_type = (AVPictureType)(s->num_refs + 1);              /* Definition of AVPictureType in avutil.h */
...
{
    DiracContext *s     = (DiracContext *)avctx->priv_data;
    DiracFrame *picture = (DiracFrame *)data;
