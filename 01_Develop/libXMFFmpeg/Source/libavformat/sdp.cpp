...
static char *extradata2psets(AVCodecContext *c)
{
    ...
    if (c->extradata[0] == 1) {
        ...
        orig_extradata = (uint8_t *)av_mallocz(orig_extradata_size +
...
{
    unsigned char *config, *encoded_config;
    ...
    config = (unsigned char *)av_malloc(config_len);
    if (!config)
        goto xiph_fail;

    encoded_config = (unsigned char *)av_malloc(AV_BASE64_SIZE(config_len));
    if (!encoded_config) {
        av_free(config);
        goto xiph_fail;
    }
    ...
    av_base64_encode((char *)encoded_config, AV_BASE64_SIZE(config_len),
                     config, config_len);
    av_free(config);

    return (char *)encoded_config;
