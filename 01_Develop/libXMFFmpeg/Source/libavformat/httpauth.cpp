...
static void update_md5_strings(struct AVMD5 *md5ctx, ...)
{
    ...
    while (1) {
        ...
        if (!str)
            break;
        av_md5_update(md5ctx, (const uint8_t *)str, strlen(str));
    }
    va_end(vl);
}
                              ...
                              const char *method)
{
    ...
    md5ctx = (AVMD5 *)av_malloc(av_md5_size);
    if (!md5ctx)
        return NULL;
                                   ...
                                   const char *path, const char *method)
{
    ...
    if (state->auth_type == HTTP_AUTH_BASIC) {
        ...
        av_base64_encode(ptr, auth_b64_len, (const uint8_t *)auth, strlen(auth));
        av_strlcat(ptr, "\r\n", len - (ptr - authstr));
