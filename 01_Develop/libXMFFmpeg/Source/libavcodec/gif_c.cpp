                                  ...
                                  uint8_t **bytestream, uint32_t *palette)
{
    ...
    bytestream_put_buffer(bytestream, (const uint8_t *)"GIF", 3);
    bytestream_put_buffer(bytestream, (const uint8_t *)"89a", 3);
...
{
    GIFContext *s = (GIFContext *)avctx->priv_data;
    int len = 0, height;
    const uint8_t *ptr;
    /* image block */
    ...
    ff_lzw_encode_init((LZWEncodeState *)s->lzw, s->buf, avctx->width*avctx->height,
                       12, FF_LZW_GIF, put_bits);

    ptr = buf;
    for (height = avctx->height; height--;) {
        len += ff_lzw_encode((LZWEncodeState *)s->lzw, ptr, avctx->width);
        ptr += linesize;
    }
    len += ff_lzw_encode_flush((LZWEncodeState *)s->lzw, flush_put_bits);
...
static av_cold int gif_encode_init(AVCodecContext *avctx)
{
    ...
    s->lzw = (LZWState *)av_mallocz(ff_lzw_encode_state_size);
    if (!s->lzw)
        return AVERROR(ENOMEM);
    s->buf = (uint8_t *)av_malloc(avctx->width*avctx->height*2);
    if (!s->buf)
         return AVERROR(ENOMEM);
    return 0;
}
...
static const enum PixelFormat _ff_gif_encoder[] = {PIX_FMT_RGB8, PIX_FMT_BGR8, PIX_FMT_RGB4_BYTE, PIX_FMT_BGR4_BYTE, PIX_FMT_GRAY8, PIX_FMT_PAL8, PIX_FMT_NONE};
