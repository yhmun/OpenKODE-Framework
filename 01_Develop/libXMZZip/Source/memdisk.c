...
#include <XMZZip/mmapped.h>
#include <XMZZip/memdisk.h>
#include "fnmatch.h"
    ...
    if (! disk)
        {
			...
			perror(error[_zzip_mem_disk_open_fail]);
#endif
			return 0;
		}
...
zzip_mem_disk_fdopen(int fd)
{
    ...
    if (! disk)
        {
#if   !defined (_WIN32_WCE)
			perror(error[_zzip_mem_disk_fdopen_fail]); 
#endif
			return 0;
	}
    ___ ZZIP_MEM_DISK *dir = zzip_mem_disk_new();
...
zzip_mem_disk_buffer(char *buffer, size_t buflen)
{
    ...
    if (! disk)
        { 
#if   !defined (_WIN32_WCE)
			perror(error[_zzip_mem_disk_buffer_fail]); 
#endif
			return 0; }
    ___ ZZIP_MEM_DISK *dir = zzip_mem_disk_new();
    zzip_mem_disk_load(dir, disk);
...
zzip_mem_entry_new(ZZIP_DISK * disk, ZZIP_DISK_ENTRY * entry)
{
    ...
    }
    {
        ...
        if (block)
        {
            item->zz_usize = (zzip_off_t)__zzip_get64(block->z_usize);
            item->zz_csize = (zzip_off_t)__zzip_get64(block->z_csize);
            item->zz_offset = (zzip_off_t)__zzip_get64(block->z_offset);
            item->zz_diskstart = (zzip_off_t)__zzip_get32(block->z_diskstart);
        }
    }
    /* NOTE:
...
#if   !defined (_WIN32_WCE)
    return (long)mktime(&date);       /* well, unix has that function... */
#else 
	return 0;
#endif
}
