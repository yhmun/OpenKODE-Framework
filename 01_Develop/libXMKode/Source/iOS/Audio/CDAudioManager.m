...
-(void) load:(NSString*) filePath
{
    ...
    if ( state == kLAS_Init || ![filePath isEqualToString:audioSourceFilePath] )
    {
        ...
        audioSourcePlayer = [(AVAudioPlayer*)[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:filePath] error:&error];
        if ( error == nil )
        {
...
-(BOOL) isDeviceMuted
{
    ...
    {
        CFComparisonResult  newDeviceIsMuted = CFStringCompare ( newAudioRoute, (CFStringRef) @"", 0 );
...
{ 
    if ( !_interrupted )
    {
        ...
        if ( alcGetCurrentContext ( ) != NULL )
        {
            ...
            {
                CDLOGINFO ( @"Denshion::CDAudioManager - Error making context current %x\n", error );
            }             
        }
    }    
} 
