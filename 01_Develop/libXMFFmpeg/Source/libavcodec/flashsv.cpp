...
static int flashsv_decode_block(AVCodecContext *avctx, AVPacket *avpkt,
                                ...
                                int blk_idx)
{
    struct FlashSVContext *s = (FlashSVContext *)avctx->priv_data;
                                ...
                                int *data_size, AVPacket *avpkt)
{
    ...
    if (s->block_size < s->block_width * s->block_height) {
        ...
        s->tmpblock = (uint8_t *)av_realloc(s->tmpblock, tmpblock_size);
        if (!s->tmpblock) {
        ...
        if (s->ver == 2) {
            ...
            s->deflate_block = (uint8_t *)av_realloc(s->deflate_block, s->deflate_block_size);
            if (!s->deflate_block) {
    ...
    if (s->is_keyframe) {
        s->keyframedata = (uint8_t *)av_realloc(s->keyframedata, avpkt->size);
        memcpy(s->keyframedata, avpkt->data, avpkt->size);
        s->blocks = (BlockInfo *)av_realloc(s->blocks,
    ...
    if (s->is_keyframe && s->ver == 2) {
        if (!s->keyframe) {
            s->keyframe = (uint8_t *)av_malloc(s->frame.linesize[0] * avctx->height);
            if (!s->keyframe) {
...
static const enum PixelFormat _ff_flashsv_decoder[] = {PIX_FMT_BGR24, PIX_FMT_NONE};
...
static const enum PixelFormat _ff_flashsv2_decoder[] = {PIX_FMT_BGR24, PIX_FMT_NONE};
