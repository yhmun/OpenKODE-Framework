...
#define ffio_wfourcc(pb,s)		ffio_wfourcc(pb, (const uint8_t *)(s))
#define	avio_write(p,b,s)		avio_write(p, (const unsigned char *)(b), s)

static const AVOption options[] = {
    { "movflags", "MOV muxer flags", offsetof(MOVMuxContext, flags), AV_OPT_TYPE_FLAGS, 0, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    { "rtphint", "Add RTP hint tracks", 0, AV_OPT_TYPE_CONST, FF_MOV_FLAG_RTP_HINT, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    { "moov_size", "maximum moov size so it can be placed at the begin", offsetof(MOVMuxContext, reserved_moov_size), AV_OPT_TYPE_INT, 0, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, 0 },
    { "empty_moov", "Make the initial moov atom empty (not supported by QuickTime)", 0, AV_OPT_TYPE_CONST, FF_MOV_FLAG_EMPTY_MOOV, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    { "frag_keyframe", "Fragment at video keyframes", 0, AV_OPT_TYPE_CONST, FF_MOV_FLAG_FRAG_KEYFRAME, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    { "separate_moof", "Write separate moof/mdat atoms for each track", 0, AV_OPT_TYPE_CONST, FF_MOV_FLAG_SEPARATE_MOOF, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    { "frag_custom", "Flush fragments on caller requests", 0, AV_OPT_TYPE_CONST, FF_MOV_FLAG_FRAG_CUSTOM, NULL, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    FF_RTP_FLAG_OPTS(MOVMuxContext, rtp_flags),
    { "skip_iods", "Skip writing iods atom.", offsetof(MOVMuxContext, iods_skip), AV_OPT_TYPE_INT, 1, NULL, 0, 1, AV_OPT_FLAG_ENCODING_PARAM},
    { "iods_audio_profile", "iods audio profile atom.", offsetof(MOVMuxContext, iods_audio_profile), AV_OPT_TYPE_INT, -1, NULL, -1, 255, AV_OPT_FLAG_ENCODING_PARAM},
    { "iods_video_profile", "iods video profile atom.", offsetof(MOVMuxContext, iods_video_profile), AV_OPT_TYPE_INT, -1, NULL, -1, 255, AV_OPT_FLAG_ENCODING_PARAM},
    { "frag_duration", "Maximum fragment duration", offsetof(MOVMuxContext, max_fragment_duration), AV_OPT_TYPE_INT, 0, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { "frag_size", "Maximum fragment size", offsetof(MOVMuxContext, max_fragment_size), AV_OPT_TYPE_INT, 0, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { "ism_lookahead", "Number of lookahead entries for ISM files", offsetof(MOVMuxContext, ism_lookahead), AV_OPT_TYPE_INT, 0, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { NULL },
};
...
static int mov_write_ctts_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    ctts_entries = (MOVStts *)av_malloc((track->entry + 1) * sizeof(*ctts_entries)); /* worst case */
...
static int mov_write_stts_tag(AVIOContext *pb, MOVTrack *track)
{
    ...
    if (track->enc->codec_type == AVMEDIA_TYPE_AUDIO && !track->audio_vbr) {
        stts_entries = (MOVStts *)av_malloc(sizeof(*stts_entries)); /* one entry */
        stts_entries[0].count = track->sampleCount;
        stts_entries[0].duration = 1;
        entries = 1;
    } else {
        stts_entries = track->entry ?
                       (MOVStts *)av_malloc(track->entry * sizeof(*stts_entries)) : /* worst case */
                       NULL;
        for (i=0; i<track->entry; i++) {
                                  ...
                                  const char *tag, const char *str)
{
    ...
    if (!t || !utf8len((const uint8_t *)t->value))
        return 0;
    avio_wb32(pb, 0);   /* size */
...
static int mov_write_chpl_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        AVDictionaryEntry *t;
        avio_wb64(pb, av_rescale_q(c->start, c->time_base, av_rational(1,10000000)));
...
{
    int len = utf8len((const uint8_t *)str)+1;
    if(len<=0)
        return;
    ...
    avio_wb16(pb, 0x01);                /* ? */
    ascii_to_wc(pb, (const uint8_t *)str);
}
...
static int mov_flush_fragment(AVFormatContext *s)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        ...
        if (write_moof) {
            ...
            track->nb_frag_info++;
            track->frag_info = (MOVFragmentInfo *)av_realloc(track->frag_info,
...
static int mov_write_packet_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (trk->vosLen == 0 && enc->extradata_size > 0) {
        ...
        trk->vosData = (uint8_t *)av_malloc(trk->vosLen);
    ...
    if ((enc->codec_id == CODEC_ID_DNXHD ||
        ...
        trk->vosLen = size;
        trk->vosData = (uint8_t *)av_malloc(size);
        if (!trk->vosData)
    ...
    if (!(trk->entry % MOV_INDEX_CLUSTER_SIZE)) {
        trk->cluster = (MOVIentry *)av_realloc_f(trk->cluster, sizeof(*trk->cluster), (trk->entry + MOV_INDEX_CLUSTER_SIZE));
        if (!trk->cluster)
            return -1;
    }
...
static void mov_create_chapter_track(AVFormatContext *s, int tracknum)
{
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        AVChapter *c = s->chapters[i];
        AVDictionaryEntry *t;
        int64_t end = av_rescale_q(c->end, c->time_base, av_rational(1,MOV_TIMESCALE));
        pkt.pts = pkt.dts = av_rescale_q(c->start, c->time_base, av_rational(1,MOV_TIMESCALE));
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    mov->tracks = (MOVTrack *)av_mallocz(mov->nb_streams*sizeof(*mov->tracks));
    if (!mov->tracks)
        return AVERROR(ENOMEM);
    ...
    for(i=0; i<s->nb_streams; i++){
        ...
        if (st->codec->extradata_size) {
            ...
            track->vosData = (uint8_t *)av_malloc(track->vosLen);
...
#if CONFIG_MOV_MUXER
MOV_CLASS(mov)
static const AVCodecTag* const _ff_mov_muxer[] = {codec_movvideo_tags, codec_movaudio_tags, 0};
...
#if CONFIG_TGP_MUXER
MOV_CLASS(tgp)
static const AVCodecTag* const _ff_tgp_muxer[] = {codec_3gp_tags, 0};
...
#if CONFIG_MP4_MUXER
MOV_CLASS(mp4)
static const AVCodecTag* const _ff_mp4_muxer[] = {ff_mp4_obj_type, 0};
...
#if CONFIG_PSP_MUXER
MOV_CLASS(psp)
static const AVCodecTag* const _ff_psp_muxer[] = {ff_mp4_obj_type, 0};
...
#if CONFIG_TG2_MUXER
MOV_CLASS(tg2)
static const AVCodecTag* const _ff_tg2_muxer[] = {codec_3gp_tags, 0};
...
MOV_CLASS(ipod)
static const AVCodecTag* const _ff_ipod_muxer[] = {codec_ipod_tags, 0};
...
MOV_CLASS(ismv)
static const AVCodecTag* const _ff_ismv_muxer[] = {ff_mp4_obj_type, 0};
