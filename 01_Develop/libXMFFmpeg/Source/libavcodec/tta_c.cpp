...
{
    TTAContext *s = (TTAContext *)avctx->priv_data;
    int i;
    ...
    if (show_bits_long(&s->gb, 32) == AV_RL32("TTA1"))
    {
        ...
        s->decode_buffer = (int32_t *)av_mallocz(sizeof(int32_t)*s->frame_length*s->channels);
        if (!s->decode_buffer)
            return AVERROR(ENOMEM);
        s->ch_ctx = (TTAChannel *)av_malloc(avctx->channels * sizeof(*s->ch_ctx));
        if (!s->ch_ctx) {
...
#define PRED(x, k) (int32_t)((((uint64_t)x << k) - x) >> k)
    ...
    skip_bits(&s->gb, 32); // frame crc
        ...
        switch(s->bps) {
            ...
            case 2: {
                uint16_t *samples = (uint16_t *)s->frame.data[0];
                for (p = s->decode_buffer; p < s->decode_buffer + (framelen * s->channels); p++)
                    *samples++ = *p;
                break;
            }
