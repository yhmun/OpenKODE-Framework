...
{
    LclDecContext *c = (LclDecContext *)avctx->priv_data;
    int zret = inflateReset(&c->zstream);
    if (zret != Z_OK) {
        av_log(avctx, AV_LOG_ERROR, "Inflate reset error: %d\n", zret);
        return -1;
    }
    c->zstream.next_in = (z_Bytef *)src;
...
static int decode_frame(AVCodecContext *avctx, void *data, int *data_size, AVPacket *avpkt)
{
    ...
    LclDecContext * const c = (LclDecContext *const)avctx->priv_data;
...
static av_cold int decode_init(AVCodecContext *avctx)
{
    ...
    if (c->decomp_size) {
        if ((c->decomp_buf = (unsigned char *)av_malloc(max_decomp_size)) == NULL) {
