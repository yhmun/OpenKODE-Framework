...
C3DSkinModel::C3DSkinModel(C3DMesh* mesh) :C3DModel(mesh),_skin(NULL)
{
                ...
                for (unsigned int j = 0; j < passCount; ++j)
                {
					...
					if(skin != NULL)
					{
						...
						for (unsigned int iBonePart=0; iBonePart < skin->_partCount; iBonePart++)
						{
							...
							if (_wireframe && (_mesh->getPrimitiveType() == PrimitiveType_TRIANGLES || _mesh->getPrimitiveType() == PrimitiveType_TRIANGLE_STRIP))// »æÖÆÏß¿ò lvlong
							{
