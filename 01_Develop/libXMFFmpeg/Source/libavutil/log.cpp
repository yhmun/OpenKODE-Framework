...
/*
#if !defined(SHP) && (defined(_WIN32) && !defined(__MINGW32CE__))
#include <windows.h>
...
void av_log_default_callback(void* ptr, int level, const char* fmt, va_list vl)
{
    ...
    if (print_prefix && (flags & AV_LOG_SKIP_REPEATED) && !strcmp(line, prev)){
        count++;
        if (is_atty == 1)
            kdLogMessagefKHR("    Last message repeated %d times\r", count);
        return;
    }
    if (count > 0) {
        kdLogMessagefKHR("    Last message repeated %d times\n", count);
        count = 0;
    }
    strcpy(prev, line);
