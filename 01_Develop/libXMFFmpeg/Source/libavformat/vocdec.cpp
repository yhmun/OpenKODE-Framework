...
{
    VocDecContext *voc = (VocDecContext *)s->priv_data;
    AVIOContext *pb = s->pb;
    int header_size;
    AVStream *st;
...
voc_get_packet(AVFormatContext *s, AVPacket *pkt, AVStream *st, int max_size)
{
    ...
    if (tmp_codec >= 0) {
        ...
        if (dec->codec_id == CODEC_ID_NONE)
            dec->codec_id = (CodecID)tmp_codec;
...
static const AVCodecTag* const _ff_voc_demuxer[] = {ff_voc_codec_tags, 0};
