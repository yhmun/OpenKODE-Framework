...
{
    AvsFormat *avs = (AvsFormat *)s->priv_data;
...
static int avs_read_packet(AVFormatContext * s, AVPacket * pkt)
{
    ...
    while (1) {
        ...
        while (avs->remaining_frame_size > 0) {
            ...
            type = (AvsBlockType)avio_r8(s->pb);
            size = avio_rl16(s->pb);
            if (size < 4)
            ...
            case AVS_VIDEO:
                if (!avs->st_video) {
					...
					avs->st_video->codec->time_base.num = 1; 
					avs->st_video->codec->time_base.den = avs->fps;
                }
