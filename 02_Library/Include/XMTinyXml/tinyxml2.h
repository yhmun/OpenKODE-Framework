...
//inline int TIXML_SNPRINTF( char* buffer, KDsize size, const char* format, ... )
//{
//    va_list va;
//    va_start( va, format );
...
#define TIXML_SNPRINTF kdSnprintf
#define TIXML_SSCANF   kdSscanf
//#endif
...
private:
    void EnsureCapacity( int cap ) {
        if ( cap > _allocated ) {
            ...
            kdMemcpy( newMem, _mem, sizeof(T)*_size );	// warning: not using constructors, only works for PODs
            if ( _mem != _pool ) {
                delete [] _mem;
            }
            _mem = newMem;
...
#ifdef DEBUG
        kdMemset( chunk, 0xfe, sizeof(Chunk) );
#endif
        chunk->next = _root;
        _root = chunk;
    }
...
public:
    ...
    static const char* SkipWhiteSpace( const char* p )	{
        while( !IsUTF8Continuation(*p) && kdIsspace( *reinterpret_cast<const unsigned char*>(p) ) ) {
            ++p;
        }
        return p;
    }
    static char* SkipWhiteSpace( char* p )				{
        while( !IsUTF8Continuation(*p) && kdIsspace( *reinterpret_cast<unsigned char*>(p) ) )		{
            ++p;
        }
        return p;
    }
    static bool IsWhiteSpace( char p )					{
        return !IsUTF8Continuation(p) && kdIsspace( static_cast<unsigned char>(p) );
    }
    
    inline static bool IsNameStartChar( unsigned char ch ) {
        return ( ( ch < 128 ) ? kdIsalpha( ch ) : 1 )
               || ch == ':'
               || ch == '_';
    }
    ...
    inline static bool StringEqual( const char* p, const char* q, int nChar=KDINT_MAX )  {
        int n = 0;
        if ( p == q ) {
            return true;
        }
...
public:
    ...
    */
    XMLError Parse( const char* xml, KDsize nBytes=(KDsize)(-1) );
    ...
    */
    ...
    /**
    	...
    	an errorID.
    */
    XMLError SaveFile( KDFile* fp, bool compact = false );
...
public:
    ...
    */
    XMLPrinter( KDFile* file=0, bool compact = false );
    ~XMLPrinter()	{}
