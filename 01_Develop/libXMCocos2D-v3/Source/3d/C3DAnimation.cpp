...
C3DAnimation::C3DAnimation(const char* id)
    : _id(id), _duration(0),
	 _defaultClip(NULL), _clips(NULL),_currentClip(NULL), _state(STOPPED)
{
...
C3DAnimation* C3DAnimation::clone()
{
	...
	for ( KDuint k = 0; k < getClipCount ( ); k++ )
	{
		C3DAnimationClip* clipSource = getClip(k);
		C3DAnimationClip* clip = clipSource->clone(animation);
		/*C3DAnimationClip* clip = animation->createClip(clipSource->getID(), clipSource->getStartTime(), clipSource->getEndTime());
		clip->setSpeed(clipSource->getSpeed());
		clip->setRepeatCount(clipSource->getRepeatCount());*/
