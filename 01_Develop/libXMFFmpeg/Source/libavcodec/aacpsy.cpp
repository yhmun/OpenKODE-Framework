...
{
    f /= 1000.0f;
    return    3.64f * pow(f, -0.8f)
            - 6.8f  * exp(-0.6f  * (f - 3.4f) * (f - 3.4f))
            + 6.0f  * exp(-0.15f * (f - 8.7f) * (f - 8.7f))
            + (0.6f + 0.04f * add) * 0.001f * f * f * f * f;
}
...
static av_cold int psy_3gpp_init(FFPsyContext *ctx) {
    ...
    for (j = 0; j < 2; j++) {
        ...
        for (g = 0; g < ctx->num_bands[j] - 1; g++) {
            ...
            coeff->spread_low[0] = pow(10.0f, -bark_width * PSY_3GPP_THR_SPREAD_LOW);
            coeff->spread_hi [0] = pow(10.0f, -bark_width * PSY_3GPP_THR_SPREAD_HI);
            coeff->spread_low[1] = pow(10.0f, -bark_width * en_spread_low);
            coeff->spread_hi [1] = pow(10.0f, -bark_width * en_spread_hi);
    ...
    pctx->ch = (AacPsyChannel *)av_mallocz(sizeof(AacPsyChannel) * ctx->avctx->channels);
                                                 ...
                                                 int channel, int prev_type)
{
    ...
    if (la) {
        ...
        pch->next_window_seq = (WindowSequence)next_type;
    } else {
        for (i = 0; i < 3; i++)
...
static void lame_apply_block_type(AacPsyChannel *ctx, FFPsyWindowInfo *wi, int uselongblock)
{
    ...
    ctx->next_window_seq = (WindowSequence)blocktype;
}
