...
int vc1_decode_sequence_header(AVCodecContext *avctx, VC1Context *v, GetBitContext *gb)
{
    ...
    if (v->profile == PROFILE_ADVANCED) {
        v->zz_8x4 = (const uint8_t *)ff_vc1_adv_progressive_8x4_zz;
        v->zz_4x8 = (const uint8_t *)ff_vc1_adv_progressive_4x8_zz;
...
static int decode_sequence_header_adv(VC1Context *v, GetBitContext *gb)
{
    ...
    if (get_bits1(gb)) { //Display Info - decoding is not affected by it
        ...
        if (ar && ar < 14) {
            ...
            w = get_bits(gb, 8) + 1;
            h = get_bits(gb, 8) + 1;
            v->s.avctx->sample_aspect_ratio = av_rational(w, h);
        } else {
...
int vc1_parse_frame_header_adv(VC1Context *v, GetBitContext* gb)
{
    ...
    if (v->second_field) {
        ...
        v->s.current_picture_ptr->f.pict_type = (AVPictureType)v->s.pict_type;
    ...
    v->field_mode = 0;
    if (v->interlace) {
        v->fcm = (FrameCodingMode)decode012(gb);
        if (v->fcm) {
