...
{
    SnowContext *s = (SnowContext *)avctx->priv_data;
    int plane_index;
    ...
    s->m.me.temp      =
    s->m.me.scratchpad= (uint8_t *)av_mallocz((avctx->width+64)*2*16*2*sizeof(uint8_t));
    s->m.me.map       = (uint32_t *)av_mallocz(ME_MAP_SIZE*sizeof(uint32_t));
    s->m.me.score_map = (uint32_t *)av_mallocz(ME_MAP_SIZE*sizeof(uint32_t));
    s->m.obmc_scratchpad= (uint8_t *)av_mallocz(MB_SIZE*MB_SIZE*12*sizeof(uint32_t));
    ...
    if(avctx->flags&CODEC_FLAG_PASS1){
        if(!avctx->stats_out)
            avctx->stats_out = (char *)av_mallocz(256);
    }
...
//    avcodec_get_chroma_sub_sample(avctx->pix_fmt, &s->chroma_h_shift, &s->chroma_v_shift);
    ...
    if(s->avctx->me_method == ME_ITER){
        ...
        for(i=0; i<s->max_ref_frames; i++){
            s->ref_mvs[i]= (int16_t (*)[2])av_mallocz(size*sizeof(int16_t[2]));
            s->ref_scores[i]= (uint32_t *)av_mallocz(size*sizeof(uint32_t));
        }
    }
...
static int encode_q_branch(SnowContext *s, int level, int x, int y){
    ...
    int trx_= (x+1)<<rem_depth;
    int try_= (y+1)<<rem_depth;
    const BlockNode *left  = x ? &s->block[index-1] : &null_block;
    const BlockNode *top   = y ? &s->block[index-w] : &null_block;
    const BlockNode *right = trx_<w ? &s->block[index+1] : &null_block;
    const BlockNode *bottom= try_<h ? &s->block[index+w] : &null_block;
    const BlockNode *tl    = y && x ? &s->block[index-w-1] : left;
    const BlockNode *tr    = y && trx_<w && ((x&1)==0 || level==0) ? &s->block[index-w+(1<<rem_depth)] : tl; //FIXME use lt
    int pl = left->color[0];
...
static int get_block_rd(SnowContext *s, int mb_x, int mb_y, int plane_index, const uint8_t *obmc_edged){
    ...
    uint8_t* tmp = (uint8_t *)av_malloc(sizeof(uint8_t)*ref_stride*(2*MB_SIZE+HTAPS_MAX-1));
    const int b_stride = s->b_width << s->block_max_depth;
    const int b_height = s->b_height<< s->block_max_depth;
    const int w= p->width;
    const int h= p->height;
...
static int encode_subband_c0run(SnowContext *s, SubBand *b, IDWTELEM *src, IDWTELEM *parent, int stride, int orientation){
    ...
    if(1){
        int run=0;
        int* runs = (int *)av_malloc(sizeof(int)*w*h);
        int run_index=0;
        int max_index;
    ...
    for(pass=0; pass<25; pass++){
        ...
        for(mb_y= 0; mb_y<b_height; mb_y++){
            for(mb_x= 0; mb_x<b_width; mb_x++){
                ...
                uint8_t** obmc_edged = (uint8_t**)av_malloc(sizeof(uint8_t)*(b_w*2)*(b_w*2));
...
static int qscale2qlog(int qscale){
    return (int)lrint(QROOT*log(qscale / (float)FF_QP2LAMBDA)/log(2.0))
           + 61*QROOT/8; ///< 64 > 60
}
...
static void calculate_visual_weight(SnowContext *s, Plane *p){
    ...
    for(level=0; level<s->spatial_decomposition_count; level++){
        for(orientation=level ? 1 : 0; orientation<4; orientation++){
            ...
            b->qlog= (int)(log(352256.0/sqrt((double)error)) / log(pow(2.0, 1.0/QROOT))+0.5);
        }
    }
}
...
static int encode_frame(AVCodecContext *avctx, unsigned char *buf, int buf_size, void *data){
    ...
    if(avctx->flags&CODEC_FLAG_PASS2){
        s->m.pict_type =
        pict->pict_type= (AVPictureType)s->m.rc_context.entry[avctx->frame_number].new_pict_type;
...
static const AVOption options[] = {
    { "memc_only",      "Only do ME/MC (I frames -> ref, P frame -> ME+MC).",   OFFSET(memc_only), AV_OPT_TYPE_INT, 0, NULL, 0, 1, VE },
    { NULL },
};
