...
static const AVOption options[]={
    {"nitris_compat", "encode with Avid Nitris compatibility", offsetof(DNXHDEncContext, nitris_compat), AV_OPT_TYPE_INT, 0, NULL, 0, 1, VE},
{NULL}
};
static const AVClass av_class = { "dnxhd", av_default_item_name, options, LIBAVUTIL_VERSION_INT };
...
static int dnxhd_init_vlc(DNXHDEncContext *ctx)
{
    ...
    FF_ALLOCZ_OR_GOTO(uint32_t*, ctx->m.avctx, ctx->vlc_codes, max_level*4*sizeof(*ctx->vlc_codes), fail);
    FF_ALLOCZ_OR_GOTO(uint8_t* , ctx->m.avctx, ctx->vlc_bits,  max_level*4*sizeof(*ctx->vlc_bits) , fail);
    FF_ALLOCZ_OR_GOTO(uint16_t*, ctx->m.avctx, ctx->run_codes, 63*2,                                fail);
    FF_ALLOCZ_OR_GOTO(uint8_t* , ctx->m.avctx, ctx->run_bits,  63,                                  fail);
...
static int dnxhd_init_qmat(DNXHDEncContext *ctx, int lbias, int cbias)
{
    ...
    FF_ALLOCZ_OR_GOTO(int (*)[64], ctx->m.avctx, ctx->qmatrix_l,   (ctx->m.avctx->qmax+1) * 64 *     sizeof(int),      fail);
    FF_ALLOCZ_OR_GOTO(int (*)[64], ctx->m.avctx, ctx->qmatrix_c,   (ctx->m.avctx->qmax+1) * 64 *     sizeof(int),      fail);
    FF_ALLOCZ_OR_GOTO(uint16_t (*)[2][64], ctx->m.avctx, ctx->qmatrix_l16, (ctx->m.avctx->qmax+1) * 64 * 2 * sizeof(uint16_t), fail);
    FF_ALLOCZ_OR_GOTO(uint16_t (*)[2][64], ctx->m.avctx, ctx->qmatrix_c16, (ctx->m.avctx->qmax+1) * 64 * 2 * sizeof(uint16_t), fail);
...
{
    FF_ALLOCZ_OR_GOTO(RCEntry (*)[8160], ctx->m.avctx, ctx->mb_rc, 8160*ctx->m.avctx->qmax*sizeof(RCEntry), fail);
    if (ctx->m.avctx->mb_decision != FF_MB_DECISION_RD)
        FF_ALLOCZ_OR_GOTO(RCCMPEntry*, ctx->m.avctx, ctx->mb_cmp, ctx->m.mb_num*sizeof(RCCMPEntry), fail);
...
{
    DNXHDEncContext *ctx = (DNXHDEncContext *)avctx->priv_data;
    ...
    FF_ALLOCZ_OR_GOTO(uint32_t*, ctx->m.avctx, ctx->slice_size, ctx->m.mb_height*sizeof(uint32_t), fail);
    FF_ALLOCZ_OR_GOTO(uint32_t*, ctx->m.avctx, ctx->slice_offs, ctx->m.mb_height*sizeof(uint32_t), fail);
    FF_ALLOCZ_OR_GOTO(uint16_t*, ctx->m.avctx, ctx->mb_bits,    ctx->m.mb_num   *sizeof(uint16_t), fail);
    FF_ALLOCZ_OR_GOTO(uint8_t* , ctx->m.avctx, ctx->mb_qscale,  ctx->m.mb_num   *sizeof(uint8_t),  fail);
    ...
    for (i = 1; i < avctx->thread_count; i++) {
        ctx->thread[i] = (DNXHDEncContext *)av_malloc(sizeof(DNXHDEncContext));
...
static void radix_sort(RCCMPEntry *data, int size)
{
    ...
    RCCMPEntry *tmp = (RCCMPEntry *)av_malloc(sizeof(*tmp) * size);
...
static int dnxhd_encode_picture(AVCodecContext *avctx, unsigned char *buf, int buf_size, void *data)
{
    ...
    dnxhd_load_picture(ctx, (const AVFrame *)data);
...
static const enum PixelFormat _ff_dnxhd_encoder[] = {PIX_FMT_YUV422P, PIX_FMT_YUV422P10, PIX_FMT_NONE};
