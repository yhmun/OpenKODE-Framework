...
static int mov_read_udta_string(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (parse)
        ...
        if (data_type == 3 || (data_type == 0 && langcode < 0x800)) { // MAC Encoded
            ...
            avio_read(pb, (unsigned char *)str, str_size);
            str[str_size] = 0;
        }
...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        avio_read(pb, (unsigned char *)str, str_len);
        str[str_len] = 0;
        avpriv_new_chapter(c->fc, i, av_rational(1,10000000), start, AV_NOPTS_VALUE, str);
    }
    return 0;
}
...
static int mov_read_dref(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(*sc->drefs))
        return AVERROR_INVALIDDATA;
    sc->drefs = (MOVDref *)av_mallocz(entries * sizeof(*sc->drefs));
    if (!sc->drefs)
    ...
    for (i = 0; i < sc->drefs_count; i++) {
        ...
        if (dref->type == MKTAG('a','l','i','s') && size > 150) {
            ...
            avio_read(pb, (unsigned char *)dref->volume, 27);
            ...
            len = avio_r8(pb);
            len = FFMIN(len, 63);
            avio_read(pb, (unsigned char *)dref->filename, 63);
            dref->filename[len] = 0;
            ...
            for (type = 0; type != -1 && avio_tell(pb) < next; ) {
                ...
                if (type == 2) { // absolute path
                    av_free(dref->path);
                    dref->path = (char *)av_mallocz(len+1);
                    if (!dref->path)
                        return AVERROR(ENOMEM);
                    avio_read(pb, (unsigned char *)dref->path, len);
                    ...
                    av_free(dref->dir);
                    dref->dir = (char *)av_malloc(len+1);
                    if (!dref->dir)
                        return AVERROR(ENOMEM);
                    avio_read(pb, (unsigned char *)dref->dir, len);
                    dref->dir[len] = 0;
                    for (j = 0; j < len; j++)
                        if (dref->dir[j] == ':')
                            dref->dir[j] = '/';
...
static int mov_read_hdlr(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (title_size > 0) {
        title_str = (char *)av_malloc(title_size + 1); /* Add null terminator */
        if (!title_str)
            return AVERROR(ENOMEM);
        avio_read(pb, (unsigned char *)title_str, title_size);
...
static int mov_read_dac3(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
	...
	switch ( acmod )
	{
		case 0 : st->codec->channels = 2 + lfeon; break;
		case 1 : st->codec->channels = 1 + lfeon; break;
		case 2 : st->codec->channels = 2 + lfeon; break;
		case 3 : st->codec->channels = 3 + lfeon; break;
		case 4 : st->codec->channels = 3 + lfeon; break;
		case 5 : st->codec->channels = 4 + lfeon; break;
		case 6 : st->codec->channels = 4 + lfeon; break;
		case 7 : st->codec->channels = 5 + lfeon; break;
	}
    //st->codec->channels = ((int[]){2,1,2,3,3,4,4,5})[acmod] + lfeon;
    st->codec->channel_layout = avpriv_ac3_channel_layout_tab[acmod];
    if (lfeon)
        st->codec->channel_layout |= AV_CH_LOW_FREQUENCY;
    st->codec->audio_service_type = (AVAudioServiceType)bsmod;
...
static int mov_read_ftyp(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (comp_brand_size < 0)
        return AVERROR_INVALIDDATA;
    comp_brands_str = (char *)av_malloc(comp_brand_size + 1); /* Add null terminator */
    if (!comp_brands_str)
        return AVERROR(ENOMEM);
    avio_read(pb, (unsigned char *)comp_brands_str, comp_brand_size);
...
static int mov_read_smi(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codec->extradata = (uint8_t *)av_mallocz(atom.size + 0x5a + FF_INPUT_BUFFER_PADDING_SIZE);
                              ...
                              enum CodecID codec_id)
{
    ...
    if (size > INT_MAX || (uint64_t)atom.size > INT_MAX)
        return AVERROR_INVALIDDATA;
    buf= (uint8_t *)av_realloc(st->codec->extradata, size);
    if (!buf)
        return AVERROR(ENOMEM);
...
static int mov_read_wave(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (st->codec->codec_id == CODEC_ID_QDM2 || st->codec->codec_id == CODEC_ID_QDMC) {
        ...
        av_free(st->codec->extradata);
        st->codec->extradata = (uint8_t *)av_mallocz(atom.size + FF_INPUT_BUFFER_PADDING_SIZE);
        if (!st->codec->extradata)
            return AVERROR(ENOMEM);
...
static int mov_read_strf(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_free(st->codec->extradata);
    st->codec->extradata = (uint8_t *)av_mallocz(atom.size - 40 + FF_INPUT_BUFFER_PADDING_SIZE);
    if (!st->codec->extradata)
        return AVERROR(ENOMEM);
...
int ff_mov_read_stsd_entries(MOVContext *c, AVIOContext *pb, int entries)
{
    ...
    for (pseudo_stream_id=0; pseudo_stream_id<entries; pseudo_stream_id++) {
        ...
        if (st->codec->codec_type==AVMEDIA_TYPE_VIDEO) {
            ...
            MOVAtom fake_atom; 
			fake_atom.size = size - (avio_tell(pb) - start_pos);
...
static int mov_read_stsc(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(*sc->stsc_data))
        return AVERROR_INVALIDDATA;
    sc->stsc_data = (MOVStsc *)av_malloc(entries * sizeof(*sc->stsc_data));
    if (!sc->stsc_data)
        return AVERROR(ENOMEM);
...
static int mov_read_stps(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(*sc->stps_data))
        return AVERROR_INVALIDDATA;
    sc->stps_data = (unsigned int *)av_malloc(entries * sizeof(*sc->stps_data));
    if (!sc->stps_data)
        return AVERROR(ENOMEM);
...
static int mov_read_stss(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(int))
        return AVERROR_INVALIDDATA;
    sc->keyframes = (int *)av_malloc(entries * sizeof(int));
    if (!sc->keyframes)
        return AVERROR(ENOMEM);
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(int) || entries >= (UINT_MAX - 4) / field_size)
        return AVERROR_INVALIDDATA;
    sc->sample_sizes = (int *)av_malloc(entries * sizeof(int));
    if (!sc->sample_sizes)
        return AVERROR(ENOMEM);

    num_bytes = (entries*field_size+4)>>3;

    buf = (unsigned char *)av_malloc(num_bytes+FF_INPUT_BUFFER_PADDING_SIZE);
    if (!buf) {
...
static int mov_read_stts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->stts_data = (MOVStts *)av_malloc(entries * sizeof(*sc->stts_data));
    if (!sc->stts_data)
        return AVERROR(ENOMEM);
...
static int mov_read_ctts(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (entries >= UINT_MAX / sizeof(*sc->ctts_data))
        return AVERROR_INVALIDDATA;
    sc->ctts_data = (MOVStts *)av_malloc(entries * sizeof(*sc->ctts_data));
    if (!sc->ctts_data)
        return AVERROR(ENOMEM);
...
static void mov_build_index(MOVContext *mov, AVStream *st)
{
    ...
    if (!(st->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
        ...
        if (sc->sample_count >= UINT_MAX / sizeof(*st->index_entries))
            return;
        st->index_entries = (AVIndexEntry *)av_malloc(sc->sample_count*sizeof(*st->index_entries));
        if (!st->index_entries)
            return;
        ...
        if (total >= UINT_MAX / sizeof(*st->index_entries))
            return;
        st->index_entries = (AVIndexEntry *)av_malloc(total*sizeof(*st->index_entries));
        if (!st->index_entries)
            return;
...
static int mov_read_trex(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if ((uint64_t)c->trex_count+1 >= UINT_MAX / sizeof(*c->trex_data))
        return AVERROR_INVALIDDATA;
    trex = (MOVTrackExt *)av_realloc(c->trex_data, (c->trex_count+1)*sizeof(*c->trex_data));
    if (!trex)
        return AVERROR(ENOMEM);
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!sc->ctts_count && sc->sample_count)
    {
        ...
        ctts_data = (MOVStts *)av_malloc(sizeof(*sc->ctts_data));
        if (!ctts_data)
            return AVERROR(ENOMEM);
    ...
    if ((uint64_t)entries+sc->ctts_count >= UINT_MAX/sizeof(*sc->ctts_data))
        return AVERROR_INVALIDDATA;
    ctts_data = (MOVStts *)av_realloc(sc->ctts_data,
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->fc->nb_streams < 1)
        return 0;
    sc = (MOVStreamContext *)c->fc->streams[c->fc->nb_streams-1]->priv_data;
...
static const MOVParseTableEntry mov_default_parse_table[56] = {
...
static void mov_read_chapters(AVFormatContext *s)
{
    ...
    for (i = 0; i < st->nb_index_entries; i++) {
        ...
        if (len > sample->size-2)
            continue;
        title_len = 2*len + 1;
        if (!(title = (uint8_t *)av_mallocz(title_len)))
            goto finish;
        ...
        if (!len) {
            ...
            ch = avio_rb16(sc->pb);
            if (ch == 0xfeff)
                avio_get_str16be(sc->pb, len, (char *)title, title_len);
            else if (ch == 0xfffe)
                avio_get_str16le(sc->pb, len, (char *)title, title_len);
            else {
                AV_WB16(title, ch);
                if (len == 1 || len == 2)
                    title[len] = 0;
                else
                    avio_get_str(sc->pb, INT_MAX, (char *)(title + 2), len - 1);
            }
        }

        avpriv_new_chapter(s, i, st->time_base, sample->timestamp, end, (const char *)title);
        av_freep(&title);
    }
finish:
                                             ...
                                             uint32_t value)
{
    ...
    struct ff_timecode tc;
        ...
        tc.drop  = st->codec->flags2 & CODEC_FLAG2_DROP_FRAME_TIMECODE;
		tc.rate.num = st->codec->time_base.den;
        tc.rate.den = st->codec->time_base.num;
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        MOVStreamContext *msc = (MOVStreamContext *)avst->priv_data;
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (!sample) {
        mov->found_mdat = 0;
		{
			MOVAtom _m = { AV_RL32("root"), INT64_MAX };
        if (s->pb->seekable||
            mov_read_default(mov, s->pb, _m) < 0 ||
            url_feof(s->pb))
            return AVERROR_EOF;
		}
...
static const AVOption options[] = {
    {"use_absolute_path",
        ...
        offsetof(MOVContext, use_absolute_path), FF_OPT_TYPE_INT, 0, NULL,
        0, 1, AV_OPT_FLAG_VIDEO_PARAM|AV_OPT_FLAG_DECODING_PARAM},
    {NULL}
};
static const AVClass av_class = {"mov,mp4,m4a,3gp,3g2,mj2", av_default_item_name, options, LIBAVUTIL_VERSION_INT};
