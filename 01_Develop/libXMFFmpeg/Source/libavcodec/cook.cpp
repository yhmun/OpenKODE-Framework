...
static av_cold void init_pow2table(void)
{
    int i;
    for (i = -63; i < 64; i++) {
        pow2tab[63 + i] = pow(2.0, i);
        rootpow2tab[63 + i] = sqrt(pow(2.0, i));
    }
}
...
static av_cold void init_gain_table(COOKContext *q)
{
    ...
    for (i = 0; i < 23; i++)
        q->gain_table[i] = pow((double)pow2tab[i + 52],
...
static av_cold int init_cook_vlc_tables(COOKContext *q)
{
    ...
    for (i = 0; i < 7; i++) {
        result |= init_vlc(&q->sqvh[i], vhvlcKDsizeab[i], vhKDsizeab[i],
                           cvh_huffbits[i], 1, 1,
...
static av_cold int init_cook_mlt(COOKContext *q)
{
    ...
    if ((q->mlt_window = (float *)av_malloc(mlt_size * sizeof(*q->mlt_window))) == 0)
...
{
    int i;
    COOKContext *q = (COOKContext *)avctx->priv_data;
...
static av_cold int cook_decode_init(AVCodecContext *avctx)
{
    ...
    q->decoded_bytes_buffer =
        (uint8_t *)av_mallocz(avctx->block_align
