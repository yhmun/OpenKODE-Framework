...
static int decode_p_frame(FourXContext *f, const uint8_t *buf, int length){
    ...
    init_get_bits(&f->gb, (const uint8_t *)f->bitstream_buffer, 8*bitstream_size);
...
static int decode_i_frame(FourXContext *f, const uint8_t *buf, int length){
    ...
    init_get_bits(&f->pre_gb, (const uint8_t *)f->bitstream_buffer, 8*prestream_size);
                        ...
                        AVPacket *avpkt)
{
    ...
    FourXContext * const f = (FourXContext * const)avctx->priv_data;
    ...
    if(frame_4cc == AV_RL32("cfrm")){
        ...
        if (data_size > UINT_MAX -  cfrm->size - FF_INPUT_BUFFER_PADDING_SIZE)
            return AVERROR_INVALIDDATA;
        cfrm->data= (uint8_t *)av_fast_realloc(cfrm->data, &cfrm->allocated_size, cfrm->size + data_size + FF_INPUT_BUFFER_PADDING_SIZE);
