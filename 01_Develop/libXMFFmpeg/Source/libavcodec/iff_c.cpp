...
{
    IffContext *s = (IffContext *)avctx->priv_data;
    int count, i;
    ...
    if (buf_size > 8) {
        ...
        if (s->masking == MASK_HAS_MASK) {
            if (s->bpp >= 8) {
                ...
                s->mask_buf = (uint32_t *)av_malloc((s->planesize * 32) + FF_INPUT_BUFFER_PADDING_SIZE);
                if (!s->mask_buf)
                    return AVERROR(ENOMEM);
                s->mask_palbuf = (uint32_t *)av_malloc((2 << s->bpp) * sizeof(uint32_t) + FF_INPUT_BUFFER_PADDING_SIZE);
                if (!s->mask_palbuf) {
                    av_freep(&s->mask_buf);
                    return AVERROR(ENOMEM);
                }
        ...
        if (s->ham) {
            ...
            s->ham_buf = (uint8_t *)av_malloc((s->planesize * 8) + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!s->ham_buf)
                return AVERROR(ENOMEM);

            ham_count = 8 * (1 << s->ham);
            s->ham_palbuf = (uint32_t *)av_malloc((ham_count << !!(s->masking == MASK_HAS_MASK)) * sizeof (uint32_t) + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!s->ham_palbuf) {
                av_freep(&s->ham_buf);
                return AVERROR(ENOMEM);
            }
...
static av_cold int decode_init(AVCodecContext *avctx)
{
    ...
    s->planebuf = (uint8_t *)av_malloc(s->planesize + FF_INPUT_BUFFER_PADDING_SIZE);
    if (!s->planebuf)
        return AVERROR(ENOMEM);
