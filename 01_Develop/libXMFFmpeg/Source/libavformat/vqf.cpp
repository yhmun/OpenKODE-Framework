                         ...
                         unsigned int tag_len, unsigned int remaining)
{
    ...
    buf = (char *)av_malloc(len+1);
    if (!buf)
        return;
    avio_read(s->pb, (unsigned char *)buf, len);
    buf[len] = 0;
    AV_WL32(key, tag);
...
{
    VqfContext *c = (VqfContext *)s->priv_data;
    ...
    if (!(st->codec->extradata = (uint8_t *)av_malloc(12 + FF_INPUT_BUFFER_PADDING_SIZE)))
        return AVERROR(ENOMEM);
