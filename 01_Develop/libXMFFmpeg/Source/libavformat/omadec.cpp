...
#include "XMFFmpeg/libavutil/des.h"
#include "oma.h"
#include "pcm.h"
#include "riff.h"
#include "id3v2.h"
...
{
    OMAContext *oc = (OMAContext *)s->priv_data;
...
static int decrypt_init(AVFormatContext *s, ID3v2ExtraMeta *em, uint8_t *header)
{
    ...
    while (em) {
        if (!strcmp(em->tag, "GEOB") &&
            (geob = (ID3v2ExtraMetaGEOB *)em->data) &&
	...
	{
		const uint8_t _t[8] = {0}; 
    if (!memcmp(oc->r_val, _t, 8) ||
                           ...
                           AVFormatParameters *ap)
{
	...
	{
		const uint8_t _t[] ={'E', 'A', '3'};
    if (memcmp(buf, _t,3) || buf[4] != 0 || buf[5] != EA3_HEADER_SIZE) {
    ...
    switch (buf[32]) {
        case OMA_CODECID_ATRAC3:
            ...
            edata = (uint8_t *)av_mallocz(14 + FF_INPUT_BUFFER_PADDING_SIZE);
            if (!edata)
                return AVERROR(ENOMEM);
...
static const AVCodecTag* const _ff_oma_demuxer[] = {ff_oma_codec_tags, 0};
