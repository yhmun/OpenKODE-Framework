...
{
    __xmlRaiseError(KD_NULL, KD_NULL, KD_NULL, KD_NULL, KD_NULL, XML_FROM_XPOINTER,
           ...
           const char * msg, const xmlChar *extra)
{
	...
	__xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
			KD_NULL, KD_NULL, XML_FROM_XPOINTER, error,
	...
	__xmlRaiseError(KD_NULL, KD_NULL, KD_NULL,
			KD_NULL, ctxt->context->debugNode, XML_FROM_XPOINTER,
			error, XML_ERR_ERROR, KD_NULL, 0,
			(const char *) extra, (const char *) ctxt->base, KD_NULL,
...
xmlXPtrGetArity(xmlNodePtr cur) {
	...
	return(-1);
    cur = cur->children;
    for (i = 0;cur != KD_NULL;cur = cur->next) {
...
xmlXPtrGetIndex(xmlNodePtr cur) {
    ...
    if (cur == KD_NULL) 
	return(-1);
    for (i = 1;cur != KD_NULL;cur = cur->prev) {
	if ((cur->type == XML_ELEMENT_NODE) ||
	    (cur->type == XML_DOCUMENT_NODE) ||
	    (cur->type == XML_HTML_DOCUMENT_NODE)) {
	    i++;
 ...
 * @no:  the child number
 *
 * Returns the @no'th element child of @cur or KD_NULL
 */
static xmlNodePtr
...
xmlXPtrNewPoint(xmlNodePtr node, int indx) {
    ...
    kdMemset (ret, 0 , (KDsize) sizeof(xmlXPathObject));
...
xmlXPtrRangesEqual(xmlXPathObjectPtr range1, xmlXPathObjectPtr range2) {
    if (range1 == range2)
	return(1);
    if ((range1 == KD_NULL) || (range2 == KD_NULL))
	return(0);
 ...
 * @val:  an initial xmlXPathObjectPtr, or KD_NULL
 *
...
xmlXPtrLocationSetCreate(xmlXPathObjectPtr val) {
    ...
    kdMemset (ret, 0 , (KDsize) sizeof(xmlLocationSet));
    if (val != KD_NULL) {
        ret->locTab = (xmlXPathObjectPtr *) xmlMalloc(XML_RANGESET_DEFAULT *
					     sizeof(xmlXPathObjectPtr));
	if (ret->locTab == KD_NULL) {
	    xmlXPtrErrMemory("allocating locationset");
	    xmlFree(ret);
	    return(KD_NULL);
	}
	kdMemset (ret->locTab, 0 ,
	       XML_RANGESET_DEFAULT * (KDsize) sizeof(xmlXPathObjectPtr));
...
xmlXPtrLocationSetAdd(xmlLocationSetPtr cur, xmlXPathObjectPtr val) {
    ...
    if (cur->locMax == 0) {
        cur->locTab = (xmlXPathObjectPtr *) xmlMalloc(XML_RANGESET_DEFAULT *
					     sizeof(xmlXPathObjectPtr));
	if (cur->locTab == KD_NULL) {
	    xmlXPtrErrMemory("adding location to set");
	    return;
	}
	kdMemset (cur->locTab, 0 ,
	       XML_RANGESET_DEFAULT * (KDsize) sizeof(xmlXPathObjectPtr));
...
xmlXPtrLocationSetMerge(xmlLocationSetPtr val1, xmlLocationSetPtr val2) {
    ...
    if (val1 == KD_NULL) return(KD_NULL);
 ...
 * @end:  the end NodePtr value or KD_NULL
 *
...
xmlXPtrNewLocationSetNodeSet(xmlNodeSetPtr set) {
	...
	newset = xmlXPtrLocationSetCreate(KD_NULL);
	if (newset == KD_NULL)
	    return(ret);
...
xmlXPtrGetChildNo(xmlXPathParserContextPtr ctxt, int indx) {
    ...
    if ((indx <= 0) || (oldset == KD_NULL) || (oldset->nodeNr != 1)) {
	xmlXPathFreeObject(obj);
	valuePush(ctxt, xmlXPathNewNodeSet(KD_NULL));
	return;
    }
...
xmlXPtrEvalXPtrPart(xmlXPathParserContextPtr ctxt, xmlChar *name) {
    ...
    *cur = 0;
	...
	if (buffer[0] == '/') {
	    xmlXPathRoot(ctxt);
	    xmlXPtrEvalChildSeq(ctxt, KD_NULL);
	} else {
...
xmlXPtrEvalFullXPtr(xmlXPathParserContextPtr ctxt, xmlChar *name) {
	...
	if (ctxt->value != KD_NULL) {
		...
		case XPATH_LOCATIONSET: {
		    ...
		    if ((loc != KD_NULL) && (loc->locNr > 0))
			return;
		    break;
		}
		case XPATH_NODESET: {
		    xmlNodeSetPtr loc = ctxt->value->nodesetval;
		    if ((loc != KD_NULL) && (loc->nodeNr > 0))
			return;
		    break;
		}
		default:
 ...
 * @here:  the node that directly contains the XPointer being evaluated or KD_NULL
...
xmlXPtrEval(const xmlChar *str, xmlXPathContextPtr ctx) {
    ...
    xmlXPathObjectPtr res = KD_NULL, tmp;
    xmlXPathObjectPtr init = KD_NULL;
    int stack = 0;

    xmlXPathInit();

    if ((ctx == KD_NULL) || (str == KD_NULL))
	return(KD_NULL);
...
xmlXPtrBuildRangeNodeList(xmlXPathObjectPtr range) {
    ...
    xmlNodePtr list = KD_NULL, last = KD_NULL, parent = KD_NULL, tmp;
	...
	if (cur == end) {
		...
		continue; /* while */
		   ...
		   (list == KD_NULL) /* looks superfluous but ... */ ) {
	    if ((cur->type == XML_TEXT_NODE) ||
	    ...
	    if (tmp != KD_NULL) {
		if ((list == KD_NULL) || ((last == KD_NULL) && (parent == KD_NULL)))  {
		    STRANGE
		    return(KD_NULL);
		}
		if (last != KD_NULL)
	...
	cur = xmlXPtrAdvanceNode(cur, KD_NULL);
    }
    return(list);
}
...
xmlXPtrHereFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    if (ctxt->context->here == KD_NULL)
	XP_ERROR(XPTR_SYNTAX_ERROR);
    
    valuePush(ctxt, xmlXPtrNewLocationSetNodes(ctxt->context->here, KD_NULL));
}
...
xmlXPtrOriginFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    if (ctxt->context->origin == KD_NULL)
	XP_ERROR(XPTR_SYNTAX_ERROR);
    
    valuePush(ctxt, xmlXPtrNewLocationSetNodes(ctxt->context->origin, KD_NULL));
}
...
xmlXPtrStartPointFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    xmlLocationSetPtr newset = KD_NULL;
    xmlLocationSetPtr oldset = KD_NULL;
 ...
 *
 * Returns a new location or KD_NULL in case of error
 */
static xmlXPathObjectPtr
...
xmlXPtrMatchString(const xmlChar *string, xmlNodePtr start, int startindex,
    ...
    if (start == KD_NULL)
	return(-1);
    if ((end == KD_NULL) || (endindex == KD_NULL))
	return(-1);
    cur = start;
    if (cur == KD_NULL)
	return(-1);
...
xmlXPtrSearchString(const xmlChar *string, xmlNodePtr *start, int *startindex,
    ...
    if (string == KD_NULL)
	return(-1);
    if ((start == KD_NULL) || (startindex == KD_NULL))
	return(-1);
    if ((end == KD_NULL) || (endindex == KD_NULL))
	return(-1);
    cur = *start;
...
xmlXPtrGetStartPoint(xmlXPathObjectPtr obj, xmlNodePtr *node, int *indx) {
    if ((obj == KD_NULL) || (node == KD_NULL) || (indx == KD_NULL))
	return(-1);
...
xmlXPtrStringRangeFunction(xmlXPathParserContextPtr ctxt, int nargs) {
    ...
    xmlXPathObjectPtr position = KD_NULL;
    xmlXPathObjectPtr number = KD_NULL;
...
xmlXPtrEvalRangePredicate(xmlXPathParserContextPtr ctxt) {
    ...
    if ((oldset == KD_NULL) || (oldset->locNr == 0)) {
