                      ...
                      const uint8_t *buf, unsigned int buf_size)
{
    ...
    sub->rects[rect]->pict.data[0] = (uint8_t *)av_malloc(sub->rects[rect]->w * sub->rects[rect]->h);
                                       ...
                                       const uint8_t *buf, int buf_size)
{
    ...
    av_freep(&ctx->presentation.objects);
    ctx->presentation.objects = (PGSSubPictureReference *)av_malloc(sizeof(PGSSubPictureReference) * ctx->presentation.object_count);
                               ...
                               const uint8_t *buf, int buf_size)
{
    ...
    for (rect = 0; rect < sub->num_rects; ++rect) {
        ...
        sub->rects[rect]       = (AVSubtitleRect *)av_mallocz(sizeof(*sub->rects[rect]));
        ...
        sub->rects[rect]->pict.data[1] = (uint8_t *)av_mallocz(AVPALETTE_SIZE);
