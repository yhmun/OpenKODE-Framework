...
static const AVOption options[] = {
    { "mpegts_transport_stream_id", "Set transport_stream_id field.",
      offsetof(MpegTSWrite, transport_stream_id), AV_OPT_TYPE_INT, /*{.dbl = */0x0001/*}*/, NULL, 0x0001, 0xffff, AV_OPT_FLAG_ENCODING_PARAM},
    { "mpegts_original_network_id", "Set original_network_id field.",
      offsetof(MpegTSWrite, original_network_id), AV_OPT_TYPE_INT, 0x0001, NULL,0x0001, 0xffff, AV_OPT_FLAG_ENCODING_PARAM},
    { "mpegts_service_id", "Set service_id field.",
      offsetof(MpegTSWrite, service_id), AV_OPT_TYPE_INT, 0x0001,NULL, 0x0001, 0xffff, AV_OPT_FLAG_ENCODING_PARAM},
    { "mpegts_pmt_start_pid", "Set the first pid of the PMT.",
      offsetof(MpegTSWrite, pmt_start_pid), AV_OPT_TYPE_INT, 0x1000, NULL,0x0010, 0x1f00, AV_OPT_FLAG_ENCODING_PARAM},
    { "mpegts_start_pid", "Set the first pid.",
      offsetof(MpegTSWrite, start_pid), AV_OPT_TYPE_INT, 0x0100, NULL,0x0100, 0x0f00, AV_OPT_FLAG_ENCODING_PARAM},
    {"mpegts_m2ts_mode", "Enable m2ts mode.",
        offsetof(MpegTSWrite, m2ts_mode), AV_OPT_TYPE_INT, -1, NULL,
        -1,1, AV_OPT_FLAG_ENCODING_PARAM},
    { "muxrate", NULL, offsetof(MpegTSWrite, mux_rate), AV_OPT_TYPE_INT, 1, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { "pes_payload_size", "Minimum PES packet payload in bytes",
      offsetof(MpegTSWrite, pes_payload_size), AV_OPT_TYPE_INT, DEFAULT_PES_PAYLOAD_SIZE, NULL, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { NULL },
};
...
static av_always_inline void put16(uint8_t **q_ptr, int val)
{
    uint8_t *q;
    q = *q_ptr;
    *q++ = val >> 8;
...
static void mpegts_prefix_m2ts_header(AVFormatContext *s)
{
    MpegTSWrite *ts = (MpegTSWrite *)s->priv_data;
    if (ts->m2ts_mode) {
        int64_t pcr = get_pcr((const MpegTSWrite *)s->priv_data, s->pb);
...
static int mpegts_write_header(AVFormatContext *s)
{
    ...
    pids = (int *)av_malloc(s->nb_streams * sizeof(*pids));
    if (!pids)
    ...
    for(i = 0;i < s->nb_streams; i++) {
        ...
        if (!ts_st)
            goto fail;
        st->priv_data = ts_st;
        ts_st->payload = (uint8_t *)av_mallocz(ts->pes_payload_size);
        if (!ts_st->payload)
            goto fail;
        ...
        if (st->codec->codec_id == CODEC_ID_AAC &&
            ...
            ts_st->adts = (ADTSContext *)av_mallocz(sizeof(*ts_st->adts));
            if (!ts_st->adts)
                goto fail;
    ...
    if (st->codec->codec_id == CODEC_ID_H264) {
        ...
        if ((AV_RB16(pkt->data) & 0xfff0) != 0xfff0) {
            ...
            if ((unsigned)new_size >= INT_MAX)
                return AVERROR_INVALIDDATA;
            data = (uint8_t *)av_malloc(new_size);
            if (!data)
                return AVERROR(ENOMEM);
