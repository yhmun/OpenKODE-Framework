...
static int png_decode_idat(PNGDecContext *s, int length)
{
    ...
    s->zstream.next_in = (z_Bytef *)s->gb.buffer;
                        ...
                        AVPacket *avpkt)
{
    ...
    PNGDecContext * const s = (PNGDecContext *const)avctx->priv_data;
    ...
    for(;;) {
        ...
        case MKTAG('I', 'D', 'A', 'T'):
            ...
            if (!(s->state & PNG_IDAT)) {
                ...
                s->last_row = (uint8_t *)av_mallocz(s->row_size);
                if (!s->last_row)
                    goto fail;
                if (s->interlace_type ||
                    s->color_type == PNG_COLOR_TYPE_RGB_ALPHA) {
                    s->tmp_row = (uint8_t *)av_malloc(s->row_size);
                    if (!s->tmp_row)
                        goto fail;
                }
                /* compressed row */
                crow_buf_base = (uint8_t *)av_malloc(s->row_size + 16);
                if (!crow_buf_base)
                    goto fail;
...
{
    PNGDecContext *s = (PNGDecContext *)avctx->priv_data;
