...
{
    struct variant *var = (struct variant *)av_mallocz(sizeof(struct variant));
    if (!var)
        return NULL;
    ...
    while (!url_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (is_segment) {
                ...
                seg = (struct segment *)av_malloc(sizeof(struct segment));
                if (!seg) {
                    ret = AVERROR(ENOMEM);
                    goto fail;
                }
...
{
    struct variant *v = (struct variant *)opaque;
    AppleHTTPContext *c = (AppleHTTPContext *)v->parent->priv_data;
    int ret;
	unsigned int i;
...
{
    AppleHTTPContext *c = (AppleHTTPContext *)s->priv_data;
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        v->read_buffer = (uint8_t *)av_malloc(INITIAL_BUFFER_SIZE);
...
{
	const char *str = (const char *)p->buf;
    /* Require #EXTM3U at the start, and either one of the ones below
     * somewhere for a proper match. */
    if (strncmp(str, "#EXTM3U", 7))
        return 0;
    if (strstr(str, "#EXT-X-STREAM-INF:")     ||
        strstr(str, "#EXT-X-TARGETDURATION:") ||
        strstr(str, "#EXT-X-MEDIA-SEQUENCE:"))
