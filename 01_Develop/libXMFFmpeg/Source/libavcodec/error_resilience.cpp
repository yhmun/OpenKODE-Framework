...
{
    int b_x, b_y;
    int16_t  (*col )[4] = (int16_t (*)[4])av_malloc(stride*h*sizeof( int16_t)*4);
    uint16_t (*dist)[4] = (uint16_t (*)[4])av_malloc(stride*h*sizeof(uint16_t)*4);
...
{
    uint8_t *fixed = (uint8_t *)av_malloc(s->mb_stride * s->mb_height);
#define MV_FROZEN    3
#define MV_CHANGED   2
...
void ff_er_frame_end(MpegEncContext *s)
{
    ...
    if (s->current_picture.f.motion_val[0] == NULL) {
        ...
        for (i = 0; i < 2; i++) {
            pic->f.ref_index[i]     = (int8_t *)av_mallocz(s->mb_stride * s->mb_height * 4 * sizeof(uint8_t));
            pic->motion_val_base[i] = (int16_t (*)[2])av_mallocz((size + 4) * 2 * sizeof(uint16_t));
