...
#include "rtpdec.h"
#include "rdt.h"
#include "XMFFmpeg/libavutil/base64.h"
...
{
    RDTDemuxContext *s = (RDTDemuxContext *)av_mallocz(sizeof(RDTDemuxContext));
    if (!s)
        return NULL;
    ...
    if (rdt->audio_pkt_cnt == 0) {
        ...
        if (res > 0) {
            if (st->codec->codec_id == CODEC_ID_AAC) {
                ...
                rdt->rmctx->pb = avio_alloc_context ((unsigned char *)rdt->buffer, len - pos, 0,
                                                    NULL, NULL, NULL, NULL);
            }
            goto get_cache;
        }
                    ...
                    uint8_t **bufptr, int len)
{
    ...
    if (!buf && s->prev_stream_id != -1) {
        ...
        rv= s->parse_packet(s->ic, (PayloadContext *)s->dynamic_protocol_context,
...
rdt_parse_b64buf (unsigned int *target_len, const char *p)
{
    ...
    *target_len = len * 3 / 4;
    target = (unsigned char *)av_mallocz(*target_len + FF_INPUT_BUFFER_PADDING_SIZE);
        ...
        for (n = 0; n < s->nb_streams; n++)
            if (s->streams[n]->id == stream->id) {
                ...
                if (rdt->nb_rmst < count) {
                    RMStream **rmst= (RMStream **)av_realloc(rdt->rmst, count*sizeof(*rmst));
                    if (!rmst)
...
{
    PayloadContext *rdt = (PayloadContext *)av_mallocz(sizeof(PayloadContext));
