...
int av_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;){
        ...
        if(st->request_probe>0){
            ...
            pd->buf = (unsigned char *)av_realloc(pd->buf, pd->buf_size+pkt->size+AVPROBE_PADDING_SIZE);
...
static int get_std_framerate(int i){
	...
	{
		const int _r[] = {24,30,60,12,15};
		return _r[i-60*12]*1000*12;
	}
}
    ...
    for(;;) {
        ...
        if (st->codec_info_nb_frames>1) {
            ...
            if (st->avg_frame_rate.num > 0)
                t = FFMAX(t, av_rescale_q(st->codec_info_nb_frames, av_rational(st->avg_frame_rate.den, st->avg_frame_rate.num), AV_TIME_BASE_Q));
...
AVStream *avformat_new_stream(AVFormatContext *s, AVCodec *c)
{
    ...
    if (s->nb_streams >= INT_MAX/sizeof(*streams))
        return NULL;
    streams = (AVStream **)av_realloc(s->streams, (s->nb_streams + 1) * sizeof(*streams));
    if (!streams)
        return NULL;
    s->streams = streams;
    ...
    if (!st)
        return NULL;
	if (!(st->info = (struct AVStream::Info*)av_mallocz(sizeof(struct AVStream::Info)))) {
        av_free(st);
        return NULL;
    }
    ...
    st->sample_aspect_ratio = av_rational(0,1);
...
static int compute_pkt_fields2(AVFormatContext *s, AVStream *st, AVPacket *pkt){
    int delay = FFMAX(st->codec->has_b_frames, (int)(!!st->codec->max_b_frames));
...
int64_t av_gettime(void)
{
	...
	return (int64_t)(kdGetTimeUST() / 1000ULL);
}
