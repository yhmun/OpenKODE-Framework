...
{
    int i, j, ret;
    IMCContext *q = (IMCContext *)avctx->priv_data;
    double r1, r2;
                                         ...
                                         float* flcoeffs2)
{
    ...
    flcoeffs2[0] = log(flcoeffs1[0])/log(2.f);
    tmp = flcoeffs1[0];
    tmp2 = flcoeffs2[0];
...
static int bit_allocation (IMCContext* q, int stream_format_code, int freebits, int flag) {
    ...
    for(i = 0; i < BANDS-1; i++) {
        q->flcoeffs4[i] = q->flcoeffs3[i] - log(q->flcoeffs5[i])/log(2.f);
    }
    ...
    if (freebits > summer) {
        ...
        do{
            ...
            if (highest > -1.e20f) {
                workT[found_indx] -= 2.0f;
                if (++(q->bitsBandT[found_indx]) == 6)
                    workT[found_indx] = -1.e20f;
...
static void imc_adjust_bit_allocation (IMCContext* q, int summer) {
    ...
    for(i = 0; i < BANDS; i++) {
        workT[i] = (q->bitsBandT[i] == 6) ? -1.e20f : (q->bitsBandT[i] * -2 + q->flcoeffs4[i] - 0.415f);
    }
